...
def create_missing_tables(conn):
    ...
    tables = {
        'question_categories': """ CREATE TABLE IF NOT EXISTS question_categories ( id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, code TEXT UNIQUE, description TEXT, sort_order INTEGER DEFAULT 0, status INTEGER DEFAULT 1, created_at TEXT DEFAULT CURRENT_TIMESTAMP ) """,
        'question_tags': """ CREATE TABLE IF NOT EXISTS question_tags ( id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, description TEXT, sort_order INTEGER DEFAULT 0, created_at TEXT DEFAULT CURRENT_TIMESTAMP ) """,
        'courses': """ CREATE TABLE IF NOT EXISTS courses ( id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, description TEXT, code TEXT UNIQUE, status TEXT DEFAULT 'active', category_id INTEGER, start_date TEXT, end_date TEXT, total_hours INTEGER DEFAULT 0, created_at TEXT DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (category_id) REFERENCES question_categories (id) ) """,
        'system_params': """ CREATE TABLE IF NOT EXISTS system_params ( id INTEGER PRIMARY KEY AUTOINCREMENT, key TEXT UNIQUE NOT NULL, value TEXT, description TEXT, type TEXT DEFAULT 'string', constraints TEXT, updated_at TEXT DEFAULT CURRENT_TIMESTAMP ) """,
    }
    
    for name, create_sql in tables.items():
        try:
            cursor.execute(create_sql)
            logger.info(f"  表 {name} 创建/检查完成")
        except Exception as e:
            logger.info(f"  创建表 {name} 失败: {e}")
...
def seed_users(conn):
    ...
    for user in users:
        try:
            ...
            logger.info(f"  用户 {user[0]} 插入失败: {e}")
...
def seed_exams(conn):
    ...
    for exam in exams:
        try:
            ...
            logger.info(f"  考试 {exam[0]} 插入失败: {e}")
...
def seed_courses(conn):
    ...
    for course in courses:
        try:
            ...
            logger.info(f"  课程 {course[0]} 插入失败: {e}")
...
def seed_system_params(conn):
    ...
    for param in params:
        try:
            ...
            logger.info(f"  参数 {param[0]} 插入失败: {e}")
...
def seed_error_reports_table(conn):
    cursor = conn.cursor()
    try:
        cursor.execute(""" CREATE TABLE IF NOT EXISTS error_reports ( id INTEGER PRIMARY KEY AUTOINCREMENT, message TEXT NOT NULL, url TEXT, line INTEGER DEFAULT 0, column INTEGER DEFAULT 0, error TEXT, timestamp REAL, user_agent TEXT, user_id INTEGER, created_at TEXT, FOREIGN KEY (user_id) REFERENCES users (id) ) """)
        conn.commit()
        logger.info("  错误报告表已创建")
    except Exception as e:
        logger.info(f"  创建错误报告表失败: {e}")
...
def main():
    logger.info("=== MTSCOS AI 数据库种子数据填充 ===")
