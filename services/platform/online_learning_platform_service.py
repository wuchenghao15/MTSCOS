...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
""" MTSCOS 在线学习平台服务 (v15.9.0) ==================================== 提供在线课程、直播课堂、录播教学、互动答疑、作业考试等综合在线学习服务。 支持成人教育和K12教育的差异化需求。  核心能力： 1. 课程管理 - 直播课、录播课、混合课、微课管理 2. 直播课堂 - 在线直播、互动白板、屏幕共享、录制 3. 录播课程 - 视频管理、章节编排、断点续播 4. 互动教学 - 答题、投票、讨论、举手、分组 5. 学习进度 - 观看时长、完成度、学习轨迹 6. 在线答疑 - 提问、解答、知识库 7. 作业考试 - 在线作业、自动批改、在线考试 8. 学习社区 - 课程讨论、学习小组、互助 """
import os
import json
import uuid
import sqlite3
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS online_courses ( course_id TEXT PRIMARY KEY, course_name TEXT NOT NULL, course_type TEXT NOT NULL, subject TEXT, education_type TEXT, teacher_id TEXT, teacher_name TEXT, grade_level TEXT, description TEXT, cover_image TEXT, difficulty TEXT, total_hours REAL DEFAULT 0, enrolled_count INTEGER DEFAULT 0, rating REAL DEFAULT 0, rating_count INTEGER DEFAULT 0, price REAL DEFAULT 0, is_free INTEGER DEFAULT 1, status TEXT DEFAULT 'draft', tags TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_chapters ( chapter_id TEXT PRIMARY KEY, course_id TEXT NOT NULL, chapter_name TEXT NOT NULL, chapter_order INTEGER DEFAULT 0, description TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_lessons ( lesson_id TEXT PRIMARY KEY, chapter_id TEXT NOT NULL, course_id TEXT NOT NULL, lesson_name TEXT NOT NULL, lesson_order INTEGER DEFAULT 0, lesson_type TEXT, duration_minutes INTEGER DEFAULT 0, video_url TEXT, content TEXT, is_preview INTEGER DEFAULT 0, view_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS live_sessions ( session_id TEXT PRIMARY KEY, course_id TEXT NOT NULL, lesson_id TEXT, session_name TEXT NOT NULL, teacher_id TEXT, teacher_name TEXT, start_time TEXT, end_time TEXT, live_status TEXT DEFAULT 'scheduled', stream_url TEXT, record_url TEXT, max_participants INTEGER DEFAULT 100, actual_participants INTEGER DEFAULT 0, is_recorded INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS live_attendance ( id INTEGER PRIMARY KEY AUTOINCREMENT, session_id TEXT NOT NULL, user_id TEXT NOT NULL, user_name TEXT, join_time TEXT, leave_time TEXT, duration_minutes INTEGER DEFAULT 0, interaction_count INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_progress ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, lesson_id TEXT NOT NULL, user_id TEXT NOT NULL, user_name TEXT, status TEXT DEFAULT 'not_started', progress_percent REAL DEFAULT 0, watch_duration REAL DEFAULT 0, last_position REAL DEFAULT 0, last_learn_time TEXT, completed_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS interactions ( interaction_id TEXT PRIMARY KEY, session_id TEXT NOT NULL, course_id TEXT NOT NULL, interaction_type TEXT NOT NULL, user_id TEXT, user_name TEXT, content TEXT, options TEXT, result TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS online_qa ( qa_id TEXT PRIMARY KEY, course_id TEXT NOT NULL, lesson_id TEXT, user_id TEXT NOT NULL, user_name TEXT, question TEXT NOT NULL, question_type TEXT DEFAULT 'general', status TEXT DEFAULT 'open', answer TEXT, answered_by TEXT, answered_at TEXT, answer_count INTEGER DEFAULT 0, helpful_count INTEGER DEFAULT 0, is_pinned INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS online_assignments ( assignment_id TEXT PRIMARY KEY, course_id TEXT NOT NULL, lesson_id TEXT, assignment_name TEXT NOT NULL, assignment_type TEXT NOT NULL, description TEXT, total_score REAL DEFAULT 100, deadline TEXT, auto_grade INTEGER DEFAULT 0, allow_resubmit INTEGER DEFAULT 0, max_attempts INTEGER DEFAULT 1, submit_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS assignment_submissions ( submission_id TEXT PRIMARY KEY, assignment_id TEXT NOT NULL, user_id TEXT NOT NULL, user_name TEXT, content TEXT, file_url TEXT, score REAL, feedback TEXT, graded_by TEXT, graded_at TEXT, attempt_number INTEGER DEFAULT 1, submit_time TEXT, status TEXT DEFAULT 'submitted', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_communities ( community_id TEXT PRIMARY KEY, community_name TEXT NOT NULL, community_type TEXT NOT NULL, course_id TEXT, creator_id TEXT NOT NULL, creator_name TEXT, description TEXT, member_count INTEGER DEFAULT 1, max_members INTEGER DEFAULT 100, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS community_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, community_id TEXT NOT NULL, user_id TEXT NOT NULL, user_name TEXT, role TEXT DEFAULT 'member', joined_at TEXT, contribution_score REAL DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS community_posts ( post_id TEXT PRIMARY KEY, community_id TEXT NOT NULL, user_id TEXT NOT NULL, user_name TEXT, title TEXT NOT NULL, content TEXT, post_type TEXT DEFAULT 'discussion', view_count INTEGER DEFAULT 0, like_count INTEGER DEFAULT 0, reply_count INTEGER DEFAULT 0, is_pinned INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_enrollments ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, user_id TEXT NOT NULL, user_name TEXT, education_type TEXT, enroll_time TEXT, progress_percent REAL DEFAULT 0, last_learn_time TEXT, completed_lessons INTEGER DEFAULT 0, total_lessons INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT ) ''')
                conn.commit()
    ...
    def create_course(self, course_name: str, course_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO online_courses ( course_id, course_name, course_type, subject, education_type, teacher_id, teacher_name, grade_level, description, cover_image, difficulty, total_hours, enrolled_count, rating, rating_count, price, is_free, status, tags, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, ?, ?, ?, ?, ?, ?) ''', (course_id, course_name, course_type, subject,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO course_chapters ( chapter_id, course_id, chapter_name, chapter_order, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (chapter_id, course_id, chapter_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO course_lessons ( lesson_id, chapter_id, course_id, lesson_name, lesson_order, lesson_type, duration_minutes, video_url, content, is_preview, view_count, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?) ''', (lesson_id, chapter_id, course_id, lesson_name,
        ...
        try:
            with self._get_connection() as conn:
                ...
                for ch in cursor.fetchall():
                    chapter = dict(ch)
                    cursor.execute('SELECT * FROM course_lessons WHERE chapter_id = ? ORDER BY lesson_order',
                    (chapter['chapter_id'],))
    ...
    def create_live_session(self, course_id: str, session_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO live_sessions ( session_id, course_id, lesson_id, session_name, teacher_id, teacher_name, start_time, end_time, live_status, stream_url, record_url, max_participants, actual_participants, is_recorded, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'scheduled', ?, ?, ?, 0, ?, ?, ?) ''', (session_id, course_id, kwargs.get('lesson_id'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE live_sessions SET live_status = ?, end_time = ?, record_url = COALESCE(?, record_url), is_recorded = ?, updated_at = ? WHERE session_id = ? AND live_status = 'live' ''', (new_status, kwargs.get('end_time', now), record_url,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(
                    'SELECT id FROM live_attendance WHERE session_id = ? AND user_id = ? AND leave_time IS NULL',
                                 (session_id, user_id))
                    existing = cursor.fetchone()
                    if existing:
                        return {'success': False, 'error': '用户已签到且未签退'}
                    cursor.execute(''' INSERT INTO live_attendance ( session_id, user_id, user_name, join_time, leave_time, duration_minutes, interaction_count, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (session_id, user_id, kwargs.get('user_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if row:
                        cursor.execute(''' UPDATE learning_progress SET status = ?, progress_percent = ?, watch_duration = ?, last_position = ?, last_learn_time = ?, completed_at = COALESCE(completed_at, ?), updated_at = ? WHERE id = ? ''', (status, progress_percent, new_watch,
                              kwargs.get('last_position', 0), now, completed_at, now, row[0]))
                    else:
                        cursor.execute(''' INSERT INTO learning_progress ( course_id, lesson_id, user_id, user_name, status, progress_percent, watch_duration, last_position, last_learn_time, completed_at, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (course_id, lesson_id, user_id,
    ...
    def get_user_progress(self, course_id: str, user_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(
                'SELECT * FROM learning_progress WHERE course_id = ? AND user_id = ? ORDER BY last_learn_time DESC',
                             (course_id, user_id))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO interactions ( interaction_id, session_id, course_id, interaction_type, user_id, user_name, content, options, result, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (interaction_id, session_id, course_id, interaction_type,
    ...
    def ask_question(self, course_id: str, user_id: str, question: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO online_qa ( qa_id, course_id, lesson_id, user_id, user_name, question, question_type, status, answer_count, helpful_count, is_pinned, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'open', 0, 0, 0, ?, ?) ''', (qa_id, course_id, kwargs.get('lesson_id'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE online_qa SET answer = ?, answered_by = ?, answered_at = ?, status = ?, answer_count = answer_count + 1, updated_at = ? WHERE qa_id = ? ''', (answer, answered_by, now, new_status, now, qa_id))
                    conn.commit()
    ...
    def create_assignment(self, course_id: str, assignment_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO online_assignments ( assignment_id, course_id, lesson_id, assignment_name, assignment_type, description, total_score, deadline, auto_grade, allow_resubmit, max_attempts, submit_count, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?) ''', (assignment_id, course_id, kwargs.get('lesson_id'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if assignment[0] and kwargs.get('auto_score') is not None:
                        ...
                        status = 'graded'
                    cursor.execute(''' INSERT INTO assignment_submissions ( submission_id, assignment_id, user_id, user_name, content, file_url, score, feedback, graded_by, graded_at, attempt_number, submit_time, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (submission_id, assignment_id, user_id,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE assignment_submissions SET score = ?, feedback = ?, graded_by = ?, graded_at = ?, status = 'graded' WHERE submission_id = ? ''', (score, kwargs.get('feedback'), kwargs.get('graded_by'),
                          now, submission_id))
                    conn.commit()
    ...
    def create_community(self, community_name: str, community_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_communities ( community_id, community_name, community_type, course_id, creator_id, creator_name, description, member_count, max_members, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, 1, ?, ?) ''', (community_id, community_name, community_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not community[0]:
                        return {'success': False, 'error': '社区已关闭'}
                    cursor.execute(''' INSERT INTO community_posts ( post_id, community_id, user_id, user_name, title, content, post_type, view_count, like_count, reply_count, is_pinned, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, 0, ?, ?, ?) ''', (post_id, community_id, user_id, kwargs.get('user_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT COUNT(*) FROM course_lessons WHERE course_id = ?', (course_id,))
                    total_lessons = cursor.fetchone()[0]
                    cursor.execute(''' INSERT INTO course_enrollments ( course_id, user_id, user_name, education_type, enroll_time, progress_percent, last_learn_time, completed_lessons, total_lessons, status, created_at ) VALUES (?, ?, ?, ?, ?, 0, ?, 0, ?, 'active', ?) ''', (course_id, user_id, kwargs.get('user_name'),
    ...
    def list_user_courses(self, user_id: str, page: int = 1,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT e.*, c.course_name, c.course_type, c.subject, c.difficulty, c.cover_image, c.total_hours FROM course_enrollments e LEFT JOIN online_courses c ON e.course_id = c.course_id WHERE e.user_id = ? '''
                params = [user_id]
                cursor.execute(f'SELECT COUNT(*) as cnt FROM ({query})', params)
                total = cursor.fetchone()['cnt']
