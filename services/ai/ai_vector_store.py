...
class AIVectorStore:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_vector_collections ( collection_id TEXT PRIMARY KEY, name TEXT NOT NULL, dim INTEGER NOT NULL, metric TEXT DEFAULT 'cosine', description TEXT, index_type TEXT DEFAULT 'flat', item_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_vector_items ( item_id TEXT PRIMARY KEY, collection_id TEXT NOT NULL, vector TEXT NOT NULL, document TEXT, metadata TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_vector_query_log ( log_id TEXT PRIMARY KEY, collection_id TEXT, query_text TEXT, top_k INTEGER, metric TEXT, duration_ms INTEGER, result_count INTEGER, created_at TEXT ) ''')
    ...
    def create_collection(self, name: str, dim: int = None, metric: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' INSERT INTO ai_vector_collections (collection_id, name, dim, metric, description, index_type, item_count, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?) ''', (collection_id, name, dim, metric, description, index_type,
    ...
    def list_collections(self) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT collection_id, name, dim, metric, description, index_type, item_count, created_at FROM ai_vector_collections ORDER BY created_at DESC ''')
                return [
                    {
    ...
    def add(self, collection_id: str, vector: List[float], document: str = '',
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT OR REPLACE INTO ai_vector_items (item_id, collection_id, vector, document, metadata, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (
            ...
            with self._get_connection() as conn:
                ...
                if coll['index_type'] == 'lsh' and collection_id in self._lsh_indexes:
                    ...
                    if candidate_ids:
                        ...
                        cursor.execute(f''' SELECT item_id, vector, document, metadata FROM ai_vector_items WHERE collection_id = ? AND item_id IN ({placeholders}) ''', [collection_id] + candidate_ids)
                    else:
                ...
                cursor.execute(''' INSERT INTO ai_vector_query_log (log_id, collection_id, query_text, top_k, metric, duration_ms, result_count, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, collection_id, '', top_k, metric, duration_ms, len(results),
