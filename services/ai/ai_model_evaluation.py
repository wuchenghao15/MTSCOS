...
class AIModelEvaluation:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_model_evaluations ( eval_id TEXT PRIMARY KEY, model_id TEXT NOT NULL, eval_type TEXT, metrics TEXT, status TEXT DEFAULT 'completed', created_at TEXT, notes TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_test_cases ( case_id TEXT PRIMARY KEY, name TEXT, category TEXT, input_data TEXT, expected_output TEXT, status TEXT DEFAULT 'active', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_test_runs ( run_id TEXT PRIMARY KEY, eval_id TEXT, case_id TEXT, actual_output TEXT, passed INTEGER, error_message TEXT, duration_ms INTEGER, executed_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_ab_experiments ( experiment_id TEXT PRIMARY KEY, name TEXT, model_a TEXT, model_b TEXT, traffic_split INTEGER DEFAULT 50, status TEXT DEFAULT 'running', metrics_a TEXT, metrics_b TEXT, winner TEXT, started_at TEXT, ended_at TEXT ) ''')
            ...
            with self._get_connection() as conn:
                ...
                for case in DEFAULT_TEST_CASES:
                    ...
                    if not cursor.fetchone():
                        cursor.execute(''' INSERT INTO ai_test_cases (case_id, name, category, input_data, expected_output, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def evaluate(self, model_id: str, predictions: List[Any], ground_truth: List[Any],
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO ai_model_evaluations (eval_id, model_id, eval_type, metrics, status, created_at, notes) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def run_regression_test(self, model_id: str, predict_fn=None) -> Dict:
        """运行回归测试套件  predict_fn: 可调用对象，接收 input_data，返回 prediction dict """
        ...
        for case in cases:
            ...
            try:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ai_test_runs (run_id, eval_id, case_id, actual_output, passed, error_message, duration_ms, executed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def create_ab_experiment(self, name: str, model_a: str, model_b: str,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO ai_ab_experiments (experiment_id, name, model_a, model_b, traffic_split, status, started_at) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def update_ab_metrics(self, experiment_id: str, model: str, metrics: Dict) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f''' UPDATE ai_ab_experiments SET {field} = ? WHERE experiment_id = ? ''', (json.dumps(metrics, ensure_ascii=False), experiment_id))
                conn.commit()
            return {'success': True}
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def conclude_ab_experiment(self, experiment_id: str) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' UPDATE ai_ab_experiments SET status = 'completed', winner = ?, ended_at = ? WHERE experiment_id = ? ''', (winner, datetime.now().isoformat(), experiment_id))
                conn.commit()
    ...
    def add_test_case(self, case_id: str, name: str, category: str,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT OR REPLACE INTO ai_test_cases (case_id, name, category, input_data, expected_output, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def get_model_history(self, model_id: str, limit: int = 20) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT eval_id, eval_type, metrics, created_at, notes FROM ai_model_evaluations WHERE model_id = ? ORDER BY created_at DESC LIMIT ? ''', (model_id, limit))
                return [
                    {
