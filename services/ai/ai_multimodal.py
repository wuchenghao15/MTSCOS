...
MODAL_ENCODERS = {
    ...
    'image': lambda data, dim=128: encode_image_from_pixels(data, dim) if isinstance(data,
    list) else encode_image_features(data.get('width', 224), data.get('height', 224), data.get('channels', 3), dim),
    'audio': lambda data, dim=128: encode_audio_features(data.get('duration', 1.0), data.get('sample_rate', 44100),
    dim) if isinstance(data, dict) else encode_audio_features(float(data), dim=dim),
}
