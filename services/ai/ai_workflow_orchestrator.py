...
DEFAULT_WORKFLOWS = [
    {
        ...
        'nodes': [
            ...
            {'id': 'clean', 'name': '数据清洗', 'type': 'transform', 'params': {'mapping': {'raw': 'cleaned'}},
            'next': ['branch']},
            {'id': 'branch', 'name': '质量检查', 'type': 'condition', 'params': {'field': 'cleaned', 'operator': '!=',
            'value': None}, 'next': ['analyze', 'notify']},
            {'id': 'analyze', 'name': '数据分析', 'type': 'log', 'params': {'message': '执行分析'}, 'next': ['output']},
            {'id': 'output', 'name': '结果输出', 'type': 'notify', 'params': {'channel': 'log', 'message': '输出结果'},
            'next': []},
            {'id': 'notify', 'name': '异常通知', 'type': 'notify', 'params': {'channel': 'alert', 'message': '数据质量异常'},
            'next': []},
        ],
        'edges': [
    ...
    },
    {
        ...
        'nodes': [
            ...
            {'id': 'grade', 'name': '告警分级', 'type': 'condition', 'params': {'field': 'severity', 'operator': '>=',
            'value': 3}, 'next': ['urgent', 'normal']},
            {'id': 'urgent', 'name': '紧急响应', 'type': 'notify', 'params': {'channel': 'sms', 'message': '紧急告警'},
            'next': ['recover']},
            {'id': 'normal', 'name': '常规响应', 'type': 'notify', 'params': {'channel': 'email', 'message': '常规告警'},
            'next': ['recover']},
    ...
    },
    {
        ...
        'nodes': [
            ...
            {'id': 'merge', 'name': '汇总结果', 'type': 'aggregate', 'params': {'keys': ['infer', 'load']},
            'next': ['report']},
            {'id': 'report', 'name': '生成报告', 'type': 'notify', 'params': {'channel': 'log', 'message': '报告已生成'},
            'next': []},
        ],
        'edges': [
            {'from': 'load', 'to': 'infer'},
...
class AIWorkflowOrchestrator:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_workflows ( workflow_id TEXT PRIMARY KEY, name TEXT NOT NULL, description TEXT, version TEXT DEFAULT '1.0.0', definition TEXT NOT NULL, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_workflow_instances ( instance_id TEXT PRIMARY KEY, workflow_id TEXT NOT NULL, status TEXT DEFAULT 'pending', context TEXT, current_node TEXT, started_at TEXT, completed_at TEXT, error_message TEXT, triggered_by TEXT, retry_count INTEGER DEFAULT 0 ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_workflow_node_history ( id INTEGER PRIMARY KEY AUTOINCREMENT, instance_id TEXT NOT NULL, node_id TEXT NOT NULL, node_name TEXT, status TEXT, started_at TEXT, completed_at TEXT, duration_ms INTEGER, input_data TEXT, output_data TEXT, error_message TEXT, retry_count INTEGER DEFAULT 0 ) ''')
            ...
            with self._get_connection() as conn:
                ...
                for wf in DEFAULT_WORKFLOWS:
                    ...
                    if not cursor.fetchone():
                        cursor.execute(''' INSERT INTO ai_workflows (workflow_id, name, description, version, definition, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def create_workflow(self, workflow_id: str, name: str, nodes: List[Dict],
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT OR REPLACE INTO ai_workflows (workflow_id, name, description, version, definition, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def trigger(self, workflow_id: str, initial_context: Optional[Dict] = None,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO ai_workflow_instances (instance_id, workflow_id, status, context, current_node, started_at, triggered_by) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def _record_node_history(self, instance_id: str, node: Dict, status: str,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO ai_workflow_node_history (instance_id, node_id, node_name, status, started_at, completed_at, duration_ms, input_data, output_data, error_message) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def _update_instance_status(self, instance_id: str, status: str, error: Optional[str]):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE ai_workflow_instances SET status = ?, completed_at = ?, error_message = ? WHERE instance_id = ? ''', (status, datetime.now().isoformat(), error, instance_id))
                conn.commit()
        except Exception as e:
    ...
    def _update_instance_progress(self, instance_id: str, node_id: str, context: Dict):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE ai_workflow_instances SET current_node = ?, context = ? WHERE instance_id = ? ''', (node_id, json.dumps(context, ensure_ascii=False), instance_id))
                conn.commit()
        except Exception as e:
    ...
    def get_instance_history(self, instance_id: str) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT node_id, node_name, status, started_at, completed_at, duration_ms, error_message FROM ai_workflow_node_history WHERE instance_id = ? ORDER BY id ASC ''', (instance_id,))
                return [
