...
class AIExperimentTracker:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS experiments ( experiment_id TEXT PRIMARY KEY, experiment_name TEXT NOT NULL, project_name TEXT DEFAULT 'default', status TEXT DEFAULT 'created', hyperparameters TEXT, metrics TEXT, tags TEXT, notes TEXT, parent_id TEXT, model_artifact TEXT, start_time TEXT, end_time TEXT, duration_sec REAL, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS experiment_metrics_log ( log_id INTEGER PRIMARY KEY AUTOINCREMENT, experiment_id TEXT NOT NULL, step INTEGER, metric_name TEXT, metric_value REAL, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS model_registry ( model_id TEXT PRIMARY KEY, model_name TEXT NOT NULL, version TEXT, experiment_id TEXT, metrics TEXT, artifact_path TEXT, status TEXT DEFAULT 'registered', tags TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS hyperparameter_searches ( search_id TEXT PRIMARY KEY, search_name TEXT, method TEXT, param_space TEXT, n_trials INTEGER, best_score REAL, best_params TEXT, status TEXT DEFAULT 'running', created_at TEXT, completed_at TEXT ) ''')
    ...
    def create_experiment(self, experiment_name: str,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO experiments (experiment_id, experiment_name, project_name, status, hyperparameters, metrics, tags, notes, parent_id, start_time, created_at) VALUES (?, ?, ?, 'running', ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def log_metric(self, experiment_id: str, metric_name: str,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO experiment_metrics_log (experiment_id, step, metric_name, metric_value, created_at) VALUES (?, ?, ?, ?, ?) ''', (
                ...
                if row:
                    ...
                    cursor.execute(''' UPDATE experiments SET metrics = ? WHERE experiment_id = ? ''', (json.dumps(metrics, ensure_ascii=False), experiment_id))
        ...
        try:
            with self._get_connection() as conn:
                ...
                if final_metrics:
                    ...
                    cursor.execute(''' UPDATE experiments SET status = 'completed', end_time = ?, duration_sec = ?, metrics = ?, model_artifact = ? WHERE experiment_id = ? ''', (
                        end_time.isoformat(), duration,
                        json.dumps(metrics, ensure_ascii=False),
                        model_artifact, experiment_id
                    ))
                else:
                    cursor.execute(''' UPDATE experiments SET status = 'completed', end_time = ?, duration_sec = ? WHERE experiment_id = ? ''', (end_time.isoformat(), duration, experiment_id))
    ...
    def fail_experiment(self, experiment_id: str, error: str = '') -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE experiments SET status = 'failed', end_time = ?, notes = COALESCE(notes, '') || ? WHERE experiment_id = ? ''', (datetime.now().isoformat(), f'\n错误: {error}', experiment_id))
                conn.commit()
            return {'success': True}
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def get_metric_history(self, experiment_id: str, metric_name: str = None,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                if metric_name:
                    cursor.execute(''' SELECT step, metric_name, metric_value, created_at FROM experiment_metrics_log WHERE experiment_id = ? AND metric_name = ? ORDER BY step LIMIT ? ''', (experiment_id, metric_name, limit))
                else:
                    cursor.execute(''' SELECT step, metric_name, metric_value, created_at FROM experiment_metrics_log WHERE experiment_id = ? ORDER BY step LIMIT ? ''', (experiment_id, limit))
                return [
                    {
    ...
    def run_hyperparameter_search(self, search_name: str, method: str,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO hyperparameter_searches (search_id, search_name, method, param_space, n_trials, status, created_at) VALUES (?, ?, ?, ?, ?, 'running', ?) ''', (
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE hyperparameter_searches SET status = 'completed', best_score = ?, best_params = ?, completed_at = ? WHERE search_id = ? ''', (
                    best_score,
    ...
    def register_model(self, model_name: str, version: str,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO model_registry (model_id, model_name, version, experiment_id, metrics, artifact_path, tags, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def list_models(self, model_name: str = None, limit: int = 20) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                if model_name:
                    cursor.execute(''' SELECT model_id, model_name, version, metrics, status, tags, created_at FROM model_registry WHERE model_name = ? ORDER BY created_at DESC LIMIT ? ''', (model_name, limit))
                else:
                    cursor.execute(''' SELECT model_id, model_name, version, metrics, status, tags, created_at FROM model_registry ORDER BY created_at DESC LIMIT ? ''', (limit,))
                return [
                    {
