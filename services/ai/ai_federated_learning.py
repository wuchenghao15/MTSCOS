...
def fedavg(weights_list: List[Dict], sample_counts: List[int] = None) -> Dict:
    """FedAvg：按样本数加权平均  weights_list: 每个客户端的模型权重字典 {layer_name: [values]} sample_counts: 每个客户端的训练样本数 """
    if not weights_list:
        return {}
...
class AIFederatedLearning:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS fl_clients ( client_id TEXT PRIMARY KEY, client_name TEXT, client_type TEXT DEFAULT 'standard', status TEXT DEFAULT 'inactive', data_size INTEGER DEFAULT 0, data_distribution TEXT, compute_capacity REAL DEFAULT 1.0, trust_score REAL DEFAULT 0.5, total_rounds INTEGER DEFAULT 0, success_rounds INTEGER DEFAULT 0, last_seen TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS fl_tasks ( task_id TEXT PRIMARY KEY, task_name TEXT NOT NULL, model_name TEXT, aggregation_method TEXT DEFAULT 'fedavg', target_rounds INTEGER DEFAULT 10, current_round INTEGER DEFAULT 0, min_clients INTEGER DEFAULT 2, max_clients INTEGER DEFAULT 100, status TEXT DEFAULT 'created', global_model TEXT, dp_enabled INTEGER DEFAULT 0, dp_method TEXT, dp_epsilon REAL, created_at TEXT, completed_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS fl_rounds ( round_id TEXT PRIMARY KEY, task_id TEXT NOT NULL, round_number INTEGER NOT NULL, status TEXT DEFAULT 'in_progress', participating_clients INTEGER DEFAULT 0, aggregated_model TEXT, accuracy REAL, loss REAL, created_at TEXT, completed_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS fl_client_updates ( update_id TEXT PRIMARY KEY, task_id TEXT, round_id TEXT, client_id TEXT, round_number INTEGER, model_weights TEXT, sample_count INTEGER, accuracy REAL, loss REAL, training_time_sec INTEGER, status TEXT DEFAULT 'submitted', submitted_at TEXT ) ''')
    ...
    def register_client(self, client_id: str, client_name: str = '',
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' INSERT INTO fl_clients (client_id, client_name, client_type, status, data_size, compute_capacity, trust_score, created_at) VALUES (?, ?, ?, 'inactive', ?, ?, 0.5, ?) ''', (
    ...
    def update_client_status(self, client_id: str, status: str,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f''' UPDATE fl_clients SET {', '.join(updates)} WHERE client_id = ? ''', values)
                conn.commit()
                return {'success': True}
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def update_trust_score(self, client_id: str, delta: float) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE fl_clients SET trust_score = MAX(0.0, MIN(1.0, trust_score + ?)) WHERE client_id = ? ''', (delta, client_id))
                conn.commit()
                return {'success': True}
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def create_task(self, task_name: str, model_name: str = 'default_model',
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO fl_tasks (task_id, task_name, model_name, aggregation_method, target_rounds, min_clients, status, global_model, dp_enabled, dp_method, dp_epsilon, created_at) VALUES (?, ?, ?, ?, ?, ?, 'created', ?, ?, ?, ?, ?) ''', (
    ...
    def start_task(self, task_id: str) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE fl_tasks SET status = 'running', current_round = 1 WHERE task_id = ? ''', (task_id,))
                conn.commit()
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def list_tasks(self) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT task_id, task_name, status, current_round, target_rounds, created_at FROM fl_tasks ORDER BY created_at DESC ''')
                return [
                    {
    ...
    def _start_round(self, task_id: str, round_number: int) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO fl_rounds (round_id, task_id, round_number, status, created_at) VALUES (?, ?, ?, 'in_progress', ?) ''', (round_id, task_id, round_number, datetime.now().isoformat()))
                conn.commit()
    ...
    def submit_update(self, task_id: str, client_id: str, model_weights: Dict,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO fl_client_updates (update_id, task_id, client_id, round_number, model_weights, sample_count, accuracy, loss, training_time_sec, status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'submitted', ?) ''', (
                ...
                # 更新客户端统计
                cursor.execute(''' UPDATE fl_clients SET total_rounds = total_rounds + 1, success_rounds = success_rounds + 1, last_seen = ? WHERE client_id = ? ''', (datetime.now().isoformat(), client_id))
    ...
    def aggregate_and_next_round(self, task_id: str) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT client_id, model_weights, sample_count, accuracy, loss FROM fl_client_updates WHERE task_id = ? AND round_number = ? AND status = 'submitted' ''', (task_id, current_round))
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE fl_rounds SET status = 'completed', participating_clients = ?, aggregated_model = ?, accuracy = ?, loss = ?, completed_at = ? WHERE task_id = ? AND round_number = ? ''', (
                    len(updates),
                ...
                if next_round <= task['target_rounds']:
                    cursor.execute(''' UPDATE fl_tasks SET global_model = ?, current_round = ? WHERE task_id = ? ''', (json.dumps(aggregated, ensure_ascii=False), next_round, task_id))
                    # 启动下一轮
                    self._start_round(task_id, next_round)
                else:
                    # 任务完成
                    cursor.execute(''' UPDATE fl_tasks SET global_model = ?, status = 'completed', completed_at = ? WHERE task_id = ? ''', (json.dumps(aggregated, ensure_ascii=False),
    ...
    def detect_anomalous_clients(self, task_id: str, round_number: int = None) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT client_id, accuracy, loss, model_weights, sample_count FROM fl_client_updates WHERE task_id = ? AND round_number = ? AND status = 'submitted' ''', (task_id, round_number))
                updates = cursor.fetchall()
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def get_task_stats(self, task_id: str) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT round_number, participating_clients, accuracy, loss, status FROM fl_rounds WHERE task_id = ? ORDER BY round_number ''', (task_id,))
                rounds = [
                    {
                        'round': r[0],
                        'clients': r[1],
                ...
                cursor.execute(''' SELECT COUNT(DISTINCT client_id) FROM fl_client_updates WHERE task_id = ? ''', (task_id,))
                total_clients = cursor.fetchone()[0]

                cursor.execute(''' SELECT COUNT(*) FROM fl_client_updates WHERE task_id = ? ''', (task_id,))
