        ...
        for i, name in enumerate(feature_names):
            if abs(best_counterfactual[i] - instance[i]) > 1e-6:
                changes.append({
                    ...
                    'change_percent': round((best_counterfactual[i] - instance[i]) / max(abs(instance[i]), 1e-10) * 100,
                    2)
                })
...
def shap_approximate(predict_fn, instance: List[float], feature_names: List[str],
    ...
    for feat_idx in range(n_features):
        ...
        for _ in range(n_samples):
            ...
            val_without = float(pred_without) if isinstance(pred_without, (int,
            float)) else (1.0 if pred_without else 0.0)
