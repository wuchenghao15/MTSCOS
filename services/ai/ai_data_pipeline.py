...
class AIDataPipeline:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_data_pipelines ( pipeline_id TEXT PRIMARY KEY, name TEXT NOT NULL, description TEXT, steps TEXT NOT NULL, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_data_pipeline_runs ( run_id TEXT PRIMARY KEY, pipeline_id TEXT, input_data TEXT, output_data TEXT, step_results TEXT, status TEXT, duration_ms INTEGER, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_pipeline(self, name: str, steps: List[Dict],
        ...
        """创建预处理管道  steps 格式: [ {'processor': 'clean_missing', 'params': {'strategy': 'mean'}}, {'processor': 'normalize_minmax', 'params': {}} ] """
        # 校验步骤
        for step in steps:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO ai_data_pipelines (pipeline_id, name, description, steps, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) ''', (
        ...
        try:
            ...
            try:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO ai_data_pipeline_runs (run_id, pipeline_id, input_data, output_data, step_results, status, duration_ms, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def list_runs(self, limit: int = 20) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT run_id, pipeline_id, status, duration_ms, created_at FROM ai_data_pipeline_runs ORDER BY created_at DESC LIMIT ? ''', (limit,))
                return [
                    {
