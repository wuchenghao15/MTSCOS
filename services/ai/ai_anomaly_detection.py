...
class AIAnomalyDetection:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_anomaly_detectors ( detector_id TEXT PRIMARY KEY, name TEXT NOT NULL, metric TEXT NOT NULL, method TEXT NOT NULL, params TEXT, enabled INTEGER DEFAULT 1, description TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_anomaly_events ( event_id TEXT PRIMARY KEY, detector_id TEXT, metric TEXT, method TEXT, anomaly_data TEXT, severity TEXT DEFAULT 'medium', status TEXT DEFAULT 'open', created_at TEXT, resolved_at TEXT, resolved_by TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_anomaly_baselines ( baseline_id TEXT PRIMARY KEY, metric TEXT NOT NULL, baseline_value REAL, std_value REAL, sample_count INTEGER, updated_at TEXT ) ''')
            ...
            with self._get_connection() as conn:
                ...
                for det in DEFAULT_DETECTOR_CONFIGS:
                    ...
                    if not cursor.fetchone():
                        cursor.execute(''' INSERT INTO ai_anomaly_detectors (detector_id, name, metric, method, params, enabled, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def add_detector(self, detector_id: str, name: str, metric: str, method: str,
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT OR REPLACE INTO ai_anomaly_detectors (detector_id, name, metric, method, params, enabled, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
        ...
        try:
            with self._get_connection() as conn:
                ...
                if result['anomaly_count'] > 0:
                    ...
                    cursor.execute(''' INSERT INTO ai_anomaly_events (event_id, detector_id, metric, method, anomaly_data, severity, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def update_baseline(self, metric: str, values: List[float]) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT OR REPLACE INTO ai_anomaly_baselines (baseline_id, metric, baseline_value, std_value, sample_count, updated_at) VALUES (?, ?, ?, ?, ?, ?) ''', (baseline_id, metric, round(m, 4), round(s, 4), len(values),
    ...
    def resolve_event(self, event_id: str, resolved_by: str = 'system') -> Dict:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE ai_anomaly_events SET status = 'resolved', resolved_at = ?, resolved_by = ? WHERE event_id = ? AND status = 'open' ''', (datetime.now().isoformat(), resolved_by, event_id))
                conn.commit()
    ...
    def list_events(self, status: Optional[str] = None, limit: int = 50) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                if status:
                    cursor.execute(''' SELECT event_id, detector_id, metric, method, anomaly_data, severity, status, created_at FROM ai_anomaly_events WHERE status = ? ORDER BY created_at DESC LIMIT ? ''', (status, limit))
                else:
                    cursor.execute(''' SELECT event_id, detector_id, metric, method, anomaly_data, severity, status, created_at FROM ai_anomaly_events ORDER BY created_at DESC LIMIT ? ''', (limit,))
                return [
                    {
