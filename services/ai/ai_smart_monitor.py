...
class AISmartMonitor:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_monitor_services ( service_id TEXT PRIMARY KEY, name TEXT NOT NULL, category TEXT, critical INTEGER DEFAULT 0, status TEXT DEFAULT 'unknown', last_check TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_monitor_metrics ( id INTEGER PRIMARY KEY AUTOINCREMENT, service_id TEXT, metric_name TEXT, metric_value REAL, metric_unit TEXT, tags TEXT, timestamp TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_monitor_alerts ( alert_id TEXT PRIMARY KEY, rule_id TEXT, service_id TEXT, metric_name TEXT, metric_value REAL, severity TEXT, message TEXT, status TEXT DEFAULT 'open', created_at TEXT, resolved_at TEXT, resolved_by TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_monitor_health_logs ( log_id TEXT PRIMARY KEY, health_score REAL, status TEXT, details TEXT, created_at TEXT ) ''')
            ...
            with self._get_connection() as conn:
                ...
                for svc in MONITORED_SERVICES:
                    ...
                    if not cursor.fetchone():
                        cursor.execute(''' INSERT INTO ai_monitor_services (service_id, name, category, critical, status, last_check, created_at) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (
        ...
        try:
            with self._get_connection() as conn:
                ...
                for m in self._metrics_buffer:
                    cursor.execute(''' INSERT INTO ai_monitor_metrics (service_id, metric_name, metric_value, metric_unit, tags, timestamp) VALUES (?, ?, ?, ?, ?, ?) ''', (m['service_id'], m['metric_name'], m['metric_value'],
    ...
    def check_service(self, service_id: str) -> Dict:
        ...
        try:
            ...
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE ai_monitor_services SET status = ?, last_check = ? WHERE service_id = ? ''', (status, datetime.now().isoformat(), service_id))
                conn.commit()
        ...
        for rule in ALERT_RULES:
            ...
            if rule['metric'] in system_metrics:
                ...
                try:
                    with self._get_connection() as conn:
                        cursor = conn.cursor()
                        cursor.execute(''' SELECT metric_value FROM ai_monitor_metrics WHERE metric_name = ? ORDER BY timestamp DESC LIMIT 1 ''', (rule['metric'],))
                        row = cursor.fetchone()
                        if row:
                            metric_value = row[0]
                except Exception:
            ...
            if triggered:
                ...
                try:
                    with self._get_connection() as conn:
                        cursor = conn.cursor()
                        cursor.execute(''' INSERT INTO ai_monitor_alerts (alert_id, rule_id, service_id, metric_name, metric_value, severity, message, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def compute_health_score(self) -> Dict:
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO ai_monitor_health_logs (log_id, health_score, status, details, created_at) VALUES (?, ?, ?, ?, ?) ''', (log_id, round(health_score, 2), status,
    ...
    def list_alerts(self, status: Optional[str] = None, limit: int = 50) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                if status:
                    cursor.execute(''' SELECT alert_id, rule_id, service_id, metric_name, metric_value, severity, message, status, created_at FROM ai_monitor_alerts WHERE status = ? ORDER BY created_at DESC LIMIT ? ''', (status, limit))
                else:
                    cursor.execute(''' SELECT alert_id, rule_id, service_id, metric_name, metric_value, severity, message, status, created_at FROM ai_monitor_alerts ORDER BY created_at DESC LIMIT ? ''', (limit,))
                return [
                    {
    ...
    def resolve_alert(self, alert_id: str, resolved_by: str = 'system') -> Dict:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' UPDATE ai_monitor_alerts SET status = 'resolved', resolved_at = ?, resolved_by = ? WHERE alert_id = ? AND status = 'open' ''', (datetime.now().isoformat(), resolved_by, alert_id))
                conn.commit()
    ...
    def get_health_history(self, limit: int = 24) -> List[Dict]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT log_id, health_score, status, created_at FROM ai_monitor_health_logs ORDER BY created_at DESC LIMIT ? ''', (limit,))
                return [
