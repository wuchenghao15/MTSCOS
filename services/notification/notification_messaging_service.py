...
class NotificationMessagingService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS notifications ( notification_id TEXT PRIMARY KEY, title TEXT NOT NULL, content TEXT NOT NULL, notification_type TEXT DEFAULT 'system', priority INTEGER DEFAULT 2, education_type TEXT, target_scope TEXT DEFAULT 'all', target_data TEXT, sender_id INTEGER, sender_name TEXT, is_pinned INTEGER DEFAULT 0, is_published INTEGER DEFAULT 0, publish_time TEXT, expire_time TEXT, read_count INTEGER DEFAULT 0, total_recipients INTEGER DEFAULT 0, attachments TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS notification_reads ( id INTEGER PRIMARY KEY AUTOINCREMENT, notification_id TEXT NOT NULL, user_id INTEGER NOT NULL, read_at TEXT, UNIQUE(notification_id, user_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS messages ( message_id TEXT PRIMARY KEY, sender_id INTEGER NOT NULL, sender_name TEXT, receiver_id INTEGER, receiver_type TEXT DEFAULT 'user', group_id TEXT, subject TEXT, content TEXT NOT NULL, message_type TEXT DEFAULT 'text', parent_id TEXT, education_type TEXT, category TEXT, is_read INTEGER DEFAULT 0, read_at TEXT, is_starred INTEGER DEFAULT 0, is_archived INTEGER DEFAULT 0, attachments TEXT, sent_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS message_groups ( group_id TEXT PRIMARY KEY, group_name TEXT NOT NULL, group_type TEXT DEFAULT 'normal', creator_id INTEGER, member_count INTEGER DEFAULT 0, max_members INTEGER DEFAULT 100, description TEXT, avatar_url TEXT, is_muted INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS message_group_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, group_id TEXT NOT NULL, user_id INTEGER NOT NULL, role TEXT DEFAULT 'member', joined_at TEXT, last_read_message_id TEXT, unread_count INTEGER DEFAULT 0, is_muted INTEGER DEFAULT 0, UNIQUE(group_id, user_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS message_templates ( template_id TEXT PRIMARY KEY, template_name TEXT NOT NULL, template_type TEXT, subject TEXT, content TEXT NOT NULL, variables TEXT, category TEXT, created_by INTEGER, is_active INTEGER DEFAULT 1, usage_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS home_school_messages ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, teacher_id INTEGER NOT NULL, parent_id INTEGER, direction TEXT DEFAULT 'to_parent', category TEXT, subject TEXT, content TEXT NOT NULL, is_read INTEGER DEFAULT 0, read_at TEXT, teacher_name TEXT, parent_name TEXT, attachments TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS push_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, notification_id TEXT, message_id TEXT, user_id INTEGER NOT NULL, channel TEXT, status TEXT DEFAULT 'pending', retry_count INTEGER DEFAULT 0, error_message TEXT, sent_at TEXT, delivered_at TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_notification(self, title: str, content: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            target_data = json.dumps(kwargs.get('target_data'),
            ensure_ascii=False) if kwargs.get('target_data') else None
            attachments = json.dumps(kwargs.get('attachments'),
            ensure_ascii=False) if kwargs.get('attachments') else None
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO notifications ( notification_id, title, content, notification_type, priority, education_type, target_scope, target_data, sender_id, sender_name, is_pinned, is_published, publish_time, expire_time, total_recipients, attachments, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def publish_notification(self, notification_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE notifications SET is_published = 1, publish_time = ?, updated_at = ? WHERE notification_id = ? AND is_published = 0 ''', (now, now, notification_id))
    ...
    def list_notifications(self, education_type: str = None, notification_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'notifications': notifications, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def read_notification(self, notification_id: str, user_id: int) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR IGNORE INTO notification_reads (notification_id, user_id, read_at) VALUES (?, ?, ?) ''', (notification_id, user_id, now))
                    if cursor.rowcount > 0:
                        cursor.execute(''' UPDATE notifications SET read_count = read_count + 1 WHERE notification_id = ? ''', (notification_id,))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_unread_notification_count(self, user_id: int, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                query = ''' SELECT COUNT(*) FROM notifications n WHERE n.is_published = 1 AND (n.expire_time IS NULL OR n.expire_time > ?) AND NOT EXISTS ( SELECT 1 FROM notification_reads nr WHERE nr.notification_id = n.notification_id AND nr.user_id = ? ) '''
    ...
    def send_message(self, sender_id: int, content: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO messages ( message_id, sender_id, sender_name, receiver_id, receiver_type, group_id, subject, content, message_type, parent_id, education_type, category, is_read, attachments, sent_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?) ''', (
                    ...
                    if kwargs.get('group_id'):
                        cursor.execute(''' UPDATE message_group_members SET unread_count = unread_count + 1 WHERE group_id = ? AND user_id != ? ''', (kwargs['group_id'], sender_id))
                    conn.commit()
    ...
    def read_message(self, message_id: str, user_id: int) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE messages SET is_read = 1, read_at = ? WHERE message_id = ? AND receiver_id = ? AND is_read = 0 ''', (now, message_id, user_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_unread_message_count(self, user_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT SUM(unread_count) FROM message_group_members WHERE user_id = ? ''', (user_id,))
                group_count = cursor.fetchone()[0] or 0
                return {'success': True, 'unread_direct': direct_count, 'unread_group': group_count,
                'total_unread': direct_count + group_count}
        except Exception as e:
    ...
    def create_message_group(self, group_name: str, creator_id: int, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO message_groups ( group_id, group_name, group_type, creator_id, member_count, max_members, description, avatar_url, is_muted, created_at, updated_at ) VALUES (?, ?, ?, ?, 1, ?, ?, ?, 0, ?, ?) ''', (group_id, group_name, kwargs.get('group_type', 'normal'),
                          creator_id, kwargs.get('max_members', 100),
                          kwargs.get('description'), kwargs.get('avatar_url'), now, now))
                    cursor.execute(''' INSERT INTO message_group_members (group_id, user_id, role, joined_at, unread_count, is_muted) VALUES (?, ?, 'admin', ?, 0, 0) ''', (group_id, creator_id, now))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO message_group_members (group_id, user_id, role, joined_at, unread_count, is_muted) VALUES (?, ?, ?, ?, 0, 0) ''', (group_id, user_id, role, now))
                    if cursor.rowcount > 0:
    ...
    def send_home_school_message(self, student_id: int, teacher_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO home_school_messages ( student_id, teacher_id, parent_id, direction, category, subject, content, is_read, teacher_name, parent_name, attachments, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?) ''', (
    ...
    def create_template(self, template_name: str, content: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO message_templates ( template_id, template_name, template_type, subject, content, variables, category, created_by, is_active, usage_count, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 0, ?, ?) ''', (template_id, template_name, kwargs.get('template_type'),
    ...
    def use_template(self, template_id: str, variables: dict = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('UPDATE message_templates SET usage_count = usage_count + 1 WHERE template_id = ?',
                (template_id,))
                conn.commit()
    ...
    def star_message(self, message_id: str, user_id: int) -> Dict[str, Any]:
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE messages SET is_starred = 1 WHERE message_id = ? AND (receiver_id = ? OR sender_id = ?) ''', (message_id, user_id, user_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_notification_stats(self, notification_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('SELECT total_recipients, read_count FROM notifications WHERE notification_id = ?',
                (notification_id,))
                row = cursor.fetchone()
                if not row:
