    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS decision_analysis ( analysis_id TEXT PRIMARY KEY, decision_type TEXT NOT NULL, education_type TEXT, title TEXT NOT NULL, description TEXT, status TEXT DEFAULT 'pending', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS analysis_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, analysis_id TEXT NOT NULL, data_source TEXT, analysis_method TEXT, result_data TEXT, conclusion TEXT, confidence REAL, recorded_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS prediction_models ( model_id TEXT PRIMARY KEY, model_name TEXT NOT NULL, model_type TEXT, education_type TEXT, description TEXT, accuracy REAL, trained_at TEXT, status TEXT DEFAULT 'active', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS prediction_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, model_id TEXT NOT NULL, prediction_type TEXT, input_data TEXT, prediction_result TEXT, confidence REAL, actual_result TEXT, prediction_date TEXT, recorded_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS risk_assessment ( assessment_id TEXT PRIMARY KEY, risk_type TEXT NOT NULL, education_type TEXT, title TEXT NOT NULL, description TEXT, risk_level TEXT, impact_score REAL, probability REAL, status TEXT DEFAULT 'assessing', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS assessment_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, assessment_id TEXT NOT NULL, factor_name TEXT, factor_score REAL, mitigation_strategy TEXT, responsible_party TEXT, deadline TEXT, recorded_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS optimization_suggestions ( suggestion_id TEXT PRIMARY KEY, optimization_type TEXT NOT NULL, education_type TEXT, title TEXT NOT NULL, description TEXT, optimization_method TEXT, expected_improvement REAL, status TEXT DEFAULT 'proposed', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS suggestion_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, suggestion_id TEXT NOT NULL, implementation_step TEXT, resource_requirement TEXT, estimated_cost REAL, timeline TEXT, responsible_party TEXT, status TEXT DEFAULT 'pending', recorded_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS decision_support ( support_id TEXT PRIMARY KEY, decision_id TEXT, education_type TEXT, title TEXT NOT NULL, data_summary TEXT, options_analysis TEXT, recommendation TEXT, confidence_level REAL, status TEXT DEFAULT 'supporting', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS support_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, support_id TEXT NOT NULL, support_type TEXT, data_source TEXT, analysis_result TEXT, supporting_evidence TEXT, recorded_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS intelligent_recommendation ( recommendation_id TEXT PRIMARY KEY, recommendation_type TEXT NOT NULL, education_type TEXT, target_user_id INTEGER, target_user_name TEXT, recommended_items TEXT, algorithm_used TEXT, confidence REAL, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS recommendation_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, recommendation_id TEXT NOT NULL, item_id TEXT, item_name TEXT, relevance_score REAL, user_feedback TEXT, feedback_score INTEGER, recorded_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS data_analysis ( analysis_id TEXT PRIMARY KEY, data_type TEXT NOT NULL, education_type TEXT, title TEXT NOT NULL, description TEXT, data_source TEXT, analysis_method TEXT, status TEXT DEFAULT 'analyzing', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS analysis_data ( id INTEGER PRIMARY KEY AUTOINCREMENT, analysis_id TEXT NOT NULL, raw_data TEXT, processed_data TEXT, key_insights TEXT, metrics TEXT, recorded_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS visualization_display ( display_id TEXT PRIMARY KEY, visualization_type TEXT NOT NULL, education_type TEXT, title TEXT NOT NULL, data_source TEXT, chart_config TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS display_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, display_id TEXT NOT NULL, view_count INTEGER DEFAULT 0, export_count INTEGER DEFAULT 0, last_viewed_at TEXT, recorded_at TEXT ) ''')
                conn.commit()
    ...
    def create_decision_analysis(self, decision_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO decision_analysis ( analysis_id, decision_type, education_type, title, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'pending', ?, ?) ''', (analysis_id, decision_type, education_type,
    ...
    def execute_analysis(self, analysis_id: str, data_source: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT decision_type, education_type FROM decision_analysis WHERE analysis_id = ?',
                    (analysis_id,))
                    analysis = cursor.fetchone()
                    if not analysis:
                        return {'success': False, 'error': '决策分析不存在'}
                    result_data = json.dumps(kwargs.get('result_data', {}), ensure_ascii=False)
                    cursor.execute(''' INSERT INTO analysis_records ( analysis_id, data_source, analysis_method, result_data, conclusion, confidence, recorded_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (analysis_id, data_source, analysis_method,
                          result_data, kwargs.get('conclusion'),
                          kwargs.get('confidence', 0.85), now))
                    cursor.execute('UPDATE decision_analysis SET status = ?, updated_at = ? WHERE analysis_id = ?',
    ...
    def get_analysis_results(self, analysis_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not analysis:
                    return {'success': False, 'error': '决策分析不存在'}
                cursor.execute('SELECT * FROM analysis_records WHERE analysis_id = ? ORDER BY recorded_at DESC',
                (analysis_id,))
    ...
    def create_prediction_model(self, model_name: str, model_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO prediction_models ( model_id, model_name, model_type, education_type, description, accuracy, trained_at, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?) ''', (model_id, model_name, model_type, education_type,
                          kwargs.get('description'),
                          kwargs.get('accuracy', float(config.get('accuracy', '中').replace('高', '0.8').replace('中',
                          '0.6').replace('很', ''))),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO prediction_records ( model_id, prediction_type, input_data, prediction_result, confidence, prediction_date, recorded_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (model_id, prediction_type,
    ...
    def get_prediction_history(self, model_id: str = None,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT pr.*, pm.model_name, pm.model_type, pm.education_type FROM prediction_records pr JOIN prediction_models pm ON pr.model_id = pm.model_id WHERE 1=1 '''
                params = []
                if model_id:
    ...
    def create_risk_assessment(self, risk_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO risk_assessment ( assessment_id, risk_type, education_type, title, description, risk_level, impact_score, probability, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'assessing', ?, ?) ''', (assessment_id, risk_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO assessment_records ( assessment_id, factor_name, factor_score, mitigation_strategy, responsible_party, deadline, recorded_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (assessment_id, factor_name, factor_score,
    ...
    def create_optimization_suggestion(self, optimization_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO optimization_suggestions ( suggestion_id, optimization_type, education_type, title, description, optimization_method, expected_improvement, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'proposed', ?, ?) ''', (suggestion_id, optimization_type, education_type,
                          title, kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO suggestion_records ( suggestion_id, implementation_step, resource_requirement, estimated_cost, timeline, responsible_party, status, recorded_at ) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?) ''', (suggestion_id, implementation_step,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute(
                        'UPDATE suggestion_records SET status = ? WHERE suggestion_id = ? AND status = ?', ('completed',
    ...
    def get_suggestion_details(self, suggestion_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not suggestion:
                    return {'success': False, 'error': '优化建议不存在'}
                cursor.execute('SELECT * FROM suggestion_records WHERE suggestion_id = ? ORDER BY recorded_at',
                (suggestion_id,))
                records = [dict(r) for r in cursor.fetchall()]
    ...
    def create_decision_support(self, decision_id: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO decision_support ( support_id, decision_id, education_type, title, data_summary, options_analysis, recommendation, confidence_level, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'supporting', ?, ?) ''', (support_id, decision_id, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO support_records ( support_id, support_type, data_source, analysis_result, supporting_evidence, recorded_at ) VALUES (?, ?, ?, ?, ?, ?) ''', (support_id, support_type, data_source,
    ...
    def generate_recommendation(self, recommendation_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO intelligent_recommendation ( recommendation_id, recommendation_type, education_type, target_user_id, target_user_name, recommended_items, algorithm_used, confidence, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (recommendation_id, recommendation_type, education_type,
    ...
    def add_recommendation_item(self, recommendation_id: str, item_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT * FROM intelligent_recommendation WHERE recommendation_id = ?',
                    (recommendation_id,))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '推荐不存在'}
                    cursor.execute(''' INSERT INTO recommendation_records ( recommendation_id, item_id, item_name, relevance_score, user_feedback, feedback_score, recorded_at ) VALUES (?, ?, ?, ?, NULL, NULL, ?) ''', (recommendation_id, item_id, item_name, relevance_score, now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def create_data_analysis(self, data_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO data_analysis ( analysis_id, data_type, education_type, title, description, data_source, analysis_method, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'analyzing', ?, ?) ''', (analysis_id, data_type, education_type,
                          title, kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO analysis_data ( analysis_id, raw_data, processed_data, key_insights, metrics, recorded_at ) VALUES (?, ?, ?, ?, ?, ?) ''', (analysis_id, json.dumps(raw_data, ensure_ascii=False),
    ...
    def get_data_analysis_results(self, analysis_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not analysis:
                    return {'success': False, 'error': '数据分析不存在'}
                cursor.execute('SELECT * FROM analysis_data WHERE analysis_id = ? ORDER BY recorded_at DESC',
                (analysis_id,))
    ...
    def create_visualization(self, visualization_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO visualization_display ( display_id, visualization_type, education_type, title, data_source, chart_config, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (display_id, visualization_type, education_type,
