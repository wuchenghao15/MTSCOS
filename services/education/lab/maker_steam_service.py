    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                # STEAM课程表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS steam_courses ( course_id TEXT PRIMARY KEY, course_name TEXT NOT NULL, course_type TEXT NOT NULL, discipline TEXT, education_type TEXT, grade_level TEXT, teacher_id TEXT, teacher_name TEXT, description TEXT, objectives TEXT, duration_hours INTEGER DEFAULT 32, difficulty TEXT DEFAULT 'intermediate', max_students INTEGER DEFAULT 30, enrolled_count INTEGER DEFAULT 0, materials_needed TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                # 课程课时表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_lessons_steam ( lesson_id TEXT PRIMARY KEY, course_id TEXT NOT NULL, lesson_name TEXT NOT NULL, lesson_order INTEGER, duration_minutes INTEGER DEFAULT 45, content TEXT, activity_type TEXT, materials TEXT, created_at TEXT ) ''')
                # 课程报名表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS steam_enrollments ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, student_id TEXT NOT NULL, student_name TEXT, enroll_date TEXT, attendance_count INTEGER DEFAULT 0, project_count INTEGER DEFAULT 0, final_score REAL, grade TEXT, status TEXT DEFAULT 'enrolled', created_at TEXT, UNIQUE(course_id, student_id) ) ''')
                # 创客空间表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS maker_spaces ( space_id TEXT PRIMARY KEY, space_name TEXT NOT NULL, space_type TEXT, location TEXT, area REAL, capacity INTEGER DEFAULT 30, manager_id TEXT, manager_name TEXT, open_hours TEXT, safety_rules TEXT, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT ) ''')
                # 空间设备表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS space_equipment ( equipment_id TEXT PRIMARY KEY, space_id TEXT NOT NULL, equipment_name TEXT NOT NULL, equipment_type TEXT, brand TEXT, model TEXT, quantity INTEGER DEFAULT 1, available_quantity INTEGER DEFAULT 1, status TEXT DEFAULT 'operational', purchase_date TEXT, last_maintenance TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 设备预约表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS equipment_bookings ( id INTEGER PRIMARY KEY AUTOINCREMENT, equipment_id TEXT NOT NULL, space_id TEXT, user_id TEXT NOT NULL, user_name TEXT, booking_date TEXT, start_time TEXT, end_time TEXT, purpose TEXT, status TEXT DEFAULT 'pending', created_at TEXT ) ''')
                # 材料库存表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS materials_inventory ( material_id TEXT PRIMARY KEY, space_id TEXT, material_name TEXT NOT NULL, category TEXT, unit TEXT DEFAULT '件', quantity REAL DEFAULT 0, min_quantity REAL DEFAULT 10, unit_price REAL DEFAULT 0, supplier TEXT, last_restock TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 材料使用记录表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS material_usage ( id INTEGER PRIMARY KEY AUTOINCREMENT, material_id TEXT NOT NULL, space_id TEXT, user_id TEXT, user_name TEXT, quantity_used REAL, project_id TEXT, usage_date TEXT, purpose TEXT, created_at TEXT ) ''')
                # 创新项目表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS innovation_projects ( project_id TEXT PRIMARY KEY, project_name TEXT NOT NULL, project_type TEXT, discipline TEXT, description TEXT, objectives TEXT, leader_id TEXT, leader_name TEXT, members TEXT, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'planning', budget REAL DEFAULT 0, space_id TEXT, outcome TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 项目里程碑表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_milestones ( milestone_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, milestone_name TEXT NOT NULL, target_date TEXT, achieved_date TEXT, description TEXT, status TEXT DEFAULT 'pending', created_at TEXT ) ''')
                # 项目成果表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_outcomes ( outcome_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, outcome_type TEXT, title TEXT NOT NULL, description TEXT, file_url TEXT, rating REAL DEFAULT 0, created_at TEXT ) ''')
                # 机器人项目表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS robot_projects ( robot_id TEXT PRIMARY KEY, project_id TEXT, robot_type TEXT, robot_name TEXT NOT NULL, controller TEXT, sensors TEXT, actuators TEXT, program_language TEXT, description TEXT, performance_score REAL DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                # 3D打印任务表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS print_jobs ( job_id TEXT PRIMARY KEY, user_id TEXT NOT NULL, user_name TEXT, file_name TEXT NOT NULL, file_url TEXT, material TEXT, color TEXT, quality TEXT DEFAULT 'standard', infill_percent INTEGER DEFAULT 20, estimated_time INTEGER, actual_time INTEGER, status TEXT DEFAULT 'queued', printer_id TEXT, created_at TEXT, completed_at TEXT ) ''')
                # STEAM竞赛表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS steam_competitions ( competition_id TEXT PRIMARY KEY, competition_name TEXT NOT NULL, competition_type TEXT, level TEXT, organizer TEXT, description TEXT, start_date TEXT, end_date TEXT, registration_deadline TEXT, max_participants INTEGER DEFAULT 50, registered_count INTEGER DEFAULT 0, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT ) ''')
                # 竞赛获奖表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS competition_awards ( award_id TEXT PRIMARY KEY, competition_id TEXT NOT NULL, project_id TEXT, user_id TEXT, user_name TEXT, award_level TEXT, award_category TEXT, certificate_url TEXT, description TEXT, awarded_date TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_course(self, course_name: str, course_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO steam_courses ( course_id, course_name, course_type, discipline, education_type, grade_level, teacher_id, teacher_name, description, objectives, duration_hours, difficulty, max_students, enrolled_count, materials_needed, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'active', ?, ?) ''', (course_id, course_name, course_type, discipline,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO course_lessons_steam ( lesson_id, course_id, lesson_name, lesson_order, duration_minutes, content, activity_type, materials, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (lesson_id, course_id, lesson_name, order,
    ...
    def enroll_course(self, course_id: str, student_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT max_students, enrolled_count, status FROM steam_courses WHERE course_id = ?',
                    (course_id,))
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO steam_enrollments ( course_id, student_id, student_name, enroll_date, status, created_at ) VALUES (?, ?, ?, ?, 'enrolled', ?) ''', (course_id, student_id, kwargs.get('student_name'),
                          now[:10], now))
                ...
                for key in ('discipline', 'objectives', 'materials_needed'):
                    if course.get(key):
                        try:
                            ...
                            pass
                cursor.execute('SELECT * FROM course_lessons_steam WHERE course_id = ? ORDER BY lesson_order ASC',
                (course_id,))
    ...
    def register_maker_space(self, space_name: str, space_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO maker_spaces ( space_id, space_name, space_type, location, area, capacity, manager_id, manager_name, open_hours, safety_rules, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?) ''', (space_id, space_name, space_type,
    ...
    def add_equipment(self, space_id: str, equipment_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO space_equipment ( equipment_id, space_id, equipment_name, equipment_type, brand, model, quantity, available_quantity, status, purchase_date, last_maintenance, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'operational', ?, ?, ?, ?) ''', (equipment_id, space_id, equipment_name, equipment_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO equipment_bookings ( equipment_id, space_id, user_id, user_name, booking_date, start_time, end_time, purpose, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'approved', ?) ''', (equipment_id, equip[2], user_id,
    ...
    def add_material(self, space_id: str, material_name: str, category: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO materials_inventory ( material_id, space_id, material_name, category, unit, quantity, min_quantity, unit_price, supplier, last_restock, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (material_id, space_id, material_name, category,
                          kwargs.get('unit', '件'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO material_usage ( material_id, space_id, user_id, user_name, quantity_used, project_id, usage_date, purpose, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (material_id, mat[2], user_id, kwargs.get('user_name'),
    ...
    def create_project(self, project_name: str, project_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO innovation_projects ( project_id, project_name, project_type, discipline, description, objectives, leader_id, leader_name, members, start_date, end_date, status, budget, space_id, outcome, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'planning', ?, ?, NULL, ?, ?) ''', (project_id, project_name, project_type, discipline,
    ...
    def add_project_member(self, project_id: str, user_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT members, project_type FROM innovation_projects WHERE project_id = ?',
                    (project_id,))
                    row = cursor.fetchone()
                    if not row:
    ...
    def create_milestone(self, project_id: str, milestone_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO project_milestones ( milestone_id, project_id, milestone_name, target_date, achieved_date, description, status, created_at ) VALUES (?, ?, ?, ?, NULL, ?, 'pending', ?) ''', (milestone_id, project_id, milestone_name,
    ...
    def complete_milestone(self, milestone_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE project_milestones SET status = 'achieved', achieved_date = ? WHERE milestone_id = ? AND status = 'pending' ''', (kwargs.get('achieved_date', now[:10]), milestone_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def record_outcome(self, project_id: str, outcome_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO project_outcomes ( outcome_id, project_id, outcome_type, title, description, file_url, rating, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (outcome_id, project_id, outcome_type, title,
                ...
                for key in ('discipline', 'members'):
                    if project.get(key):
                        try:
                            ...
                            pass
                cursor.execute('SELECT * FROM project_milestones WHERE project_id = ? ORDER BY target_date ASC',
                (project_id,))
                project['milestones'] = [dict(m) for m in cursor.fetchall()]
                cursor.execute('SELECT * FROM project_outcomes WHERE project_id = ? ORDER BY created_at DESC',
                (project_id,))
    ...
    def create_robot_project(self, project_id: str, robot_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO robot_projects ( robot_id, project_id, robot_type, robot_name, controller, sensors, actuators, program_language, description, performance_score, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (robot_id, project_id, robot_type,
    ...
    def submit_print_job(self, user_id: str, file_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO print_jobs ( job_id, user_id, user_name, file_name, file_url, material, color, quality, infill_percent, estimated_time, actual_time, status, printer_id, created_at, completed_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 'queued', ?, ?, NULL) ''', (job_id, user_id, kwargs.get('user_name'), file_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    if status == 'completed':
                        cursor.execute(''' UPDATE print_jobs SET status = ?, actual_time = ?, completed_at = ? WHERE job_id = ? ''', (status, kwargs.get('actual_time'), now, job_id))
                    else:
    ...
    def create_competition(self, competition_name: str, competition_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO steam_competitions ( competition_id, competition_name, competition_type, level, organizer, description, start_date, end_date, registration_deadline, max_participants, registered_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'open', ?, ?) ''', (competition_id, competition_name, competition_type,
    ...
    def record_award(self, competition_id: str, project_id: str, user_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO competition_awards ( award_id, competition_id, project_id, user_id, user_name, award_level, award_category, certificate_url, description, awarded_date, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (award_id, competition_id, project_id, user_id,
