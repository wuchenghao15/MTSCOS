...
class LabExperimentService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS laboratories ( lab_id TEXT PRIMARY KEY, lab_name TEXT NOT NULL, lab_type TEXT NOT NULL, location TEXT, building TEXT, floor INTEGER, room_number TEXT, area REAL, capacity INTEGER DEFAULT 30, manager_id INTEGER, manager_name TEXT, safety_level INTEGER DEFAULT 1, safety_rules TEXT, open_time TEXT, close_time TEXT, is_available INTEGER DEFAULT 1, description TEXT, photo_url TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS lab_reservations ( reservation_id TEXT PRIMARY KEY, lab_id TEXT NOT NULL, lab_name TEXT, reserved_by INTEGER, reserved_by_name TEXT, course_name TEXT, experiment_name TEXT, reserve_date TEXT, start_time TEXT, end_time TEXT, student_count INTEGER DEFAULT 0, purpose TEXT, status TEXT DEFAULT 'pending', approved_by INTEGER, approved_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS experiments ( experiment_id TEXT PRIMARY KEY, experiment_name TEXT NOT NULL, experiment_type TEXT, subject TEXT, lab_type TEXT, education_type TEXT, grade_level INTEGER, description TEXT, objectives TEXT, materials TEXT, procedure TEXT, safety_notes TEXT, duration_minutes INTEGER DEFAULT 90, group_size INTEGER DEFAULT 2, difficulty TEXT DEFAULT 'medium', is_virtual INTEGER DEFAULT 0, virtual_url TEXT, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS experiment_sessions ( session_id TEXT PRIMARY KEY, experiment_id TEXT NOT NULL, experiment_name TEXT, lab_id TEXT, lab_name TEXT, teacher_id INTEGER, teacher_name TEXT, class_id TEXT, class_name TEXT, session_date TEXT, start_time TEXT, end_time TEXT, student_count INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', notes TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS lab_equipment ( equipment_id TEXT PRIMARY KEY, equipment_name TEXT NOT NULL, equipment_code TEXT UNIQUE, category TEXT, lab_id TEXT, lab_name TEXT, model TEXT, manufacturer TEXT, purchase_date TEXT, purchase_price REAL, status TEXT DEFAULT 'normal', location TEXT, last_maintenance TEXT, next_maintenance TEXT, description TEXT, photo_url TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS equipment_borrows ( borrow_id TEXT PRIMARY KEY, equipment_id TEXT NOT NULL, equipment_name TEXT, borrower_id INTEGER, borrower_name TEXT, borrow_date TEXT, expected_return TEXT, actual_return TEXT, purpose TEXT, status TEXT DEFAULT 'borrowed', condition_on_borrow TEXT, condition_on_return TEXT, approved_by INTEGER, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS consumables ( consumable_id TEXT PRIMARY KEY, name TEXT NOT NULL, category TEXT, lab_id TEXT, lab_name TEXT, unit TEXT, total_quantity INTEGER DEFAULT 0, available_quantity INTEGER DEFAULT 0, warning_threshold INTEGER DEFAULT 10, unit_price REAL, expiry_date TEXT, storage_location TEXT, supplier TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS consumable_transactions ( transaction_id TEXT PRIMARY KEY, consumable_id TEXT NOT NULL, consumable_name TEXT, transaction_type TEXT, quantity INTEGER, operator_id INTEGER, operator_name TEXT, transaction_date TEXT, purpose TEXT, related_session TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS safety_inspections ( inspection_id TEXT PRIMARY KEY, lab_id TEXT NOT NULL, lab_name TEXT, inspection_type TEXT, inspector TEXT, inspection_date TEXT, items_checked TEXT, issues_found TEXT, severity TEXT, status TEXT DEFAULT 'open', rectification TEXT, rectified_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS safety_incidents ( incident_id TEXT PRIMARY KEY, lab_id TEXT, lab_name TEXT, incident_type TEXT, severity TEXT, description TEXT, occurred_at TEXT, reported_by TEXT, reported_at TEXT, affected_persons TEXT, cause_analysis TEXT, measures TEXT, status TEXT DEFAULT 'reported', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS experiment_reports ( report_id TEXT PRIMARY KEY, session_id TEXT NOT NULL, experiment_id TEXT NOT NULL, experiment_name TEXT, student_id INTEGER NOT NULL, student_name TEXT, group_members TEXT, hypothesis TEXT, procedure_detail TEXT, data_record TEXT, analysis TEXT, conclusion TEXT, reflection TEXT, file_url TEXT, status TEXT DEFAULT 'draft', submitted_at TEXT, graded_by INTEGER, graded_at TEXT, score REAL, feedback TEXT, created_at TEXT, updated_at TEXT ) ''')
                conn.commit()
    ...
    def create_lab(self, lab_name: str, lab_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO laboratories ( lab_id, lab_name, lab_type, location, building, floor, room_number, area, capacity, manager_id, manager_name, safety_level, safety_rules, open_time, close_time, is_available, description, photo_url, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?) ''', (lab_id, lab_name, lab_type, kwargs.get('location'),
    ...
    def reserve_lab(self, lab_id: str, reserved_by: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT lab_name, is_available, safety_level FROM laboratories WHERE lab_id = ?',
                    (lab_id,))
                    lab = cursor.fetchone()
                    if not lab:
                    ...
                    cursor.execute(''' SELECT COUNT(*) FROM lab_reservations WHERE lab_id = ? AND reserve_date = ? AND ((start_time < ? AND end_time > ?) OR (start_time < ? AND end_time > ?)) AND status IN ('pending', 'approved') ''', (lab_id, reserve_date, end_time, start_time, end_time, start_time))
                    if cursor.fetchone()[0] > 0:
                        return {'success': False, 'error': '该时段已被预约'}
                    status = 'approved' if lab[2] <= 1 else 'pending'
                    cursor.execute(''' INSERT INTO lab_reservations ( reservation_id, lab_id, lab_name, reserved_by, reserved_by_name, course_name, experiment_name, reserve_date, start_time, end_time, student_count, purpose, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (reservation_id, lab_id, lab[0], reserved_by,
    ...
    def approve_reservation(self, reservation_id: str, approved: bool,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE lab_reservations SET status = ?, approved_by = ?, approved_at = ? WHERE reservation_id = ? AND status = 'pending' ''', (status, approved_by, now, reservation_id))
    ...
    def create_experiment(self, experiment_name: str, experiment_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO experiments ( experiment_id, experiment_name, experiment_type, subject, lab_type, education_type, grade_level, description, objectives, materials, procedure, safety_notes, duration_minutes, group_size, difficulty, is_virtual, virtual_url, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (experiment_id, experiment_name, experiment_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not lab:
                        return {'success': False, 'error': '实验室不存在'}
                    cursor.execute(''' INSERT INTO experiment_sessions ( session_id, experiment_id, experiment_name, lab_id, lab_name, teacher_id, teacher_name, class_id, class_name, session_date, start_time, end_time, student_count, status, notes, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'scheduled', ?, ?, ?) ''', (session_id, experiment_id, exp[0],
    ...
    def add_equipment(self, equipment_name: str, category: str,
        ...
        try:
            ...
            f"EQ{datetime.now().strftime('%Y%m%d')}{uuid.uuid4().hex[:4].upper()}")
            now = datetime.now().isoformat()
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    ...
                    lab = cursor.fetchone()
                    cursor.execute(''' INSERT INTO lab_equipment ( equipment_id, equipment_name, equipment_code, category, lab_id, lab_name, model, manufacturer, purchase_date, purchase_price, status, location, last_maintenance, next_maintenance, description, photo_url, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'normal', ?, ?, ?, ?, ?, ?, ?) ''', (equipment_id, equipment_name, equipment_code, category,
    ...
    def borrow_equipment(self, equipment_id: str, borrower_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT equipment_name, status FROM lab_equipment WHERE equipment_id = ?',
                    (equipment_id,))
                    eq = cursor.fetchone()
                    if not eq:
                    ...
                    ('borrowed', now, equipment_id))
                    cursor.execute(''' INSERT INTO equipment_borrows ( borrow_id, equipment_id, equipment_name, borrower_id, borrower_name, borrow_date, expected_return, purpose, status, condition_on_borrow, approved_by, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'borrowed', ?, ?, ?) ''', (borrow_id, equipment_id, eq[0], borrower_id,
    ...
    def return_equipment(self, borrow_id: str, condition: str = '正常',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT equipment_id FROM equipment_borrows WHERE borrow_id = ? AND status = ?',
                    (borrow_id, 'borrowed'))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT lab_name FROM laboratories WHERE lab_id = ?', (kwargs.get('lab_id'),))
                    lab = cursor.fetchone()
                    cursor.execute(''' INSERT INTO consumables ( consumable_id, name, category, lab_id, lab_name, unit, total_quantity, available_quantity, warning_threshold, unit_price, expiry_date, storage_location, supplier, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (consumable_id, name, category, kwargs.get('lab_id'),
    ...
    def _consumable_transaction(self, consumable_id: str, quantity: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT name, available_quantity FROM consumables WHERE consumable_id = ?',
                    (consumable_id,))
                    c = cursor.fetchone()
                    if not c:
                    ...
                    cursor.execute(''' INSERT INTO consumable_transactions ( transaction_id, consumable_id, consumable_name, transaction_type, quantity, operator_id, operator_name, transaction_date, purpose, related_session, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (transaction_id, consumable_id, c[0], txn_type, quantity,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not lab:
                        return {'success': False, 'error': '实验室不存在'}
                    items_checked = json.dumps(kwargs.get('items_checked'),
                    ensure_ascii=False) if kwargs.get('items_checked') else None
                    issues_found = json.dumps(kwargs.get('issues_found'),
                    ensure_ascii=False) if kwargs.get('issues_found') else None
                    cursor.execute(''' INSERT INTO safety_inspections ( inspection_id, lab_id, lab_name, inspection_type, inspector, inspection_date, items_checked, issues_found, severity, status, rectification, rectified_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?, ?) ''', (inspection_id, lab_id, lab[0], inspection_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO safety_incidents ( incident_id, lab_id, lab_name, incident_type, severity, description, occurred_at, reported_by, reported_at, affected_persons, cause_analysis, measures, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'reported', ?) ''', (incident_id, lab_id, lab_name, incident_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT experiment_name FROM experiments WHERE experiment_id = ?', (experiment_id,))
                    exp = cursor.fetchone()
                    group_members = json.dumps(kwargs.get('group_members'),
                    ensure_ascii=False) if kwargs.get('group_members') else None
                    cursor.execute(''' INSERT INTO experiment_reports ( report_id, session_id, experiment_id, experiment_name, student_id, student_name, group_members, hypothesis, procedure_detail, data_record, analysis, conclusion, reflection, file_url, status, submitted_at, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'submitted', ?, ?, ?) ''', (report_id, session_id, experiment_id,
    ...
    def grade_report(self, report_id: str, graded_by: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE experiment_reports SET status = 'graded', graded_by = ?, graded_at = ?, score = ?, feedback = ?, updated_at = ? WHERE report_id = ? AND status IN ('submitted', 'reviewing') ''', (graded_by, now, score,
