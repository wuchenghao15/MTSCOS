        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    f"TX{datetime.now().strftime('%Y%m%d%H%M%S')}{uuid.uuid4().hex[:4].upper()}")
                    cursor.execute('''
    ...
    def get_financial_stats(self, semester: str, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(query, params)
                row = cursor.fetchone()
                cursor.execute('SELECT COALESCE(SUM(refund_amount), 0) FROM refund_records WHERE status = ?',
                ('approved',))
                total_refunded = cursor.fetchone()[0]
                cursor.execute('SELECT COALESCE(SUM(amount), 0) FROM scholarships WHERE status = ? AND semester = ?',
                ('awarded', semester))
    ...
    def get_student_financial_summary(self, student_id: int, semester: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(query, params)
                row = cursor.fetchone()
                cursor.execute('SELECT COALESCE(SUM(amount), 0) FROM scholarships WHERE student_id = ? AND status = ?',
                (student_id, 'awarded'))
