...
class LibraryService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS books ( book_id TEXT PRIMARY KEY, isbn TEXT, title TEXT NOT NULL, subtitle TEXT, author TEXT, publisher TEXT, publish_date TEXT, category TEXT, book_type TEXT, education_type TEXT, grade_level INTEGER, total_pages INTEGER, total_copies INTEGER DEFAULT 1, available_copies INTEGER DEFAULT 1, current_borrowed INTEGER DEFAULT 0, shelf_location TEXT, price REAL DEFAULT 0, language TEXT DEFAULT '中文', summary TEXT, tags TEXT, cover_url TEXT, times_borrowed INTEGER DEFAULT 0, times_reserved INTEGER DEFAULT 0, rating REAL DEFAULT 0, rating_count INTEGER DEFAULT 0, status TEXT DEFAULT 'available', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS borrow_records ( borrow_id TEXT PRIMARY KEY, book_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, borrow_date TEXT NOT NULL, due_date TEXT NOT NULL, return_date TEXT, actual_days INTEGER, overdue_days INTEGER DEFAULT 0, fine_amount REAL DEFAULT 0, fine_paid INTEGER DEFAULT 0, status TEXT DEFAULT 'borrowed', renewed_times INTEGER DEFAULT 0, operator_borrow TEXT, operator_return TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS book_reservations ( reservation_id TEXT PRIMARY KEY, book_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, reservation_date TEXT NOT NULL, available_date TEXT, expire_date TEXT, status TEXT DEFAULT 'pending', picked_up INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS reader_profiles ( id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER UNIQUE NOT NULL, reader_name TEXT, level INTEGER DEFAULT 1, credit_score INTEGER DEFAULT 100, total_borrowed INTEGER DEFAULT 0, total_returned INTEGER DEFAULT 0, total_overdue INTEGER DEFAULT 0, total_lost INTEGER DEFAULT 0, current_borrowed INTEGER DEFAULT 0, current_reserved INTEGER DEFAULT 0, joined_date TEXT, last_visit TEXT, favorite_categories TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS book_reviews ( id INTEGER PRIMARY KEY AUTOINCREMENT, book_id TEXT NOT NULL, user_id INTEGER NOT NULL, rating INTEGER NOT NULL, review TEXT, is_visible INTEGER DEFAULT 1, likes INTEGER DEFAULT 0, created_at TEXT, UNIQUE(book_id, user_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS book_recommendations ( id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, book_id TEXT NOT NULL, reason TEXT, score REAL DEFAULT 0, source TEXT, is_shown INTEGER DEFAULT 0, created_at TEXT, UNIQUE(user_id, book_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS book_purchase_requests ( request_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, book_title TEXT NOT NULL, author TEXT, isbn TEXT, publisher TEXT, reason TEXT, status TEXT DEFAULT 'pending', reviewer_id INTEGER, review_comment TEXT, reviewed_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS reading_statistics ( id INTEGER PRIMARY KEY AUTOINCREMENT, stat_date TEXT NOT NULL, education_type TEXT, total_borrow_count INTEGER DEFAULT 0, total_return_count INTEGER DEFAULT 0, total_reserve_count INTEGER DEFAULT 0, most_borrowed_category TEXT, most_popular_book TEXT, active_readers INTEGER DEFAULT 0, updated_at TEXT, UNIQUE(stat_date, education_type) ) ''')
                conn.commit()
    ...
    def add_book(self, title: str, author: str, category: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO books ( book_id, isbn, title, subtitle, author, publisher, publish_date, category, book_type, education_type, grade_level, total_pages, total_copies, available_copies, shelf_location, price, language, summary, tags, cover_url, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'available', ?, ?) ''', (book_id, kwargs.get('isbn'), title, kwargs.get('subtitle'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO borrow_records ( borrow_id, book_id, user_id, user_name, borrow_date, due_date, status, renewed_times, operator_borrow, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'borrowed', 0, ?, ?) ''', (borrow_id, book_id, user_id, kwargs.get('user_name'),
                          borrow_date, due_date, kwargs.get('operator_name'), borrow_date))
                    cursor.execute(''' UPDATE books SET available_copies = available_copies - 1, current_borrowed = current_borrowed + 1, times_borrowed = times_borrowed + 1, updated_at = ? WHERE book_id = ? AND available_copies > 0 ''', (now.isoformat(), book_id))
                    cursor.execute(''' INSERT INTO reader_profiles (user_id, reader_name, current_borrowed, total_borrowed, created_at, updated_at) VALUES (?, ?, 1, 1, ?, ?) ON CONFLICT(user_id) DO UPDATE SET current_borrowed = current_borrowed + 1, total_borrowed = total_borrowed + 1, last_visit = ?, updated_at = ? ''', (user_id, kwargs.get('user_name', ''), borrow_date, borrow_date, borrow_date, borrow_date))
                    conn.commit()
    ...
    def _init_reader(self, cursor, user_id: int, user_name: str = None):
        now = datetime.now().isoformat()
        cursor.execute(''' INSERT INTO reader_profiles (user_id, reader_name, level, credit_score, current_borrowed, total_borrowed, joined_date, last_visit, created_at, updated_at) VALUES (?, ?, 1, 100, 0, 0, ?, ?, ?, ?) ''', (user_id, user_name or '', now, now, now, now))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    fine_amount = min(overdue_days * OVERDUE_FINE_RULE['per_day'],
                    OVERDUE_FINE_RULE['max_amount']) if overdue_days > 0 else 0
                    status = 'overdue' if overdue_days > 0 else 'returned'
                    cursor.execute(''' UPDATE borrow_records SET return_date = ?, actual_days = ?, overdue_days = ?, fine_amount = ?, status = ?, operator_return = ? WHERE borrow_id = ? ''', (now.isoformat(), actual_days, overdue_days, fine_amount,
                          status, kwargs.get('operator_name'), borrow_id))
                    cursor.execute(''' UPDATE books SET available_copies = available_copies + 1, current_borrowed = MAX(current_borrowed - 1, 0), updated_at = ? WHERE book_id = ? ''', (now.isoformat(), book_id))
                    cursor.execute(''' UPDATE reader_profiles SET current_borrowed = MAX(current_borrowed - 1, 0), total_returned = total_returned + 1, total_overdue = total_overdue + ?, last_visit = ?, updated_at = ? WHERE user_id = ? ''', (1 if overdue_days > 0 else 0, now.isoformat(), now.isoformat(), user_id))
                    if overdue_days > 0:
                        cursor.execute(''' UPDATE reader_profiles SET credit_score = MAX(credit_score - ?, 0) WHERE user_id = ? ''', (min(overdue_days, 10), user_id))
                    self._check_reservations(cursor, book_id)
                    conn.commit()
                    logger.info(f'归还图书: {borrow_id}, 逾期{overdue_days}天, 罚款{fine_amount}元')
                    return {'success': True, 'actual_days': actual_days, 'overdue_days': overdue_days,
    ...
    def _check_reservations(self, cursor, book_id: str):
        cursor.execute(''' SELECT reservation_id, user_id FROM book_reservations WHERE book_id = ? AND status = 'pending' ORDER BY reservation_date LIMIT 1 ''', (book_id,))
        reservation = cursor.fetchone()
        if reservation:
            now = datetime.now()
            expire_date = (now + timedelta(days=3)).isoformat()
            cursor.execute(''' UPDATE book_reservations SET status = 'available', available_date = ?, expire_date = ? WHERE reservation_id = ? ''', (now.isoformat(), expire_date, reservation[0]))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE borrow_records SET due_date = ?, renewed_times = renewed_times + 1, status = 'renewed' WHERE borrow_id = ? ''', (new_due_date, borrow_id))
                    conn.commit()
                    return {'success': True, 'new_due_date': new_due_date,
                    'remaining_renews': max_renew - renewed_times - 1}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO book_reservations ( reservation_id, book_id, user_id, user_name, reservation_date, expire_date, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?) ''', (reservation_id, book_id, user_id, kwargs.get('user_name'),
                          now, expire_date, now))
                    cursor.execute(''' UPDATE books SET times_reserved = times_reserved + 1, updated_at = ? WHERE book_id = ? ''', (now, book_id))
                    cursor.execute(''' UPDATE reader_profiles SET current_reserved = current_reserved + 1, updated_at = ? WHERE user_id = ? ''', (now, user_id))
                    conn.commit()
    ...
    def get_borrow_records(self, user_id: int = None, status: str = None,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT br.*, b.title, b.author, b.cover_url FROM borrow_records br JOIN books b ON br.book_id = b.book_id WHERE 1=1 '''
                params = []
                if user_id:
    ...
    def review_book(self, book_id: str, user_id: int, rating: int, review: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO book_reviews (book_id, user_id, rating, review, created_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(book_id, user_id) DO UPDATE SET rating = excluded.rating, review = excluded.review ''', (book_id, user_id, rating, review, now))
                    cursor.execute(''' UPDATE books SET rating = ( SELECT AVG(rating) FROM book_reviews WHERE book_id = ? AND is_visible = 1 ), rating_count = ( SELECT COUNT(*) FROM book_reviews WHERE book_id = ? AND is_visible = 1 ), updated_at = ? WHERE book_id = ? ''', (book_id, book_id, now, book_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def request_purchase(self, user_id: int, book_title: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO book_purchase_requests ( request_id, user_id, book_title, author, isbn, publisher, reason, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?) ''', (request_id, user_id, book_title, kwargs.get('author'),
        ...
        try:
            with self._get_connection() as conn:
                ...
                if user_id:
                    ...
                    cursor.execute('SELECT COUNT(*) FROM borrow_records WHERE user_id = ? AND status = ?', (user_id,
                    'borrowed'))
                    ...
                    cursor.execute('SELECT SUM( CASE WHEN status = "returned" THEN actual_days ELSE 0 END) FROM borrow_records WHERE user_id = ?',
                    (user_id,))
