    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS shared_resources ( resource_id TEXT PRIMARY KEY, title TEXT NOT NULL, resource_type TEXT NOT NULL, education_type TEXT, description TEXT, file_url TEXT, thumbnail_url TEXT, file_size INTEGER DEFAULT 0, sharing_model TEXT DEFAULT 'free', price REAL DEFAULT 0, subscription_period TEXT, author_id INTEGER, author_name TEXT, institution_id INTEGER, institution_name TEXT, category_id TEXT, tags TEXT, license_type TEXT DEFAULT 'CC-BY', status TEXT DEFAULT 'draft', download_count INTEGER DEFAULT 0, view_count INTEGER DEFAULT 0, favorite_count INTEGER DEFAULT 0, average_rating REAL DEFAULT 0, rating_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_categories ( category_id TEXT PRIMARY KEY, category_name TEXT NOT NULL, parent_id TEXT, education_type TEXT, description TEXT, sort_order INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_tags ( tag_id TEXT PRIMARY KEY, tag_name TEXT NOT NULL, education_type TEXT, usage_count INTEGER DEFAULT 0, created_at TEXT, UNIQUE(tag_name) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_versions ( version_id TEXT PRIMARY KEY, resource_id TEXT NOT NULL, version_number TEXT NOT NULL, change_log TEXT, file_url TEXT, file_size INTEGER DEFAULT 0, author_id INTEGER, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_downloads ( id INTEGER PRIMARY KEY AUTOINCREMENT, resource_id TEXT NOT NULL, user_id INTEGER, user_name TEXT, download_time TEXT, education_type TEXT, UNIQUE(resource_id, user_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_favorites ( id INTEGER PRIMARY KEY AUTOINCREMENT, resource_id TEXT NOT NULL, user_id INTEGER, user_name TEXT, favorite_time TEXT, UNIQUE(resource_id, user_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_reviews ( review_id TEXT PRIMARY KEY, resource_id TEXT NOT NULL, reviewer_id INTEGER, reviewer_name TEXT, rating INTEGER DEFAULT 0, comment TEXT, helpful_count INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS sharing_agreements ( agreement_id TEXT PRIMARY KEY, resource_id TEXT NOT NULL, user_id INTEGER, agreement_type TEXT, accepted INTEGER DEFAULT 0, accepted_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_alliances ( alliance_id TEXT PRIMARY KEY, alliance_name TEXT NOT NULL, alliance_type TEXT, description TEXT, leader_id INTEGER, leader_name TEXT, institution_id INTEGER, member_count INTEGER DEFAULT 1, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS alliance_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, alliance_id TEXT NOT NULL, institution_id INTEGER, institution_name TEXT, join_date TEXT, role TEXT DEFAULT 'member', UNIQUE(alliance_id, institution_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS collaboration_projects ( project_id TEXT PRIMARY KEY, project_name TEXT NOT NULL, collaboration_type TEXT, description TEXT, leader_id INTEGER, leader_name TEXT, institution_id INTEGER, partner_ids TEXT, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'planning', resource_count INTEGER DEFAULT 0, participant_count INTEGER DEFAULT 0, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_transactions ( transaction_id TEXT PRIMARY KEY, resource_id TEXT NOT NULL, buyer_id INTEGER, buyer_name TEXT, seller_id INTEGER, seller_name TEXT, transaction_type TEXT, amount REAL DEFAULT 0, currency TEXT DEFAULT 'CNY', status TEXT DEFAULT 'pending', payment_method TEXT, completed_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_licenses ( license_id TEXT PRIMARY KEY, resource_id TEXT NOT NULL, license_type TEXT, license_key TEXT, user_id INTEGER, valid_from TEXT, valid_until TEXT, status TEXT DEFAULT 'active', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS intellectual_property ( ip_id TEXT PRIMARY KEY, resource_id TEXT NOT NULL, ip_type TEXT, registration_number TEXT, owner_id INTEGER, owner_name TEXT, institution_id INTEGER, registration_date TEXT, expiry_date TEXT, status TEXT DEFAULT 'registered', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS shared_workspaces ( workspace_id TEXT PRIMARY KEY, workspace_name TEXT NOT NULL, description TEXT, institution_id INTEGER, owner_id INTEGER, owner_name TEXT, access_type TEXT DEFAULT 'private', member_count INTEGER DEFAULT 1, resource_count INTEGER DEFAULT 0, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS workspace_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, workspace_id TEXT NOT NULL, user_id INTEGER, user_name TEXT, role TEXT DEFAULT 'member', join_date TEXT, UNIQUE(workspace_id, user_id) ) ''')
                conn.commit()
    ...
    def publish_resource(self, title: str, resource_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO shared_resources ( resource_id, title, resource_type, education_type, description, file_url, thumbnail_url, file_size, sharing_model, price, subscription_period, author_id, author_name, institution_id, institution_name, category_id, tags, license_type, status, download_count, view_count, favorite_count, average_rating, rating_count, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, 0, 0, ?, ?) ''', (resource_id, title, resource_type,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if updates:
                        ...
                        cursor.execute(f'UPDATE shared_resources SET {", ".join(updates)} WHERE resource_id = ?',
                        params)
                        conn.commit()
                        return {'success': True}
    ...
    def create_category(self, category_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO resource_categories ( category_id, category_name, parent_id, education_type, description, sort_order, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (category_id, category_name, kwargs.get('parent_id'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not resource:
                        return {'success': False, 'error': '资源不存在'}
                    cursor.execute(''' INSERT INTO resource_versions ( version_id, resource_id, version_number, change_log, file_url, file_size, author_id, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (version_id, resource_id, version_number, change_log,
    ...
    def list_versions(self, resource_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM resource_versions WHERE resource_id = ? ORDER BY created_at DESC',
                (resource_id,))
    ...
    def rollback_version(self, resource_id: str, version_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT file_url, file_size FROM resource_versions WHERE version_id = ? AND resource_id = ?', (version_id,
                    resource_id))
    ...
    def create_alliance(self, alliance_name: str, alliance_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO resource_alliances ( alliance_id, alliance_name, alliance_type, description, leader_id, leader_name, institution_id, member_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, 'active', ?, ?) ''', (alliance_id, alliance_name, alliance_type,
    ...
    def create_collaboration_project(self, project_name: str, collaboration_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO collaboration_projects ( project_id, project_name, collaboration_type, description, leader_id, leader_name, institution_id, partner_ids, start_date, end_date, status, resource_count, participant_count, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?, ?) ''', (project_id, project_name, collaboration_type,
                          kwargs.get('description'), leader_id, leader_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO resource_transactions ( transaction_id, resource_id, buyer_id, buyer_name, seller_id, seller_name, transaction_type, amount, currency, status, payment_method, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (transaction_id, resource_id, buyer_id, buyer_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not resource:
                        return {'success': False, 'error': '资源不存在'}
                    cursor.execute(''' INSERT INTO resource_licenses ( license_id, resource_id, license_type, license_key, user_id, valid_from, valid_until, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?) ''', (license_id, resource_id, resource[0], license_key,
    ...
    def get_user_purchases(self, user_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(
                'SELECT * FROM resource_transactions WHERE buyer_id = ? AND status = "completed" ORDER BY created_at  DESC', (user_id,))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not resource:
                        return {'success': False, 'error': '资源不存在'}
                    cursor.execute(''' INSERT INTO intellectual_property ( ip_id, resource_id, ip_type, registration_number, owner_id, owner_name, institution_id, registration_date, expiry_date, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'registered', ?) ''', (ip_id, resource_id, ip_type, kwargs.get('registration_number'),
    ...
    def create_workspace(self, workspace_name: str, owner_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO shared_workspaces ( workspace_id, workspace_name, description, institution_id, owner_id, owner_name, access_type, member_count, resource_count, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, 0, ?, ?, ?) ''', (workspace_id, workspace_name, kwargs.get('description'),
    ...
    def list_workspaces(self, user_id: int = None, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                if user_id:
                    cursor.execute(
                    'SELECT w.* FROM shared_workspaces w JOIN workspace_members m ON w.workspace_id = m.workspace_id  WHERE m.user_id = ? ORDER BY w.created_at DESC', (user_id,))
                else:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT OR REPLACE INTO resource_reviews (review_id, resource_id, reviewer_id, reviewer_name, rating, comment, helpful_count, created_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?)',
                                 (review_id, resource_id, reviewer_id, reviewer_name, rating, kwargs.get('comment'),
                                 now))
    ...
    def mark_review_helpful(self, review_id: str) -> Dict[str, Any]:
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE resource_reviews SET helpful_count = helpful_count + 1 WHERE review_id = ?',
                    (review_id,))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def get_trending_resources(self, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT r.*, COUNT(d.id) as recent_downloads FROM shared_resources r LEFT JOIN resource_downloads d ON r.resource_id = d.resource_id AND d.download_time >= ? WHERE r.status = "published" '''
    ...
    def get_similar_resources(self, resource_id: str, limit: int = 5) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT resource_type, category_id, tags FROM shared_resources WHERE resource_id = ?',
                (resource_id,))
                resource = cursor.fetchone()
                if not resource:
                    return {'success': False, 'error': '资源不存在'}
