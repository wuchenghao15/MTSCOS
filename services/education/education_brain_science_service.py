...
ASSESSMENT_TYPES = {
    ...
    'learning_style': {'name': '学习风格', 'dimensions': ['visual', 'auditory', 'kinesthetic', 'reading', 'social',
    'independent', 'reflective', 'active']},
...
ATTENTION_STATES = {
    'highly_focused': {'name': '高度集中', 'description': '注意力高度集中，学习效率高', 'EEG_pattern': '高beta波、低theta波',
    'color': '#2ecc71'},
...
MEMORY_TYPES = {
    'sensory': {'name': '感觉记忆', 'duration': '<1秒', 'capacity': '大', 'encoding': '感觉信息', 'applications': ['感官训练',
    '即时感知']},
    'short_term': {'name': '短期记忆', 'duration': '<1分钟', 'capacity': '7±2', 'encoding': '听觉、视觉', 'applications': ['即时记忆',
    '临时存储']},
    'working': {'name': '工作记忆', 'duration': '<30秒', 'capacity': '4±1', 'encoding': '多模态', 'applications': ['问题解决',
    '思维操作']},
    'long_term': {'name': '长期记忆', 'duration': '无限', 'capacity': '无限', 'encoding': '语义、情景', 'applications': ['知识存储',
    '技能习得']},
    'procedural': {'name': '程序性记忆', 'duration': '无限', 'capacity': '无限', 'encoding': '动作序列', 'applications': ['技能学习',
    '习惯养成']},
...
class EducationBrainScienceService:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    conn.commit()
                    return {'success': True, 'effectiveness_id': effectiveness_id,
    ...
    def assess_brain_health(self, student_id: int, student_name: str,
        ...
        try:
            ...
            now = datetime.now().isoformat()
            scores = [health_metrics.get(k, 0) for k in ['brain_age', 'cognitive_reserve', 'sleep_quality',
            'stress_level', 'physical_activity', 'nutrition_score']]
            ...
            recommendations = self._generate_health_recommendations(risk_level, health_metrics,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    conn.commit()
                    return {'success': True, 'health_id': health_id, 'overall_health_score': round(overall, 2),
                    'risk_level': risk_level}
        except Exception as e:
            logger.error(f'评估脑健康失败: {e}')
            return {'success': False, 'error': str(e)}

    def _generate_health_recommendations(self, risk_level: str, metrics: Dict[str, float],
    education_type: str) -> List[str]:
        recommendations = []
        type_suffix = '（成人版）' if education_type == 'adult' else '（K12版）' if education_type == 'k12' else ''
        if risk_level == 'high':
    ...
    def generate_learning_insights(self, student_id: int, education_type: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(
                'SELECT * FROM assessment_results WHERE student_id = ? ORDER BY completed_at DESC LIMIT 5', (student_id,
                ))
                assessments = [dict(a) for a in cursor.fetchall()]
                cursor.execute(
                'SELECT * FROM attention_monitoring WHERE student_id = ? ORDER BY session_date DESC LIMIT 10',
                (student_id,))
                attention_data = [dict(a) for a in cursor.fetchall()]
                cursor.execute(
                'SELECT * FROM learning_effectiveness WHERE student_id = ? ORDER BY assessed_at DESC LIMIT 3',
                (student_id,))
