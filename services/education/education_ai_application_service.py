#!/usr/bin/env python3
# -*- coding: utf-8 -*-
""" MTSCOS 教育AI应用服务 (v15.17.0) =============================== 提供AI助教、AI导师、智能批改、智能组卷、智能答疑、AI写作、AI翻译、AI学习助手等综合教育服务。  核心能力： 1. AI助手 - AI助教/AI导师/AI辅导员/AI班主任/AI学习伙伴/AI作业助手/AI考试助手/AI研究助手 2. 智能批改 - 自动批改/智能批改/AI辅助批改/人工批改/混合批改 3. 智能组卷 - 单选/多选/判断/填空/简答/论述/计算/案例分析/听力/阅读 4. 智能答疑 - 精确匹配/关键词匹配/语义相似度/模糊匹配/手写识别/语音识别 5. AI写作 - 作文/论文/报告/邮件/简历/摘要/文案/创意写作 6. AI翻译 - 中文/英语/日语/韩语/法语/德语/西班牙语/俄语 7. AI学习助手 - 学习规划/进度追踪/错题分析/知识点推荐/学习提醒/考试预测/学习社区/学习档案 8. 会话管理 - 会话创建/消息记录/会话恢复/会话结束 9. 配置管理 - 助手配置/模型配置/系统配置 10. 统计分析 - 使用统计/效果评估 """
import os
import json
import uuid
import sqlite3
...
AI_MODEL_TYPES = {
    ...
    'claude3': {'name': 'Claude-3', 'provider': 'Anthropic', 'capability': 'high', 'supports_adult': True,
    'supports_k12': True},
    'qwen': {'name': 'Qwen', 'provider': '阿里云', 'capability': 'medium', 'supports_adult': True, 'supports_k12': True},
    'llama3': {'name': 'Llama-3', 'provider': 'Meta', 'capability': 'medium', 'supports_adult': True,
    'supports_k12': True},
    'gemini': {'name': 'Gemini', 'provider': 'Google', 'capability': 'high', 'supports_adult': True,
    'supports_k12': True},
    'mistral': {'name': 'Mistral', 'provider': 'Mistral AI', 'capability': 'medium', 'supports_adult': True,
    'supports_k12': True},
    'ernie': {'name': 'ERNIE', 'provider': '百度', 'capability': 'medium', 'supports_adult': True, 'supports_k12': True},
    'baichuan': {'name': 'Baichuan', 'provider': '百川智能', 'capability': 'medium', 'supports_adult': True,
    'supports_k12': True}
}
...
ANSWER_METHODS = {
    'exact_match': {'name': '精确匹配', 'description': '完全一致判定', 'suitable_for': ['single_choice', 'multiple_choice',
    'judgment']},
    'keyword_match': {'name': '关键词匹配', 'description': '关键词命中判定', 'suitable_for': ['fill_blank', 'short_answer']},
    'semantic_similarity': {'name': '语义相似度', 'description': 'AI语义分析', 'suitable_for': ['short_answer', 'essay',
    'case_analysis']},
    'fuzzy_match': {'name': '模糊匹配', 'description': '部分匹配判定', 'suitable_for': ['fill_blank']},
    'handwriting_recognition': {'name': '手写识别', 'description': 'OCR手写识别', 'suitable_for': ['fill_blank',
    'short_answer']},
...
LEARNING_ASSISTANTS = {
    ...
    'knowledge_recommendation': {'name': '知识点推荐', 'description': '智能推荐学习内容', 'supports_adult': True,
    'supports_k12': True},
...
class EducationAIApplicationService:
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_assistants ( assistant_id TEXT PRIMARY KEY, assistant_name TEXT NOT NULL, assistant_type TEXT NOT NULL, ai_model TEXT DEFAULT 'gpt4', education_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, description TEXT, avatar_url TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS assistant_config ( config_id TEXT PRIMARY KEY, assistant_id TEXT NOT NULL, config_key TEXT NOT NULL, config_value TEXT, config_type TEXT DEFAULT 'string', description TEXT, created_at TEXT, FOREIGN KEY (assistant_id) REFERENCES ai_assistants(assistant_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_sessions ( session_id TEXT PRIMARY KEY, assistant_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, subject TEXT, session_type TEXT, status TEXT DEFAULT 'active', start_time TEXT, end_time TEXT, message_count INTEGER DEFAULT 0, created_at TEXT, FOREIGN KEY (assistant_id) REFERENCES ai_assistants(assistant_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS session_messages ( message_id TEXT PRIMARY KEY, session_id TEXT NOT NULL, sender TEXT NOT NULL, content TEXT NOT NULL, message_type TEXT DEFAULT 'text', timestamp TEXT, is_read INTEGER DEFAULT 0, FOREIGN KEY (session_id) REFERENCES ai_sessions(session_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_corrections ( correction_id TEXT PRIMARY KEY, correction_type TEXT NOT NULL, education_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, title TEXT, description TEXT, status TEXT DEFAULT 'pending', created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS correction_results ( result_id TEXT PRIMARY KEY, correction_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, answer_content TEXT, correct_answer TEXT, score REAL, max_score REAL, correction_method TEXT, ai_feedback TEXT, teacher_comment TEXT, status TEXT DEFAULT 'completed', created_at TEXT, FOREIGN KEY (correction_id) REFERENCES ai_corrections(correction_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_exam_papers ( paper_id TEXT PRIMARY KEY, paper_name TEXT NOT NULL, education_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, total_score REAL DEFAULT 100, duration INTEGER DEFAULT 90, question_count INTEGER DEFAULT 0, difficulty TEXT DEFAULT 'medium', status TEXT DEFAULT 'draft', created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS paper_templates ( template_id TEXT PRIMARY KEY, template_name TEXT NOT NULL, education_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, structure TEXT, default_settings TEXT, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_qna ( qna_id TEXT PRIMARY KEY, question TEXT NOT NULL, answer TEXT NOT NULL, subject TEXT, education_type TEXT, grade_level INTEGER, tags TEXT, usage_count INTEGER DEFAULT 0, accuracy REAL DEFAULT 1.0, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS qna_history ( history_id TEXT PRIMARY KEY, qna_id TEXT, user_id INTEGER NOT NULL, user_question TEXT NOT NULL, matched_question TEXT, answer TEXT, similarity REAL, is_satisfied INTEGER DEFAULT 1, feedback TEXT, timestamp TEXT, FOREIGN KEY (qna_id) REFERENCES ai_qna(qna_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_writing ( writing_id TEXT PRIMARY KEY, writing_type TEXT NOT NULL, education_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, topic TEXT, requirements TEXT, word_count INTEGER DEFAULT 0, status TEXT DEFAULT 'draft', created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS writing_results ( result_id TEXT PRIMARY KEY, writing_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, content TEXT, ai_score REAL, ai_feedback TEXT, teacher_score REAL, teacher_comment TEXT, final_score REAL, status TEXT DEFAULT 'completed', created_at TEXT, FOREIGN KEY (writing_id) REFERENCES ai_writing(writing_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_translation ( translation_id TEXT PRIMARY KEY, source_text TEXT NOT NULL, source_language TEXT NOT NULL, target_language TEXT NOT NULL, education_type TEXT, subject TEXT, context TEXT, status TEXT DEFAULT 'pending', created_by INTEGER, created_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS translation_history ( history_id TEXT PRIMARY KEY, source_text TEXT, translated_text TEXT, source_language TEXT, target_language TEXT, user_id INTEGER, education_type TEXT, timestamp TEXT, rating INTEGER DEFAULT 5 ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_learning_plans ( plan_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, plan_name TEXT NOT NULL, start_date TEXT, end_date TEXT, total_hours REAL DEFAULT 0, completed_hours REAL DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS plan_executions ( execution_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, activity_name TEXT NOT NULL, activity_type TEXT, planned_date TEXT, completed_date TEXT, duration REAL DEFAULT 0, status TEXT DEFAULT 'pending', notes TEXT, created_at TEXT, FOREIGN KEY (plan_id) REFERENCES ai_learning_plans(plan_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS ai_study_assistant ( assistant_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, assistant_type TEXT NOT NULL, education_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, settings TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS assistant_interactions ( interaction_id TEXT PRIMARY KEY, assistant_id TEXT NOT NULL, user_id INTEGER NOT NULL, interaction_type TEXT, content TEXT, result TEXT, timestamp TEXT, FOREIGN KEY (assistant_id) REFERENCES ai_study_assistant(assistant_id) ) ''')
                    conn.commit()
    ...
    def create_assistant(self, assistant_name: str, assistant_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ai_assistants ( assistant_id, assistant_name, assistant_type, ai_model, education_type, subject, grade_level, description, avatar_url, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (assistant_id, assistant_name, assistant_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR REPLACE INTO assistant_config ( config_id, assistant_id, config_key, config_value, config_type, description, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (config_id, assistant_id, config_key, config_value,
    ...
    def create_correction(self, correction_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ai_corrections ( correction_id, correction_type, education_type, subject, grade_level, title, description, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?) ''', (correction_id, correction_type, education_type,
                          kwargs.get('subject'), kwargs.get('grade_level'),
    ...
    def submit_answer(self, correction_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT correction_type, education_type FROM ai_corrections WHERE correction_id = ?',
                    (correction_id,))
                    ...
                    cursor.execute(''' INSERT INTO correction_results ( result_id, correction_id, student_id, student_name, answer_content, correct_answer, score, max_score, correction_method, ai_feedback, teacher_comment, status, created_at ) VALUES (?, ?, ?, ?, ?, NULL, NULL, ?, ?, NULL, NULL, 'pending', ?) ''', (result_id, correction_id, student_id,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not correction:
                        return {'success': False, 'error': '批改任务不存在'}
                    cursor.execute('SELECT result_id, answer_content FROM correction_results WHERE correction_id = ? AND status = ?', (correction_id,
                    'pending'))
                    ...
                    for result in pending_results:
                        ...
                        cursor.execute(''' UPDATE correction_results SET score = ?, ai_feedback = ?, status = 'completed' WHERE result_id = ? ''', (score, ai_feedback, result[0]))
                    cursor.execute('UPDATE ai_corrections SET status = ?, updated_at = ? WHERE correction_id = ?',
                    ('completed', now, correction_id))
                    conn.commit()
    ...
    def create_exam_paper(self, paper_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ai_exam_papers ( paper_id, paper_name, education_type, subject, grade_level, total_score, duration, question_count, difficulty, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, 'draft', ?, ?, ?) ''', (paper_id, paper_name, education_type, kwargs.get('subject'),
    ...
    def add_qna(self, question: str, answer: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ai_qna ( qna_id, question, answer, subject, education_type, grade_level, tags, usage_count, accuracy, is_active, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 1.0, 1, ?) ''', (qna_id, question, answer, kwargs.get('subject'),
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                if qnas:
                    ...
                    cursor.execute('UPDATE ai_qna SET usage_count = usage_count + 1 WHERE qna_id = ?',
                    (matched_qna['qna_id'],))
                    cursor.execute(''' INSERT INTO qna_history ( history_id, qna_id, user_id, user_question, matched_question, answer, similarity, is_satisfied, feedback, timestamp ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, NULL, ?) ''', (history_id, matched_qna['qna_id'], user_id, user_question,
    ...
    def create_writing_task(self, writing_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ai_writing ( writing_id, writing_type, education_type, subject, grade_level, topic, requirements, word_count, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?) ''', (writing_id, writing_type, education_type, kwargs.get('subject'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO writing_results ( result_id, writing_id, student_id, student_name, content, ai_score, ai_feedback, teacher_score, teacher_comment, final_score, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, 'completed', ?) ''', (result_id, writing_id, student_id, kwargs.get('student_name'),
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE writing_results SET teacher_score = ?, teacher_comment = ?, final_score = ? WHERE result_id = ? ''', (teacher_score, kwargs.get('teacher_comment'), final_score, result_id))
                    conn.commit()
    ...
    def translate(self, source_text: str, source_language: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ai_translation ( translation_id, source_text, source_language, target_language, education_type, subject, context, status, created_by, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'completed', ?, ?) ''', (translation_id, source_text, source_language, target_language,
                          kwargs.get('education_type'), kwargs.get('subject'),
                          kwargs.get('context'), kwargs.get('created_by'), now))
                    if kwargs.get('user_id'):
                        cursor.execute(''' INSERT INTO translation_history ( history_id, source_text, translated_text, source_language, target_language, user_id, education_type, timestamp, rating ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 5) ''', (f"trh_{uuid.uuid4().hex[:12]}", source_text, translated_text,
    ...
    def rate_translation(self, history_id: str, rating: int) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE translation_history SET rating = ? WHERE history_id = ?', (rating,
                    history_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def get_supported_languages(self) -> Dict[str, Any]:
        try:
            languages = [{'code': k, 'name': v['name'], 'full_code': v['code']} for k,
            v in TRANSLATION_LANGUAGES.items()]
    ...
    def create_learning_plan(self, user_id: int, plan_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ai_learning_plans ( plan_id, user_id, user_name, education_type, subject, grade_level, plan_name, start_date, end_date, total_hours, completed_hours, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'active', ?, ?) ''', (plan_id, user_id, kwargs.get('user_name'), education_type,
                          kwargs.get('subject'), kwargs.get('grade_level'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO plan_executions ( execution_id, plan_id, activity_name, activity_type, planned_date, completed_date, duration, status, notes, created_at ) VALUES (?, ?, ?, ?, ?, NULL, ?, 'pending', ?, ?) ''', (execution_id, plan_id, activity_name, kwargs.get('activity_type'),
    ...
    def complete_activity(self, execution_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT plan_id, duration FROM plan_executions WHERE execution_id = ?',
                    (execution_id,))
    ...
    def get_learning_progress(self, user_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT plan_id, plan_name, total_hours, completed_hours, status, created_at FROM ai_learning_plans WHERE user_id = ? AND status = 'active' ''', (user_id,))
    ...
    def create_session(self, assistant_id: str, user_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT education_type, subject FROM ai_assistants WHERE assistant_id = ?',
                    (assistant_id,))
                    assistant = cursor.fetchone()
                    if not assistant:
                        return {'success': False, 'error': '助手不存在'}
                    cursor.execute(''' INSERT INTO ai_sessions ( session_id, assistant_id, user_id, user_name, education_type, subject, session_type, status, start_time, end_time, message_count, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, NULL, 0, ?) ''', (session_id, assistant_id, user_id, kwargs.get('user_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO session_messages ( message_id, session_id, sender, content, message_type, timestamp, is_read ) VALUES (?, ?, ?, ?, ?, ?, 0) ''', (message_id, session_id, sender, content,
                          kwargs.get('message_type', 'text'), now))
                    cursor.execute('UPDATE ai_sessions SET message_count = message_count + 1 WHERE session_id = ?',
                    (session_id,))
                    conn.commit()
    ...
    def get_model_config(self, model_type: str = None) -> Dict[str, Any]:
        try:
            ...
            models = [{'code': k, 'name': v['name'], 'provider': v['provider'], 'capability': v['capability']} for k,
            v in AI_MODEL_TYPES.items()]
    ...
    def get_correction_config(self, correction_type: str = None) -> Dict[str, Any]:
        try:
            ...
            corrections = [{'code': k, 'name': v['name'], 'description': v['description'],
            'accuracy': v['accuracy']} for k, v in CORRECTION_TYPES.items()]
