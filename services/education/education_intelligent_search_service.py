    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' CREATE TABLE IF NOT EXISTS search_index ( index_id TEXT PRIMARY KEY, index_name TEXT NOT NULL, index_type TEXT NOT NULL, education_type TEXT, source_type TEXT, description TEXT, document_count INTEGER DEFAULT 0, last_updated TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS index_config ( config_id TEXT PRIMARY KEY, index_id TEXT NOT NULL, config_key TEXT NOT NULL, config_value TEXT, description TEXT, created_at TEXT, FOREIGN KEY(index_id) REFERENCES search_index(index_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS search_query ( query_id TEXT PRIMARY KEY, query_text TEXT NOT NULL, query_type TEXT NOT NULL, education_type TEXT, language TEXT DEFAULT 'zh', is_advanced INTEGER DEFAULT 0, filters TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS query_records ( record_id TEXT PRIMARY KEY, query_id TEXT NOT NULL, user_id INTEGER, user_type TEXT, search_type TEXT, index_type TEXT, ranking_method TEXT, filters TEXT, result_count INTEGER DEFAULT 0, click_count INTEGER DEFAULT 0, query_time INTEGER DEFAULT 0, created_at TEXT, FOREIGN KEY(query_id) REFERENCES search_query(query_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS search_results ( result_id TEXT PRIMARY KEY, query_id TEXT NOT NULL, document_id TEXT NOT NULL, document_title TEXT, document_type TEXT, education_type TEXT, source_url TEXT, snippet TEXT, score REAL DEFAULT 0, rank INTEGER DEFAULT 0, is_recommended INTEGER DEFAULT 0, created_at TEXT, FOREIGN KEY(query_id) REFERENCES search_query(query_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS result_records ( record_id TEXT PRIMARY KEY, result_id TEXT NOT NULL, user_id INTEGER, click_time TEXT, dwell_time INTEGER DEFAULT 0, is_favorite INTEGER DEFAULT 0, feedback TEXT, created_at TEXT, FOREIGN KEY(result_id) REFERENCES search_results(result_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS intelligent_qna ( qna_id TEXT PRIMARY KEY, question TEXT NOT NULL, question_type TEXT, education_type TEXT, subject TEXT, difficulty TEXT, answer TEXT, answer_source TEXT, confidence REAL DEFAULT 0, is_valid INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS qna_records ( record_id TEXT PRIMARY KEY, qna_id TEXT NOT NULL, user_id INTEGER, user_type TEXT, question TEXT, answer TEXT, is_satisfied INTEGER DEFAULT 0, feedback TEXT, created_at TEXT, FOREIGN KEY(qna_id) REFERENCES intelligent_qna(qna_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS personalized_search ( ps_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_type TEXT, education_type TEXT, interests TEXT, learning_history TEXT, learning_goals TEXT, knowledge_level TEXT, learning_style TEXT, behavior_patterns TEXT, social_relations TEXT, time_preferences TEXT, updated_at TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS search_history ( history_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, search_type TEXT, query_text TEXT, education_type TEXT, results_count INTEGER DEFAULT 0, clicked_result_id TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS search_recommendation ( rec_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, education_type TEXT, recommendation_type TEXT, recommendation_method TEXT, recommended_items TEXT, confidence REAL DEFAULT 0, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS recommendation_records ( record_id TEXT PRIMARY KEY, rec_id TEXT NOT NULL, user_id INTEGER, item_id TEXT, item_type TEXT, is_clicked INTEGER DEFAULT 0, is_favorite INTEGER DEFAULT 0, feedback TEXT, created_at TEXT, FOREIGN KEY(rec_id) REFERENCES search_recommendation(rec_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS search_logs ( log_id TEXT PRIMARY KEY, user_id INTEGER, user_type TEXT, education_type TEXT, search_type TEXT, query_text TEXT, query_params TEXT, result_count INTEGER DEFAULT 0, response_time INTEGER DEFAULT 0, error_message TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS log_records ( record_id TEXT PRIMARY KEY, log_id TEXT NOT NULL, action_type TEXT, action_detail TEXT, created_at TEXT, FOREIGN KEY(log_id) REFERENCES search_logs(log_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS search_statistics ( stat_id TEXT PRIMARY KEY, stat_type TEXT NOT NULL, education_type TEXT, period TEXT, total_queries INTEGER DEFAULT 0, avg_response_time REAL DEFAULT 0, avg_result_count REAL DEFAULT 0, click_through_rate REAL DEFAULT 0, user_satisfaction REAL DEFAULT 0, top_searches TEXT, data_date TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS stat_data ( data_id TEXT PRIMARY KEY, stat_id TEXT NOT NULL, metric_name TEXT NOT NULL, metric_value REAL DEFAULT 0, metric_unit TEXT, dimension TEXT, created_at TEXT, FOREIGN KEY(stat_id) REFERENCES search_statistics(stat_id) ) ''')
    ...
    def full_text_search(self, query: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, language, is_advanced, filters, created_at) VALUES (?, ?, 'full_text', ?, ?, ?, ?, ?) ''', (query_id, query, education_type,
                          kwargs.get('language', 'zh'),
                          1 if kwargs.get('is_advanced') else 0,
                          filters, now))

                    cursor.execute(''' INSERT INTO query_records (record_id, query_id, user_id, user_type, search_type, index_type, ranking_method, filters, result_count, created_at) VALUES (?, ?, ?, ?, 'full_text', 'text', ?, ?, ?, ?) ''', (f"qrc_{uuid.uuid4().hex[:12]}", query_id,
    ...
    def advanced_full_text_search(self, query: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, language, is_advanced, filters, created_at) VALUES (?, ?, 'full_text', ?, ?, 1, ?, ?) ''', (query_id, query, education_type,
    ...
    def full_text_search_with_filters(self, query: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, filters, created_at) VALUES (?, ?, 'full_text', ?, ?, ?) ''', (query_id, query, education_type, json.dumps(filters), now))
                    conn.commit()
    ...
    def semantic_search(self, query: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'semantic', ?, ?) ''', (query_id, query, education_type, now))

                    cursor.execute(''' INSERT INTO query_records (record_id, query_id, user_id, search_type, index_type, ranking_method, created_at) VALUES (?, ?, ?, 'semantic', 'vector', 'relevance', ?) ''', (f"qrc_{uuid.uuid4().hex[:12]}", query_id,
    ...
    def semantic_search_by_vector(self, vector: List[float], education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'semantic', ?, ?) ''', (query_id, 'vector_search', education_type, now))
                    conn.commit()
    ...
    def image_search_by_url(self, image_url: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'image', ?, ?) ''', (query_id, image_url, education_type, now))

                    cursor.execute(''' INSERT INTO query_records (record_id, query_id, user_id, search_type, index_type, created_at) VALUES (?, ?, ?, 'image', 'image', ?) ''', (f"qrc_{uuid.uuid4().hex[:12]}", query_id, kwargs.get('user_id'), now))
                    conn.commit()
    ...
    def image_search_by_features(self, features: List[float], education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'image', ?, ?) ''', (query_id, 'feature_search', education_type, now))
                    conn.commit()
    ...
    def image_search_with_text(self, text: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'image', ?, ?) ''', (query_id, text, education_type, now))
                    conn.commit()
    ...
    def reverse_image_search(self, image_url: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'image', ?, ?) ''', (query_id, f'reverse:{image_url}', education_type, now))
                    conn.commit()
    ...
    def voice_search_by_text(self, text: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'voice', ?, ?) ''', (query_id, text, education_type, now))

                    cursor.execute(''' INSERT INTO query_records (record_id, query_id, user_id, search_type, index_type, created_at) VALUES (?, ?, ?, 'voice', 'voice', ?) ''', (f"qrc_{uuid.uuid4().hex[:12]}", query_id, kwargs.get('user_id'), now))
                    conn.commit()
    ...
    def voice_search_by_audio(self, audio_url: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'voice', ?, ?) ''', (query_id, audio_url, education_type, now))
                    conn.commit()
    ...
    def voice_search_with_transcription(self, audio_url: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'voice', ?, ?) ''', (query_id, transcription['text'], education_type, now))
                    conn.commit()
    ...
    def multilingual_voice_search(self, audio_url: str, language: str = 'zh',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, language, created_at) VALUES (?, ?, 'voice', ?, ?, ?) ''', (query_id, audio_url, education_type, language, now))
                    conn.commit()
    ...
    def voice_command_search(self, command: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'voice', ?, ?) ''', (query_id, command, education_type, now))
                    conn.commit()
    ...
    def video_search_by_text(self, query: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'video', ?, ?) ''', (query_id, query, education_type, now))

                    cursor.execute(''' INSERT INTO query_records (record_id, query_id, user_id, search_type, index_type, created_at) VALUES (?, ?, ?, 'video', 'video', ?) ''', (f"qrc_{uuid.uuid4().hex[:12]}", query_id, kwargs.get('user_id'), now))
                    conn.commit()
    ...
    def video_search_by_keyframe(self, keyframe_url: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'video', ?, ?) ''', (query_id, keyframe_url, education_type, now))
                    conn.commit()
    ...
    def video_search_by_transcript(self, transcript: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'video', ?, ?) ''', (query_id, transcript[:100], education_type, now))
                    conn.commit()
    ...
    def intelligent_qna_query(self, question: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO intelligent_qna (qna_id, question, question_type, education_type, subject, difficulty, answer, confidence, is_valid, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?) ''', (qna_id, question, question_type, education_type,
                          kwargs.get('subject'), kwargs.get('difficulty', 'medium'),
                          f'这是针对"{question}"的智能回答',
                          round(0.85 + kwargs.get('confidence', 0) * 0.15, 2), now))

                    cursor.execute(''' INSERT INTO qna_records (record_id, qna_id, user_id, user_type, question, answer, created_at) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (f"qnr_{uuid.uuid4().hex[:12]}", qna_id, kwargs.get('user_id'),
    ...
    def qna_with_context(self, question: str, context: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO intelligent_qna (qna_id, question, question_type, education_type, answer, confidence, is_valid, created_at) VALUES (?, ?, 'contextual', ?, ?, ?, 1, ?) ''', (qna_id, question, education_type,
    ...
    def qna_for_education_level(self, question: str, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO intelligent_qna (qna_id, question, question_type, education_type, answer, confidence, is_valid, created_at) VALUES (?, ?, ?, ?, ?, ?, 1, ?) ''', (qna_id, question, 'adaptive', education_type, answer, 0.9, now))
                    conn.commit()
    ...
    def create_user_profile(self, user_id: int, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO personalized_search ( ps_id, user_id, user_type, education_type, interests, learning_history, learning_goals, knowledge_level, learning_style, behavior_patterns, social_relations, time_preferences, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (ps_id, user_id, kwargs.get('user_type', 'student'), education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO search_query (query_id, query_text, query_type, education_type, created_at) VALUES (?, ?, 'personalized', ?, ?) ''', (query_id, query, education_type, now))
                    conn.commit()
    ...
    def generate_recommendations(self, user_id: int, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_recommendation ( rec_id, user_id, education_type, recommendation_type, recommendation_method, recommended_items, confidence, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (rec_id, user_id, education_type, 'content', method,
    ...
    def collaborative_filtering_recommend(self, user_id: int, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_recommendation ( rec_id, user_id, education_type, recommendation_type, recommendation_method, recommended_items, confidence, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (rec_id, user_id, education_type, 'collaborative', 'collaborative',
    ...
    def content_based_recommend(self, user_id: int, education_type: str = 'adult',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_recommendation ( rec_id, user_id, education_type, recommendation_type, recommendation_method, recommended_items, confidence, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (rec_id, user_id, education_type, 'content', 'content',
    ...
    def context_aware_recommend(self, user_id: int, context: Dict[str, Any],
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO search_recommendation ( rec_id, user_id, education_type, recommendation_type, recommendation_method, recommended_items, confidence, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (rec_id, user_id, education_type, 'context', 'context',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO search_statistics ( stat_id, stat_type, education_type, period, total_queries, avg_response_time, avg_result_count, click_through_rate, user_satisfaction, top_searches, data_date, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (stat_id, 'overview', education_type, period,
