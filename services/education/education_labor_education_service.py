    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_courses ( course_id TEXT PRIMARY KEY, course_name TEXT NOT NULL, labor_type TEXT NOT NULL, course_type TEXT NOT NULL, education_type TEXT, grade_level INTEGER, teacher_id INTEGER, teacher_name TEXT, semester TEXT, weekly_hours INTEGER DEFAULT 2, location TEXT, max_students INTEGER DEFAULT 30, enrolled_count INTEGER DEFAULT 0, description TEXT, objectives TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_schedules ( schedule_id TEXT PRIMARY KEY, course_id TEXT NOT NULL, weekday INTEGER, start_time TEXT, end_time TEXT, location TEXT, week_range TEXT, created_at TEXT, FOREIGN KEY(course_id) REFERENCES labor_courses(course_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_practice ( practice_id TEXT PRIMARY KEY, practice_name TEXT NOT NULL, practice_type TEXT NOT NULL, education_type TEXT, grade_level INTEGER, organizer TEXT, description TEXT, location TEXT, start_date TEXT, end_date TEXT, max_participants INTEGER DEFAULT 50, registered_count INTEGER DEFAULT 0, safety_requirements TEXT, status TEXT DEFAULT 'scheduled', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS practice_records ( record_id TEXT PRIMARY KEY, practice_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, registration_date TEXT, attendance INTEGER DEFAULT 0, performance TEXT, score REAL, reflection TEXT, status TEXT DEFAULT 'registered', FOREIGN KEY(practice_id) REFERENCES labor_practice(practice_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_skills ( skill_id TEXT PRIMARY KEY, skill_name TEXT NOT NULL, skill_category TEXT NOT NULL, education_type TEXT, description TEXT, proficiency_levels TEXT, duration_hours INTEGER, prerequisites TEXT, status TEXT DEFAULT 'active', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS skill_training ( training_id TEXT PRIMARY KEY, skill_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, education_type TEXT, start_date TEXT, end_date TEXT, hours_completed INTEGER DEFAULT 0, proficiency_level TEXT, assessment_result TEXT, certificate_no TEXT, status TEXT DEFAULT 'in_progress', FOREIGN KEY(skill_id) REFERENCES labor_skills(skill_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_literacy ( literacy_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, student_name TEXT, education_type TEXT, grade_level INTEGER, academic_year TEXT, total_hours INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS literacy_assessment ( assessment_id TEXT PRIMARY KEY, literacy_id TEXT NOT NULL, assessment_date TEXT, dimension TEXT, score REAL, comment TEXT, assessor TEXT, created_at TEXT, FOREIGN KEY(literacy_id) REFERENCES labor_literacy(literacy_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_education_base ( base_id TEXT PRIMARY KEY, base_name TEXT NOT NULL, base_type TEXT NOT NULL, education_type TEXT, address TEXT, contact_person TEXT, contact_phone TEXT, description TEXT, capacity INTEGER, facilities TEXT, cooperation_status TEXT DEFAULT 'active', cooperation_agreement TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS base_resources ( resource_id TEXT PRIMARY KEY, base_id TEXT NOT NULL, resource_name TEXT NOT NULL, resource_type TEXT, quantity INTEGER DEFAULT 1, status TEXT DEFAULT 'available', created_at TEXT, FOREIGN KEY(base_id) REFERENCES labor_education_base(base_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_education_resource ( resource_id TEXT PRIMARY KEY, resource_name TEXT NOT NULL, resource_type TEXT NOT NULL, education_type TEXT, description TEXT, file_path TEXT, url TEXT, provider TEXT, usage_count INTEGER DEFAULT 0, status TEXT DEFAULT 'available', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_management ( id INTEGER PRIMARY KEY AUTOINCREMENT, resource_id TEXT NOT NULL, operation_type TEXT, operator TEXT, operation_time TEXT, details TEXT, FOREIGN KEY(resource_id) REFERENCES labor_education_resource(resource_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_education_evaluation ( evaluation_id TEXT PRIMARY KEY, evaluation_name TEXT NOT NULL, education_type TEXT, target_type TEXT, dimensions TEXT, weight_config TEXT, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS evaluation_results ( result_id TEXT PRIMARY KEY, evaluation_id TEXT NOT NULL, target_id INTEGER, target_name TEXT, education_type TEXT, scores TEXT, total_score REAL, grade TEXT, feedback TEXT, evaluator TEXT, evaluation_date TEXT, FOREIGN KEY(evaluation_id) REFERENCES labor_education_evaluation(evaluation_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_education_research ( research_id TEXT PRIMARY KEY, research_name TEXT NOT NULL, research_type TEXT NOT NULL, education_type TEXT, lead_researcher TEXT, team_members TEXT, description TEXT, objectives TEXT, start_date TEXT, end_date TEXT, budget REAL, status TEXT DEFAULT 'in_progress', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS research_records ( record_id TEXT PRIMARY KEY, research_id TEXT NOT NULL, record_type TEXT, content TEXT, author TEXT, record_date TEXT, file_path TEXT, FOREIGN KEY(research_id) REFERENCES labor_education_research(research_id) ) ''')
                conn.commit()
    ...
    def create_labor_course(self, course_name: str, labor_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_courses ( course_id, course_name, labor_type, course_type, education_type, grade_level, teacher_id, teacher_name, semester, weekly_hours, location, max_students, enrolled_count, description, objectives, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'active', ?, ?) ''', (course_id, course_name, labor_type, course_type,
    ...
    def enroll_labor_course(self, course_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT max_students, enrolled_count, status, education_type FROM labor_courses WHERE course_id = ? ''', (course_id,))
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO labor_literacy (literacy_id, student_id, student_name, education_type, grade_level, academic_year, total_hours, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 0, 'active', ?) ''', (f"lit_{uuid.uuid4().hex[:12]}", student_id, student_name,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not cursor.fetchone():
                        return {'success': False, 'error': '课程不存在'}
                    cursor.execute(''' INSERT INTO course_schedules (schedule_id, course_id, weekday, start_time, end_time, location, week_range, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (schedule_id, course_id, weekday, start_time, end_time,
    ...
    def create_labor_practice(self, practice_name: str, practice_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_practice ( practice_id, practice_name, practice_type, education_type, grade_level, organizer, description, location, start_date, end_date, max_participants, registered_count, safety_requirements, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'scheduled', ?, ?) ''', (practice_id, practice_name, practice_type,
                          kwargs.get('education_type'), kwargs.get('grade_level'),
    ...
    def register_practice(self, practice_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT max_participants, registered_count, status, education_type FROM labor_practice WHERE practice_id = ? ''', (practice_id,))
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO practice_records (record_id, practice_id, student_id, student_name, registration_date, status) VALUES (?, ?, ?, ?, ?, 'registered') ''', (record_id, practice_id, student_id, student_name, now[:10]))
    ...
    def create_skill(self, skill_name: str, skill_category: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_skills ( skill_id, skill_name, skill_category, education_type, description, proficiency_levels, duration_hours, prerequisites, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?) ''', (skill_id, skill_name, skill_category,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO skill_training (training_id, skill_id, student_id, student_name, education_type, start_date, hours_completed, status) VALUES (?, ?, ?, ?, ?, ?, 0, 'in_progress') ''', (training_id, skill_id, student_id, student_name, education_type, now[:10]))
                    conn.commit()
    ...
    def record_training_hours(self, training_id: str, hours: float) -> Dict[str, Any]:
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT hours_completed, status FROM skill_training WHERE training_id = ?',
                    (training_id,))
                    ...
                    cursor.execute('UPDATE skill_training SET hours_completed = ? WHERE training_id = ?', (new_hours,
                    training_id))
                    conn.commit()
    ...
    def create_literacy_record(self, student_id: int, student_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_literacy ( literacy_id, student_id, student_name, education_type, grade_level, academic_year, total_hours, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 0, 'active', ?, ?) ''', (literacy_id, student_id, student_name, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO literacy_assessment (assessment_id, literacy_id, assessment_date, dimension, score, comment, assessor, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (assessment_id, literacy_id, now[:10], dimension, score,
    ...
    def create_education_base(self, base_name: str, base_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_education_base ( base_id, base_name, base_type, education_type, address, contact_person, contact_phone, description, capacity, facilities, cooperation_status, cooperation_agreement, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?) ''', (base_id, base_name, base_type, kwargs.get('education_type'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO base_resources (resource_id, base_id, resource_name, resource_type, quantity, status, created_at) VALUES (?, ?, ?, ?, ?, 'available', ?) ''', (resource_id, base_id, resource_name, resource_type,
    ...
    def create_resource(self, resource_name: str, resource_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_education_resource ( resource_id, resource_name, resource_type, education_type, description, file_path, url, provider, usage_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 'available', ?, ?) ''', (resource_id, resource_name, resource_type,
                          kwargs.get('education_type'), kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(
                    'UPDATE labor_education_resource SET usage_count = usage_count + 1 WHERE resource_id = ?',
                    (resource_id,))
    ...
    def create_evaluation(self, evaluation_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_education_evaluation ( evaluation_id, evaluation_name, education_type, target_type, dimensions, weight_config, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (evaluation_id, evaluation_name, kwargs.get('education_type'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO evaluation_results ( result_id, evaluation_id, target_id, target_name, education_type, scores, total_score, grade, feedback, evaluator, evaluation_date ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (result_id, evaluation_id, target_id, target_name,
    ...
    def update_evaluation_feedback(self, result_id: str, feedback: str) -> Dict[str, Any]:
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE evaluation_results SET feedback = ? WHERE result_id = ?', (feedback,
                    result_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def create_research_project(self, research_name: str, research_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_education_research ( research_id, research_name, research_type, education_type, lead_researcher, team_members, description, objectives, start_date, end_date, budget, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'in_progress', ?, ?) ''', (research_id, research_name, research_type,
    ...
    def add_research_record(self, research_id: str, record_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT research_id FROM labor_education_research WHERE research_id = ?',
                    (research_id,))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '研究项目不存在'}
                    cursor.execute(''' INSERT INTO research_records (record_id, research_id, record_type, content, author, record_date, file_path) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (record_id, research_id, record_type, content,
    ...
    def get_labor_education_statistics(self, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT SUM(total_hours) FROM labor_literacy WHERE 1=1' + (
                ' AND education_type = ?' if education_type else ''), params)
                total_hours = cursor.fetchone()[0]
                stats['total_labor_hours'] = total_hours or 0

                cursor.execute('SELECT AVG(total_score) FROM evaluation_results WHERE 1=1' + (
                ' AND education_type = ?' if education_type else ''), params)
