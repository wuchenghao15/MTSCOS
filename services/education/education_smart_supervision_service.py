...
class EducationSmartSupervisionService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_plans ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, supervision_type TEXT NOT NULL, plan_level TEXT NOT NULL, education_type TEXT NOT NULL, start_date TEXT, end_date TEXT, description TEXT, status TEXT DEFAULT 'draft', priority INTEGER DEFAULT 3, target_school TEXT, target_level TEXT, created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS plan_details ( detail_id INTEGER PRIMARY KEY AUTOINCREMENT, plan_id TEXT NOT NULL, item_name TEXT NOT NULL, item_description TEXT, target_value TEXT, actual_value TEXT, status TEXT DEFAULT 'pending', FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_process ( process_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, process_name TEXT NOT NULL, monitoring_method TEXT, start_time TEXT, end_time TEXT, status TEXT DEFAULT 'not_started', location TEXT, participants TEXT, created_at TEXT, FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS process_records ( record_id INTEGER PRIMARY KEY AUTOINCREMENT, process_id TEXT NOT NULL, record_time TEXT NOT NULL, record_type TEXT, content TEXT, attachments TEXT, operator TEXT, FOREIGN KEY(process_id) REFERENCES supervision_process(process_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_data ( data_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, data_type TEXT, data_category TEXT, data_value TEXT, data_unit TEXT, collection_time TEXT, source TEXT, education_type TEXT, FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS data_analysis ( analysis_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, analysis_method TEXT, analysis_type TEXT, input_data TEXT, output_result TEXT, analysis_time TEXT, analyst TEXT, FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_reports ( report_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, report_type TEXT NOT NULL, report_title TEXT NOT NULL, report_content TEXT, status TEXT DEFAULT 'draft', education_type TEXT, created_by TEXT, created_at TEXT, updated_at TEXT, FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS report_versions ( version_id INTEGER PRIMARY KEY AUTOINCREMENT, report_id TEXT NOT NULL, version_number TEXT NOT NULL, content TEXT, change_log TEXT, created_at TEXT, FOREIGN KEY(report_id) REFERENCES supervision_reports(report_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS rectification_tasks ( task_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, report_id TEXT, task_name TEXT NOT NULL, problem_description TEXT, responsible_unit TEXT, responsible_person TEXT, deadline TEXT, status TEXT DEFAULT 'pending', priority INTEGER DEFAULT 2, education_type TEXT, created_at TEXT, updated_at TEXT, FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id), FOREIGN KEY(report_id) REFERENCES supervision_reports(report_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS task_progress ( progress_id INTEGER PRIMARY KEY AUTOINCREMENT, task_id TEXT NOT NULL, progress_percent INTEGER DEFAULT 0, progress_description TEXT, update_time TEXT, updater TEXT, attachments TEXT, FOREIGN KEY(task_id) REFERENCES rectification_tasks(task_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS evaluation_system ( eval_id TEXT PRIMARY KEY, system_name TEXT NOT NULL, education_type TEXT NOT NULL, version TEXT DEFAULT '1.0', description TEXT, status TEXT DEFAULT 'active', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS evaluation_scores ( score_id INTEGER PRIMARY KEY AUTOINCREMENT, plan_id TEXT NOT NULL, eval_id TEXT NOT NULL, dimension TEXT, score REAL, max_score REAL, weight REAL, comments TEXT, FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id), FOREIGN KEY(eval_id) REFERENCES evaluation_system(eval_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS expert_management ( expert_id TEXT PRIMARY KEY, expert_name TEXT NOT NULL, expert_role TEXT NOT NULL, education_type TEXT, title TEXT, organization TEXT, expertise TEXT, qualifications TEXT, level TEXT DEFAULT 'medium', status TEXT DEFAULT 'active', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS expert_assignments ( assignment_id INTEGER PRIMARY KEY AUTOINCREMENT, expert_id TEXT NOT NULL, plan_id TEXT NOT NULL, role_in_plan TEXT, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'assigned', FOREIGN KEY(expert_id) REFERENCES expert_management(expert_id), FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS information_publication ( pub_id TEXT PRIMARY KEY, info_type TEXT NOT NULL, title TEXT NOT NULL, content TEXT, public_level TEXT DEFAULT 'internal', education_type TEXT, status TEXT DEFAULT 'draft', publish_date TEXT, created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS publication_records ( record_id INTEGER PRIMARY KEY AUTOINCREMENT, pub_id TEXT NOT NULL, view_count INTEGER DEFAULT 0, download_count INTEGER DEFAULT 0, feedback TEXT, record_time TEXT, FOREIGN KEY(pub_id) REFERENCES information_publication(pub_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_alerts ( alert_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, alert_type TEXT, alert_level TEXT DEFAULT 'warning', alert_message TEXT, status TEXT DEFAULT 'active', created_at TEXT, FOREIGN KEY(plan_id) REFERENCES supervision_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS alert_rules ( rule_id INTEGER PRIMARY KEY AUTOINCREMENT, rule_name TEXT NOT NULL, rule_type TEXT, condition TEXT, threshold TEXT, alert_level TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_stats ( stat_id INTEGER PRIMARY KEY AUTOINCREMENT, stat_type TEXT NOT NULL, education_type TEXT NOT NULL, period TEXT, stat_data TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_supervision_plan(self, plan_name: str, supervision_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO supervision_plans ( plan_id, plan_name, supervision_type, plan_level, education_type, start_date, end_date, description, status, priority, target_school, target_level, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?) ''', (plan_id, plan_name, supervision_type, plan_level,
    ...
    def create_monitoring_process(self, plan_id: str, process_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO supervision_process ( process_id, plan_id, process_name, monitoring_method, start_time, end_time, status, location, participants, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'not_started', ?, ?, ?) ''', (process_id, plan_id, process_name, monitoring_method,
    ...
    def record_process_data(self, process_id: str, record_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO process_records ( process_id, record_time, record_type, content, attachments, operator ) VALUES (?, ?, ?, ?, ?, ?) ''', (process_id, now, record_type, content,
    ...
    def collect_supervision_data(self, plan_id: str, data_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO supervision_data ( data_id, plan_id, data_type, data_category, data_value, data_unit, collection_time, source, education_type ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (data_id, plan_id, data_type, data_category,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO data_analysis ( analysis_id, plan_id, analysis_method, analysis_type, input_data, output_result, analysis_time, analyst ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (analysis_id, plan_id, analysis_method,
    ...
    def create_report(self, plan_id: str, report_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO supervision_reports ( report_id, plan_id, report_type, report_title, report_content, status, education_type, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?) ''', (report_id, plan_id, report_type, report_title,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'SELECT version_number FROM report_versions WHERE report_id = ? ORDER BY version_id DESC LIMIT 1',
                    (report_id,))
    ...
    def _generate_report_content(self, plan_id: str, education_type: str) -> str:
        return f"""督导报告自动生成 ================  计划ID: {plan_id} 教育类型: {education_type} 生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  一、督导概况 本次督导按照计划要求，对相关单位进行了全面检查评估。  二、评估结果 根据评价体系，综合得分：85分（满分100分）  三、存在问题 1. 教学设施有待改善 2. 师资培训需要加强  四、整改建议 1. 加大设施投入 2. 完善培训机制  五、结论 整体情况良好，建议继续保持并改进不足。 """
    ...
    def create_rectification_task(self, plan_id: str, task_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO rectification_tasks ( task_id, plan_id, report_id, task_name, problem_description, responsible_unit, responsible_person, deadline, status, priority, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?) ''', (task_id, plan_id, kwargs.get('report_id'), task_name,
    ...
    def update_task_progress(self, task_id: str, progress_percent: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO task_progress ( task_id, progress_percent, progress_description, update_time, updater, attachments ) VALUES (?, ?, ?, ?, ?, ?) ''', (task_id, progress_percent, kwargs.get('progress_description'),
    ...
    def create_evaluation_system(self, system_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO evaluation_system ( eval_id, system_name, education_type, version, description, status, created_at ) VALUES (?, ?, ?, ?, ?, 'active', ?) ''', (eval_id, system_name, education_type,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    for score in scores:
                        cursor.execute(''' INSERT INTO evaluation_scores ( plan_id, eval_id, dimension, score, max_score, weight, comments ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (plan_id, eval_id, score.get('dimension'),
    ...
    def register_expert(self, expert_name: str, expert_role: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO expert_management ( expert_id, expert_name, expert_role, education_type, title, organization, expertise, qualifications, level, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?) ''', (expert_id, expert_name, expert_role,
    ...
    def assign_expert(self, expert_id: str, plan_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO expert_assignments ( expert_id, plan_id, role_in_plan, start_date, end_date, status ) VALUES (?, ?, ?, ?, ?, 'assigned') ''', (expert_id, plan_id, role_in_plan,
    ...
    def publish_information(self, info_type: str, title: str, content: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO information_publication ( pub_id, info_type, title, content, public_level, education_type, status, publish_date, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'published', ?, ?, ?, ?) ''', (pub_id, info_type, title, content,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'SELECT view_count FROM publication_records WHERE pub_id = ? ORDER BY record_id DESC LIMIT 1',
                    (pub_id,))
                    latest = cursor.fetchone()
                    view_count = (latest[0] + 1) if latest else 1
                    cursor.execute(''' INSERT INTO publication_records ( pub_id, view_count, download_count, feedback, record_time ) VALUES (?, ?, ?, ?, ?) ''', (pub_id, view_count, kwargs.get('download_count', 0),
    ...
    def get_publication_detail(self, pub_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT * FROM publication_records WHERE pub_id = ? ORDER BY record_time DESC LIMIT 1',
                (pub_id,))
    ...
    def create_alert_rule(self, rule_name: str, rule_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO alert_rules ( rule_name, rule_type, condition, threshold, alert_level, education_type, is_active, created_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?) ''', (rule_name, rule_type, condition, threshold,
    ...
    def trigger_alert(self, plan_id: str, alert_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO supervision_alerts ( alert_id, plan_id, alert_type, alert_level, alert_message, status, created_at ) VALUES (?, ?, ?, ?, ?, 'active', ?) ''', (alert_id, plan_id, alert_type,
