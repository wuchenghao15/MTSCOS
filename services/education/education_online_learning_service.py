    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS online_courses ( course_id TEXT PRIMARY KEY, course_name TEXT NOT NULL, course_type TEXT NOT NULL, education_type TEXT, subject TEXT, grade_level INTEGER, teacher_id INTEGER, teacher_name TEXT, description TEXT, duration INTEGER, total_chapters INTEGER, difficulty TEXT, max_students INTEGER DEFAULT 1000, enrolled_count INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, education_type TEXT, enroll_date TEXT, progress REAL DEFAULT 0, completed_chapters INTEGER DEFAULT 0, total_time REAL DEFAULT 0, final_score REAL, certificate_issued INTEGER DEFAULT 0, status TEXT DEFAULT 'learning', UNIQUE(course_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_platform ( platform_id TEXT PRIMARY KEY, platform_name TEXT NOT NULL, platform_type TEXT NOT NULL, education_type TEXT, description TEXT, url TEXT, api_key TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS platform_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, platform_id TEXT NOT NULL, user_id INTEGER, user_name TEXT, access_time TEXT, duration REAL, actions TEXT, FOREIGN KEY(platform_id) REFERENCES learning_platform(platform_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_data ( data_id TEXT PRIMARY KEY, analysis_type TEXT NOT NULL, student_id INTEGER NOT NULL, education_type TEXT, course_id TEXT, metrics TEXT, analysis_result TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS data_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, data_id TEXT NOT NULL, record_type TEXT, record_data TEXT, timestamp TEXT, FOREIGN KEY(data_id) REFERENCES learning_data(data_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_community ( community_id TEXT PRIMARY KEY, community_type TEXT NOT NULL, education_type TEXT, name TEXT NOT NULL, description TEXT, course_id TEXT, creator_id INTEGER, creator_name TEXT, member_count INTEGER DEFAULT 1, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS community_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, community_id TEXT NOT NULL, user_id INTEGER, user_name TEXT, action_type TEXT, content TEXT, created_at TEXT, FOREIGN KEY(community_id) REFERENCES learning_community(community_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_certification ( cert_id TEXT PRIMARY KEY, cert_type TEXT NOT NULL, education_type TEXT, student_id INTEGER NOT NULL, student_name TEXT, course_id TEXT, cert_no TEXT, issue_date TEXT, expiry_date TEXT, status TEXT DEFAULT 'issued', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS certification_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, cert_id TEXT NOT NULL, record_type TEXT, record_data TEXT, created_at TEXT, FOREIGN KEY(cert_id) REFERENCES learning_certification(cert_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_resources ( resource_id TEXT PRIMARY KEY, resource_type TEXT NOT NULL, education_type TEXT, course_id TEXT, title TEXT NOT NULL, description TEXT, file_url TEXT, file_size INTEGER, duration REAL, uploader_id INTEGER, uploader_name TEXT, views INTEGER DEFAULT 0, downloads INTEGER DEFAULT 0, is_public INTEGER DEFAULT 1, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS resource_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, resource_id TEXT NOT NULL, user_id INTEGER, action_type TEXT, timestamp TEXT, FOREIGN KEY(resource_id) REFERENCES learning_resources(resource_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_support ( support_id TEXT PRIMARY KEY, support_type TEXT NOT NULL, education_type TEXT, student_id INTEGER NOT NULL, student_name TEXT, course_id TEXT, question TEXT, answer TEXT, status TEXT DEFAULT 'pending', response_time TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS support_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, support_id TEXT NOT NULL, action_type TEXT, action_data TEXT, created_at TEXT, FOREIGN KEY(support_id) REFERENCES learning_support(support_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_assessment ( assessment_id TEXT PRIMARY KEY, assessment_method TEXT NOT NULL, education_type TEXT, course_id TEXT, title TEXT NOT NULL, description TEXT, duration INTEGER, max_score REAL DEFAULT 100, passing_score REAL DEFAULT 60, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS assessment_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, assessment_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, education_type TEXT, score REAL, status TEXT DEFAULT 'submitted', submission_time TEXT, feedback TEXT, FOREIGN KEY(assessment_id) REFERENCES learning_assessment(assessment_id), UNIQUE(assessment_id, student_id) ) ''')
                conn.commit()
    ...
    def create_course(self, course_name: str, course_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO online_courses ( course_id, course_name, course_type, education_type, subject, grade_level, teacher_id, teacher_name, description, duration, total_chapters, difficulty, max_students, enrolled_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'active', ?, ?) ''', (course_id, course_name, course_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE course_records SET progress = ?, completed_chapters = COALESCE(?, completed_chapters), total_time = COALESCE(total_time + ?, total_time), status = ? WHERE course_id = ? AND student_id = ? ''', (progress, kwargs.get('completed_chapters'), kwargs.get('time_added', 0), status, course_id,
                    student_id))
                    if cursor.rowcount > 0:
                        conn.commit()
    ...
    def create_platform(self, platform_name: str, platform_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_platform ( platform_id, platform_name, platform_type, education_type, description, url, api_key, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (platform_id, platform_name, platform_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO platform_records (platform_id, user_id, user_name, access_time, duration, actions) VALUES (?, ?, ?, ?, ?, ?) ''', (platform_id, user_id, user_name, now, kwargs.get('duration', 0), kwargs.get('actions', '[]')))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_platform_stats(self, platform_id: str, **kwargs) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(''' SELECT COUNT(*) as total_access, SUM(duration) as total_duration FROM platform_records WHERE platform_id = ? ''', (platform_id,))
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if updates:
                        ...
                        cursor.execute(f'UPDATE learning_platform SET {", ".join(updates)} WHERE platform_id = ?',
                        params)
                        conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def collect_learning_data(self, student_id: int, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_data ( data_id, analysis_type, student_id, education_type, course_id, metrics, analysis_result, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (data_id, analysis_type, student_id, education_type,
                ...
                with self._get_connection() as conn:
                    ...
                    if data[1] == 'behavior':
                        result = {'learning_hours': metrics.get('total_time', 0) / 3600,
                                  ...
                                  'interaction_level': 'high' if metrics.get('interactions',
                                  0) > 100 else 'medium' if metrics.get('interactions', 0) > 50 else 'low'}
                    elif data[1] == 'effect':
                        ...
                        result = {'dropout_risk': 'low' if metrics.get('engagement',
                        0) > 80 else 'medium' if metrics.get('engagement', 0) > 50 else 'high',
                                  'expected_score': metrics.get('predicted_score', 0),
                                  'completion_probability': metrics.get('completion_prob', 0)}
                    cursor.execute('UPDATE learning_data SET analysis_result = ? WHERE data_id = ?',
                    (json.dumps(result), data_id))
                    conn.commit()
    ...
    def evaluate_effect(self, student_id: int, course_id: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT AVG(final_score) as avg_score, COUNT(*) as course_count, SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) as completed_count FROM course_records WHERE student_id = ? '''
                params = [student_id]
                if course_id:
                    query += ' AND course_id = ?'
                ...
                stats = cursor.fetchone()
                completion_rate = (
                stats['completed_count'] / stats['course_count'] * 100) if stats['course_count'] > 0 else 0
                return {
                    'success': True,
    ...
    def predict_learning(self, student_id: int, education_type: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT AVG(progress) as avg_progress, AVG(final_score) as avg_score, COUNT(*) as total_courses FROM course_records WHERE student_id = ? AND education_type = ? ''', (student_id, education_type))
                stats = cursor.fetchone()
    ...
    def create_community(self, community_type: str, name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_community ( community_id, community_type, education_type, name, description, course_id, creator_id, creator_name, member_count, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 1, ?, ?) ''', (community_id, community_type, education_type, name,
                          kwargs.get('description', config.get('description', '')),
    ...
    def ask_question(self, community_id: str, user_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT is_active, community_type FROM learning_community WHERE community_id = ?',
                    (community_id,))
                    community = cursor.fetchone()
                    if not community or community[0] != 1:
                        return {'success': False, 'error': '社区未启用'}
    ...
    def get_community_stats(self, community_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT COUNT( *) as post_count FROM community_records WHERE community_id = ? AND action_type = \'post\'',
                (community_id,))
                post_count = cursor.fetchone()['post_count']
                cursor.execute('SELECT COUNT( *) as question_count FROM community_records WHERE community_id = ? AND action_type = \'question\'',
                (community_id,))
    ...
    def create_certification(self, cert_type: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_certification ( cert_id, cert_type, education_type, student_id, student_name, course_id, cert_no, issue_date, expiry_date, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'issued', ?) ''', (cert_id, cert_type, education_type, student_id,
    ...
    def upload_resource(self, resource_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_resources ( resource_id, resource_type, education_type, course_id, title, description, file_url, file_size, duration, uploader_id, uploader_name, views, downloads, is_public, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 1, 'active', ?, ?) ''', (resource_id, resource_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if action_type == 'view':
                        cursor.execute('UPDATE learning_resources SET views = views + 1 WHERE resource_id = ?',
                        (resource_id,))
                    elif action_type == 'download':
                        cursor.execute('UPDATE learning_resources SET downloads = downloads + 1 WHERE resource_id = ?',
                        (resource_id,))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def recommend_resources(self, student_id: int, education_type: str,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT r.* FROM learning_resources r WHERE r.is_public = 1 AND r.status = 'active' AND r.education_type = ? ORDER BY r.views DESC, r.downloads DESC LIMIT ? ''', (education_type, count))
    ...
    def create_support_ticket(self, support_type: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_support ( support_id, support_type, education_type, student_id, student_name, course_id, question, answer, status, response_time, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?) ''', (support_id, support_type, education_type, student_id,
    ...
    def create_assessment(self, assessment_method: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_assessment ( assessment_id, assessment_method, education_type, course_id, title, description, duration, max_score, passing_score, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (assessment_id, assessment_method, education_type,
                          kwargs.get('course_id'), title,
                          kwargs.get('description', config.get('description', '')),
    ...
    def submit_assessment(self, assessment_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT is_active, auto_grade FROM learning_assessment WHERE assessment_id = ?',
                    (assessment_id,))
    ...
    def grade_assessment(self, assessment_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT passing_score FROM learning_assessment WHERE assessment_id = ?',
                    (assessment_id,))
                    assessment = cursor.fetchone()
                    if not assessment:
    ...
    def get_assessment_results(self, assessment_id: str, **kwargs) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT r.*, a.passing_score FROM assessment_records r JOIN learning_assessment a ON r.assessment_id = a.assessment_id WHERE r.assessment_id = ? ''', (assessment_id,))
                results = []
    ...
    def get_learning_statistics(self, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f''' SELECT COUNT(*) as total_courses FROM online_courses WHERE {where_clause} ''', params)
                total_courses = cursor.fetchone()['total_courses']
                cursor.execute(f''' SELECT COUNT(*) as total_students FROM course_records WHERE {where_clause} ''', params)
                total_students = cursor.fetchone()['total_students']
                cursor.execute(f''' SELECT AVG(progress) as avg_progress FROM course_records WHERE {where_clause} ''', params)
                avg_progress = cursor.fetchone()['avg_progress'] or 0
                cursor.execute(f''' SELECT COUNT(*) as total_certificates FROM learning_certification WHERE {where_clause} ''', params)
                total_certificates = cursor.fetchone()['total_certificates']
                cursor.execute(f''' SELECT COUNT(*) as total_resources FROM learning_resources WHERE {where_clause} ''', params)
                total_resources = cursor.fetchone()['total_resources']
                cursor.execute(f''' SELECT COUNT(*) as total_communities FROM learning_community WHERE {where_clause} ''', params)
                total_communities = cursor.fetchone()['total_communities']
                cursor.execute(f''' SELECT COUNT(*) as total_support_tickets FROM learning_support WHERE {where_clause} ''', params)
                total_support_tickets = cursor.fetchone()['total_support_tickets']
                cursor.execute(f''' SELECT COUNT( *) as resolved_tickets FROM learning_support WHERE status = 'resolved' AND {where_clause} ''', params)
