...
# 语言类证书
LANGUAGE_CERTS = {
    'jlpt': {'name': '日语能力测试', 'organization': '日本国际交流基金会', 'levels': ['N5', 'N4', 'N3', 'N2', 'N1'],
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                # 证书目录表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS certifications ( cert_id TEXT PRIMARY KEY, cert_name TEXT NOT NULL, category TEXT NOT NULL, sub_category TEXT, issuing_authority TEXT, level TEXT, validity_period TEXT, exam_fee REAL DEFAULT 0, prerequisite TEXT, description TEXT, education_type TEXT DEFAULT 'common', is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                # 考点表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exam_centers ( center_id TEXT PRIMARY KEY, center_name TEXT NOT NULL, center_code TEXT, address TEXT, city TEXT, contact_phone TEXT, contact_email TEXT, capacity INTEGER DEFAULT 0, room_count INTEGER DEFAULT 0, facilities TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                # 考场表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exam_rooms ( room_id TEXT PRIMARY KEY, center_id TEXT NOT NULL, room_name TEXT NOT NULL, room_code TEXT, seat_count INTEGER DEFAULT 0, facilities TEXT, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                # 考试安排表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exam_schedules ( schedule_id TEXT PRIMARY KEY, cert_id TEXT NOT NULL, exam_name TEXT NOT NULL, exam_date TEXT, registration_deadline TEXT, center_id TEXT, fee REAL DEFAULT 0, max_candidates INTEGER DEFAULT 0, registered_count INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', description TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 报名记录表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS registrations ( reg_id TEXT PRIMARY KEY, schedule_id TEXT NOT NULL, user_id TEXT NOT NULL, candidate_name TEXT, id_number TEXT, phone TEXT, email TEXT, education_type TEXT DEFAULT 'adult', current_level TEXT, status TEXT DEFAULT 'pending', apply_level TEXT, fee_paid REAL DEFAULT 0, paid_at TEXT, review_note TEXT, reviewed_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 准考证表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS admission_tickets ( ticket_id TEXT PRIMARY KEY, reg_id TEXT NOT NULL, user_id TEXT NOT NULL, candidate_name TEXT, schedule_id TEXT NOT NULL, center_id TEXT, room_id TEXT, seat_number TEXT, exam_date TEXT, exam_time TEXT, ticket_code TEXT, issued_at TEXT ) ''')
                # 成绩记录表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exam_scores ( score_id TEXT PRIMARY KEY, reg_id TEXT NOT NULL, user_id TEXT NOT NULL, schedule_id TEXT NOT NULL, cert_id TEXT NOT NULL, total_score REAL, section_scores TEXT, pass_threshold REAL, is_passed INTEGER, rank INTEGER, graded_at TEXT, graded_by TEXT, created_at TEXT ) ''')
                # 证书表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS certificates ( certificate_id TEXT PRIMARY KEY, cert_id TEXT NOT NULL, user_id TEXT NOT NULL, candidate_name TEXT, cert_number TEXT UNIQUE, issue_date TEXT, expiry_date TEXT, score_id TEXT, level_achieved TEXT, status TEXT DEFAULT 'issued', issued_by TEXT, verification_code TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 培训机构表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS training_organizations ( org_id TEXT PRIMARY KEY, org_name TEXT NOT NULL, org_type TEXT, license_number TEXT, legal_person TEXT, address TEXT, contact_phone TEXT, courses INTEGER DEFAULT 0, teacher_count INTEGER DEFAULT 0, rating REAL DEFAULT 0, is_approved INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                # 培训课程表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS training_courses ( course_id TEXT PRIMARY KEY, org_id TEXT NOT NULL, cert_id TEXT, course_name TEXT NOT NULL, duration_hours INTEGER DEFAULT 0, tuition REAL DEFAULT 0, schedule TEXT, max_students INTEGER DEFAULT 0, enrolled_count INTEGER DEFAULT 0, description TEXT, teacher TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                # 课程报名表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_enrollments ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, user_id TEXT NOT NULL, student_name TEXT, enroll_date TEXT, progress REAL DEFAULT 0, completion_status TEXT DEFAULT 'enrolled', score REAL, created_at TEXT ) ''')
                # 证书验证记录表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS cert_verifications ( verify_id TEXT PRIMARY KEY, certificate_id TEXT, cert_number TEXT, verification_code TEXT, verify_time TEXT, verify_ip TEXT, verify_result TEXT, verifier_info TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def register_certification(self, cert_name: str, category: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO certifications ( cert_id, cert_name, category, sub_category, issuing_authority, level, validity_period, exam_fee, prerequisite, description, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (cert_id, cert_name, category,
    ...
    def register_exam_center(self, center_name: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO exam_centers ( center_id, center_name, center_code, address, city, contact_phone, contact_email, capacity, room_count, facilities, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?) ''', (center_id, center_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO exam_rooms ( room_id, center_id, room_name, room_code, seat_count, facilities, is_active, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (room_id, center_id, room_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO exam_schedules ( schedule_id, cert_id, exam_name, exam_date, registration_deadline, center_id, fee, max_candidates, registered_count, status, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?) ''', (schedule_id, cert_id, exam_name, exam_date,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    # 重复报名检查
                    cursor.execute(
                    'SELECT reg_id FROM registrations WHERE schedule_id = ? AND user_id = ? AND status != ?',
                                 (schedule_id, user_id, 'cancelled'))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已报名该考试'}
                    cursor.execute(''' INSERT INTO registrations ( reg_id, schedule_id, user_id, candidate_name, id_number, phone, email, education_type, current_level, status, apply_level, fee_paid, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, 0, ?, ?) ''', (reg_id, schedule_id, user_id, candidate_name,
    ...
    def issue_admission_ticket(self, reg_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT r.reg_id, r.user_id, r.candidate_name, r.schedule_id, e.center_id, e.exam_date FROM registrations r JOIN exam_schedules e ON r.schedule_id = e.schedule_id WHERE r.reg_id = ? AND r.status = ? ''', (reg_id, 'paid'))
                    reg = cursor.fetchone()
                    if not reg:
                        return {'success': False, 'error': '报名记录不存在或未缴费'}
                    # 分配考场与座位
                    cursor.execute(
                    'SELECT room_id FROM exam_rooms WHERE center_id = ? AND is_active = 1 ORDER BY seat_count DESC  LIMIT 1', (reg[4],))
                    room = cursor.fetchone()
                    room_id = room[0] if room else None
                    cursor.execute('SELECT COUNT(*) FROM admission_tickets WHERE schedule_id = ? AND room_id = ?',
                    (reg[3], room_id))
                    seat_no = (cursor.fetchone()[0] or 0) + 1
                    ticket_code = f"ADT{datetime.now().strftime('%Y%m%d')}{uuid.uuid4().hex[:6].upper()}"
                    cursor.execute(''' INSERT INTO admission_tickets ( ticket_id, reg_id, user_id, candidate_name, schedule_id, center_id, room_id, seat_number, exam_date, exam_time, ticket_code, issued_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (ticket_id, reg_id, reg[1], reg[2], reg[3], reg[4],
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO exam_scores ( score_id, reg_id, user_id, schedule_id, cert_id, total_score, section_scores, pass_threshold, is_passed, rank, graded_at, graded_by, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (score_id, reg_id,
                          kwargs.get('user_id'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO certificates ( certificate_id, cert_id, user_id, candidate_name, cert_number, issue_date, expiry_date, score_id, level_achieved, status, issued_by, verification_code, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'issued', ?, ?, ?, ?) ''', (certificate_id, cert_id, user_id,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO cert_verifications ( verify_id, certificate_id, cert_number, verification_code, verify_time, verify_ip, verify_result, verifier_info, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (verify_id, certificate_id, cert_number,
    ...
    def renew_certificate(self, certificate_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT certificate_id, expiry_date FROM certificates WHERE certificate_id = ?',
                    (certificate_id,))
                    cert = cursor.fetchone()
                    if not cert:
                        return {'success': False, 'error': '证书不存在'}
    ...
    def register_training_org(self, org_name: str, org_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO training_organizations ( org_id, org_name, org_type, license_number, legal_person, address, contact_phone, courses, teacher_count, rating, is_approved, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?) ''', (org_id, org_name, org_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO training_courses ( course_id, org_id, cert_id, course_name, duration_hours, tuition, schedule, max_students, enrolled_count, description, teacher, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?) ''', (course_id, org_id, cert_id, course_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT id FROM course_enrollments WHERE course_id = ? AND user_id = ?', (course_id,
                    user_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已报名该课程'}
                    cursor.execute(''' INSERT INTO course_enrollments (course_id, user_id, student_name, enroll_date, progress, completion_status, created_at) VALUES (?, ?, ?, ?, 0, 'enrolled', ?) ''', (course_id, user_id, student_name, now[:10], now))
    ...
    def get_statistics(self, education_type: str = None) -> Dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f"SELECT substr(created_at, 1, 7) as month, COUNT(*) as cnt FROM registrations WHERE 1=1{reg_cond} GROUP BY month ORDER BY month DESC LIMIT 12",
                reg_params)
