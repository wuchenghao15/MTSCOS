...
class InternshipCareerService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS internship_positions ( position_id TEXT PRIMARY KEY, company_id TEXT, company_name TEXT NOT NULL, position_title TEXT NOT NULL, industry TEXT, city TEXT, salary_range TEXT, internship_type TEXT NOT NULL, duration_days INTEGER, start_date TEXT, headcount INTEGER DEFAULT 1, applied_count INTEGER DEFAULT 0, accepted_count INTEGER DEFAULT 0, description TEXT, requirements TEXT, benefits TEXT, mentor_assigned INTEGER DEFAULT 0, status TEXT DEFAULT 'draft', education_type TEXT, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS internship_applications ( application_id TEXT PRIMARY KEY, position_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, education_type TEXT, apply_date TEXT, status TEXT DEFAULT 'applied', resume_url TEXT, cover_letter TEXT, interview_date TEXT, interview_result TEXT, offer_date TEXT, start_date TEXT, end_date TEXT, mentor_name TEXT, weekly_report_count INTEGER DEFAULT 0, final_score REAL, evaluation TEXT, certificate_url TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS companies ( company_id TEXT PRIMARY KEY, company_name TEXT NOT NULL, industry TEXT, company_size TEXT, city TEXT, address TEXT, contact_person TEXT, contact_phone TEXT, contact_email TEXT, website TEXT, description TEXT, logo_url TEXT, cooperation_type TEXT, is_partner INTEGER DEFAULT 0, total_positions INTEGER DEFAULT 0, total_hired INTEGER DEFAULT 0, rating REAL DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_assessments ( assessment_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, assessment_type TEXT NOT NULL, answers TEXT, result TEXT, result_summary TEXT, score REAL, taken_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS employment_records ( record_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, student_name TEXT, education_type TEXT, graduation_year TEXT, company_name TEXT, position_title TEXT, industry TEXT, city TEXT, salary REAL, employment_status TEXT, employ_date TEXT, is_related INTEGER DEFAULT 1, source TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS weekly_reports ( report_id TEXT PRIMARY KEY, application_id TEXT NOT NULL, student_id INTEGER NOT NULL, week_number INTEGER, report_date TEXT, content TEXT, achievements TEXT, challenges TEXT, plan_next_week TEXT, mentor_comment TEXT, mentor_score INTEGER, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_guidance ( guidance_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, guidance_type TEXT, topic TEXT, content TEXT, advisor_id INTEGER, advisor_name TEXT, session_date TEXT, duration_minutes INTEGER, rating INTEGER, feedback TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS campus_recruitment ( recruit_id TEXT PRIMARY KEY, company_id TEXT, company_name TEXT NOT NULL, event_name TEXT, event_date TEXT, location TEXT, description TEXT, positions TEXT, target_majors TEXT, registration_count INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', created_at TEXT, updated_at TEXT ) ''')
                conn.commit()
    ...
    def register_company(self, company_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO companies ( company_id, company_name, industry, company_size, city, address, contact_person, contact_phone, contact_email, website, description, logo_url, cooperation_type, is_partner, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (company_id, company_name, kwargs.get('industry'),
    ...
    def create_position(self, company_id: str, company_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO internship_positions ( position_id, company_id, company_name, position_title, industry, city, salary_range, internship_type, duration_days, start_date, headcount, description, requirements, benefits, mentor_assigned, status, education_type, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?, ?, ?) ''', (position_id, company_id, company_name, position_title,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO internship_applications ( application_id, position_id, student_id, student_name, education_type, apply_date, status, resume_url, cover_letter, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'applied', ?, ?, ?, ?) ''', (application_id, position_id, student_id,
    ...
    def review_application(self, application_id: str, accepted: bool,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT position_id FROM internship_applications WHERE application_id = ?',
                    (application_id,))
                    app = cursor.fetchone()
                    if not app:
                    ...
                    cursor.execute(f''' UPDATE internship_applications SET {", ".join(update_fields)} WHERE application_id = ? AND status IN ('applied', 'ongoing') ''', params)
    ...
    def start_internship(self, application_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE internship_applications SET status = 'ongoing', start_date = ?, updated_at = ? WHERE application_id = ? AND status = 'accepted' ''', (now[:10], now, application_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def complete_internship(self, application_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE internship_applications SET status = 'completed', end_date = ?, final_score = ?, evaluation = ?, certificate_url = ?, updated_at = ? WHERE application_id = ? AND status = 'ongoing' ''', (kwargs.get('end_date', now[:10]),
    ...
    def submit_weekly_report(self, application_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT weekly_report_count FROM internship_applications WHERE application_id = ?',
                    (application_id,))
                    app = cursor.fetchone()
                    if not app:
                        return {'success': False, 'error': '实习申请不存在'}
                    week_number = app[0] + 1
                    cursor.execute(''' INSERT INTO weekly_reports ( report_id, application_id, student_id, week_number, report_date, content, achievements, challenges, plan_next_week, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (report_id, application_id, student_id, week_number,
    ...
    def review_weekly_report(self, report_id: str, mentor_comment: str,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE weekly_reports SET mentor_comment = ?, mentor_score = ? WHERE report_id = ? ''', (mentor_comment, mentor_score, report_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def take_career_assessment(self, student_id: int, assessment_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_assessments ( assessment_id, student_id, assessment_type, answers, result, result_summary, score, taken_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (assessment_id, student_id, assessment_type,
    ...
    def create_career_guidance(self, student_id: int, guidance_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_guidance ( guidance_id, student_id, guidance_type, topic, content, advisor_id, advisor_name, session_date, duration_minutes, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (guidance_id, student_id, guidance_type, topic,
    ...
    def get_student_guidance(self, student_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM career_guidance WHERE student_id = ? ORDER BY session_date DESC',
                (student_id,))
    ...
    def record_employment(self, student_id: int, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO employment_records ( record_id, student_id, student_name, education_type, graduation_year, company_name, position_title, industry, city, salary, employment_status, employ_date, is_related, source, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (record_id, student_id, kwargs.get('student_name'),
    ...
    def get_employment_statistics(self, graduation_year: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f'SELECT industry, COUNT(*) FROM ({query}) GROUP BY industry ORDER BY COUNT(*) DESC',
                params)
                ...
                employed = by_status.get('employed', 0) + by_status.get('further_study',
                0) + by_status.get('entrepreneur', 0)
    ...
    def create_campus_recruitment(self, company_id: str, company_name: str,
        ...
        try:
            ...
            target_majors = json.dumps(kwargs.get('target_majors'),
            ensure_ascii=False) if kwargs.get('target_majors') else None
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO campus_recruitment ( recruit_id, company_id, company_name, event_name, event_date, location, description, positions, target_majors, registration_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'scheduled', ?, ?) ''', (recruit_id, company_id, company_name, event_name,
