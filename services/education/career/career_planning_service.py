...
# 霍兰德6种职业兴趣类型
HOLLAND_TYPES = {
    'R': {'name': '实用型', 'traits': ['动手能力强', '喜欢具体任务', '重视实际', '不善言辞'], 'suitable_jobs': ['工程师', '技术员', '机械师', '农艺师',
    '运动员']},
    'I': {'name': '研究型', 'traits': ['善于观察思考', '逻辑分析强', '喜欢探究', '独立工作'], 'suitable_jobs': ['科学家', '研究员', '医师', '程序员',
    '分析师']},
    'A': {'name': '艺术型', 'traits': ['富有想象', '追求个性', '情感丰富', '不喜约束'], 'suitable_jobs': ['设计师', '作家', '音乐家', '演员',
    '摄影师']},
    'S': {'name': '社会型', 'traits': ['乐于助人', '善于沟通', '合作意识强', '责任感强'], 'suitable_jobs': ['教师', '心理咨询师', '社工', '护士',
    '人力资源']},
    'E': {'name': '企业型', 'traits': ['领导欲强', '敢于冒险', '目标导向', '善说服人'], 'suitable_jobs': ['管理者', '销售经理', '创业者', '律师',
    '市场专员']},
    'C': {'name': '常规型', 'traits': ['细心条理', '服从规范', '注重细节', '稳定可靠'], 'suitable_jobs': ['会计', '行政专员', '银行职员', '统计员',
    '档案管理']}
}
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_profiles ( profile_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, grade_level TEXT, interests TEXT, strengths TEXT, weaknesses TEXT, values TEXT, holland_code TEXT, mbti_type TEXT, career_goal TEXT, target_major TEXT, target_school TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_assessments ( assessment_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, inventory_type TEXT NOT NULL, status TEXT DEFAULT 'in_progress', questions_answered INTEGER DEFAULT 0, total_questions INTEGER DEFAULT 0, answers TEXT, result_code TEXT, result_summary TEXT, result_details TEXT, score REAL, started_at TEXT, completed_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_explorations ( exploration_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, career_category TEXT, career_name TEXT, exploration_type TEXT, description TEXT, duration_hours REAL, outcome TEXT, rating INTEGER, reflection TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_info_library ( career_id TEXT PRIMARY KEY, career_name TEXT NOT NULL, career_category TEXT, description TEXT, required_education TEXT, required_skills TEXT, salary_range TEXT, employment_outlook TEXT, work_environment TEXT, related_majors TEXT, growth_path TEXT, view_count INTEGER DEFAULT 0, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_plans ( plan_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, current_stage TEXT, target_stage TEXT, target_school TEXT, target_major TEXT, application_year TEXT, requirements TEXT, progress INTEGER DEFAULT 0, status TEXT DEFAULT 'planning', notes TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS subject_selections ( selection_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, grade_level TEXT, combination TEXT, subjects TEXT, selected_at TEXT, advisor_id INTEGER, advisor_name TEXT, advice TEXT, rationale TEXT, status TEXT DEFAULT 'pending', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_courses ( course_id TEXT PRIMARY KEY, course_name TEXT NOT NULL, course_type TEXT, description TEXT, target_audience TEXT, instructor TEXT, duration_hours REAL, schedule TEXT, location TEXT, max_participants INTEGER DEFAULT 50, enrolled_count INTEGER DEFAULT 0, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, register_time TEXT, attendance_status TEXT DEFAULT 'registered', completion INTEGER DEFAULT 0, created_at TEXT, UNIQUE(course_id, user_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS counseling_sessions ( session_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, counselor_id INTEGER, counselor_name TEXT, counseling_type TEXT, topic TEXT, content TEXT, suggestions TEXT, next_steps TEXT, session_date TEXT, duration INTEGER, status TEXT DEFAULT 'booked', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_milestones ( milestone_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, milestone_name TEXT NOT NULL, milestone_type TEXT, target_date TEXT, achieved_date TEXT, status TEXT DEFAULT 'pending', description TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_goal_tracking ( goal_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, goal_name TEXT NOT NULL, goal_type TEXT, description TEXT, target_date TEXT, progress INTEGER DEFAULT 0, milestones TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS career_events ( event_id TEXT PRIMARY KEY, event_name TEXT NOT NULL, event_type TEXT, description TEXT, organizer TEXT, start_date TEXT, end_date TEXT, location TEXT, max_participants INTEGER DEFAULT 100, registered_count INTEGER DEFAULT 0, target_audience TEXT, created_at TEXT, updated_at TEXT ) ''')
                conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO career_profiles ( profile_id, user_id, user_name, education_type, grade_level, interests, strengths, weaknesses, values, holland_code, mbti_type, career_goal, target_major, target_school, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (profile_id, user_id, user_name,
    ...
    def start_assessment(self, user_id: int, inventory_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_assessments ( assessment_id, user_id, user_name, inventory_type, status, questions_answered, total_questions, answers, started_at, created_at ) VALUES (?, ?, ?, ?, 'in_progress', 0, ?, '[]', ?, ?) ''', (assessment_id, user_id, kwargs.get('user_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE career_assessments SET status = 'completed', questions_answered = ?, answers = ?, result_code = ?, result_summary = ?, result_details = ?, score = ?, completed_at = ? WHERE assessment_id = ? ''', (len(answers), json.dumps(answers, ensure_ascii=False),
    ...
    def add_career_to_library(self, career_name: str, career_category: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_info_library ( career_id, career_name, career_category, description, required_education, required_skills, salary_range, employment_outlook, work_environment, related_majors, growth_path, view_count, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 1, ?, ?) ''', (career_id, career_name, career_category,
    ...
    def record_exploration(self, user_id: int, career_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_explorations ( exploration_id, user_id, career_category, career_name, exploration_type, description, duration_hours, outcome, rating, reflection, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (exploration_id, user_id,
    ...
    def create_education_plan(self, user_id: int, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_plans ( plan_id, user_id, user_name, education_type, current_stage, target_stage, target_school, target_major, application_year, requirements, progress, status, notes, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'planning', ?, ?, ?) ''', (plan_id, user_id, kwargs.get('user_name'),
                          kwargs.get('education_type'),
    ...
    def record_subject_selection(self, user_id: int, combination: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO subject_selections ( selection_id, user_id, user_name, grade_level, combination, subjects, selected_at, advisor_id, advisor_name, advice, rationale, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?) ''', (selection_id, user_id, kwargs.get('user_name'),
    ...
    def create_career_course(self, course_name: str, course_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_courses ( course_id, course_name, course_type, description, target_audience, instructor, duration_hours, schedule, location, max_participants, enrolled_count, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?) ''', (course_id, course_name, course_type,
                          kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO course_registrations (course_id, user_id, user_name, register_time, attendance_status, completion, created_at) VALUES (?, ?, ?, ?, 'registered', 0, ?) ''', (course_id, user_id, kwargs.get('user_name'), now, now))
                    if cursor.rowcount > 0:
    ...
    def create_career_event(self, event_name: str, event_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_events ( event_id, event_name, event_type, description, organizer, start_date, end_date, location, max_participants, registered_count, target_audience, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?) ''', (event_id, event_name, event_type,
                          kwargs.get('description'),
    ...
    def book_counseling(self, user_id: int, counselor_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO counseling_sessions ( session_id, user_id, user_name, counselor_id, counselor_name, counseling_type, topic, session_date, duration, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'booked', ?) ''', (session_id, user_id, kwargs.get('user_name'),
    ...
    def record_counseling(self, session_id: str, content: str,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE counseling_sessions SET content = ?, suggestions = ?, next_steps = ?, status = 'completed' WHERE session_id = ? AND status = 'booked' ''', (content, kwargs.get('suggestions'),
    ...
    def create_goal(self, user_id: int, goal_name: str, goal_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_goal_tracking ( goal_id, user_id, goal_name, goal_type, description, target_date, progress, milestones, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 0, '[]', 'active', ?, ?) ''', (goal_id, user_id, goal_name, goal_type,
                          kwargs.get('description'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if milestones is not None:
                        cursor.execute(''' UPDATE career_goal_tracking SET progress = ?, status = ?, milestones = ?, updated_at = ? WHERE goal_id = ? ''', (progress, status,
                              json.dumps(milestones, ensure_ascii=False),
                              now, goal_id))
                    else:
                        cursor.execute(''' UPDATE career_goal_tracking SET progress = ?, status = ?, updated_at = ? WHERE goal_id = ? ''', (progress, status, now, goal_id))
                    if cursor.rowcount > 0:
                        conn.commit()
    ...
    def create_milestone(self, user_id: int, milestone_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO career_milestones ( milestone_id, user_id, milestone_name, milestone_type, target_date, achieved_date, status, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, NULL, 'pending', ?, ?, ?) ''', (milestone_id, user_id, milestone_name, milestone_type,
    ...
    def complete_milestone(self, milestone_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE career_milestones SET status = 'achieved', achieved_date = ?, updated_at = ? WHERE milestone_id = ? AND status = 'pending' ''', (achieved_date, now, milestone_id))
                    if cursor.rowcount > 0:
                        conn.commit()
