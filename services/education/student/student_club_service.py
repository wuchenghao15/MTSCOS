    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                # 社团表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS clubs ( club_id TEXT PRIMARY KEY, club_name TEXT NOT NULL, category TEXT, description TEXT, logo TEXT, education_type TEXT, founder_id TEXT, founder_name TEXT, president_id TEXT, president_name TEXT, instructor_id TEXT, instructor_name TEXT, established_date TEXT, member_count INTEGER DEFAULT 0, max_members INTEGER DEFAULT 50, meeting_schedule TEXT, location TEXT, contact_info TEXT, status TEXT DEFAULT 'pending', level TEXT, total_score REAL DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                # 社团成员表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, club_id TEXT NOT NULL, student_id TEXT NOT NULL, student_name TEXT, role TEXT DEFAULT 'member', join_date TEXT, status TEXT DEFAULT 'active', attendance_rate REAL DEFAULT 0, contribution_score REAL DEFAULT 0, total_activities INTEGER DEFAULT 0, leave_reason TEXT, left_date TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 社团活动表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_activities ( activity_id TEXT PRIMARY KEY, club_id TEXT NOT NULL, activity_name TEXT NOT NULL, activity_type TEXT, description TEXT, objectives TEXT, start_date TEXT, end_date TEXT, start_time TEXT, end_time TEXT, location TEXT, venue_booking_id TEXT, max_participants INTEGER DEFAULT 50, registered_count INTEGER DEFAULT 0, actual_participants INTEGER DEFAULT 0, budget REAL DEFAULT 0, status TEXT DEFAULT 'planning', approval_status TEXT DEFAULT 'pending', approved_by TEXT, approved_at TEXT, outcome TEXT, summary TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 活动报名表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS activity_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, activity_id TEXT NOT NULL, club_id TEXT, student_id TEXT NOT NULL, student_name TEXT, register_time TEXT, attend_status TEXT DEFAULT 'registered', role_in_activity TEXT, created_at TEXT ) ''')
                # 社团经费表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_funds ( fund_id TEXT PRIMARY KEY, club_id TEXT NOT NULL, fund_type TEXT, amount REAL DEFAULT 0, direction TEXT, category TEXT, description TEXT, transaction_date TEXT, operator_id TEXT, operator_name TEXT, balance_after REAL DEFAULT 0, receipt_url TEXT, status TEXT DEFAULT 'confirmed', created_at TEXT ) ''')
                # 经费预算表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS fund_budgets ( budget_id TEXT PRIMARY KEY, club_id TEXT NOT NULL, fiscal_year INTEGER, fiscal_semester TEXT, total_budget REAL DEFAULT 0, total_income REAL DEFAULT 0, total_expense REAL DEFAULT 0, category_budgets TEXT, status TEXT DEFAULT 'draft', created_at TEXT, updated_at TEXT ) ''')
                # 社团成果表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_achievements ( achievement_id TEXT PRIMARY KEY, club_id TEXT NOT NULL, achievement_type TEXT, title TEXT NOT NULL, description TEXT, file_url TEXT, event_name TEXT, event_date TEXT, achievement_date TEXT, participants TEXT, rating REAL DEFAULT 0, is_public INTEGER DEFAULT 1, created_at TEXT ) ''')
                # 社团评价表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_evaluations ( evaluation_id TEXT PRIMARY KEY, club_id TEXT NOT NULL, evaluator_id TEXT, evaluator_name TEXT, evaluator_type TEXT, evaluation_period TEXT, dimension_scores TEXT, total_score REAL DEFAULT 0, level TEXT, comment TEXT, suggestions TEXT, created_at TEXT ) ''')
                # 成员评价表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS member_evaluations ( evaluation_id TEXT PRIMARY KEY, club_id TEXT NOT NULL, member_id TEXT NOT NULL, member_name TEXT, evaluator_id TEXT, evaluator_name TEXT, evaluator_type TEXT, evaluation_period TEXT, attendance_score REAL DEFAULT 0, contribution_score REAL DEFAULT 0, skill_score REAL DEFAULT 0, attitude_score REAL DEFAULT 0, total_score REAL DEFAULT 0, comment TEXT, created_at TEXT ) ''')
                # 活动评价表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS activity_evaluations ( evaluation_id TEXT PRIMARY KEY, activity_id TEXT NOT NULL, club_id TEXT, evaluator_id TEXT, evaluator_name TEXT, evaluator_type TEXT, organization_score REAL DEFAULT 0, content_score REAL DEFAULT 0, satisfaction_score REAL DEFAULT 0, total_score REAL DEFAULT 0, comment TEXT, suggestions TEXT, created_at TEXT ) ''')
                # 指导教师表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_instructors ( id INTEGER PRIMARY KEY AUTOINCREMENT, club_id TEXT NOT NULL, instructor_id TEXT, instructor_name TEXT, assigned_date TEXT, end_date TEXT, responsibilities TEXT, guidance_hours REAL DEFAULT 0, evaluation_score REAL DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT ) ''')
                # 招新活动表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_recruitments ( recruitment_id TEXT PRIMARY KEY, club_id TEXT NOT NULL, recruitment_name TEXT NOT NULL, academic_year TEXT, semester TEXT, start_date TEXT, end_date TEXT, target_audience TEXT, description TEXT, requirements TEXT, quota INTEGER DEFAULT 30, applied_count INTEGER DEFAULT 0, accepted_count INTEGER DEFAULT 0, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT ) ''')
                # 招新报名表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS recruitment_applications ( application_id TEXT PRIMARY KEY, recruitment_id TEXT NOT NULL, club_id TEXT, student_id TEXT NOT NULL, student_name TEXT, grade TEXT, apply_date TEXT, self_intro TEXT, skills TEXT, interview_score REAL, interview_notes TEXT, status TEXT DEFAULT 'pending', accepted_at TEXT, created_at TEXT ) ''')
                # 社团场地表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_venues ( venue_id TEXT PRIMARY KEY, venue_name TEXT NOT NULL, location TEXT, capacity INTEGER DEFAULT 50, facilities TEXT, available_times TEXT, manager_id TEXT, manager_name TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                # 场地预约表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS venue_bookings ( booking_id TEXT PRIMARY KEY, venue_id TEXT NOT NULL, club_id TEXT, activity_id TEXT, booker_id TEXT, booker_name TEXT, booking_date TEXT, start_time TEXT, end_time TEXT, purpose TEXT, status TEXT DEFAULT 'pending', created_at TEXT ) ''')
                conn.commit()
    ...
    def create_club(self, club_name: str, category: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO clubs ( club_id, club_name, category, description, logo, education_type, founder_id, founder_name, president_id, president_name, instructor_id, instructor_name, established_date, member_count, max_members, meeting_schedule, location, contact_info, status, level, total_score, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, 'pending', NULL, 0, ?, ?) ''', (club_id, club_name, category,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO club_members (club_id, student_id, student_name, role, join_date, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'active', ?, ?) ''', (club_id, student_id, kwargs.get('student_name'),
                          kwargs.get('role', 'member'), now[:10], now, now))
                    cursor.execute('UPDATE clubs SET member_count = member_count + 1, updated_at = ? WHERE club_id = ?',
                    (now, club_id))
                    conn.commit()
    ...
    def create_recruitment(self, club_id: str, recruitment_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO club_recruitments ( recruitment_id, club_id, recruitment_name, academic_year, semester, start_date, end_date, target_audience, description, requirements, quota, applied_count, accepted_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 'open', ?, ?) ''', (recruitment_id, club_id, recruitment_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(
                    'SELECT application_id FROM recruitment_applications WHERE recruitment_id = ? AND student_id = ?',
                    (recruitment_id, student_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已报名该招新'}
                    cursor.execute(''' INSERT INTO recruitment_applications ( application_id, recruitment_id, club_id, student_id, student_name, grade, apply_date, self_intro, skills, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?) ''', (application_id, recruitment_id, rec[0], student_id,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not cursor.fetchone():
                        cursor.execute(''' INSERT INTO club_members (club_id, student_id, student_name, role, join_date, status, created_at, updated_at) VALUES (?, ?, ?, 'member', ?, 'active', ?, ?) ''', (app[1], app[2], app[3], now[:10], now, now))
    ...
    def list_recruitments(self, page: int = 1, page_size: int = 20, **filters) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'recruitments': recruitments, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def create_activity(self, club_id: str, activity_name: str, activity_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO club_activities ( activity_id, club_id, activity_name, activity_type, description, objectives, start_date, end_date, start_time, end_time, location, venue_booking_id, max_participants, registered_count, actual_participants, budget, status, approval_status, approved_by, approved_at, outcome, summary, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, 0, 0, ?, 'planning', ?, NULL, NULL, NULL, NULL, ?, ?) ''', (activity_id, club_id, activity_name, activity_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT id FROM activity_registrations WHERE activity_id = ? AND student_id = ?',
                    (activity_id, student_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已报名该活动'}
                    cursor.execute(''' INSERT INTO activity_registrations (activity_id, club_id, student_id, student_name, register_time, attend_status, role_in_activity, created_at) VALUES (?, ?, ?, ?, ?, 'registered', ?, ?) ''', (activity_id, act[0], student_id, kwargs.get('student_name'),
    ...
    def register_venue(self, venue_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO club_venues ( venue_id, venue_name, location, capacity, facilities, available_times, manager_id, manager_name, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (venue_id, venue_name, kwargs.get('location'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' SELECT booking_id FROM venue_bookings WHERE venue_id = ? AND booking_date = ? AND status != ? AND start_time < ? AND end_time > ? ''', (venue_id, booking_date, 'cancelled', end_time, start_time))
                    if cursor.fetchone():
                        return {'success': False, 'error': '场地该时段已被预约'}
                    cursor.execute(''' INSERT INTO venue_bookings ( booking_id, venue_id, club_id, activity_id, booker_id, booker_name, booking_date, start_time, end_time, purpose, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'confirmed', ?) ''', (booking_id, venue_id, club_id, kwargs.get('activity_id'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    # 获取当前余额
                    cursor.execute(
                    'SELECT balance_after FROM club_funds WHERE club_id = ? ORDER BY created_at DESC LIMIT 1', (club_id,
                    ))
                    row = cursor.fetchone()
                    ...
                    if direction == 'income':
                        ...
                        new_balance = current_balance - amount
                    cursor.execute(''' INSERT INTO club_funds ( fund_id, club_id, fund_type, amount, direction, category, description, transaction_date, operator_id, operator_name, balance_after, receipt_url, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'confirmed', ?) ''', (fund_id, club_id, fund_type, amount, direction,
    ...
    def create_budget(self, club_id: str, fiscal_year: int, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO fund_budgets ( budget_id, club_id, fiscal_year, fiscal_semester, total_budget, total_income, total_expense, category_budgets, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 0, 0, ?, 'draft', ?, ?) ''', (budget_id, club_id, fiscal_year, kwargs.get('fiscal_semester'),
    ...
    def record_achievement(self, club_id: str, achievement_type: str, title: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO club_achievements ( achievement_id, club_id, achievement_type, title, description, file_url, event_name, event_date, achievement_date, participants, rating, is_public, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (achievement_id, club_id, achievement_type, title,
    ...
    def evaluate_club(self, club_id: str, evaluator_id: str, dimension_scores: dict, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO club_evaluations ( evaluation_id, club_id, evaluator_id, evaluator_name, evaluator_type, evaluation_period, dimension_scores, total_score, level, comment, suggestions, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (evaluation_id, club_id, evaluator_id,
    ...
    def evaluate_member(self, club_id: str, member_id: str, evaluator_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO member_evaluations ( evaluation_id, club_id, member_id, member_name, evaluator_id, evaluator_name, evaluator_type, evaluation_period, attendance_score, contribution_score, skill_score, attitude_score, total_score, comment, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (evaluation_id, club_id, member_id, kwargs.get('member_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO activity_evaluations ( evaluation_id, activity_id, club_id, evaluator_id, evaluator_name, evaluator_type, organization_score, content_score, satisfaction_score, total_score, comment, suggestions, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (evaluation_id, activity_id, club_id, evaluator_id,
                          kwargs.get('evaluator_name'), kwargs.get('evaluator_type'),
    ...
    def assign_instructor(self, club_id: str, instructor_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO club_instructors ( club_id, instructor_id, instructor_name, assigned_date, end_date, responsibilities, guidance_hours, evaluation_score, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 'active', ?) ''', (club_id, instructor_id, kwargs.get('instructor_name'),
    ...
    def list_instructors(self, club_id: str = None, page: int = 1, page_size: int = 20) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'instructors': instructors, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def get_statistics(self, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                # 成员人数分布（按社团类别）
                if education_type:
                    cursor.execute(''' SELECT c.category, COUNT(m.id) as cnt FROM club_members m JOIN clubs c ON m.club_id = c.club_id WHERE m.status = 'active' AND c.education_type = ? GROUP BY c.category ''', [education_type])
                else:
                    cursor.execute(''' SELECT c.category, COUNT(m.id) as cnt FROM club_members m JOIN clubs c ON m.club_id = c.club_id WHERE m.status = 'active' GROUP BY c.category ''')
                ...
                cursor.execute(f''' SELECT COUNT(DISTINCT a.activity_id) as total, SUM(CASE WHEN a.status = 'completed' THEN 1 ELSE 0 END) as completed, SUM(CASE WHEN a.status = 'in_progress' THEN 1 ELSE 0 END) as in_progress, SUM(a.registered_count) as registered FROM club_activities a JOIN clubs c ON a.club_id = c.club_id WHERE 1=1{act_cond} ''', act_params)
                ...
                # 经费收支统计
                cursor.execute(f''' SELECT f.direction, SUM(f.amount) as total FROM club_funds f JOIN clubs c ON f.club_id = c.club_id WHERE 1=1{act_cond} GROUP BY f.direction ''', act_params)
