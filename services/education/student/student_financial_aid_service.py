    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS aid_policies ( policy_id TEXT PRIMARY KEY, policy_name TEXT NOT NULL, aid_type TEXT, target_group TEXT, eligibility TEXT, amount_range TEXT, application_period_start TEXT, application_period_end TEXT, required_materials TEXT, description TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS aid_applications ( application_id TEXT PRIMARY KEY, policy_id TEXT, student_id INTEGER, student_name TEXT, education_type TEXT, grade_level TEXT, apply_type TEXT, apply_amount REAL, reason TEXT, family_income REAL, family_members INTEGER, difficulty_level TEXT, materials TEXT, status TEXT DEFAULT 'pending', submitted_at TEXT, reviewed_by TEXT, reviewed_at TEXT, review_comment TEXT, approved_amount REAL, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS scholarships ( scholarship_id TEXT PRIMARY KEY, scholarship_name TEXT NOT NULL, scholarship_type TEXT, level TEXT, amount REAL, donor TEXT, donor_type TEXT, eligibility TEXT, quota INTEGER DEFAULT 0, applied_count INTEGER DEFAULT 0, awarded_count INTEGER DEFAULT 0, academic_year TEXT, semester TEXT, description TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS scholarship_applications ( application_id TEXT PRIMARY KEY, scholarship_id TEXT, student_id INTEGER, student_name TEXT, grade_level TEXT, gpa REAL, rank_in_grade INTEGER, achievements TEXT, self_statement TEXT, materials TEXT, status TEXT DEFAULT 'pending', score REAL, ranked_position INTEGER, reviewed_by TEXT, reviewed_at TEXT, review_comment TEXT, awarded_amount REAL, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS difficulty_assessments ( assessment_id TEXT PRIMARY KEY, student_id INTEGER, student_name TEXT, education_type TEXT, grade_level TEXT, family_income REAL, family_members INTEGER, per_capita_income REAL, difficulty_level TEXT, special_circumstances TEXT, support_documents TEXT, assessor_id TEXT, assessor_name TEXT, assessment_date TEXT, valid_until TEXT, status TEXT DEFAULT 'pending', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS tuition_waivers ( waiver_id TEXT PRIMARY KEY, student_id INTEGER, student_name TEXT, education_type TEXT, waiver_type TEXT, original_fee REAL, waiver_percent REAL, waiver_amount REAL, actual_fee REAL, reason TEXT, materials TEXT, status TEXT DEFAULT 'pending', approved_by TEXT, approved_at TEXT, academic_year TEXT, semester TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS work_study_positions ( position_id TEXT PRIMARY KEY, position_name TEXT NOT NULL, category TEXT, department TEXT, supervisor_id TEXT, supervisor_name TEXT, weekly_hours INTEGER DEFAULT 10, hourly_wage REAL DEFAULT 15, total_positions INTEGER DEFAULT 1, filled_positions INTEGER DEFAULT 0, requirements TEXT, semester TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS work_study_records ( record_id TEXT PRIMARY KEY, position_id TEXT, student_id INTEGER, student_name TEXT, start_date TEXT, end_date TEXT, total_hours REAL DEFAULT 0, total_earned REAL DEFAULT 0, monthly_hours TEXT, evaluation TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS student_loans ( loan_id TEXT PRIMARY KEY, student_id INTEGER, student_name TEXT, education_type TEXT, loan_type TEXT, loan_amount REAL, loan_date TEXT, repayment_start_date TEXT, repayment_end_date TEXT, interest_rate REAL, total_repay REAL, repaid_amount REAL DEFAULT 0, remaining_amount REAL, status TEXT DEFAULT 'pending', contract_number TEXT, guarantor TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS aid_disbursements ( disbursement_id TEXT PRIMARY KEY, application_id TEXT, student_id INTEGER, student_name TEXT, aid_type TEXT, amount REAL, payment_method TEXT, bank_account TEXT, payment_date TEXT, batch_number TEXT, status TEXT DEFAULT 'pending', operator_id TEXT, operator_name TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS aid_statistics ( stat_id TEXT PRIMARY KEY, academic_year TEXT, semester TEXT, education_type TEXT, total_applications INTEGER DEFAULT 0, approved_count INTEGER DEFAULT 0, total_amount REAL DEFAULT 0, by_type TEXT, by_level TEXT, generated_at TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_aid_policy(self, policy_name: str, aid_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO aid_policies ( policy_id, policy_name, aid_type, target_group, eligibility, amount_range, application_period_start, application_period_end, required_materials, description, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (policy_id, policy_name, aid_type,
    ...
    def apply_difficulty_assessment(self, student_id: int, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO difficulty_assessments ( assessment_id, student_id, student_name, education_type, grade_level, family_income, family_members, per_capita_income, difficulty_level, special_circumstances, support_documents, assessor_id, assessor_name, assessment_date, valid_until, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (assessment_id, student_id, kwargs.get('student_name'),
    ...
    def review_difficulty_assessment(self, assessment_id: str, difficulty_level: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE difficulty_assessments SET difficulty_level = ?, status = ?, assessor_id = ?, assessor_name = ?, assessment_date = ?, valid_until = ?, updated_at = ? WHERE assessment_id = ? AND status = 'pending' ''', (difficulty_level, status,
                          kwargs.get('assessor_id'), kwargs.get('assessor_name'),
    ...
    def get_difficulty_assessment(self, student_id: int) -> Dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT * FROM difficulty_assessments WHERE student_id = ? AND status = 'approved' ORDER BY created_at DESC LIMIT 1 ''', (student_id,))
                row = cursor.fetchone()
                if not row:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO aid_applications ( application_id, policy_id, student_id, student_name, education_type, grade_level, apply_type, apply_amount, reason, family_income, family_members, difficulty_level, materials, status, submitted_at, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?) ''', (application_id, policy_id, student_id, kwargs.get('student_name'),
                          kwargs.get('education_type'), kwargs.get('grade_level'),
    ...
    def review_aid_application(self, application_id: str, approved: bool,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE aid_applications SET status = ?, reviewed_by = ?, reviewed_at = ?, review_comment = ?, approved_amount = ?, updated_at = ? WHERE application_id = ? AND status IN ('pending', 'under_review') ''', (status, kwargs.get('reviewed_by'), now,
    ...
    def create_scholarship(self, scholarship_name: str, scholarship_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO scholarships ( scholarship_id, scholarship_name, scholarship_type, level, amount, donor, donor_type, eligibility, quota, applied_count, awarded_count, academic_year, semester, description, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?, ?, ?, ?, ?, ?) ''', (scholarship_id, scholarship_name, scholarship_type, level, amount,
    ...
    def apply_scholarship(self, scholarship_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT quota, applied_count, is_active FROM scholarships WHERE scholarship_id = ?',
                    (scholarship_id,))
                    ...
                    cursor.execute(
                    'SELECT application_id FROM scholarship_applications WHERE scholarship_id = ? AND student_id = ?',
                                 (scholarship_id, student_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已申请该奖学金'}
                    cursor.execute(''' INSERT INTO scholarship_applications ( application_id, scholarship_id, student_id, student_name, grade_level, gpa, rank_in_grade, achievements, self_statement, materials, status, score, ranked_position, awarded_amount, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', 0, 0, 0, ?, ?) ''', (application_id, scholarship_id, student_id, kwargs.get('student_name'),
    ...
    def review_scholarship_application(self, application_id: str, score: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT scholarship_id FROM scholarship_applications WHERE application_id = ?',
                    (application_id,))
                    row = cursor.fetchone()
                    if not row:
                        return {'success': False, 'error': '申请不存在'}
                    scholarship_id = row[0]
                    cursor.execute(''' UPDATE scholarship_applications SET score = ?, status = 'under_review', reviewed_by = ?, reviewed_at = ?, review_comment = ?, updated_at = ? WHERE application_id = ? ''', (score, kwargs.get('reviewed_by'), now,
                          kwargs.get('review_comment'), now, application_id))
                    # 计算排名：按分数降序排名
                    cursor.execute(''' SELECT application_id FROM scholarship_applications WHERE scholarship_id = ? AND score IS NOT NULL ORDER BY score DESC ''', (scholarship_id,))
                    ranked_ids = [r[0] for r in cursor.fetchall()]
                    for idx, aid in enumerate(ranked_ids, start=1):
                        cursor.execute(
                        'UPDATE scholarship_applications SET ranked_position = ? WHERE application_id = ?',
                                     (idx, aid))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'SELECT scholarship_id FROM scholarship_applications WHERE application_id = ? AND status =  "under_review"',
                                 (application_id,))
                    row = cursor.fetchone()
                    if not row:
    ...
    def apply_tuition_waiver(self, student_id: int, waiver_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO tuition_waivers ( waiver_id, student_id, student_name, education_type, waiver_type, original_fee, waiver_percent, waiver_amount, actual_fee, reason, materials, status, academic_year, semester, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?) ''', (waiver_id, student_id, kwargs.get('student_name'),
    ...
    def review_tuition_waiver(self, waiver_id: str, approved: bool,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE tuition_waivers SET status = ?, approved_by = ?, approved_at = ?, updated_at = ? WHERE waiver_id = ? AND status = 'pending' ''', (status, kwargs.get('approved_by'), now, now, waiver_id))
                    if cursor.rowcount > 0:
                        conn.commit()
    ...
    def create_work_study_position(self, position_name: str, category: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO work_study_positions ( position_id, position_name, category, department, supervisor_id, supervisor_name, weekly_hours, hourly_wage, total_positions, filled_positions, requirements, semester, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?) ''', (position_id, position_name, category,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(
                    'SELECT record_id FROM work_study_records WHERE position_id = ? AND student_id = ? AND status =  "active"',
                                 (position_id, student_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已申请该岗位'}
                    cursor.execute(''' INSERT INTO work_study_records ( record_id, position_id, student_id, student_name, start_date, end_date, total_hours, total_earned, monthly_hours, evaluation, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, NULL, 0, 0, ?, NULL, 'active', ?, ?) ''', (record_id, position_id, student_id, kwargs.get('student_name'),
    ...
    def record_work_study(self, record_id: str, hours: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT r.position_id, p.hourly_wage FROM work_study_records r JOIN work_study_positions p ON r.position_id = p.position_id WHERE r.record_id = ? AND r.status = 'active' ''', (record_id,))
                    row = cursor.fetchone()
                    if not row:
                    ...
                    cursor.execute(''' UPDATE work_study_records SET total_hours = total_hours + ?, total_earned = total_earned + ?, monthly_hours = ?, updated_at = ? WHERE record_id = ? ''', (hours, earned, json.dumps(monthly, ensure_ascii=False), now, record_id))
                    conn.commit()
    ...
    def apply_loan(self, student_id: int, loan_type: str, loan_amount: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO student_loans ( loan_id, student_id, student_name, education_type, loan_type, loan_amount, loan_date, repayment_start_date, repayment_end_date, interest_rate, total_repay, repaid_amount, remaining_amount, status, contract_number, guarantor, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'pending', ?, ?, ?, ?) ''', (loan_id, student_id, kwargs.get('student_name'),
    ...
    def approve_loan(self, loan_id: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE student_loans SET status = 'approved', contract_number = ?, updated_at = ? WHERE loan_id = ? AND status = 'pending' ''', (contract_number, now, loan_id))
                    if cursor.rowcount > 0:
                        conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE student_loans SET repaid_amount = ?, remaining_amount = ?, status = ?, updated_at = ? WHERE loan_id = ? ''', (new_repaid, new_remaining, status, now, loan_id))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO aid_disbursements ( disbursement_id, application_id, student_id, student_name, aid_type, amount, payment_method, bank_account, payment_date, batch_number, status, operator_id, operator_name, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'issued', ?, ?, ?) ''', (disbursement_id, application_id, app[0], app[1], app[2], amount,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO aid_statistics ( stat_id, academic_year, semester, education_type, total_applications, approved_count, total_amount, by_type, by_level, generated_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (stat_id, academic_year, semester, education_type,
    ...
    def get_statistics(self, education_type: str = None) -> Dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                result['aid_type_distribution'] = {row[0]: {'count': row[1],
                'amount': row[2]} for row in cursor.fetchall()}
                # 困难等级分布
