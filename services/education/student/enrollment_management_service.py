#!/usr/bin/env python3
# -*- coding: utf-8 -*-
""" MTSCOS 招生报名管理服务 (v15.4.0) ==================================== 提供招生计划、在线报名、入学审核和录取管理等综合服务。  核心能力： 1. 招生计划 - 招生方案制定、名额管理 2. 在线报名 - 报名表单、材料提交 3. 入学审核 - 资格审核、材料审核 4. 录取管理 - 录取通知、分班 5. 招生统计 - 报名统计、录取率分析 6. 招生宣传 - 招生简章、专业介绍 7. 成人招生 - 成人教育招生管理 8. K12招生 - 九年制义务教育招生 """
import os
import json
import uuid
import sqlite3
...
class EnrollmentManagementService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS enrollment_plans ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, enrollment_type TEXT NOT NULL, education_type TEXT NOT NULL, semester TEXT, academic_year TEXT, target_grade INTEGER, target_direction TEXT, max_students INTEGER DEFAULT 30, current_applications INTEGER DEFAULT 0, accepted_count INTEGER DEFAULT 0, enrolled_count INTEGER DEFAULT 0, start_date TEXT, end_date TEXT, requirements TEXT, description TEXT, status TEXT DEFAULT 'draft', created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS applications ( application_id TEXT PRIMARY KEY, application_no TEXT UNIQUE, plan_id TEXT NOT NULL, applicant_name TEXT NOT NULL, applicant_id_number TEXT, gender TEXT, birth_date TEXT, phone TEXT, email TEXT, address TEXT, education_type TEXT, target_grade INTEGER, target_direction TEXT, previous_school TEXT, gpa REAL, application_data TEXT, documents TEXT, status TEXT DEFAULT 'draft', submitted_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS application_reviews ( id INTEGER PRIMARY KEY AUTOINCREMENT, application_id TEXT NOT NULL, review_type TEXT NOT NULL, reviewer_id INTEGER, reviewer_name TEXT, result TEXT, score REAL, comment TEXT, reviewed_at TEXT, UNIQUE(application_id, review_type) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS admission_notices ( notice_id TEXT PRIMARY KEY, application_id TEXT NOT NULL, student_id INTEGER, plan_id TEXT, notice_type TEXT DEFAULT 'admission', title TEXT, content TEXT, class_id TEXT, grade_level INTEGER, registration_deadline TEXT, requirements TEXT, sent_at TEXT, is_read INTEGER DEFAULT 0, read_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS enrollment_promotions ( promotion_id TEXT PRIMARY KEY, plan_id TEXT, title TEXT NOT NULL, content TEXT, target_audience TEXT, publish_channel TEXT, views INTEGER DEFAULT 0, clicks INTEGER DEFAULT 0, is_published INTEGER DEFAULT 0, published_at TEXT, created_by INTEGER, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS interview_schedules ( interview_id TEXT PRIMARY KEY, application_id TEXT NOT NULL, plan_id TEXT, interview_date TEXT, interview_time TEXT, location TEXT, interviewer_id INTEGER, interviewer_name TEXT, duration_minutes INTEGER DEFAULT 30, status TEXT DEFAULT 'scheduled', result TEXT, score REAL, feedback TEXT, created_at TEXT, updated_at TEXT ) ''')
                conn.commit()
    ...
    def create_enrollment_plan(self, plan_name: str, enrollment_type: str,
        ...
        try:
            ...
            requirements = json.dumps(kwargs.get('requirements'),
            ensure_ascii=False) if kwargs.get('requirements') else None
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO enrollment_plans ( plan_id, plan_name, enrollment_type, education_type, semester, academic_year, target_grade, target_direction, max_students, current_applications, accepted_count, enrolled_count, start_date, end_date, requirements, description, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, ?, ?, ?, ?, ?, ?, ?, ?) ''', (plan_id, plan_name, enrollment_type, education_type,
    ...
    def submit_application(self, plan_id: str, applicant_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            now = datetime.now().isoformat()
            app_data = json.dumps(kwargs.get('application_data'),
            ensure_ascii=False) if kwargs.get('application_data') else None
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO applications ( application_id, application_no, plan_id, applicant_name, applicant_id_number, gender, birth_date, phone, email, address, education_type, target_grade, target_direction, previous_school, gpa, application_data, documents, status, submitted_at, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'submitted', ?, ?, ?) ''', (application_id, application_no, plan_id, applicant_name,
        ...
        try:
            with self._get_connection() as conn:
                ...
                if row:
                    ...
                    cursor.execute('SELECT * FROM application_reviews WHERE application_id = ? ORDER BY reviewed_at',
                    (application_id,))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR REPLACE INTO application_reviews ( application_id, review_type, reviewer_id, reviewer_name, result, score, comment, reviewed_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (application_id, review_type, kwargs.get('reviewer_id'),
    ...
    def schedule_interview(self, application_id: str, interview_date: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO interview_schedules ( interview_id, application_id, plan_id, interview_date, interview_time, location, interviewer_id, interviewer_name, duration_minutes, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'scheduled', ?, ?) ''', (interview_id, application_id, kwargs.get('plan_id'),
    ...
    def record_interview_result(self, interview_id: str, result: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE interview_schedules SET status = 'completed', result = ?, score = ?, feedback = ?, updated_at = ? WHERE interview_id = ? AND status = 'scheduled' ''', (result, score, kwargs.get('feedback'), now, interview_id))
                    cursor.execute('SELECT application_id FROM interview_schedules WHERE interview_id = ?',
                    (interview_id,))
                    row = cursor.fetchone()
                    if row:
                        cursor.execute('UPDATE applications SET status = ?, updated_at = ? WHERE application_id = ?',
    ...
    def issue_admission_notice(self, application_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT plan_id, applicant_name FROM applications WHERE application_id = ? AND status = ?', (application_id,
                    'accepted'))
                    app = cursor.fetchone()
                    if not app:
                        return {'success': False, 'error': '报名申请不存在或未录取'}
                    cursor.execute(''' INSERT INTO admission_notices ( notice_id, application_id, plan_id, notice_type, title, content, class_id, grade_level, registration_deadline, requirements, sent_at, is_read, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?) ''', (notice_id, application_id, app[0],
    ...
    def enroll_student(self, application_id: str, student_id: int) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT plan_id FROM applications WHERE application_id = ? AND status = ?',
        ...
        try:
            with self._get_connection() as conn:
                ...
                if plan_id:
                    ...
                    cursor.execute('SELECT status, COUNT(*) FROM applications WHERE plan_id = ? GROUP BY status',
                    (plan_id,))
                    ...
                    cursor.execute(''' SELECT COUNT(DISTINCT plan_id) as total_plans, SUM(current_applications) as total_applications, SUM(accepted_count) as total_accepted, SUM(enrolled_count) as total_enrolled FROM enrollment_plans ''')
                    row = cursor.fetchone()
                    return {
                        'success': True,
                        'stats': {
    ...
    def create_promotion(self, plan_id: str, title: str, content: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO enrollment_promotions ( promotion_id, plan_id, title, content, target_audience, publish_channel, views, clicks, is_published, published_at, created_by, created_at ) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0, NULL, ?, ?) ''', (promotion_id, plan_id, title, content,
