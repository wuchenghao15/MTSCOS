    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_accounts ( parent_id TEXT PRIMARY KEY, user_id INTEGER UNIQUE, phone TEXT, email TEXT, real_name TEXT, id_number TEXT, relation_type TEXT, is_verified INTEGER DEFAULT 0, verified_at TEXT, notification_prefs TEXT, permissions TEXT, avatar_url TEXT, last_login TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_student_relations ( id INTEGER PRIMARY KEY AUTOINCREMENT, parent_id TEXT NOT NULL, student_id INTEGER NOT NULL, relation_type TEXT DEFAULT 'father', is_primary INTEGER DEFAULT 0, can_pickup INTEGER DEFAULT 1, emergency_contact INTEGER DEFAULT 1, created_at TEXT, UNIQUE(parent_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS leave_requests ( leave_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, parent_id TEXT, leave_type TEXT NOT NULL, start_date TEXT NOT NULL, end_date TEXT NOT NULL, total_days REAL, reason TEXT NOT NULL, attachment_url TEXT, status TEXT DEFAULT 'pending', approved_by INTEGER, approved_by_name TEXT, approved_at TEXT, reject_reason TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_reports ( report_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, parent_id TEXT, period TEXT NOT NULL, period_start TEXT, period_end TEXT, title TEXT, summary TEXT, academic_data TEXT, behavior_data TEXT, attendance_data TEXT, suggestions TEXT, teacher_comment TEXT, generated_at TEXT, is_read INTEGER DEFAULT 0, read_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_consents ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, parent_id TEXT, consent_type TEXT NOT NULL, consent_value TEXT, is_agreed INTEGER DEFAULT 0, agreed_at TEXT, expires_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_view_logs ( id INTEGER PRIMARY KEY AUTOINCREMENT, parent_id TEXT NOT NULL, student_id INTEGER NOT NULL, view_type TEXT, view_content TEXT, ip_address TEXT, user_agent TEXT, created_at TEXT ) ''')
                conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO parent_accounts ( parent_id, user_id, phone, email, real_name, id_number, relation_type, is_verified, notification_prefs, permissions, avatar_url, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, 'active', ?, ?) ''', (parent_id, user_id, kwargs.get('phone'), kwargs.get('email'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT id FROM parent_student_relations WHERE parent_id = ? AND student_id = ?',
                    (parent_id, student_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '该学生已绑定'}
                    if is_primary:
                        cursor.execute('UPDATE parent_student_relations SET is_primary = 0 WHERE parent_id = ?',
                        (parent_id,))
                    cursor.execute(''' INSERT INTO parent_student_relations (parent_id, student_id, relation_type, is_primary, can_pickup, emergency_contact, created_at) VALUES (?, ?, ?, ?, 1, 1, ?) ''', (parent_id, student_id, relation_type, 1 if is_primary else 0, now))
                    conn.commit()
    ...
    def get_parent_children(self, parent_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(''' SELECT psr.*, pa.real_name as parent_name FROM parent_student_relations psr JOIN parent_accounts pa ON psr.parent_id = pa.parent_id WHERE psr.parent_id = ? ORDER BY psr.is_primary DESC, psr.created_at ''', (parent_id,))
    ...
    def get_student_overview(self, student_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                try:
                    cursor.execute(''' SELECT class_id FROM class_students WHERE student_id = ? AND enrollment_status = 'enrolled' ''', (student_id,))
                    overview['class_id'] = cursor.fetchone()
                except:
                    overview['class_id'] = None
                try:
                    cursor.execute(''' SELECT AVG(score) as avg_score, COUNT(*) as exam_count, MAX(score) as best_score, MIN(score) as lowest_score FROM exam_scores WHERE student_id = ? AND is_absent = 0 ''', (student_id,))
                    row = cursor.fetchone()
                    overview['academic'] = dict(row) if row else None
                except:
                    overview['academic'] = None
                try:
                    cursor.execute(''' SELECT behavior_type, COUNT(*) as cnt FROM behavior_records WHERE student_id = ? GROUP BY behavior_type ''', (student_id,))
                    overview['behavior'] = {r[0]: r[1] for r in cursor.fetchall()}
                except:
                    overview['behavior'] = {}
                try:
                    cursor.execute(''' SELECT status, COUNT(*) as cnt FROM attendance_records WHERE student_id = ? AND record_date >= ? GROUP BY status ''', (student_id, (datetime.now() - timedelta(days=30)).isoformat()[:10]))
                    overview['attendance_30days'] = {r[0]: r[1] for r in cursor.fetchall()}
                except:
                    overview['attendance_30days'] = {}
                try:
                    cursor.execute(''' SELECT COUNT(*) as unread FROM messages WHERE receiver_id = ? AND is_read = 0 ''', (student_id,))
    ...
    def apply_leave(self, student_id: int, leave_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO leave_requests ( leave_id, student_id, parent_id, leave_type, start_date, end_date, total_days, reason, attachment_url, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (leave_id, student_id, kwargs.get('parent_id'),
    ...
    def approve_leave(self, leave_id: str, approved_by: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE leave_requests SET status = ?, approved_by = ?, approved_by_name = ?, approved_at = ?, reject_reason = ?, updated_at = ? WHERE leave_id = ? AND status = 'pending' ''', (status, approved_by, kwargs.get('approved_by_name'),
    ...
    def get_student_scores(self, student_id: int, semester: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT es.*, e.exam_name, e.exam_type, e.exam_date, e.full_score FROM exam_scores es JOIN exams e ON es.exam_id = e.exam_id WHERE es.student_id = ? AND es.is_absent = 0 '''
                params = [student_id]
                if semester:
    ...
    def get_student_behavior(self, student_id: int, days: int = 30) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT * FROM behavior_records WHERE student_id = ? AND record_date >= ? ORDER BY created_at DESC ''', (student_id, start_date))
    ...
    def get_student_attendance(self, student_id: int, days: int = 30) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                start_date = (datetime.now() - timedelta(days=days)).isoformat()[:10]
                cursor.execute(''' SELECT * FROM attendance_records WHERE student_id = ? AND record_date >= ? ORDER BY record_date DESC ''', (student_id, start_date))
                records = [dict(r) for r in cursor.fetchall()]
                stats = {}
                for r in records:
    ...
    def generate_report(self, student_id: int, period: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO parent_reports ( report_id, student_id, parent_id, period, period_start, period_end, title, summary, academic_data, behavior_data, attendance_data, suggestions, teacher_comment, generated_at, is_read ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0) ''', (report_id, student_id, parent_id, period,
    ...
    def read_report(self, report_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE parent_reports SET is_read = 1, read_at = ? WHERE report_id = ?', (now,
                    report_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def log_parent_view(self, parent_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO parent_view_logs (parent_id, student_id, view_type, view_content, ip_address, created_at) VALUES (?, ?, ?, ?, ?, ?) ''', (parent_id, student_id, view_type, view_content, ip_address, now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def verify_parent(self, parent_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE parent_accounts SET is_verified = 1, verified_at = ? WHERE parent_id = ?',
                    (now, parent_id))
                    conn.commit()
