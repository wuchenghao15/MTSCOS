...
class StudentBehaviorService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS behavior_records ( record_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, behavior_type TEXT NOT NULL, category TEXT, behavior_code TEXT, description TEXT, score_change REAL DEFAULT 0, record_date TEXT, recorded_by INTEGER, recorded_by_name TEXT, class_id TEXT, education_type TEXT, location TEXT, witnesses TEXT, attachments TEXT, remark TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS behavior_scores ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, semester TEXT NOT NULL, total_score REAL DEFAULT 100, positive_count INTEGER DEFAULT 0, negative_count INTEGER DEFAULT 0, positive_score REAL DEFAULT 0, negative_score REAL DEFAULT 0, reward_count INTEGER DEFAULT 0, punishment_count INTEGER DEFAULT 0, moral_grade TEXT, updated_at TEXT, UNIQUE(student_id, semester) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS reward_records ( reward_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, reward_type TEXT NOT NULL, reward_name TEXT, description TEXT, score_change REAL DEFAULT 0, reward_date TEXT, awarded_by INTEGER, awarded_by_name TEXT, semester TEXT, is_revoked INTEGER DEFAULT 0, revoked_at TEXT, revoke_reason TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS punishment_records ( punishment_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, punishment_type TEXT NOT NULL, punishment_name TEXT, description TEXT, score_change REAL DEFAULT 0, punishment_date TEXT, issued_by INTEGER, issued_by_name TEXT, semester TEXT, duration_days INTEGER, is_active INTEGER DEFAULT 1, is_revoked INTEGER DEFAULT 0, revoked_at TEXT, revoke_reason TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS moral_evaluations ( evaluation_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, semester TEXT NOT NULL, dimensions TEXT NOT NULL, total_score REAL, grade TEXT, evaluator_id INTEGER, evaluator_name TEXT, teacher_comment TEXT, student_self_eval TEXT, parent_feedback TEXT, class_id TEXT, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS moral_archive ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, semester TEXT NOT NULL, academic_year TEXT, behavior_score REAL, moral_grade TEXT, reward_count INTEGER DEFAULT 0, punishment_count INTEGER DEFAULT 0, summary TEXT, created_at TEXT, UNIQUE(student_id, semester) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS behavior_warnings ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, warning_type TEXT NOT NULL, description TEXT, threshold REAL, current_value REAL, is_resolved INTEGER DEFAULT 0, resolved_at TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def record_behavior(self, student_id: int, behavior_type: str,
        ...
        try:
            ...
            attachments = json.dumps(kwargs.get('attachments'),
            ensure_ascii=False) if kwargs.get('attachments') else None
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO behavior_records ( record_id, student_id, behavior_type, category, behavior_code, description, score_change, record_date, recorded_by, recorded_by_name, class_id, education_type, location, witnesses, attachments, remark, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (record_id, student_id, behavior_type,
    ...
    def _update_behavior_score(self, cursor, student_id: int, behavior_type: str, score_change: float):
        ...
        if row:
            ...
            cursor.execute(''' UPDATE behavior_scores SET total_score = ?, positive_count = ?, negative_count = ?, positive_score = ?, negative_score = ?, updated_at = ? WHERE id = ? ''', (round(total, 2), pos_cnt, neg_cnt, round(pos_score, 2),
            ...
            cursor.execute(''' INSERT INTO behavior_scores (student_id, semester, total_score, positive_count, negative_count, positive_score, negative_score, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (student_id, semester, round(total, 2), pos_cnt, neg_cnt,
    ...
    def record_reward(self, student_id: int, reward_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO reward_records ( reward_id, student_id, reward_type, reward_name, description, score_change, reward_date, awarded_by, awarded_by_name, semester, is_revoked, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?) ''', (reward_id, student_id, reward_type,
                          ...
                          semester, now))
                    cursor.execute(''' UPDATE behavior_scores SET reward_count = reward_count + 1, total_score = total_score + ?, updated_at = ? WHERE student_id = ? AND semester = ? ''', (score_change, now, student_id, semester))
                    if cursor.rowcount == 0:
                        cursor.execute(''' INSERT INTO behavior_scores (student_id, semester, total_score, reward_count, updated_at) VALUES (?, ?, ?, 1, ?) ''', (student_id, semester, 100 + score_change, now))
                    conn.commit()
    ...
    def record_punishment(self, student_id: int, punishment_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO punishment_records ( punishment_id, student_id, punishment_type, punishment_name, description, score_change, punishment_date, issued_by, issued_by_name, semester, duration_days, is_active, is_revoked, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0, ?) ''', (punishment_id, student_id, punishment_type,
                    ...
                    cursor.execute(''' UPDATE behavior_scores SET punishment_count = punishment_count + 1, total_score = total_score + ?, updated_at = ? WHERE student_id = ? AND semester = ? ''', (score_change, now, student_id, semester))
                    if cursor.rowcount == 0:
                        cursor.execute(''' INSERT INTO behavior_scores (student_id, semester, total_score, punishment_count, updated_at) VALUES (?, ?, ?, 1, ?) ''', (student_id, semester, 100 + score_change, now))
                    conn.commit()
    ...
    def revoke_punishment(self, punishment_id: str, reason: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE punishment_records SET is_revoked = 1, is_active = 0, revoked_at = ?, revoke_reason = ? WHERE punishment_id = ? AND is_revoked = 0 ''', (now, reason, punishment_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_behavior_score(self, student_id: int, semester: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM behavior_scores WHERE student_id = ? AND semester = ?', (student_id,
                semester))
                row = cursor.fetchone()
                if row:
                    score = dict(row)
    ...
    def evaluate_moral(self, student_id: int, semester: str, dimensions: dict,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO moral_evaluations ( evaluation_id, student_id, semester, dimensions, total_score, grade, evaluator_id, evaluator_name, teacher_comment, student_self_eval, parent_feedback, class_id, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (evaluation_id, student_id, semester, dims_json,
                    ...
                    cursor.execute(''' UPDATE behavior_scores SET moral_grade = ?, updated_at = ? WHERE student_id = ? AND semester = ? ''', (grade['grade'], now, student_id, semester))
                    conn.commit()
    ...
    def get_moral_evaluation(self, student_id: int, semester: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM moral_evaluations WHERE student_id = ? AND semester = ?', (student_id,
                semester))
                row = cursor.fetchone()
                if row:
                    eval_data = dict(row)
    ...
    def get_behavior_summary(self, student_id: int, semester: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT behavior_type, COUNT(*) as cnt, SUM(score_change) as total_score FROM behavior_records WHERE student_id = ? AND created_at >= ? GROUP BY behavior_type ''', (student_id, f'{semester.split("-")[0]}-09-01' if '-' in semester else '2026-01-01'))
                type_stats = {r[0]: {'count': r[1], 'score': r[2]} for r in cursor.fetchall()}
                cursor.execute(''' SELECT category, COUNT(*) as cnt FROM behavior_records WHERE student_id = ? GROUP BY category ''', (student_id,))
                cat_stats = {r[0]: r[1] for r in cursor.fetchall()}
                cursor.execute('SELECT COUNT( *) FROM reward_records WHERE student_id = ? AND semester = ? AND is_revoked = 0', (student_id,
                semester))
    ...
    def generate_behavior_warnings(self) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT student_id, total_score FROM behavior_scores WHERE semester = ?',
                    (self._get_current_semester(),))
                    students = cursor.fetchall()
                    for student_id, score in students:
                        if score < 60:
                            cursor.execute(''' INSERT INTO behavior_warnings ( student_id, warning_type, description, threshold, current_value, created_at ) VALUES (?, 'low_score', ?, 60, ?, ?) ''', (student_id, f'行为分数{score}分，低于及格线60分', score, now))
                            warnings_created += 1
                        if score < 80:
                            cursor.execute(''' INSERT INTO behavior_warnings ( student_id, warning_type, description, threshold, current_value, created_at ) VALUES (?, 'decline_risk', ?, 80, ?, ?) ''', (student_id, f'行为分数{score}分，接近警戒线', score, now))
                            warnings_created += 1
                    conn.commit()
    ...
    def get_student_warnings(self, student_id: int, resolved: int = 0) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT * FROM behavior_warnings WHERE student_id = ? AND is_resolved = ? ORDER BY created_at DESC ''', (student_id, resolved))
    ...
    def archive_moral_record(self, student_id: int, semester: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO moral_archive ( student_id, semester, academic_year, behavior_score, moral_grade, reward_count, punishment_count, summary, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(student_id, semester) DO UPDATE SET behavior_score = excluded.behavior_score, moral_grade = excluded.moral_grade, reward_count = excluded.reward_count, punishment_count = excluded.punishment_count, summary = excluded.summary ''', (student_id, semester, kwargs.get('academic_year'),
