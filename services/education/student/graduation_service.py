...
class GraduationService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS graduation_plans ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, education_type TEXT NOT NULL, graduation_type TEXT NOT NULL, academic_year TEXT, semester TEXT, grade_level INTEGER, class_id TEXT, conditions TEXT, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'draft', total_students INTEGER DEFAULT 0, approved_count INTEGER DEFAULT 0, rejected_count INTEGER DEFAULT 0, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS graduation_applications ( application_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, apply_date TEXT, status TEXT DEFAULT 'applied', review_stage TEXT DEFAULT 'initial', reviewer_id INTEGER, review_comment TEXT, review_date TEXT, credit_earned REAL DEFAULT 0, credit_required REAL DEFAULT 0, gpa REAL DEFAULT 0, gpa_required REAL DEFAULT 0, attendance_rate REAL DEFAULT 0, conditions_met TEXT, conditions_failed TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS certificates ( certificate_id TEXT PRIMARY KEY, certificate_no TEXT UNIQUE, student_id INTEGER NOT NULL, student_name TEXT, education_type TEXT, certificate_type TEXT NOT NULL, certificate_title TEXT, plan_id TEXT, issue_date TEXT, valid_from TEXT, valid_to TEXT, issuer TEXT, issuer_title TEXT, status TEXT DEFAULT 'pending', verification_code TEXT, file_url TEXT, grade_level INTEGER, major TEXT, honor_level TEXT, issued_by INTEGER, printed_at TEXT, distributed_at TEXT, distributed_to TEXT, revoked_at TEXT, revoke_reason TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS certificate_templates ( template_id TEXT PRIMARY KEY, template_name TEXT NOT NULL, certificate_type TEXT NOT NULL, education_type TEXT, template_content TEXT, style_config TEXT, is_active INTEGER DEFAULT 1, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS graduation_statistics ( id INTEGER PRIMARY KEY AUTOINCREMENT, stat_year TEXT NOT NULL, education_type TEXT NOT NULL, total_students INTEGER DEFAULT 0, graduated_count INTEGER DEFAULT 0, graduation_rate REAL DEFAULT 0, employment_rate REAL DEFAULT 0, further_study_rate REAL DEFAULT 0, average_gpa REAL DEFAULT 0, top_score REAL DEFAULT 0, honor_graduate_count INTEGER DEFAULT 0, updated_at TEXT, UNIQUE(stat_year, education_type) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS certificate_verification ( id INTEGER PRIMARY KEY AUTOINCREMENT, certificate_id TEXT NOT NULL, verification_code TEXT, verify_count INTEGER DEFAULT 0, last_verify_at TEXT, verifier_info TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS reissue_records ( reissue_id TEXT PRIMARY KEY, certificate_id TEXT NOT NULL, student_id INTEGER, apply_date TEXT, reason TEXT, status TEXT DEFAULT 'pending', new_certificate_id TEXT, approved_by INTEGER, approved_at TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_graduation_plan(self, plan_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO graduation_plans ( plan_id, plan_name, education_type, graduation_type, academic_year, semester, grade_level, class_id, conditions, start_date, end_date, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?) ''', (plan_id, plan_name, education_type, graduation_type,
    ...
    def publish_plan(self, plan_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE graduation_plans SET status = 'published', updated_at = ? WHERE plan_id = ? AND status = 'draft' ''', (now, plan_id))
    ...
    def apply_graduation(self, plan_id: str, student_id: int, **kwargs) -> Dict[str, Any]:
        try:
            ...
            now = datetime.now().isoformat()
            conditions_met = json.dumps(kwargs.get('conditions_met'),
            ensure_ascii=False) if kwargs.get('conditions_met') else None
            conditions_failed = json.dumps(kwargs.get('conditions_failed'),
            ensure_ascii=False) if kwargs.get('conditions_failed') else None
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    ...
                    cursor.execute(''' SELECT application_id FROM graduation_applications WHERE plan_id = ? AND student_id = ? ''', (plan_id, student_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已提交过毕业申请'}
                    cursor.execute(''' INSERT INTO graduation_applications ( application_id, plan_id, student_id, student_name, apply_date, status, review_stage, credit_earned, credit_required, gpa, gpa_required, attendance_rate, conditions_met, conditions_failed, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'applied', 'initial', ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (application_id, plan_id, student_id,
    ...
    def review_application(self, application_id: str, approved: bool,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT plan_id FROM graduation_applications WHERE application_id = ?',
                    (application_id,))
                    app = cursor.fetchone()
                    if not app:
                        return {'success': False, 'error': '申请不存在'}
                    cursor.execute(''' UPDATE graduation_applications SET status = ?, review_stage = 'final', reviewer_id = ?, review_comment = ?, review_date = ?, updated_at = ? WHERE application_id = ? AND status IN (?, ?) ''', (status, kwargs.get('reviewer_id'), kwargs.get('review_comment'),
    ...
    def issue_certificate(self, student_id: int, certificate_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO certificates ( certificate_id, certificate_no, student_id, student_name, education_type, certificate_type, certificate_title, plan_id, issue_date, issuer, issuer_title, status, verification_code, grade_level, major, honor_level, issued_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'issued', ?, ?, ?, ?, ?, ?, ?) ''', (certificate_id, certificate_no, student_id,
                    ...
                    cursor.execute(''' INSERT INTO certificate_verification (certificate_id, verification_code, created_at) VALUES (?, ?, ?) ''', (certificate_id, verification_code, now))
                    conn.commit()
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    conn.row_factory = sqlite3.Row
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT * FROM certificates WHERE certificate_no = ? AND verification_code = ? ''', (certificate_no, verification_code))
                    cert = cursor.fetchone()
                    now = datetime.now().isoformat()
                    if cert:
                        cert_dict = dict(cert)
                        cursor.execute(''' UPDATE certificate_verification SET verify_count = verify_count + 1, last_verify_at = ? WHERE certificate_id = ? ''', (now, cert_dict['certificate_id']))
                        conn.commit()
                        return {
                            'success': True,
    ...
    def reissue_certificate(self, certificate_id: str, reason: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT student_id, status FROM certificates WHERE certificate_id = ?',
                    (certificate_id,))
                    cert = cursor.fetchone()
                    if not cert:
                    ...
                    cursor.execute(''' INSERT INTO reissue_records ( reissue_id, certificate_id, student_id, apply_date, reason, status, created_at ) VALUES (?, ?, ?, ?, ?, 'pending', ?) ''', (reissue_id, certificate_id, cert[0], now[:10], reason, now))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    verification_code = uuid.uuid4().hex[:8].upper()
                    cursor.execute(''' INSERT INTO certificates ( certificate_id, certificate_no, student_id, student_name, education_type, certificate_type, certificate_title, issue_date, issuer, status, verification_code, grade_level, major, issued_by, created_at, updated_at ) SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, 'replaced', ?, ?, ?, ?, ?, ? FROM certificates WHERE certificate_id = ? ''', (new_cert_id, new_cert_no, student_id, old_cert[3],
                          old_cert[4], old_cert[5], old_cert[6],
                          old_cert[7], old_cert[9], verification_code,
                          old_cert[19], old_cert[20], approved_by, now, now,
                          old_cert_id))
                    cursor.execute('UPDATE certificates SET status = ? WHERE certificate_id = ?', ('replaced',
                    old_cert_id))
                    cursor.execute(''' UPDATE reissue_records SET status = 'approved', new_certificate_id = ?, approved_by = ?, approved_at = ? WHERE reissue_id = ? ''', (new_cert_id, approved_by, now, reissue_id))
                    cursor.execute(''' INSERT INTO certificate_verification (certificate_id, verification_code, created_at) VALUES (?, ?, ?) ''', (new_cert_id, verification_code, now))
                    conn.commit()
    ...
    def distribute_certificate(self, certificate_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE certificates SET status = 'distributed', distributed_at = ?, distributed_to = ?, updated_at = ? WHERE certificate_id = ? AND status = 'printed' ''', (now, distributed_to, now, certificate_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def update_graduation_stats(self, stat_year: str, education_type: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT COUNT(*) as total, SUM(CASE WHEN status = 'approved' THEN 1 ELSE 0 END) as approved, AVG(gpa) as avg_gpa FROM graduation_applications ga JOIN graduation_plans gp ON ga.plan_id = gp.plan_id WHERE gp.academic_year = ? AND gp.education_type = ? ''', (stat_year, education_type))
                    row = cursor.fetchone()
                    total = row[0] or 0
                    approved = row[1] or 0
                    avg_gpa = row[2] or 0
                    grad_rate = round(approved / total * 100, 2) if total > 0 else 0
                    cursor.execute(''' INSERT INTO graduation_statistics ( stat_year, education_type, total_students, graduated_count, graduation_rate, average_gpa, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(stat_year, education_type) DO UPDATE SET total_students = excluded.total_students, graduated_count = excluded.graduated_count, graduation_rate = excluded.graduation_rate, average_gpa = excluded.average_gpa, updated_at = excluded.updated_at ''', (stat_year, education_type, total, approved, grad_rate, avg_gpa, now))
                    conn.commit()
    ...
    def create_certificate_template(self, template_name: str,
        ...
        try:
            ...
            now = datetime.now().isoformat()
            style_config = json.dumps(kwargs.get('style_config'),
            ensure_ascii=False) if kwargs.get('style_config') else None
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO certificate_templates ( template_id, template_name, certificate_type, education_type, template_content, style_config, is_active, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, ?) ''', (template_id, template_name, certificate_type,
