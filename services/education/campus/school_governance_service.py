        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor = conn.cursor()
                    cursor.execute('UPDATE regulations SET view_count = view_count + 1 WHERE regulation_id = ?',
                    (regulation_id,))
                    ...
                    cursor.execute(
                    'SELECT * FROM regulation_revisions WHERE regulation_id = ? ORDER BY created_at DESC',
                    (regulation_id,))
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT * FROM contract_approvals WHERE contract_id = ? ORDER BY approval_level',
                    (contract_id,))
