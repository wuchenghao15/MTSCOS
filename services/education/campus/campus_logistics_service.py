    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS facilities ( facility_id TEXT PRIMARY KEY, facility_name TEXT NOT NULL, facility_type TEXT NOT NULL, location TEXT, floor INTEGER, capacity INTEGER DEFAULT 0, area REAL DEFAULT 0, status TEXT DEFAULT 'available', equipment TEXT, responsible_person TEXT, responsible_phone TEXT, description TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS facility_reservations ( reservation_id TEXT PRIMARY KEY, facility_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, purpose TEXT, start_time TEXT, end_time TEXT, attendees INTEGER DEFAULT 1, status TEXT DEFAULT 'pending', approved_by INTEGER, approved_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS repair_requests ( repair_id TEXT PRIMARY KEY, repair_no TEXT UNIQUE, facility_id TEXT, location TEXT, repair_type TEXT NOT NULL, description TEXT NOT NULL, urgency INTEGER DEFAULT 2, reporter_id INTEGER, reporter_name TEXT, reporter_phone TEXT, photos TEXT, status TEXT DEFAULT 'pending', assigned_to TEXT, assigned_at TEXT, completed_at TEXT, verified_at TEXT, verified_by INTEGER, repair_cost REAL DEFAULT 0, repair_notes TEXT, feedback TEXT, feedback_rating INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS dormitories ( dorm_id TEXT PRIMARY KEY, building TEXT NOT NULL, floor INTEGER, room_number TEXT NOT NULL, dorm_type TEXT DEFAULT 'quad', capacity INTEGER DEFAULT 4, current_occupancy INTEGER DEFAULT 0, gender_restriction TEXT, monthly_fee REAL DEFAULT 300, status TEXT DEFAULT 'available', facilities TEXT, responsible_person TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS dorm_assignments ( id INTEGER PRIMARY KEY AUTOINCREMENT, dorm_id TEXT NOT NULL, student_id INTEGER NOT NULL, bed_number INTEGER, check_in_date TEXT, check_out_date TEXT, status TEXT DEFAULT 'active', deposit REAL DEFAULT 0, remark TEXT, created_at TEXT, UNIQUE(dorm_id, bed_number) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS materials ( material_id TEXT PRIMARY KEY, material_name TEXT NOT NULL, material_type TEXT, specification TEXT, unit TEXT DEFAULT '件', stock_quantity INTEGER DEFAULT 0, min_stock INTEGER DEFAULT 10, max_stock INTEGER DEFAULT 100, unit_price REAL DEFAULT 0, total_value REAL DEFAULT 0, location TEXT, supplier TEXT, last_restocked TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS material_transactions ( id INTEGER PRIMARY KEY AUTOINCREMENT, material_id TEXT NOT NULL, transaction_type TEXT NOT NULL, quantity INTEGER NOT NULL, unit_price REAL, total_price REAL, operator_id INTEGER, operator_name TEXT, department TEXT, purpose TEXT, transaction_date TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS bus_routes ( route_id TEXT PRIMARY KEY, route_name TEXT NOT NULL, driver_name TEXT, driver_phone TEXT, plate_number TEXT, capacity INTEGER DEFAULT 30, current_passengers INTEGER DEFAULT 0, stops TEXT, departure_time TEXT, return_time TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS bus_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, route_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, pickup_stop TEXT, days_of_week TEXT, semester TEXT, status TEXT DEFAULT 'active', created_at TEXT, UNIQUE(route_id, student_id, semester) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS visitor_logs ( id INTEGER PRIMARY KEY AUTOINCREMENT, visitor_name TEXT NOT NULL, visitor_phone TEXT, id_number TEXT, visit_purpose TEXT, host_name TEXT, host_department TEXT, check_in_time TEXT, check_out_time TEXT, vehicle_plate TEXT, temperature REAL, is_approved INTEGER DEFAULT 0, approved_by INTEGER, created_at TEXT ) ''')
                conn.commit()
    ...
    def add_facility(self, facility_name: str, facility_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO facilities ( facility_id, facility_name, facility_type, location, floor, capacity, area, status, equipment, responsible_person, responsible_phone, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (facility_id, facility_name, facility_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' SELECT reservation_id FROM facility_reservations WHERE facility_id = ? AND status IN ('pending', 'approved') AND (start_time < ? AND end_time > ?) ''', (facility_id, end_time, start_time))
                    if cursor.fetchone():
                        return {'success': False, 'error': '该时段已被预约'}
                    cursor.execute(''' INSERT INTO facility_reservations ( reservation_id, facility_id, user_id, user_name, purpose, start_time, end_time, attendees, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (reservation_id, facility_id, user_id,
    ...
    def approve_reservation(self, reservation_id: str, approved_by: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE facility_reservations SET status = ?, approved_by = ?, approved_at = ?, updated_at = ? WHERE reservation_id = ? AND status = 'pending' ''', (status, approved_by, now, now, reservation_id))
                    conn.commit()
    ...
    def submit_repair(self, repair_type: str, description: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO repair_requests ( repair_id, repair_no, facility_id, location, repair_type, description, urgency, reporter_id, reporter_name, reporter_phone, photos, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (repair_id, repair_no, kwargs.get('facility_id'),
    ...
    def assign_repair(self, repair_id: str, assigned_to: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE repair_requests SET status = 'assigned', assigned_to = ?, assigned_at = ?, updated_at = ? WHERE repair_id = ? AND status = 'pending' ''', (assigned_to, now, now, repair_id))
                    conn.commit()
    ...
    def complete_repair(self, repair_id: str, repair_notes: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE repair_requests SET status = 'completed', completed_at = ?, repair_notes = ?, repair_cost = ?, updated_at = ? WHERE repair_id = ? AND status IN ('assigned', 'in_progress') ''', (now, repair_notes, repair_cost, now, repair_id))
                    conn.commit()
    ...
    def verify_repair(self, repair_id: str, verified_by: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE repair_requests SET status = 'verified', verified_at = ?, verified_by = ?, feedback = ?, feedback_rating = ?, updated_at = ? WHERE repair_id = ? AND status = 'completed' ''', (now, verified_by, feedback, rating, now, repair_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def add_dormitory(self, building: str, room_number: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO dormitories ( dorm_id, building, floor, room_number, dorm_type, capacity, current_occupancy, gender_restriction, monthly_fee, status, facilities, responsible_person, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, 'available', ?, ?, ?, ?) ''', (dorm_id, building, kwargs.get('floor'), room_number,
    ...
    def assign_dormitory(self, dorm_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT capacity, current_occupancy, status FROM dormitories WHERE dorm_id = ?',
                    (dorm_id,))
                    ...
                    cursor.execute(
                    'SELECT id FROM dorm_assignments WHERE dorm_id = ? AND bed_number = ? AND status = ?', (dorm_id,
                    bed_number, 'active'))
                    if cursor.fetchone():
                        return {'success': False, 'error': '该床位已被占用'}
                    cursor.execute(''' INSERT INTO dorm_assignments (dorm_id, student_id, bed_number, check_in_date, status, deposit, remark, created_at) VALUES (?, ?, ?, ?, 'active', ?, ?, ?) ''', (dorm_id, student_id, bed_number,
    ...
    def check_out_dormitory(self, dorm_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE dorm_assignments SET status = 'checked_out', check_out_date = ? WHERE dorm_id = ? AND student_id = ? AND status = 'active' ''', (check_out, dorm_id, student_id))
    ...
    def add_material(self, material_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO materials ( material_id, material_name, material_type, specification, unit, stock_quantity, min_stock, max_stock, unit_price, total_value, location, supplier, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (material_id, material_name, kwargs.get('material_type'),
    ...
    def stock_in(self, material_id: str, quantity: int, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT stock_quantity, unit_price FROM materials WHERE material_id = ?',
                    (material_id,))
                    mat = cursor.fetchone()
                    if not mat:
                    ...
                    cursor.execute(''' UPDATE materials SET stock_quantity = ?, unit_price = ?, total_value = ?, last_restocked = ?, updated_at = ? WHERE material_id = ? ''', (new_qty, unit_price, total_value, now, now, material_id))
                    cursor.execute(''' INSERT INTO material_transactions (material_id, transaction_type, quantity, unit_price, total_price, operator_id, operator_name, department, purpose, transaction_date, created_at) VALUES (?, 'in', ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (material_id, quantity, unit_price, quantity * unit_price,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO material_transactions (material_id, transaction_type, quantity, unit_price, total_price, operator_id, operator_name, department, purpose, transaction_date, created_at) VALUES (?, 'out', ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (material_id, quantity, mat[1], quantity * mat[1],
                          kwargs.get('operator_id'), kwargs.get('operator_name'),
    ...
    def log_visitor(self, visitor_name: str, visit_purpose: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO visitor_logs ( visitor_name, visitor_phone, id_number, visit_purpose, host_name, host_department, check_in_time, vehicle_plate, temperature, is_approved, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (visitor_name, kwargs.get('visitor_phone'),
    ...
    def check_out_visitor(self, visitor_id: int) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE visitor_logs SET check_out_time = ? WHERE id = ? AND check_out_time IS NULL',
                    (now, visitor_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
