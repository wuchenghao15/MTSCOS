    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS innovation_projects ( project_id TEXT PRIMARY KEY, project_name TEXT NOT NULL, innovation_type TEXT NOT NULL, education_type TEXT NOT NULL, description TEXT, problem_statement TEXT, solution TEXT, target_audience TEXT, market_analysis TEXT, business_model TEXT, founder_name TEXT, founder_id INTEGER, team_size INTEGER DEFAULT 1, support_level TEXT DEFAULT 'basic', status TEXT DEFAULT 'incubating', current_stage TEXT DEFAULT 'idea', total_investment REAL DEFAULT 0, funding_raised REAL DEFAULT 0, ip_count INTEGER DEFAULT 0, competition_wins INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, project_id TEXT NOT NULL, member_id INTEGER NOT NULL, member_name TEXT, role TEXT DEFAULT 'member', join_date TEXT, UNIQUE(project_id, member_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_milestones ( milestone_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, stage TEXT NOT NULL, milestone_name TEXT NOT NULL, description TEXT, target_date TEXT, completed_date TEXT, status TEXT DEFAULT 'pending', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_documents ( doc_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, doc_type TEXT, doc_name TEXT NOT NULL, file_url TEXT, uploaded_by INTEGER, uploaded_at TEXT, is_approved INTEGER DEFAULT 0 ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS incubator_spaces ( space_id TEXT PRIMARY KEY, space_name TEXT NOT NULL, location TEXT, capacity INTEGER DEFAULT 10, occupied_count INTEGER DEFAULT 0, monthly_rent REAL DEFAULT 0, amenities TEXT, status TEXT DEFAULT 'available', education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS space_occupancy ( occupancy_id TEXT PRIMARY KEY, space_id TEXT NOT NULL, project_id TEXT NOT NULL, start_date TEXT NOT NULL, end_date TEXT, monthly_fee REAL, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, UNIQUE(space_id, project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS mentor_profiles ( mentor_id TEXT PRIMARY KEY, mentor_name TEXT NOT NULL, mentor_role TEXT NOT NULL, education_type TEXT, expertise TEXT, experience_years INTEGER DEFAULT 0, organization TEXT, title TEXT, contact_email TEXT, contact_phone TEXT, availability TEXT, hourly_rate REAL DEFAULT 0, assigned_projects INTEGER DEFAULT 0, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS mentor_assignments ( assignment_id TEXT PRIMARY KEY, mentor_id TEXT NOT NULL, project_id TEXT NOT NULL, start_date TEXT NOT NULL, end_date TEXT, hours_per_week INTEGER DEFAULT 2, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, UNIQUE(mentor_id, project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS competition_events ( event_id TEXT PRIMARY KEY, event_name TEXT NOT NULL, competition_type TEXT NOT NULL, education_type TEXT NOT NULL, organizer TEXT, description TEXT, start_date TEXT, end_date TEXT, registration_deadline TEXT, location TEXT, max_participants INTEGER DEFAULT 100, registered_count INTEGER DEFAULT 0, prize_pool REAL DEFAULT 0, status TEXT DEFAULT 'announced', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS competition_participants ( id INTEGER PRIMARY KEY AUTOINCREMENT, event_id TEXT NOT NULL, project_id TEXT NOT NULL, team_name TEXT, leader_name TEXT, submission_date TEXT, score REAL, rank INTEGER, award TEXT, status TEXT DEFAULT 'registered', UNIQUE(event_id, project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS investment_rounds ( round_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, round_type TEXT NOT NULL, round_number INTEGER DEFAULT 1, target_amount REAL NOT NULL, raised_amount REAL DEFAULT 0, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS investment_deals ( deal_id TEXT PRIMARY KEY, round_id TEXT NOT NULL, investor_name TEXT NOT NULL, investment_amount REAL NOT NULL, equity_percentage REAL, deal_date TEXT, status TEXT DEFAULT 'pending', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS intellectual_property ( ip_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, ip_type TEXT NOT NULL, ip_name TEXT NOT NULL, description TEXT, registration_no TEXT, registration_date TEXT, expiration_date TEXT, owner TEXT, status TEXT DEFAULT 'applied', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ip_transactions ( transaction_id TEXT PRIMARY KEY, ip_id TEXT NOT NULL, transaction_type TEXT, buyer_name TEXT, seller_name TEXT, transaction_amount REAL, transaction_date TEXT, status TEXT DEFAULT 'pending', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS innovation_rewards ( reward_id TEXT PRIMARY KEY, reward_name TEXT NOT NULL, education_type TEXT, description TEXT, eligibility TEXT, amount REAL DEFAULT 0, max_winners INTEGER DEFAULT 1, application_deadline TEXT, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS incubation_records ( record_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, action_type TEXT, action_description TEXT, performed_by TEXT, performed_at TEXT ) ''')
                conn.commit()
    ...
    def create_project(self, project_name: str, innovation_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO innovation_projects ( project_id, project_name, innovation_type, education_type, description, problem_statement, solution, target_audience, market_analysis, business_model, founder_name, founder_id, team_size, support_level, status, current_stage, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 'basic', 'incubating', 'idea', ?, ?) ''', (project_id, project_name, innovation_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", project_id, 'stage_update',
                                 f'阶段变更: {project[0]} -> {stage}', 'system', now[:10]))
                    conn.commit()
    ...
    def create_incubator_space(self, space_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO incubator_spaces ( space_id, space_name, location, capacity, occupied_count, monthly_rent, amenities, status, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, 0, ?, ?, 'available', ?, ?, ?) ''', (space_id, space_name, kwargs.get('location'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", project_id, 'occupy_space', f'入驻场地: {space_id}',
                                 'system', start_date))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", occupancy[1], 'release_space', '场地释放', 'system',
                                 now[:10]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def register_mentor(self, mentor_name: str, mentor_role: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO mentor_profiles ( mentor_id, mentor_name, mentor_role, education_type, expertise, experience_years, organization, title, contact_email, contact_phone, availability, hourly_rate, assigned_projects, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 1, ?, ?) ''', (mentor_id, mentor_name, mentor_role,
                ...
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute('UPDATE mentor_profiles SET assigned_projects = assigned_projects + 1, updated_at = ? WHERE mentor_id = ?',
                                     (now, mentor_id))
                        cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                     (f"rec_{uuid.uuid4().hex[:12]}", project_id, 'mentor_assigned',
                                     f'导师分配: {mentor_id}', 'system', start_date))
                        conn.commit()
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE mentor_profiles SET assigned_projects = assigned_projects - 1, updated_at = ? WHERE mentor_id = ?',
                                 (now, assignment[0]))
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", assignment[1], 'mentor_unassigned', '导师取消分配',
                                 'system', now[:10]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def create_competition(self, event_name: str, competition_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO competition_events ( event_id, event_name, competition_type, education_type, organizer, description, start_date, end_date, registration_deadline, location, max_participants, registered_count, prize_pool, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'announced', ?, ?) ''', (event_id, event_name, competition_type,
    ...
    def list_competitions(self, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'competitions': competitions, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO investment_rounds ( round_id, project_id, round_type, round_number, target_amount, raised_amount, start_date, end_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 0, ?, ?, 'open', ?, ?) ''', (round_id, project_id, round_type, count + 1,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", round_info[0], 'investment',
                                 f'投资到账: {investment_amount}', investor_name, now[:10]))
                    conn.commit()
    ...
    def register_ip(self, project_id: str, ip_type: str, ip_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO intellectual_property ( ip_id, project_id, ip_type, ip_name, description, registration_no, registration_date, expiration_date, owner, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'applied', ?, ?) ''', (ip_id, project_id, ip_type, ip_name,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE intellectual_property SET status = ?, registration_no = ?, registration_date = ?, expiration_date = ?, updated_at = ? WHERE ip_id = ?',
                                 (status, kwargs.get('registration_no'), kwargs.get('registration_date'),
                                 kwargs.get('expiration_date'), now, ip_id))
                    if cursor.rowcount > 0:
                        conn.commit()
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO project_milestones (milestone_id, project_id, stage, milestone_name, description, target_date, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                                 (f"mls_{uuid.uuid4().hex[:12]}", project_id, 'operation', '路演',
                                 kwargs.get('description', '项目路演'), pitch_date, 'pending', now, now))
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", project_id, 'pitch_scheduled', f'路演安排: {pitch_date}',
                                 kwargs.get('performed_by', 'system'), now[:10]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", project_id, 'investor_connection',
                                 f'投资人对接: {investor_name}', kwargs.get('performed_by', 'system'), now[:10]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", project_id, 'graduate', '项目毕业',
                                 kwargs.get('performed_by', 'system'), now[:10]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", project_id, 'transform',
                                 f'项目转型: {project[0]} -> {new_type}', kwargs.get('performed_by', 'system'), now[:10]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def create_reward_program(self, reward_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO innovation_rewards ( reward_id, reward_name, education_type, description, eligibility, amount, max_winners, application_deadline, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?) ''', (reward_id, reward_name, kwargs.get('education_type'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('INSERT INTO incubation_records (record_id, project_id, action_type, action_description, performed_by, performed_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"rec_{uuid.uuid4().hex[:12]}", project_id, 'reward', f'获得奖励: {reward_id}',
                                 kwargs.get('performed_by', 'system'), now[:10]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
