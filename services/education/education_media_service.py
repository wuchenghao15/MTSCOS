    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS media_content ( content_id TEXT PRIMARY KEY, title TEXT NOT NULL, media_type TEXT NOT NULL, file_path TEXT, file_name TEXT, file_size INTEGER DEFAULT 0, file_format TEXT, duration INTEGER DEFAULT 0, thumbnail_url TEXT, education_type TEXT DEFAULT 'adult', grade_level TEXT, subject TEXT, category TEXT, tags TEXT, description TEXT, creator_id INTEGER, creator_name TEXT, status TEXT DEFAULT 'uploading', view_count INTEGER DEFAULT 0, download_count INTEGER DEFAULT 0, is_published INTEGER DEFAULT 0, published_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS content_metadata ( metadata_id TEXT PRIMARY KEY, content_id TEXT NOT NULL, key TEXT NOT NULL, value TEXT, created_at TEXT, FOREIGN KEY(content_id) REFERENCES media_content(content_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS video_management ( video_id TEXT PRIMARY KEY, content_id TEXT NOT NULL, video_title TEXT NOT NULL, duration INTEGER DEFAULT 0, resolution TEXT, bitrate INTEGER DEFAULT 0, codec TEXT, frame_rate INTEGER DEFAULT 25, aspect_ratio TEXT DEFAULT '16:9', chapters TEXT, subtitles TEXT, thumbnails TEXT, education_type TEXT DEFAULT 'adult', grade_level TEXT, subject TEXT, is_processed INTEGER DEFAULT 0, processing_status TEXT DEFAULT 'pending', status TEXT DEFAULT 'draft', created_at TEXT, updated_at TEXT, FOREIGN KEY(content_id) REFERENCES media_content(content_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS video_records ( record_id TEXT PRIMARY KEY, video_id TEXT NOT NULL, action_type TEXT, user_id INTEGER, user_name TEXT, timestamp TEXT, details TEXT, FOREIGN KEY(video_id) REFERENCES video_management(video_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS audio_management ( audio_id TEXT PRIMARY KEY, content_id TEXT NOT NULL, audio_title TEXT NOT NULL, duration INTEGER DEFAULT 0, sample_rate INTEGER DEFAULT 44100, bitrate INTEGER DEFAULT 128000, channels INTEGER DEFAULT 2, codec TEXT, audio_format TEXT, cover_image TEXT, lyrics TEXT, education_type TEXT DEFAULT 'adult', grade_level TEXT, subject TEXT, is_processed INTEGER DEFAULT 0, processing_status TEXT DEFAULT 'pending', status TEXT DEFAULT 'draft', created_at TEXT, updated_at TEXT, FOREIGN KEY(content_id) REFERENCES media_content(content_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS audio_records ( record_id TEXT PRIMARY KEY, audio_id TEXT NOT NULL, action_type TEXT, user_id INTEGER, user_name TEXT, timestamp TEXT, details TEXT, FOREIGN KEY(audio_id) REFERENCES audio_management(audio_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS live_streaming ( live_id TEXT PRIMARY KEY, title TEXT NOT NULL, live_type TEXT, description TEXT, cover_image TEXT, stream_url TEXT, play_url TEXT, protocol TEXT DEFAULT 'hls', education_type TEXT DEFAULT 'adult', grade_level TEXT, subject TEXT, host_id INTEGER, host_name TEXT, start_time TEXT, end_time TEXT, duration INTEGER DEFAULT 0, viewer_count INTEGER DEFAULT 0, max_viewers INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', is_recorded INTEGER DEFAULT 1, recording_path TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS streaming_records ( record_id TEXT PRIMARY KEY, live_id TEXT NOT NULL, viewer_id INTEGER, viewer_name TEXT, join_time TEXT, leave_time TEXT, duration INTEGER DEFAULT 0, FOREIGN KEY(live_id) REFERENCES live_streaming(live_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS media_streaming ( stream_id TEXT PRIMARY KEY, content_id TEXT NOT NULL, stream_name TEXT, protocol TEXT, quality TEXT, stream_url TEXT, play_url TEXT, bandwidth INTEGER DEFAULT 0, latency REAL DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, FOREIGN KEY(content_id) REFERENCES media_content(content_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS streaming_config ( config_id TEXT PRIMARY KEY, protocol TEXT NOT NULL, quality_levels TEXT, bitrate_settings TEXT, buffer_size INTEGER DEFAULT 3000, latency_target REAL DEFAULT 3.0, transcoding_preset TEXT, adaptive_bitrate INTEGER DEFAULT 1, drm_enabled INTEGER DEFAULT 0, watermark_enabled INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS content_distribution ( distribution_id TEXT PRIMARY KEY, content_id TEXT NOT NULL, channel TEXT NOT NULL, status TEXT DEFAULT 'pending', url TEXT, cdn_provider TEXT, edge_locations TEXT, start_time TEXT, end_time TEXT, bandwidth_usage INTEGER DEFAULT 0, cost REAL DEFAULT 0, education_type TEXT DEFAULT 'adult', created_at TEXT, updated_at TEXT, FOREIGN KEY(content_id) REFERENCES media_content(content_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS distribution_records ( record_id TEXT PRIMARY KEY, distribution_id TEXT NOT NULL, timestamp TEXT, bandwidth INTEGER DEFAULT 0, requests INTEGER DEFAULT 0, cache_hit REAL DEFAULT 0, latency REAL DEFAULT 0, FOREIGN KEY(distribution_id) REFERENCES content_distribution(distribution_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS media_transcoding ( transcode_id TEXT PRIMARY KEY, content_id TEXT NOT NULL, source_format TEXT, target_format TEXT, transcoding_method TEXT, quality TEXT, resolution TEXT, bitrate INTEGER DEFAULT 0, status TEXT DEFAULT 'pending', progress INTEGER DEFAULT 0, error_message TEXT, output_path TEXT, duration INTEGER DEFAULT 0, start_time TEXT, end_time TEXT, created_at TEXT, updated_at TEXT, FOREIGN KEY(content_id) REFERENCES media_content(content_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS transcoding_records ( record_id TEXT PRIMARY KEY, transcode_id TEXT NOT NULL, action_type TEXT, timestamp TEXT, progress INTEGER DEFAULT 0, details TEXT, FOREIGN KEY(transcode_id) REFERENCES media_transcoding(transcode_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS media_storage ( storage_id TEXT PRIMARY KEY, content_id TEXT NOT NULL, storage_type TEXT NOT NULL, bucket_name TEXT, file_path TEXT, file_size INTEGER DEFAULT 0, checksum TEXT, upload_time TEXT, last_access_time TEXT, replication_status TEXT DEFAULT 'pending', backup_count INTEGER DEFAULT 0, is_archived INTEGER DEFAULT 0, archive_time TEXT, education_type TEXT DEFAULT 'adult', created_at TEXT, updated_at TEXT, FOREIGN KEY(content_id) REFERENCES media_content(content_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS storage_records ( record_id TEXT PRIMARY KEY, storage_id TEXT NOT NULL, action_type TEXT, timestamp TEXT, size_change INTEGER DEFAULT 0, details TEXT, FOREIGN KEY(storage_id) REFERENCES media_storage(storage_id) ) ''')
                conn.commit()
    ...
    def upload_media_content(self, title: str, media_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO media_content ( content_id, title, media_type, file_path, file_name, file_size, file_format, duration, thumbnail_url, education_type, grade_level, subject, category, tags, description, creator_id, creator_name, status, view_count, download_count, is_published, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'uploading', 0, 0, 0, ?, ?) ''', (content_id, title, media_type, kwargs.get('file_path'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    for key, value in metadata.items():
                        cursor.execute(''' INSERT OR REPLACE INTO content_metadata (metadata_id, content_id, key, value, created_at) VALUES (?, ?, ?, ?, ?) ''', (f"mmd_{uuid.uuid4().hex[:8]}", content_id, key, str(value), now))
    ...
    def upload_video(self, video_title: str, content_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO video_management ( video_id, content_id, video_title, duration, resolution, bitrate, codec, frame_rate, aspect_ratio, chapters, subtitles, thumbnails, education_type, grade_level, subject, is_processed, processing_status, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'pending', 'draft', ?, ?) ''', (video_id, content_id, video_title, kwargs.get('duration', 0),
                          ...
                          now, now))
                    cursor.execute('UPDATE media_content SET status = ? WHERE content_id = ?', ('processing',
                    content_id))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    conn.row_factory = sqlite3.Row
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT vm.*, mc.file_path, mc.view_count FROM video_management vm JOIN media_content mc ON vm.content_id = mc.content_id WHERE vm.video_id = ? ''', (video_id,))
                    video = cursor.fetchone()
                    if not video:
                        return {'success': False, 'error': '视频不存在'}
                    ...
                    cursor.execute('INSERT INTO video_records (record_id, video_id, action_type, user_id, user_name, timestamp, details) VALUES (?, ?, ?, ?, ?, ?, ?)',
                                 (f"vrd_{uuid.uuid4().hex[:8]}", video_id, 'play', user_id, None, now,
                                 json.dumps({'action': 'playback_start'})))
                    conn.commit()
    ...
    def upload_audio(self, audio_title: str, content_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO audio_management ( audio_id, content_id, audio_title, duration, sample_rate, bitrate, channels, codec, audio_format, cover_image, lyrics, education_type, grade_level, subject, is_processed, processing_status, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'pending', 'draft', ?, ?) ''', (audio_id, content_id, audio_title, kwargs.get('duration', 0),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    conn.row_factory = sqlite3.Row
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT am.*, mc.file_path, mc.view_count FROM audio_management am JOIN media_content mc ON am.content_id = mc.content_id WHERE am.audio_id = ? ''', (audio_id,))
                    audio = cursor.fetchone()
                    if not audio:
                        return {'success': False, 'error': '音频不存在'}
                    ...
                    cursor.execute('INSERT INTO audio_records (record_id, audio_id, action_type, user_id, user_name, timestamp, details) VALUES (?, ?, ?, ?, ?, ?, ?)',
                                 (f"ard_{uuid.uuid4().hex[:8]}", audio_id, 'play', user_id, None, now,
                                 json.dumps({'action': 'playback_start'})))
                    conn.commit()
    ...
    def create_live_stream(self, title: str, live_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO live_streaming ( live_id, title, live_type, description, cover_image, stream_url, play_url, protocol, education_type, grade_level, subject, host_id, host_name, start_time, end_time, duration, viewer_count, max_viewers, status, is_recorded, recording_path, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, 'scheduled', ?, ?, ?, ?) ''', (live_id, title, live_type, kwargs.get('description'),
    ...
    def get_live_viewers(self, live_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT ls.viewer_count, ls.max_viewers, ls.status FROM live_streaming ls WHERE ls.live_id = ? ''', (live_id,))
                live = cursor.fetchone()
                if not live:
                    return {'success': False, 'error': '直播不存在'}
                cursor.execute(''' SELECT sr.viewer_id, sr.viewer_name, sr.join_time, sr.duration FROM streaming_records sr WHERE sr.live_id = ? ORDER BY sr.join_time DESC ''', (live_id,))
    ...
    def configure_streaming(self, protocol: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO streaming_config ( config_id, protocol, quality_levels, bitrate_settings, buffer_size, latency_target, transcoding_preset, adaptive_bitrate, drm_enabled, watermark_enabled, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (config_id, protocol, json.dumps(kwargs.get('quality_levels', [])),
    ...
    def create_stream(self, content_id: str, stream_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO media_streaming ( stream_id, content_id, stream_name, protocol, quality, stream_url, play_url, bandwidth, latency, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (stream_id, content_id, stream_name, kwargs.get('protocol', 'hls'),
    ...
    def monitor_stream(self, stream_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT ms.*, mc.title, mc.media_type FROM media_streaming ms JOIN media_content mc ON ms.content_id = mc.content_id WHERE ms.stream_id = ? ''', (stream_id,))
    ...
    def distribute_content(self, content_id: str, channel: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO content_distribution ( distribution_id, content_id, channel, status, url, cdn_provider, edge_locations, start_time, end_time, bandwidth_usage, cost, education_type, created_at, updated_at ) VALUES (?, ?, ?, 'pending', ?, ?, ?, ?, ?, 0, 0, ?, ?, ?) ''', (distribution_id, content_id, channel, kwargs.get('url'),
    ...
    def start_distribution(self, distribution_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT status FROM content_distribution WHERE distribution_id = ?',
                    (distribution_id,))
    ...
    def get_distribution_stats(self, distribution_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT cd.*, mc.title, mc.media_type FROM content_distribution cd JOIN media_content mc ON cd.content_id = mc.content_id WHERE cd.distribution_id = ? ''', (distribution_id,))
                dist = cursor.fetchone()
                if not dist:
                    return {'success': False, 'error': '分发任务不存在'}
    ...
    def create_transcoding_task(self, content_id: str, source_format: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO media_transcoding ( transcode_id, content_id, source_format, target_format, transcoding_method, quality, resolution, bitrate, status, progress, error_message, output_path, duration, start_time, end_time, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', 0, ?, ?, ?, ?, ?, ?, ?) ''', (transcode_id, content_id, source_format, target_format,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO transcoding_records (record_id, transcode_id, action_type, timestamp, progress, details) VALUES (?, ?, ?, ?, ?, ?)',
                                 (f"trd_{uuid.uuid4().hex[:8]}", transcode_id, 'start', now, 0,
                                 json.dumps({'action': 'transcoding_start'})))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if progress >= 100:
                        ...
                        content_id = cursor.execute('SELECT content_id FROM media_transcoding WHERE transcode_id = ?',
                        (transcode_id,)).fetchone()[0]
                        cursor.execute('UPDATE media_content SET status = ? WHERE content_id = ?', ('ready',
                        content_id))
    ...
    def get_transcoding_status(self, transcode_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT mt.*, mc.title FROM media_transcoding mt JOIN media_content mc ON mt.content_id = mc.content_id WHERE mt.transcode_id = ? ''', (transcode_id,))
                task = cursor.fetchone()
                if not task:
                    return {'success': False, 'error': '转码任务不存在'}
                cursor.execute(
                'SELECT * FROM transcoding_records WHERE transcode_id = ? ORDER BY timestamp DESC LIMIT 10',
                (transcode_id,))
    ...
    def store_media(self, content_id: str, storage_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO media_storage ( storage_id, content_id, storage_type, bucket_name, file_path, file_size, checksum, upload_time, last_access_time, replication_status, backup_count, is_archived, archive_time, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', 0, 0, ?, ?, ?, ?) ''', (storage_id, content_id, storage_type, kwargs.get('bucket_name'),
    ...
    def migrate_storage(self, storage_id: str, target_storage_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT storage_type, content_id FROM media_storage WHERE storage_id = ?',
                    (storage_id,))
    ...
    def get_media_statistics(self, education_type: str = None, date_range: Tuple[str, str] = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True,
                        'content': {'total': content_stats[0], 'views': content_stats[1] or 0,
                        'downloads': content_stats[2] or 0},
