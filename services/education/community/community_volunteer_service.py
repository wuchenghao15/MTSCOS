    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS volunteer_profiles ( volunteer_id TEXT PRIMARY KEY, user_id INTEGER UNIQUE, real_name TEXT NOT NULL, phone TEXT, email TEXT, education_type TEXT, grade_level INTEGER, class_id TEXT, skills TEXT, interests TEXT, total_hours REAL DEFAULT 0, total_activities INTEGER DEFAULT 0, level INTEGER DEFAULT 1, badge TEXT DEFAULT 'bronze', status TEXT DEFAULT 'active', registered_at TEXT, last_active TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS volunteer_activities ( activity_id TEXT PRIMARY KEY, activity_name TEXT NOT NULL, activity_type TEXT, organizer TEXT, description TEXT, location TEXT, community TEXT, start_date TEXT, end_date TEXT, start_time TEXT, end_time TEXT, max_participants INTEGER DEFAULT 20, current_participants INTEGER DEFAULT 0, hours_per_volunteer REAL DEFAULT 2, contact_person TEXT, contact_phone TEXT, status TEXT DEFAULT 'draft', cover_image TEXT, education_type TEXT, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS activity_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, activity_id TEXT NOT NULL, volunteer_id TEXT NOT NULL, volunteer_name TEXT, register_time TEXT, status TEXT DEFAULT 'registered', check_in_time TEXT, check_out_time TEXT, actual_hours REAL DEFAULT 0, performance TEXT, feedback TEXT, rating INTEGER, UNIQUE(activity_id, volunteer_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS community_partnerships ( partnership_id TEXT PRIMARY KEY, community_name TEXT NOT NULL, partnership_type TEXT, contact_person TEXT, contact_phone TEXT, address TEXT, description TEXT, start_date TEXT, is_active INTEGER DEFAULT 1, total_activities INTEGER DEFAULT 0, total_volunteers INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS social_practices ( practice_id TEXT PRIMARY KEY, practice_name TEXT NOT NULL, practice_type TEXT, education_type TEXT, description TEXT, location TEXT, start_date TEXT, end_date TEXT, duration_days INTEGER, leader_id INTEGER, leader_name TEXT, participant_count INTEGER DEFAULT 0, report_required INTEGER DEFAULT 1, report_url TEXT, report_status TEXT DEFAULT 'not_submitted', status TEXT DEFAULT 'planned', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS practice_participants ( id INTEGER PRIMARY KEY AUTOINCREMENT, practice_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, role TEXT DEFAULT 'participant', joined_at TEXT, report_url TEXT, score REAL, UNIQUE(practice_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS donations ( donation_id TEXT PRIMARY KEY, donor_name TEXT NOT NULL, donor_id INTEGER, donation_type TEXT, amount REAL, quantity INTEGER, unit TEXT, purpose TEXT, recipient TEXT, anonymous INTEGER DEFAULT 0, message TEXT, received_at TEXT, acknowledged INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS volunteer_teams ( team_id TEXT PRIMARY KEY, team_name TEXT NOT NULL, description TEXT, leader_id INTEGER, leader_name TEXT, member_count INTEGER DEFAULT 1, education_type TEXT, is_active INTEGER DEFAULT 1, founded_date TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS team_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, team_id TEXT NOT NULL, volunteer_id TEXT NOT NULL, volunteer_name TEXT, role TEXT DEFAULT 'member', joined_at TEXT, UNIQUE(team_id, volunteer_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_certificates ( certificate_id TEXT PRIMARY KEY, volunteer_id TEXT NOT NULL, volunteer_name TEXT, certificate_type TEXT, title TEXT, hours REAL, issued_date TEXT, issued_by TEXT, verification_code TEXT, file_url TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def register_volunteer(self, real_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO volunteer_profiles ( volunteer_id, user_id, real_name, phone, email, education_type, grade_level, class_id, skills, interests, total_hours, total_activities, level, badge, status, registered_at, last_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 1, 'bronze', 'active', ?, ?, ?, ?) ''', (volunteer_id, kwargs.get('user_id'), real_name,
    ...
    def create_activity(self, activity_name: str, activity_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO volunteer_activities ( activity_id, activity_name, activity_type, organizer, description, location, community, start_date, end_date, start_time, end_time, max_participants, current_participants, hours_per_volunteer, contact_person, contact_phone, status, cover_image, education_type, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, 'published', ?, ?, ?, ?, ?) ''', (activity_id, activity_name, activity_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO activity_registrations (activity_id, volunteer_id, volunteer_name, register_time, status) VALUES (?, ?, ?, ?, 'registered') ''', (activity_id, volunteer_id, volunteer_name, now))
    ...
    def check_in(self, activity_id: str, volunteer_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE activity_registrations SET check_in_time = ?, status = 'checked_in' WHERE activity_id = ? AND volunteer_id = ? AND status = 'registered' ''', (now, activity_id, volunteer_id))
                    if cursor.rowcount > 0:
                        conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'SELECT check_in_time FROM activity_registrations WHERE activity_id = ? AND volunteer_id = ? AND  status = ?', (activity_id, volunteer_id, 'checked_in'))
                    reg = cursor.fetchone()
                    if not reg:
                        return {'success': False, 'error': '未找到签到记录'}
                    cursor.execute('SELECT hours_per_volunteer FROM volunteer_activities WHERE activity_id = ?',
                    (activity_id,))
                    act = cursor.fetchone()
                    hours = actual_hours if actual_hours else (act[0] if act else 2)
                    cursor.execute(''' UPDATE activity_registrations SET check_out_time = ?, actual_hours = ?, status = 'completed', performance = ?, feedback = ?, rating = ? WHERE activity_id = ? AND volunteer_id = ? ''', (now, hours, kwargs.get('performance'),
    ...
    def create_partnership(self, community_name: str, partnership_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO community_partnerships ( partnership_id, community_name, partnership_type, contact_person, contact_phone, address, description, start_date, is_active, total_activities, total_volunteers, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 0, 0, ?, ?) ''', (partnership_id, community_name, partnership_type,
    ...
    def list_partnerships(self, is_active: bool = True,
        ...
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'partnerships': partnerships, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def create_practice(self, practice_name: str, practice_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO social_practices ( practice_id, practice_name, practice_type, education_type, description, location, start_date, end_date, duration_days, leader_id, leader_name, participant_count, report_required, report_url, report_status, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, NULL, 'not_submitted', 'planned', ?, ?) ''', (practice_id, practice_name, practice_type,
    ...
    def join_practice(self, practice_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR IGNORE INTO practice_participants (practice_id, student_id, student_name, role, joined_at) VALUES (?, ?, ?, ?, ?) ''', (practice_id, student_id, student_name,
    ...
    def submit_practice_report(self, practice_id: str, report_url: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE social_practices SET report_url = ?, report_status = 'submitted', updated_at = ? WHERE practice_id = ? AND report_status = 'not_submitted' ''', (report_url, now, practice_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def create_team(self, team_name: str, leader_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO volunteer_teams ( team_id, team_name, description, leader_id, leader_name, member_count, education_type, is_active, founded_date, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 1, ?, 1, ?, ?, ?) ''', (team_id, team_name, kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO service_certificates ( certificate_id, volunteer_id, volunteer_name, certificate_type, title, hours, issued_date, issued_by, verification_code, file_url, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (certificate_id, volunteer_id, volunteer_name,
