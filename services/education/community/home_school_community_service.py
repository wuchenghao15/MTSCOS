    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                # 1. 家长档案表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_profiles ( parent_id TEXT PRIMARY KEY, user_id TEXT, name TEXT NOT NULL, gender TEXT, phone TEXT, email TEXT, relationship TEXT, occupation TEXT, education_level TEXT, student_ids TEXT, family_type TEXT, address TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 2. 家校沟通记录表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS teacher_parent_communications ( comm_id TEXT PRIMARY KEY, parent_id TEXT, parent_name TEXT, teacher_id TEXT, teacher_name TEXT, student_id TEXT, student_name TEXT, channel TEXT, topic TEXT, title TEXT, content TEXT, direction TEXT, status TEXT, response TEXT, response_time TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 3. 家长会表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_meetings ( meeting_id TEXT PRIMARY KEY, meeting_name TEXT NOT NULL, meeting_type TEXT, organizer_id TEXT, organizer_name TEXT, target_grade TEXT, target_class TEXT, meeting_date TEXT, start_time TEXT, end_time TEXT, location TEXT, agenda TEXT, max_attendees INTEGER DEFAULT 100, registered_count INTEGER DEFAULT 0, actual_attendees INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', summary TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 4. 家长会报名表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS meeting_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, meeting_id TEXT NOT NULL, parent_id TEXT, parent_name TEXT, student_id TEXT, student_name TEXT, register_time TEXT, attend_status TEXT DEFAULT 'registered', created_at TEXT ) ''')
                # 5. 家庭教育指导项目表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS family_education_programs ( program_id TEXT PRIMARY KEY, program_name TEXT NOT NULL, program_type TEXT, target_audience TEXT, instructor TEXT, description TEXT, content TEXT, schedule TEXT, location TEXT, max_participants INTEGER DEFAULT 50, enrolled_count INTEGER DEFAULT 0, is_free INTEGER DEFAULT 1, fee REAL DEFAULT 0, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT ) ''')
                # 6. 项目报名表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS program_enrollments ( id INTEGER PRIMARY KEY AUTOINCREMENT, program_id TEXT NOT NULL, parent_id TEXT, parent_name TEXT, student_id TEXT, enroll_time TEXT, attendance_count INTEGER DEFAULT 0, completion TEXT DEFAULT 'ongoing', feedback TEXT, created_at TEXT ) ''')
                # 7. 家长委员会表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_committees ( committee_id TEXT PRIMARY KEY, committee_name TEXT NOT NULL, committee_type TEXT, level TEXT, parent_id TEXT, parent_name TEXT, role TEXT, term_start TEXT, term_end TEXT, responsibilities TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                # 8. 家长志愿者表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS parent_volunteers ( volunteer_id TEXT PRIMARY KEY, parent_id TEXT, parent_name TEXT, student_id TEXT, specialties TEXT, available_times TEXT, total_hours REAL DEFAULT 0, activity_count INTEGER DEFAULT 0, rating REAL DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                # 9. 社区教育资源表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS community_resources ( resource_id TEXT PRIMARY KEY, resource_name TEXT NOT NULL, resource_type TEXT, provider_name TEXT, provider_contact TEXT, description TEXT, availability TEXT, location TEXT, cost REAL DEFAULT 0, is_free INTEGER DEFAULT 1, rating REAL DEFAULT 0, usage_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                # 10. 社区活动表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS community_activities ( activity_id TEXT PRIMARY KEY, activity_name TEXT NOT NULL, activity_type TEXT, organizer TEXT, description TEXT, target_audience TEXT, start_date TEXT, end_date TEXT, location TEXT, max_participants INTEGER DEFAULT 100, registered_count INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', outcome TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 11. 社区共建项目表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS community_cooperations ( coop_id TEXT PRIMARY KEY, project_name TEXT NOT NULL, partner_organization TEXT, partner_type TEXT, description TEXT, objectives TEXT, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'ongoing', contribution TEXT, outcome TEXT, contact_person TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 12. 家访记录表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS home_visits ( visit_id TEXT PRIMARY KEY, teacher_id TEXT, teacher_name TEXT, student_id TEXT, student_name TEXT, parent_id TEXT, parent_name TEXT, visit_type TEXT, planned_date TEXT, actual_date TEXT, duration INTEGER, purpose TEXT, family_situation TEXT, discussion_content TEXT, findings TEXT, recommendations TEXT, follow_up_plan TEXT, status TEXT DEFAULT 'planned', created_at TEXT, updated_at TEXT ) ''')
                # 13. 协同活动表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS collaborative_activities ( activity_id TEXT PRIMARY KEY, activity_name TEXT NOT NULL, activity_type TEXT, organizer TEXT, description TEXT, target_audience TEXT, start_date TEXT, end_date TEXT, location TEXT, max_participants INTEGER DEFAULT 100, registered_count INTEGER DEFAULT 0, parent_participants INTEGER DEFAULT 0, community_participants INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', outcome TEXT, created_at TEXT, updated_at TEXT ) ''')
                # 14. 活动报名表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS activity_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, activity_id TEXT NOT NULL, activity_type TEXT, participant_id TEXT, participant_name TEXT, participant_role TEXT, register_time TEXT, attend_status TEXT DEFAULT 'registered', created_at TEXT ) ''')
                # 15. 家庭教育档案表
                cursor.execute(''' CREATE TABLE IF NOT EXISTS family_education_profiles ( profile_id TEXT PRIMARY KEY, student_id TEXT, student_name TEXT, parent_id TEXT, parent_name TEXT, family_structure TEXT, education_environment TEXT, parent_expectations TEXT, communication_frequency TEXT, last_communication TEXT, needs TEXT, risk_factors TEXT, created_at TEXT, updated_at TEXT ) ''')
                conn.commit()
    ...
    def register_parent(self, name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO parent_profiles ( parent_id, user_id, name, gender, phone, email, relationship, occupation, education_level, student_ids, family_type, address, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (parent_id, kwargs.get('user_id'), name,
    ...
    def create_communication(self, parent_id: str, teacher_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO teacher_parent_communications ( comm_id, parent_id, parent_name, teacher_id, teacher_name, student_id, student_name, channel, topic, title, content, direction, status, response, response_time, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?) ''', (comm_id, parent_id, kwargs.get('parent_name'),
    ...
    def respond_communication(self, comm_id: str, response: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE teacher_parent_communications SET response = ?, response_time = ?, status = 'responded', updated_at = ? WHERE comm_id = ? ''', (response, now, now, comm_id))
                    if cursor.rowcount > 0:
                        conn.commit()
    ...
    def create_parent_meeting(self, meeting_name: str, meeting_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO parent_meetings ( meeting_id, meeting_name, meeting_type, organizer_id, organizer_name, target_grade, target_class, meeting_date, start_time, end_time, location, agenda, max_attendees, registered_count, actual_attendees, status, summary, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 'scheduled', NULL, ?, ?) ''', (meeting_id, meeting_name, meeting_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO meeting_registrations (meeting_id, parent_id, parent_name, student_id, student_name, register_time, attend_status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'registered', ?) ''', (meeting_id, parent_id, kwargs.get('parent_name'),
    ...
    def record_meeting_attendance(self, meeting_id: str, actual_attendees: int, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE parent_meetings SET actual_attendees = ?, summary = ?, status = 'completed', updated_at = ? WHERE meeting_id = ? ''', (actual_attendees, kwargs.get('summary'), now, meeting_id))
                    if cursor.rowcount > 0:
                        if kwargs.get('attendees'):
                            for parent_id in kwargs['attendees']:
                                cursor.execute(
                                'UPDATE meeting_registrations SET attend_status = ? WHERE meeting_id = ? AND parent_id  = ?',
    ...
    def create_family_education_program(self, program_name: str, program_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO family_education_programs ( program_id, program_name, program_type, target_audience, instructor, description, content, schedule, location, max_participants, enrolled_count, is_free, fee, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'open', ?, ?) ''', (program_id, program_name, program_type,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if program[0] and program[1] >= program[0]:
                        return {'success': False, 'error': '名额已满'}
                    cursor.execute(''' INSERT INTO program_enrollments (program_id, parent_id, parent_name, student_id, enroll_time, attendance_count, completion, feedback, created_at) VALUES (?, ?, ?, ?, ?, 0, 'ongoing', NULL, ?) ''', (program_id, parent_id, kwargs.get('parent_name'),
    ...
    def create_committee(self, committee_name: str, committee_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO parent_committees ( committee_id, committee_name, committee_type, level, parent_id, parent_name, role, term_start, term_end, responsibilities, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (committee_id, committee_name, committee_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'SELECT volunteer_id FROM parent_volunteers WHERE parent_id = ? AND status = "active"', (parent_id,
                    ))
                    if cursor.fetchone():
                        return {'success': False, 'error': '该家长已是志愿者'}
                    cursor.execute(''' INSERT INTO parent_volunteers ( volunteer_id, parent_id, parent_name, student_id, specialties, available_times, total_hours, activity_count, rating, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0, 'active', ?, ?) ''', (volunteer_id, parent_id, kwargs.get('parent_name'),
    ...
    def add_community_resource(self, resource_name: str, resource_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO community_resources ( resource_id, resource_name, resource_type, provider_name, provider_contact, description, availability, location, cost, is_free, rating, usage_count, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?) ''', (resource_id, resource_name, resource_type,
    ...
    def create_community_activity(self, activity_name: str, activity_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO community_activities ( activity_id, activity_name, activity_type, organizer, description, target_audience, start_date, end_date, location, max_participants, registered_count, status, outcome, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'scheduled', NULL, ?, ?) ''', (activity_id, activity_name, activity_type,
    ...
    def create_cooperation(self, project_name: str, partner_organization: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO community_cooperations ( coop_id, project_name, partner_organization, partner_type, description, objectives, start_date, end_date, status, contribution, outcome, contact_person, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'ongoing', ?, NULL, ?, ?, ?) ''', (coop_id, project_name, partner_organization,
    ...
    def plan_home_visit(self, teacher_id: str, student_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO home_visits ( visit_id, teacher_id, teacher_name, student_id, student_name, parent_id, parent_name, visit_type, planned_date, actual_date, duration, purpose, family_situation, discussion_content, findings, recommendations, follow_up_plan, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, ?, NULL, NULL, NULL, NULL, NULL, 'planned', ?, ?) ''', (visit_id, teacher_id, kwargs.get('teacher_name'),
    ...
    def create_collaborative_activity(self, activity_name: str, activity_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO collaborative_activities ( activity_id, activity_name, activity_type, organizer, description, target_audience, start_date, end_date, location, max_participants, registered_count, parent_participants, community_participants, status, outcome, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, 'scheduled', NULL, ?, ?) ''', (activity_id, activity_name, activity_type,
                          kwargs.get('organizer'), kwargs.get('description'),
                          kwargs.get('target_audience'),
                          kwargs.get('start_date'), kwargs.get('end_date'),
                          kwargs.get('location'), kwargs.get('max_participants', 100),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO activity_registrations (activity_id, activity_type, participant_id, participant_name, participant_role, register_time, attend_status, created_at) VALUES (?, 'collaborative', ?, ?, ?, ?, 'registered', ?) ''', (activity_id, participant_id, kwargs.get('participant_name'),
                    ...
                    if participant_role == 'parent':
                        cursor.execute(
                        'UPDATE collaborative_activities SET parent_participants = parent_participants + 1 WHERE  activity_id = ?', (activity_id,))
                    elif participant_role == 'community':
                        cursor.execute(
                        'UPDATE collaborative_activities SET community_participants = community_participants + 1 WHERE  activity_id = ?', (activity_id,))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO family_education_profiles ( profile_id, student_id, student_name, parent_id, parent_name, family_structure, education_environment, parent_expectations, communication_frequency, last_communication, needs, risk_factors, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (profile_id, student_id, kwargs.get('student_name'),
    ...
    def update_family_profile(self, profile_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(f'UPDATE family_education_profiles SET {", ".join(fields)} WHERE profile_id = ?',
                    values)
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
