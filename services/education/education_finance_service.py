...
LOAN_TYPES = {
    'student_loan': {'name': '助学贷款', 'rate': 0.03, 'max_amount': 12000, 'duration': 240, 'education_types': ['adult',
    'k12']},
    'tuition_loan': {'name': '学费贷款', 'rate': 0.045, 'max_amount': 50000, 'duration': 120, 'education_types': ['adult',
    'k12']},
    ...
    'study_abroad_loan': {'name': '留学贷款', 'rate': 0.055, 'max_amount': 200000, 'duration': 240,
    'education_types': ['adult', 'k12']},
    'consumption_loan': {'name': '消费贷款', 'rate': 0.07, 'max_amount': 50000, 'duration': 36,
    'education_types': ['adult']},
    'installment_loan': {'name': '分期贷款', 'rate': 0.035, 'max_amount': 20000, 'duration': 24,
    'education_types': ['adult', 'k12']},
    'credit_loan': {'name': '信用贷款', 'rate': 0.08, 'max_amount': 100000, 'duration': 60, 'education_types': ['adult']}
}

INVESTMENT_TYPES = {
    'education_savings': {'name': '教育储蓄', 'risk': 'low', 'min_amount': 500, 'expected_return': 0.025,
    'lock_period': 365},
    'education_fund': {'name': '教育基金', 'risk': 'medium', 'min_amount': 1000, 'expected_return': 0.05,
    'lock_period': 730},
    'education_bond': {'name': '教育债券', 'risk': 'low', 'min_amount': 100, 'expected_return': 0.03, 'lock_period': 365},
    'education_trust': {'name': '教育信托', 'risk': 'medium', 'min_amount': 100000, 'expected_return': 0.06,
    'lock_period': 1095},
    'education_insurance': {'name': '教育保险', 'risk': 'low', 'min_amount': 5000, 'expected_return': 0.035,
    'lock_period': 3650},
    'education_finance': {'name': '教育理财', 'risk': 'medium', 'min_amount': 1000, 'expected_return': 0.045,
    'lock_period': 180},
    'education_fixed_investment': {'name': '教育定投', 'risk': 'medium', 'min_amount': 100, 'expected_return': 0.055,
    'lock_period': 730},
    'education_crowdfunding': {'name': '教育众筹', 'risk': 'high', 'min_amount': 100, 'expected_return': 0.1,
    'lock_period': 365}
}
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_loans ( loan_id TEXT PRIMARY KEY, loan_type TEXT NOT NULL, education_type TEXT, applicant_id INTEGER NOT NULL, applicant_name TEXT, amount REAL NOT NULL, rate REAL NOT NULL, duration INTEGER NOT NULL, monthly_payment REAL, total_payment REAL, status TEXT DEFAULT 'applied', reason TEXT, collateral TEXT, credit_score INTEGER, approved_at TEXT, disbursed_at TEXT, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS loan_records ( record_id TEXT PRIMARY KEY, loan_id TEXT NOT NULL, record_type TEXT, amount REAL, remaining_amount REAL, due_date TEXT, paid_date TEXT, status TEXT DEFAULT 'pending', penalty REAL DEFAULT 0, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_investment ( investment_id TEXT PRIMARY KEY, investment_type TEXT NOT NULL, education_type TEXT, investor_id INTEGER NOT NULL, investor_name TEXT, amount REAL NOT NULL, expected_return REAL, actual_return REAL DEFAULT 0, risk_level TEXT, lock_period INTEGER, maturity_date TEXT, status TEXT DEFAULT 'active', purchased_at TEXT, redeemed_at TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS investment_records ( record_id TEXT PRIMARY KEY, investment_id TEXT NOT NULL, record_type TEXT, amount REAL, return_amount REAL, record_date TEXT, description TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_insurance ( policy_id TEXT PRIMARY KEY, insurance_type TEXT NOT NULL, education_type TEXT, insured_id INTEGER NOT NULL, insured_name TEXT, premium REAL NOT NULL, coverage REAL NOT NULL, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'active', beneficiary TEXT, claim_amount REAL DEFAULT 0, claimed_at TEXT, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS insurance_records ( record_id TEXT PRIMARY KEY, policy_id TEXT NOT NULL, record_type TEXT, amount REAL, description TEXT, record_date TEXT, status TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_payment ( payment_id TEXT PRIMARY KEY, order_id TEXT NOT NULL, payment_method TEXT NOT NULL, education_type TEXT, payer_id INTEGER, payer_name TEXT, amount REAL NOT NULL, fee REAL DEFAULT 0, status TEXT DEFAULT 'pending', transaction_no TEXT, paid_at TEXT, refunded_at TEXT, refund_amount REAL DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS payment_records ( record_id TEXT PRIMARY KEY, payment_id TEXT NOT NULL, record_type TEXT, amount REAL, description TEXT, record_date TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_settlement ( settlement_id TEXT PRIMARY KEY, settlement_type TEXT NOT NULL, education_type TEXT, amount REAL NOT NULL, fee REAL DEFAULT 0, status TEXT DEFAULT 'pending', settlement_date TEXT, completed_at TEXT, source_account TEXT, target_account TEXT, description TEXT, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS settlement_records ( record_id TEXT PRIMARY KEY, settlement_id TEXT NOT NULL, record_type TEXT, amount REAL, description TEXT, record_date TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_financing ( financing_id TEXT PRIMARY KEY, financing_method TEXT NOT NULL, education_type TEXT, applicant_id INTEGER NOT NULL, applicant_name TEXT, project_name TEXT, amount REAL NOT NULL, ratio REAL, status TEXT DEFAULT 'applied', business_plan TEXT, approved_at TEXT, funded_at TEXT, repayment_schedule TEXT, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS financing_records ( record_id TEXT PRIMARY KEY, financing_id TEXT NOT NULL, record_type TEXT, amount REAL, description TEXT, record_date TEXT, status TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_investment_analysis ( analysis_id TEXT PRIMARY KEY, investment_id TEXT, strategy_type TEXT NOT NULL, risk_score INTEGER, expected_return REAL, actual_return REAL DEFAULT 0, portfolio TEXT, performance_benchmark TEXT, analysis_date TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS analysis_records ( record_id TEXT PRIMARY KEY, analysis_id TEXT NOT NULL, metric_name TEXT, metric_value REAL, metric_date TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS education_cash_management ( cash_id TEXT PRIMARY KEY, management_type TEXT NOT NULL, education_type TEXT, organization_id INTEGER, organization_name TEXT, budget_amount REAL DEFAULT 0, actual_amount REAL DEFAULT 0, balance REAL DEFAULT 0, period TEXT, status TEXT DEFAULT 'active', description TEXT, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS cash_records ( record_id TEXT PRIMARY KEY, cash_id TEXT NOT NULL, record_type TEXT, amount REAL, description TEXT, record_date TEXT, balance_after REAL, created_at TEXT ) ''')
    ...
    def apply_loan(self, loan_type: str, applicant_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_loans ( loan_id, loan_type, education_type, applicant_id, applicant_name, amount, rate, duration, monthly_payment, total_payment, status, reason, collateral, credit_score, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'applied', ?, ?, ?, ?, ?) ''', (loan_id, loan_type, education_type, applicant_id,
                    ...
                    return {'success': True, 'loan_id': loan_id, 'monthly_payment': round(monthly_payment, 2),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    for i in range(duration):
                        ...
                        cursor.execute(''' INSERT INTO loan_records (record_id, loan_id, record_type, amount, remaining_amount, due_date, status, created_at) VALUES (?, ?, 'installment', ?, ?, ?, 'pending', ?) ''', (record_id, loan_id, round(monthly_payment, 2), round(remaining_amount, 2), due_date, now))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE loan_records SET remaining_amount = ? WHERE record_id = ?',
                    (round(remaining_after, 2), record_id))

                    repay_record_id = f"lrp_{uuid.uuid4().hex[:8]}"
                    cursor.execute(''' INSERT INTO loan_records (record_id, loan_id, record_type, amount, paid_date, status, created_at) VALUES (?, ?, 'repayment', ?, ?, 'completed', ?) ''', (repay_record_id, loan_id, actual_pay, now, now))

                    cursor.execute('SELECT COUNT(*) FROM loan_records WHERE loan_id = ? AND status = ?', (loan_id,
                    'pending'))
                    pending_count = cursor.fetchone()[0]
                    if pending_count == 0:
                        cursor.execute('UPDATE education_loans SET status = ?, updated_at = ? WHERE loan_id = ?',
                        ('settled', now, loan_id))
    ...
    def purchase_investment(self, investment_type: str, investor_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_investment ( investment_id, investment_type, education_type, investor_id, investor_name, amount, expected_return, risk_level, lock_period, maturity_date, status, purchased_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (investment_id, investment_type, kwargs.get('education_type', 'adult'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO investment_records (record_id, investment_id, record_type, amount, return_amount, record_date, description, created_at) VALUES (?, ?, 'redemption', ?, ?, ?, ?, ?) ''', (record_id, investment_id, investment['amount'], round(actual_return, 2), now,
                          f'提前赎回扣罚{round(penalty, 2)}元' if penalty > 0 else '正常赎回', now))

                    conn.commit()
                    return {'success': True, 'redeemed_amount': round(investment['amount'] + actual_return, 2),
    ...
    def list_investments(self, investor_id: int = None, status: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'investments': investments, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def purchase_insurance(self, insurance_type: str, insured_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_insurance ( policy_id, insurance_type, education_type, insured_id, insured_name, premium, coverage, start_date, end_date, status, beneficiary, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?) ''', (policy_id, insurance_type, education_type,
                    ...
                    return {'success': True, 'policy_id': policy_id, 'premium': config['premium'],
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO insurance_records (record_id, policy_id, record_type, amount, description, record_date, status, created_at) VALUES (?, ?, 'claim', ?, ?, ?, 'pending', ?) ''', (record_id, policy_id, amount, kwargs.get('description'), now, now))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE education_insurance SET status = 'active', premium = ?, coverage = ?, start_date = ?, end_date = ?, claim_amount = 0, claimed_at = NULL, updated_at = ? WHERE policy_id = ? ''', (config.get('premium', policy['premium']), config.get('coverage', policy['coverage']),
                          new_start_date, new_end_date, now, policy_id))

                    record_id = f"irn_{uuid.uuid4().hex[:8]}"
                    cursor.execute(''' INSERT INTO insurance_records (record_id, policy_id, record_type, amount, description, record_date, status, created_at) VALUES (?, ?, 'renewal', ?, ?, ?, 'completed', ?) ''', (record_id, policy_id, config.get('premium', policy['premium']), '保单续费', now, now))
    ...
    def create_payment(self, order_id: str, payment_method: str, amount: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_payment ( payment_id, order_id, payment_method, education_type, payer_id, payer_name, amount, fee, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (payment_id, order_id, payment_method, education_type,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        ...
                        cursor.execute(''' INSERT INTO payment_records (record_id, payment_id, record_type, amount, description, record_date, created_at) VALUES (?, ?, 'payment', ?, '支付成功', ?, ?) ''', (record_id, payment_id, transaction_no, now, now))
                        conn.commit()
                        return {'success': True}
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO payment_records (record_id, payment_id, record_type, amount, description, record_date, created_at) VALUES (?, ?, 'refund', ?, '退款', ?, ?) ''', (record_id, payment_id, refund_amount, now, now))
    ...
    def create_settlement(self, settlement_type: str, amount: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_settlement ( settlement_id, settlement_type, education_type, amount, fee, status, source_account, target_account, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?) ''', (settlement_id, settlement_type, kwargs.get('education_type', 'adult'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        ...
                        cursor.execute(''' INSERT INTO settlement_records (record_id, settlement_id, record_type, amount, description, record_date, created_at) VALUES (?, ?, 'settlement', ?, '结算完成', ?, ?) ''', (record_id, settlement_id, now, now, now))
                        conn.commit()
                        return {'success': True}
    ...
    def reconcile_settlement(self, settlement_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' INSERT INTO settlement_records (record_id, settlement_id, record_type, amount, description, record_date, created_at) VALUES (?, ?, 'reconciliation', ?, ?, ?, ?) ''', (record_id, settlement_id, discrepancy, f'对账差异: {discrepancy:.2f}',
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    for split in splits:
                        ...
                        cursor.execute(''' INSERT INTO settlement_records (record_id, settlement_id, record_type, amount, description, record_date, created_at) VALUES (?, ?, 'split', ?, ?, ?, ?) ''', (record_id, settlement_id, split['amount'], split.get('description',
                        f'分账至{split.get("target", "")}'), now, now))
    ...
    def apply_financing(self, financing_method: str, applicant_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_financing ( financing_id, financing_method, education_type, applicant_id, applicant_name, project_name, amount, ratio, status, business_plan, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'applied', ?, ?, ?) ''', (financing_id, financing_method, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO financing_records (record_id, financing_id, record_type, amount, description, record_date, status, created_at) VALUES (?, ?, 'funding', ?, '融资放款', ?, 'completed', ?) ''', (record_id, financing_id, financing[0], now, now))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO financing_records (record_id, financing_id, record_type, amount, description, record_date, status, created_at) VALUES (?, ?, 'repayment', ?, ?, ?, 'completed', ?) ''', (record_id, financing_id, amount, kwargs.get('description', '还款'), now, now))
    ...
    def create_investment_portfolio(self, strategy_type: str, investor_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_investment_analysis ( analysis_id, strategy_type, risk_score, expected_return, portfolio, analysis_date, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (analysis_id, strategy_type, config['risk_level'],
    ...
    def add_metric_record(self, analysis_id: str, metric_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO analysis_records (record_id, analysis_id, metric_name, metric_value, metric_date, created_at) VALUES (?, ?, ?, ?, ?, ?) ''', (record_id, analysis_id, metric_name, metric_value, now[:10], now))
                    conn.commit()
    ...
    def create_budget(self, organization_id: int, management_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO education_cash_management ( cash_id, management_type, education_type, organization_id, organization_name, budget_amount, actual_amount, balance, period, status, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, 'active', ?, ?, ?) ''', (cash_id, management_type, kwargs.get('education_type', 'adult'),
    ...
    def record_cash_flow(self, cash_id: str, record_type: str, amount: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT balance, actual_amount FROM education_cash_management WHERE cash_id = ?',
                    (cash_id,))
                    ...
                    cursor.execute(''' INSERT INTO cash_records (record_id, cash_id, record_type, amount, description, record_date, balance_after, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (record_id, cash_id, record_type, amount, kwargs.get('description'), now[:10],
                    round(new_balance, 2), now))
            ...
            with self._get_connection() as conn:
                ...
                for cm in cash_management:
                    ...
                    reports.append({
                        ...
                        'utilization_rate': round(cm['actual_amount'] / cm['budget_amount'] * 100,
                        2) if cm['budget_amount'] > 0 else 0
                    })
    ...
    def analyze_cash_flow(self, organization_id: int, period: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                query = ''' SELECT cr.record_type, SUM(cr.amount) as total, cr.record_date FROM cash_records cr JOIN education_cash_management cm ON cr.cash_id = cm.cash_id WHERE cm.organization_id = ? '''
                ...
                cursor.execute('SELECT SUM(balance) FROM education_cash_management WHERE organization_id = ?',
                (organization_id,))
