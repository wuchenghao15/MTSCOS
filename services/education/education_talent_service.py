    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS talent_recruitment ( recruitment_id TEXT PRIMARY KEY, title TEXT NOT NULL, recruitment_type TEXT, education_type TEXT, department TEXT, position TEXT, requirement TEXT, salary_range TEXT, location TEXT, quota INTEGER DEFAULT 1, status TEXT DEFAULT 'open', start_date TEXT, end_date TEXT, contact TEXT, description TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS recruitment_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, recruitment_id TEXT NOT NULL, applicant_id INTEGER NOT NULL, applicant_name TEXT, resume_url TEXT, status TEXT DEFAULT 'applied', apply_date TEXT, interview_date TEXT, interview_result TEXT, offer_date TEXT, accept_status TEXT, UNIQUE(recruitment_id, applicant_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS talent_training ( training_id TEXT PRIMARY KEY, title TEXT NOT NULL, training_type TEXT, education_type TEXT, department TEXT, duration TEXT, location TEXT, start_date TEXT, end_date TEXT, max_participants INTEGER DEFAULT 30, enrolled_count INTEGER DEFAULT 0, instructor TEXT, content TEXT, cost REAL DEFAULT 0, status TEXT DEFAULT 'planned', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS training_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, training_id TEXT NOT NULL, trainee_id INTEGER NOT NULL, trainee_name TEXT, enroll_date TEXT, attendance_rate REAL DEFAULT 0, final_score REAL, evaluation TEXT, completion_status TEXT DEFAULT 'in_progress', UNIQUE(training_id, trainee_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS talent_evaluation ( evaluation_id TEXT PRIMARY KEY, title TEXT NOT NULL, evaluation_method TEXT, education_type TEXT, department TEXT, period TEXT, criteria TEXT, weight TEXT, status TEXT DEFAULT 'draft', start_date TEXT, end_date TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS evaluation_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, evaluation_id TEXT NOT NULL, employee_id INTEGER NOT NULL, employee_name TEXT, self_score REAL, manager_score REAL, peer_score REAL, total_score REAL, grade TEXT, comment TEXT, status TEXT DEFAULT 'pending', created_at TEXT, UNIQUE(evaluation_id, employee_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS talent_flow ( flow_id TEXT PRIMARY KEY, flow_type TEXT, education_type TEXT, employee_id INTEGER NOT NULL, employee_name TEXT, current_department TEXT, target_department TEXT, current_position TEXT, target_position TEXT, reason TEXT, status TEXT DEFAULT 'pending', apply_date TEXT, approve_date TEXT, effective_date TEXT, approver_id INTEGER, approver_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS flow_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, flow_id TEXT NOT NULL, step TEXT, action TEXT, operator_id INTEGER, operator_name TEXT, operation_date TEXT, remark TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS talent_incentive ( incentive_id TEXT PRIMARY KEY, incentive_type TEXT, education_type TEXT, title TEXT NOT NULL, description TEXT, criteria TEXT, budget REAL DEFAULT 0, period TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS incentive_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, incentive_id TEXT NOT NULL, employee_id INTEGER NOT NULL, employee_name TEXT, amount REAL DEFAULT 0, reason TEXT, grant_date TEXT, status TEXT DEFAULT 'approved', UNIQUE(incentive_id, employee_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS talent_reserve ( reserve_id TEXT PRIMARY KEY, reserve_type TEXT, education_type TEXT, employee_id INTEGER NOT NULL, employee_name TEXT, department TEXT, position TEXT, level TEXT, target_position TEXT, evaluation_result TEXT, development_plan TEXT, status TEXT DEFAULT 'active', join_date TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS reserve_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, reserve_id TEXT NOT NULL, activity_type TEXT, activity_name TEXT, activity_date TEXT, result TEXT, evaluator TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS talent_service ( service_id TEXT PRIMARY KEY, service_type TEXT, education_type TEXT, title TEXT NOT NULL, provider TEXT, description TEXT, schedule TEXT, capacity INTEGER DEFAULT 20, used_count INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, service_id TEXT NOT NULL, employee_id INTEGER NOT NULL, employee_name TEXT, request_date TEXT, service_date TEXT, content TEXT, feedback TEXT, rating INTEGER, status TEXT DEFAULT 'completed', UNIQUE(service_id, employee_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS talent_development ( development_id TEXT PRIMARY KEY, development_path TEXT, education_type TEXT, employee_id INTEGER NOT NULL, employee_name TEXT, department TEXT, current_position TEXT, target_position TEXT, plan TEXT, milestones TEXT, start_date TEXT, expected_end_date TEXT, status TEXT DEFAULT 'in_progress', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS development_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, development_id TEXT NOT NULL, milestone TEXT, target_date TEXT, actual_date TEXT, status TEXT DEFAULT 'pending', comment TEXT, evaluator_id INTEGER, evaluator_name TEXT ) ''')
                conn.commit()
    ...
    def create_recruitment(self, title: str, recruitment_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO talent_recruitment ( recruitment_id, title, recruitment_type, education_type, department, position, requirement, salary_range, location, quota, status, start_date, end_date, contact, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?, ?, ?, ?, ?) ''', (recruitment_id, title, recruitment_type, education_type,
    ...
    def apply_recruitment(self, recruitment_id: str, applicant_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT status, end_date FROM talent_recruitment WHERE recruitment_id = ?',
                    (recruitment_id,))
    ...
    def process_interview(self, recruitment_id: str, applicant_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE recruitment_records SET interview_date = ?, interview_result = ?, status = ? WHERE recruitment_id = ? AND applicant_id = ? ''', (kwargs.get('interview_date', now[:10]),
    ...
    def create_training(self, title: str, training_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO talent_training ( training_id, title, training_type, education_type, department, duration, location, start_date, end_date, max_participants, enrolled_count, instructor, content, cost, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, 'planned', ?, ?) ''', (training_id, title, training_type, education_type,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE training_records SET attendance_rate = ?, final_score = ?, evaluation = ?, completion_status = ? WHERE training_id = ? AND trainee_id = ? ''', (kwargs.get('attendance_rate', 0), score,
    ...
    def create_evaluation(self, title: str, evaluation_method: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO talent_evaluation ( evaluation_id, title, evaluation_method, education_type, department, period, criteria, weight, status, start_date, end_date, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?) ''', (evaluation_id, title, evaluation_method, education_type,
                          kwargs.get('department'),
    ...
    def record_manager_score(self, evaluation_id: str, employee_id: int,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT self_score, peer_score FROM evaluation_records WHERE evaluation_id = ? AND employee_id = ?', (evaluation_id,
                    employee_id))
    ...
    def get_evaluation_result(self, evaluation_id: str, employee_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM evaluation_records WHERE evaluation_id = ? AND employee_id = ?',
                (evaluation_id, employee_id))
                record = cursor.fetchone()
                if record:
    ...
    def apply_flow(self, flow_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO talent_flow ( flow_id, flow_type, education_type, employee_id, employee_name, current_department, target_department, current_position, target_position, reason, status, apply_date, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?) ''', (flow_id, flow_type, education_type, employee_id,
    ...
    def create_incentive(self, incentive_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO talent_incentive ( incentive_id, incentive_type, education_type, title, description, criteria, budget, period, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (incentive_id, incentive_type, education_type, title,
    ...
    def calculate_incentive_budget(self, incentive_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT SUM(amount) as total FROM incentive_records WHERE incentive_id = ?',
                (incentive_id,))
                used = cursor.fetchone()['total'] or 0
                remaining = incentive['budget'] - used
                return {'success': True, 'total_budget': incentive['budget'], 'used_amount': used,
    ...
    def add_to_reserve(self, reserve_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO talent_reserve ( reserve_id, reserve_type, education_type, employee_id, employee_name, department, position, level, target_position, evaluation_result, development_plan, status, join_date, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?) ''', (reserve_id, reserve_type, education_type, employee_id,
    ...
    def create_service(self, service_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO talent_service ( service_id, service_type, education_type, title, provider, description, schedule, capacity, used_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 'active', ?, ?) ''', (service_id, service_type, education_type, title,
    ...
    def request_service(self, service_id: str, employee_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT capacity, used_count, status FROM talent_service WHERE service_id = ?',
                    (service_id,))
    ...
    def create_development_plan(self, development_path: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO talent_development ( development_id, development_path, education_type, employee_id, employee_name, department, current_position, target_position, plan, milestones, start_date, expected_end_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'in_progress', ?, ?) ''', (development_id, development_path, education_type,
    ...
    def get_development_progress(self, development_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT * FROM development_records WHERE development_id = ? ORDER BY target_date',
                (development_id,))
