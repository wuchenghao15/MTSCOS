    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS art_courses ( course_id TEXT PRIMARY KEY, course_name TEXT NOT NULL, art_category TEXT NOT NULL, sub_category TEXT, teacher_id INTEGER, teacher_name TEXT, education_type TEXT, grade_level INTEGER, semester TEXT, weekly_hours INTEGER DEFAULT 2, location TEXT, max_students INTEGER DEFAULT 30, enrolled_count INTEGER DEFAULT 0, description TEXT, is_grade_prep INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS art_enrollments ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, enroll_date TEXT, attendance_count INTEGER DEFAULT 0, final_score REAL, grade TEXT, UNIQUE(course_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS cultural_events ( event_id TEXT PRIMARY KEY, event_name TEXT NOT NULL, event_type TEXT, art_category TEXT, organizer TEXT, description TEXT, location TEXT, start_date TEXT, end_date TEXT, start_time TEXT, end_time TEXT, max_participants INTEGER DEFAULT 100, registered_count INTEGER DEFAULT 0, requires_tickets INTEGER DEFAULT 0, ticket_price REAL DEFAULT 0, tickets_sold INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', cover_image TEXT, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS event_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, event_id TEXT NOT NULL, student_id INTEGER, student_name TEXT, register_time TEXT, ticket_no TEXT, seat_number TEXT, status TEXT DEFAULT 'registered', attended INTEGER DEFAULT 0, UNIQUE(event_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS artworks ( artwork_id TEXT PRIMARY KEY, title TEXT NOT NULL, artwork_type TEXT, art_category TEXT, artist_id INTEGER, artist_name TEXT, education_type TEXT, creation_date TEXT, description TEXT, file_url TEXT, thumbnail_url TEXT, medium TEXT, dimensions TEXT, exhibition_id TEXT, is_displayed INTEGER DEFAULT 0, views INTEGER DEFAULT 0, average_rating REAL DEFAULT 0, rating_count INTEGER DEFAULT 0, awards TEXT, status TEXT DEFAULT 'submitted', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS artwork_ratings ( id INTEGER PRIMARY KEY AUTOINCREMENT, artwork_id TEXT NOT NULL, rater_id INTEGER, rater_name TEXT, rating INTEGER NOT NULL, comment TEXT, created_at TEXT, UNIQUE(artwork_id, rater_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exhibitions ( exhibition_id TEXT PRIMARY KEY, exhibition_name TEXT NOT NULL, theme TEXT, description TEXT, location TEXT, start_date TEXT, end_date TEXT, curator TEXT, artwork_count INTEGER DEFAULT 0, visitor_count INTEGER DEFAULT 0, status TEXT DEFAULT 'planned', cover_image TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS grade_exams ( exam_id TEXT PRIMARY KEY, grade_system TEXT, organization TEXT, level INTEGER, exam_date TEXT, registration_deadline TEXT, location TEXT, fee REAL, max_participants INTEGER DEFAULT 50, registered_count INTEGER DEFAULT 0, description TEXT, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS grade_results ( result_id TEXT PRIMARY KEY, exam_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, applied_level INTEGER, result TEXT, score REAL, certificate_no TEXT, certificate_url TEXT, exam_date TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS art_clubs ( club_id TEXT PRIMARY KEY, club_name TEXT NOT NULL, club_type TEXT, art_category TEXT, description TEXT, leader_id INTEGER, leader_name TEXT, advisor_id INTEGER, advisor_name TEXT, member_count INTEGER DEFAULT 1, education_type TEXT, is_active INTEGER DEFAULT 1, founded_date TEXT, logo_url TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, club_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, role TEXT DEFAULT 'member', joined_at TEXT, UNIQUE(club_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS heritage_programs ( program_id TEXT PRIMARY KEY, heritage_name TEXT NOT NULL, heritage_type TEXT, level TEXT, description TEXT, instructor TEXT, schedule TEXT, location TEXT, max_participants INTEGER DEFAULT 30, enrolled_count INTEGER DEFAULT 0, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                conn.commit()
    ...
    def create_art_course(self, course_name: str, art_category: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO art_courses ( course_id, course_name, art_category, sub_category, teacher_id, teacher_name, education_type, grade_level, semester, weekly_hours, location, max_students, enrolled_count, description, is_grade_prep, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'active', ?, ?) ''', (course_id, course_name, art_category,
    ...
    def enroll_art_course(self, course_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT max_students, enrolled_count, status FROM art_courses WHERE course_id = ?',
                    (course_id,))
    ...
    def create_event(self, event_name: str, event_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO cultural_events ( event_id, event_name, event_type, art_category, organizer, description, location, start_date, end_date, start_time, end_time, max_participants, registered_count, requires_tickets, ticket_price, tickets_sold, status, cover_image, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 0, 'scheduled', ?, ?, ?, ?) ''', (event_id, event_name, event_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO event_registrations (event_id, student_id, student_name, register_time, ticket_no, status) VALUES (?, ?, ?, ?, ?, 'registered') ''', (event_id, student_id, student_name, now, ticket_no))
                    if cursor.rowcount > 0:
                        cursor.execute('UPDATE cultural_events SET registered_count = registered_count + 1, updated_at = ? WHERE event_id = ?', (now, event_id))
                        if event[3]:
                            cursor.execute(
                            'UPDATE cultural_events SET tickets_sold = tickets_sold + 1 WHERE event_id = ?', (event_id,
                            ))
                        conn.commit()
    ...
    def submit_artwork(self, title: str, artwork_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO artworks ( artwork_id, title, artwork_type, art_category, artist_id, artist_name, education_type, creation_date, description, file_url, thumbnail_url, medium, dimensions, exhibition_id, is_displayed, views, average_rating, rating_count, awards, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 0, 0, 0, 0, ?, 'submitted', ?, ?) ''', (artwork_id, title, artwork_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT AVG(rating), COUNT(*) FROM artwork_ratings WHERE artwork_id = ?',
                    (artwork_id,))
    ...
    def create_exhibition(self, exhibition_name: str, start_date: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO exhibitions ( exhibition_name, theme, description, location, start_date, end_date, curator, artwork_count, visitor_count, status, cover_image, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, 'planned', ?, ?, ?) ''', (exhibition_name, kwargs.get('theme'),
                          kwargs.get('description'), kwargs.get('location'),
                          start_date, kwargs.get('end_date'),
                          kwargs.get('curator'), kwargs.get('cover_image'),
                          now, now))
                    cursor.execute(
                    'UPDATE exhibitions SET exhibition_id = ? WHERE exhibition_name = ? AND start_date = ?',
                    (exhibition_id, exhibition_name, start_date))
                    conn.commit()
    ...
    def create_grade_exam(self, grade_system: str, level: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO grade_exams ( exam_id, grade_system, organization, level, exam_date, registration_deadline, location, fee, max_participants, registered_count, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'open', ?, ?) ''', (exam_id, grade_system, kwargs.get('organization', config.get('organizations', [''])[0]),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if exam[0] and exam[1] >= exam[0]:
                        return {'success': False, 'error': '名额已满'}
                    cursor.execute('SELECT exam_id FROM grade_results WHERE exam_id = ? AND student_id = ?', (exam_id,
                    student_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已报名该考级'}
                    cursor.execute(''' INSERT INTO grade_results (result_id, exam_id, student_id, student_name, applied_level, exam_date, created_at) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (f"grs_{uuid.uuid4().hex[:12]}", exam_id, student_id,
    ...
    def record_grade_result(self, result_id: str, result: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE grade_results SET result = ?, score = ?, certificate_no = ?, certificate_url = ? WHERE result_id = ? ''', (result, kwargs.get('score'), certificate_no,
    ...
    def create_club(self, club_name: str, club_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO art_clubs ( club_id, club_name, club_type, art_category, description, leader_id, leader_name, advisor_id, advisor_name, member_count, education_type, is_active, founded_date, logo_url, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, 1, ?, ?, ?, ?) ''', (club_id, club_name, club_type,
    ...
    def create_heritage_program(self, heritage_name: str, heritage_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO heritage_programs ( program_id, heritage_name, heritage_type, level, description, instructor, schedule, location, max_participants, enrolled_count, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 1, ?, ?) ''', (program_id, heritage_name, heritage_type,
