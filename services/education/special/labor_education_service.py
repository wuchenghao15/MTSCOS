...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
""" MTSCOS 劳动教育服务 (v15.9.0) ==================================== 提供劳动课程、劳动实践、劳动技能、劳动评价、实践基地、勤工助学、 劳动档案等综合管理服务，支持成人教育和K12教育的差异化需求。  核心能力： 1. 劳动课程 - 日常生活劳动、生产劳动、服务性劳动课程 2. 劳动实践 - 实践活动、劳动周、劳动基地 3. 劳动技能 - 技能培训、技能等级、技能认证 4. 劳动评价 - 劳动态度、劳动技能、劳动成果评价 5. 实践基地 - 基地管理、合作单位、岗位安排 6. 勤工助学 - 岗位发布、申请、安排、考核 7. 劳动档案 - 劳动经历记录、劳动素养档案 8. K12劳动教育与成人职业劳动差异化 """
import os
import json
import uuid
import sqlite3
...
class LaborEducationService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_courses ( course_id TEXT PRIMARY KEY, course_name TEXT NOT NULL, course_type TEXT, labor_category TEXT, labor_type TEXT, education_type TEXT, grade_level INTEGER, teacher_id INTEGER, teacher_name TEXT, description TEXT, objectives TEXT, duration_hours INTEGER DEFAULT 32, theory_hours INTEGER DEFAULT 8, practice_hours INTEGER DEFAULT 24, max_students INTEGER DEFAULT 30, enrolled_count INTEGER DEFAULT 0, materials_needed TEXT, location TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_enrollments ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, enroll_date TEXT, attendance_count INTEGER DEFAULT 0, practice_hours REAL DEFAULT 0, theory_score REAL, practice_score REAL, final_grade TEXT, status TEXT DEFAULT 'enrolled', created_at TEXT, UNIQUE(course_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_activities ( activity_id TEXT PRIMARY KEY, activity_name TEXT NOT NULL, activity_type TEXT, labor_category TEXT, labor_type TEXT, description TEXT, organizer TEXT, start_date TEXT, end_date TEXT, location TEXT, max_participants INTEGER DEFAULT 50, registered_count INTEGER DEFAULT 0, required_hours INTEGER DEFAULT 8, status TEXT DEFAULT 'scheduled', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS activity_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, activity_id TEXT NOT NULL, student_id INTEGER, student_name TEXT, register_time TEXT, actual_hours REAL DEFAULT 0, attendance_status TEXT DEFAULT 'registered', performance TEXT, evaluation TEXT, created_at TEXT, UNIQUE(activity_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_skills ( skill_id TEXT PRIMARY KEY, skill_name TEXT NOT NULL, labor_type TEXT, description TEXT, required_hours REAL DEFAULT 20, difficulty TEXT DEFAULT 'medium', certification_available INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS skill_assessments ( assessment_id TEXT PRIMARY KEY, skill_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, assessor_id INTEGER, assessor_name TEXT, assessment_date TEXT, theory_score REAL, practice_score REAL, total_score REAL, current_level TEXT, new_level TEXT, passed INTEGER DEFAULT 0, notes TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS student_skill_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, skill_id TEXT NOT NULL, skill_name TEXT, level TEXT, acquired_date TEXT, total_hours REAL DEFAULT 0, assessment_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT, UNIQUE(student_id, skill_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS practice_bases ( base_id TEXT PRIMARY KEY, base_name TEXT NOT NULL, base_type TEXT, partner_organization TEXT, address TEXT, contact_person TEXT, contact_phone TEXT, capacity INTEGER DEFAULT 30, cooperation_type TEXT, agreement_start TEXT, agreement_end TEXT, available_positions INTEGER DEFAULT 0, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS base_positions ( position_id TEXT PRIMARY KEY, base_id TEXT NOT NULL, position_name TEXT NOT NULL, position_desc TEXT, required_skills TEXT, capacity INTEGER DEFAULT 10, filled_count INTEGER DEFAULT 0, duration_weeks INTEGER DEFAULT 4, requirements TEXT, status TEXT DEFAULT 'open', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS position_arrangements ( arrangement_id TEXT PRIMARY KEY, position_id TEXT NOT NULL, base_id TEXT, student_id INTEGER NOT NULL, student_name TEXT, start_date TEXT, end_date TEXT, supervisor TEXT, evaluation_score REAL, feedback TEXT, status TEXT DEFAULT 'arranged', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS work_study_positions ( position_id TEXT PRIMARY KEY, position_name TEXT NOT NULL, position_type TEXT, department TEXT, supervisor_id INTEGER, supervisor_name TEXT, weekly_hours INTEGER DEFAULT 8, hourly_wage REAL DEFAULT 15.0, total_positions INTEGER DEFAULT 1, filled_positions INTEGER DEFAULT 0, requirements TEXT, semester TEXT, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS work_study_applications ( application_id TEXT PRIMARY KEY, position_id TEXT NOT NULL, student_id INTEGER NOT NULL, student_name TEXT, apply_date TEXT, status TEXT DEFAULT 'pending', approved_by TEXT, approved_at TEXT, start_date TEXT, end_date TEXT, total_hours REAL DEFAULT 0, total_earned REAL DEFAULT 0, evaluation TEXT, created_at TEXT, updated_at TEXT, UNIQUE(position_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_portfolios ( portfolio_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, student_name TEXT, education_type TEXT, total_labor_hours REAL DEFAULT 0, total_courses INTEGER DEFAULT 0, total_activities INTEGER DEFAULT 0, total_skills INTEGER DEFAULT 0, skill_list TEXT, activity_history TEXT, work_study_hours REAL DEFAULT 0, evaluation_summary TEXT, attitude_score REAL, skill_score REAL, quality_score REAL, innovation_score REAL, overall_grade TEXT, created_at TEXT, updated_at TEXT, UNIQUE(student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_evaluations ( evaluation_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, student_name TEXT, evaluator_id INTEGER, evaluator_name TEXT, evaluation_type TEXT, target_id TEXT, dimension_scores TEXT, total_score REAL, grade TEXT, comment TEXT, evaluation_date TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS labor_awards ( award_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, student_name TEXT, award_name TEXT NOT NULL, award_type TEXT, award_level TEXT, award_date TEXT, description TEXT, certificate_url TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_course(self, course_name: str, course_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_courses ( course_id, course_name, course_type, labor_category, labor_type, education_type, grade_level, teacher_id, teacher_name, description, objectives, duration_hours, theory_hours, practice_hours, max_students, enrolled_count, materials_needed, location, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'active', ?, ?) ''', (course_id, course_name, course_type, labor_category,
    ...
    def enroll_course(self, course_id: str, student_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT max_students, enrolled_count, status FROM labor_courses WHERE course_id = ?',
                    (course_id,))
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO labor_enrollments (course_id, student_id, student_name, enroll_date, status, created_at) VALUES (?, ?, ?, ?, 'enrolled', ?) ''', (course_id, student_id, kwargs.get('student_name'), now[:10], now))
    ...
    def record_course_grade(self, enrollment_id: int, theory_score: float,
        ...
        try:
            ...
            grade = 'excellent' if final_score >= 90 else ('good' if final_score >= 80 else (
            'pass' if final_score >= 60 else 'fail'))
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE labor_enrollments SET theory_score = ?, practice_score = ?, final_grade = ?, attendance_count = ?, practice_hours = ?, status = 'completed' WHERE id = ? ''', (theory_score, practice_score, grade,
    ...
    def create_activity(self, activity_name: str, activity_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_activities ( activity_id, activity_name, activity_type, labor_category, labor_type, description, organizer, start_date, end_date, location, max_participants, registered_count, required_hours, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'scheduled', ?, ?) ''', (activity_id, activity_name, activity_type, labor_category,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO activity_registrations (activity_id, student_id, student_name, register_time, attendance_status, created_at) VALUES (?, ?, ?, ?, 'registered', ?) ''', (activity_id, student_id, kwargs.get('student_name'), now, now))
                    if cursor.rowcount > 0:
    ...
    def record_activity_hours(self, registration_id: int, actual_hours: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE activity_registrations SET actual_hours = ?, attendance_status = ?, performance = ?, evaluation = ? WHERE id = ? ''', (actual_hours, kwargs.get('attendance_status', 'attended'),
    ...
    def register_skill(self, skill_name: str, labor_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_skills ( skill_id, skill_name, labor_type, description, required_hours, difficulty, certification_available, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (skill_id, skill_name, labor_type, kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    # 获取学生当前技能等级
                    cursor.execute('SELECT level, total_hours FROM student_skill_records WHERE student_id = ? AND skill_id = ?', (student_id,
                    skill_id))
                    ...
                    cursor.execute(''' INSERT INTO skill_assessments ( assessment_id, skill_id, student_id, student_name, assessor_id, assessor_name, assessment_date, theory_score, practice_score, total_score, current_level, new_level, passed, notes, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (assessment_id, skill_id, student_id, kwargs.get('student_name'),
                    ...
                    # 更新学生技能档案
                    cursor.execute(''' INSERT OR REPLACE INTO student_skill_records ( student_id, skill_id, skill_name, level, acquired_date, total_hours, assessment_count, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (student_id, skill_id, skill_name, new_level,
                          now[:10], total_hours,
    ...
    def get_student_skills(self, student_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT ssr.*, ls.labor_type, ls.difficulty, ls.required_hours FROM student_skill_records ssr LEFT JOIN labor_skills ls ON ssr.skill_id = ls.skill_id WHERE ssr.student_id = ? ORDER BY ssr.updated_at DESC ''', (student_id,))
    ...
    def register_practice_base(self, base_name: str, base_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO practice_bases ( base_id, base_name, base_type, partner_organization, address, contact_person, contact_phone, capacity, cooperation_type, agreement_start, agreement_end, available_positions, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'active', ?, ?) ''', (base_id, base_name, base_type,
    ...
    def add_base_position(self, base_id: str, position_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO base_positions ( position_id, base_id, position_name, position_desc, required_skills, capacity, filled_count, duration_weeks, requirements, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, 'open', ?) ''', (position_id, base_id, position_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    base_id = position[0]
                    cursor.execute(''' INSERT INTO position_arrangements ( arrangement_id, position_id, base_id, student_id, student_name, start_date, end_date, supervisor, evaluation_score, feedback, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, 'arranged', ?, ?) ''', (arrangement_id, position_id, base_id, student_id,
                          kwargs.get('student_name'), kwargs.get('start_date'),
                          kwargs.get('end_date'), kwargs.get('supervisor'), now, now))
                    cursor.execute('UPDATE base_positions SET filled_count = filled_count + 1 WHERE position_id = ?',
                    (position_id,))
                    if position[2] + 1 >= position[1]:
                        cursor.execute('UPDATE base_positions SET status = ? WHERE position_id = ?', ('full',
                        position_id))
                    conn.commit()
    ...
    def create_work_study_position(self, position_name: str, position_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO work_study_positions ( position_id, position_name, position_type, department, supervisor_id, supervisor_name, weekly_hours, hourly_wage, total_positions, filled_positions, requirements, semester, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'open', ?, ?) ''', (position_id, position_name, position_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO work_study_applications ( application_id, position_id, student_id, student_name, apply_date, status, start_date, end_date, total_hours, total_earned, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, 0, 0, ?, ?) ''', (application_id, position_id, student_id,
    ...
    def approve_work_study(self, application_id: str, approved: bool,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT position_id, status FROM work_study_applications WHERE application_id = ?',
                    (application_id,))
                    app = cursor.fetchone()
                    if not app:
                    ...
                    cursor.execute(''' UPDATE work_study_applications SET status = ?, approved_by = ?, approved_at = ?, updated_at = ? WHERE application_id = ? ''', (status, kwargs.get('approved_by'), now, now, application_id))
                    if approved:
    ...
    def record_work_study_hours(self, application_id: str, hours: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT position_id, total_hours, total_earned FROM work_study_applications WHERE application_id = ? AND status = ?', (application_id,
                    'approved'))
                    app = cursor.fetchone()
                    if not app:
                    ...
                    cursor.execute(''' UPDATE work_study_applications SET total_hours = ?, total_earned = ?, evaluation = ?, updated_at = ? WHERE application_id = ? ''', (new_total_hours, new_total_earned, kwargs.get('evaluation'), now, application_id))
                    conn.commit()
    ...
    def create_evaluation(self, student_id: int, evaluation_type: str,
        ...
        try:
            ...
            grade = 'excellent' if total_score >= 90 else ('good' if total_score >= 80 else (
            'pass' if total_score >= 60 else 'fail'))
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_evaluations ( evaluation_id, student_id, student_name, evaluator_id, evaluator_name, evaluation_type, target_id, dimension_scores, total_score, grade, comment, evaluation_date, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (evaluation_id, student_id, kwargs.get('student_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor = conn.cursor()
                    # 汇总课程数据
                    cursor.execute(''' SELECT COUNT(*) as cnt, COALESCE(SUM(practice_hours), 0) as hours FROM labor_enrollments WHERE student_id = ? ''', (student_id,))
                    course_data = cursor.fetchone()
                    total_courses = course_data['cnt']
                    course_hours = course_data['hours']
                    # 汇总活动数据
                    cursor.execute(''' SELECT COUNT(*) as cnt, COALESCE(SUM(actual_hours), 0) as hours FROM activity_registrations WHERE student_id = ? ''', (student_id,))
                    activity_data = cursor.fetchone()
                    total_activities = activity_data['cnt']
                    activity_hours = activity_data['hours']
                    # 汇总技能数据
                    cursor.execute('SELECT skill_name, level FROM student_skill_records WHERE student_id = ?',
                    (student_id,))
                    skills = [dict(s) for s in cursor.fetchall()]
                    total_skills = len(skills)
                    skill_list = json.dumps(skills, ensure_ascii=False)
                    # 汇总勤工助学数据
                    cursor.execute(''' SELECT COALESCE(SUM(total_hours), 0) as hours FROM work_study_applications WHERE student_id = ? AND status = 'approved' ''', (student_id,))
                    ...
                    cursor.execute(''' INSERT OR REPLACE INTO labor_portfolios ( portfolio_id, student_id, student_name, education_type, total_labor_hours, total_courses, total_activities, total_skills, skill_list, activity_history, work_study_hours, evaluation_summary, attitude_score, skill_score, quality_score, innovation_score, overall_grade, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (portfolio_id, student_id, student_name,
    ...
    def record_award(self, student_id: int, award_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO labor_awards ( award_id, student_id, student_name, award_name, award_type, award_level, award_date, description, certificate_url, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (award_id, student_id, kwargs.get('student_name'), award_name,
