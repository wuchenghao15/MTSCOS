...
SMART_CAMPUS_MODULES = {
    'access_control': {'name': '智能门禁', 'description': '人脸/刷卡门禁系统', 'k12_features': ['家长接送', '出入记录'],
    'adult_features': ['访客管理', '时段控制']},
    'attendance': {'name': '智能考勤', 'description': '自动考勤统计', 'k12_features': ['家校通知', '请假管理'], 'adult_features': ['弹性考勤',
    '工时统计']},
    'surveillance': {'name': '智能监控', 'description': '高清视频监控', 'k12_features': ['区域告警', '行为分析'],
    'adult_features': ['隐私保护', 'AI识别']},
    'broadcast': {'name': '智能广播', 'description': '校园广播系统', 'k12_features': ['上下课铃', '安全播报'], 'adult_features': ['会议广播',
    '紧急通知']},
    'lighting': {'name': '智能照明', 'description': '节能照明控制', 'k12_features': ['教室调光', '定时开关'], 'adult_features': ['场景模式',
    '远程控制']},
    'aircon': {'name': '智能空调', 'description': '恒温控制管理', 'k12_features': ['定时启停', '温度限制'], 'adult_features': ['个性化设置',
    '能耗统计']},
    'water': {'name': '智能饮水', 'description': '直饮水管理', 'k12_features': ['水质监测', '用量统计'], 'adult_features': ['健康饮水',
    '智能提醒']},
    'elevator': {'name': '智能电梯', 'description': '电梯智能调度', 'k12_features': ['楼层限制', '安全监控'], 'adult_features': ['预约服务',
    '效率优化']}
}
...
class EducationSmartCampusService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_campus ( campus_id TEXT PRIMARY KEY, campus_name TEXT NOT NULL, education_type TEXT NOT NULL, address TEXT, total_area REAL, building_count INTEGER DEFAULT 0, device_count INTEGER DEFAULT 0, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS campus_config ( id INTEGER PRIMARY KEY AUTOINCREMENT, campus_id TEXT NOT NULL, module_key TEXT NOT NULL, module_name TEXT, enabled INTEGER DEFAULT 1, config_json TEXT, education_type TEXT, created_at TEXT, updated_at TEXT, UNIQUE(campus_id, module_key) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_devices ( device_id TEXT PRIMARY KEY, device_name TEXT NOT NULL, device_type TEXT NOT NULL, sub_type TEXT, campus_id TEXT, location TEXT, ip_address TEXT, mac_address TEXT, status TEXT DEFAULT 'online', last_heartbeat TEXT, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, device_id TEXT NOT NULL, record_type TEXT, value REAL, unit TEXT, record_time TEXT, education_type TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS iot_applications ( app_id TEXT PRIMARY KEY, app_name TEXT NOT NULL, app_type TEXT, campus_id TEXT, description TEXT, status TEXT DEFAULT 'active', config_json TEXT, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS iot_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, app_id TEXT NOT NULL, record_type TEXT, data_json TEXT, record_time TEXT, education_type TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_security ( security_id TEXT PRIMARY KEY, system_type TEXT NOT NULL, system_name TEXT, campus_id TEXT, location TEXT, status TEXT DEFAULT 'normal', config_json TEXT, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, security_id TEXT NOT NULL, event_type TEXT, event_level TEXT, event_desc TEXT, event_time TEXT, handled INTEGER DEFAULT 0, education_type TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_energy ( energy_id TEXT PRIMARY KEY, meter_type TEXT NOT NULL, meter_name TEXT, campus_id TEXT, location TEXT, last_reading REAL, unit TEXT, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS energy_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, energy_id TEXT NOT NULL, reading REAL, consumption REAL, unit TEXT, record_date TEXT, education_type TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_library ( library_id TEXT PRIMARY KEY, service_type TEXT NOT NULL, service_name TEXT, campus_id TEXT, description TEXT, status TEXT DEFAULT 'active', config_json TEXT, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS library_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, library_id TEXT NOT NULL, record_type TEXT, data_json TEXT, record_time TEXT, education_type TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_classroom ( classroom_id TEXT PRIMARY KEY, classroom_name TEXT NOT NULL, campus_id TEXT, floor TEXT, capacity INTEGER DEFAULT 0, tech_config TEXT, status TEXT DEFAULT 'available', education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS classroom_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, classroom_id TEXT NOT NULL, record_type TEXT, data_json TEXT, record_time TEXT, education_type TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_logistics ( logistics_id TEXT PRIMARY KEY, service_type TEXT NOT NULL, service_name TEXT, campus_id TEXT, description TEXT, status TEXT DEFAULT 'active', config_json TEXT, education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS logistics_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, logistics_id TEXT NOT NULL, record_type TEXT, data_json TEXT, record_time TEXT, education_type TEXT ) ''')
                conn.commit()
    ...
    def create_campus(self, campus_name: str, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO smart_campus ( campus_id, campus_name, education_type, address, total_area, building_count, device_count, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 0, ?, 'active', ?, ?) ''', (campus_id, campus_name, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    features = module_info.get('k12_features',
                    []) if education_type == 'k12' else module_info.get('adult_features', [])
                    config = {'features': features, 'settings': kwargs.get('settings', {})}
                    cursor.execute(''' INSERT OR REPLACE INTO campus_config ( campus_id, module_key, module_name, enabled, config_json, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (campus_id, module_key, module_info.get('name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not campus:
                        return {'success': False, 'error': '校园不存在'}
                    cursor.execute(''' INSERT INTO smart_devices ( device_id, device_name, device_type, sub_type, campus_id, location, ip_address, mac_address, status, last_heartbeat, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'online', ?, ?, ?, ?) ''', (device_id, device_name, device_type, kwargs.get('sub_type'),
                          campus_id, kwargs.get('location'), kwargs.get('ip_address'),
                          kwargs.get('mac_address'), now, campus[0], now, now))
                    cursor.execute('UPDATE smart_campus SET device_count = device_count + 1 WHERE campus_id = ?',
                    (campus_id,))
                    conn.commit()
    ...
    def update_device_status(self, device_id: str, status: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE smart_devices SET status = ?, last_heartbeat = ?, updated_at = ? WHERE device_id = ? ''', (status, now, now, device_id))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not device:
                        return {'success': False, 'error': '设备不存在'}
                    cursor.execute(''' INSERT INTO device_records (device_id, record_type, value, unit, record_time, education_type) VALUES (?, ?, ?, ?, ?, ?) ''', (device_id, record_type, value, kwargs.get('unit', ''), now, device[0]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_device_records(self, device_id: str, record_type: str = None, start_time: str = None,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not campus:
                        return {'success': False, 'error': '校园不存在'}
                    cursor.execute(''' INSERT INTO iot_applications ( app_id, app_name, app_type, campus_id, description, status, config_json, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'active', ?, ?, ?, ?) ''', (app_id, app_name, app_type, campus_id,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO iot_records (app_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (app_id, 'analysis', json.dumps(analysis_result, ensure_ascii=False), now, app[0]))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO iot_records (app_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (app_id, 'warning', json.dumps(warning_data, ensure_ascii=False), now, app[0]))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not app:
                        return {'success': False, 'error': '应用不存在'}
                    linkage_data = {'device_ids': device_ids, 'linkage_type': kwargs.get('linkage_type', 'auto'),
                    'created_at': now}
                    cursor.execute(''' INSERT INTO iot_records (app_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (app_id, 'linkage', json.dumps(linkage_data, ensure_ascii=False), now, app[0]))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not campus:
                        return {'success': False, 'error': '校园不存在'}
                    cursor.execute(''' INSERT INTO smart_security ( security_id, system_type, system_name, campus_id, location, status, config_json, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'normal', ?, ?, ?, ?) ''', (security_id, system_type, system_info.get('name'),
                          campus_id, kwargs.get('location'),
                          json.dumps({'features': system_info.get('features', []), 'settings': kwargs.get('settings',
                          {})}, ensure_ascii=False),
                          campus[0], now, now))
                    conn.commit()
    ...
    def record_security_event(self, security_id: str, event_type: str, event_level: str, event_desc: str) -> Dict[str,
    Any]:
        try:
            now = datetime.now().isoformat()
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO security_records (security_id, event_type, event_level, event_desc, event_time, handled, education_type) VALUES (?, ?, ?, ?, ?, 0, ?) ''', (security_id, event_type, event_level, event_desc, now, security[0]))
                    cursor.execute('UPDATE smart_security SET status = ? WHERE security_id = ?',
                    ('warning' if event_level == 'high' else 'normal', security_id))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not campus:
                        return {'success': False, 'error': '校园不存在'}
                    cursor.execute(''' INSERT INTO security_records (security_id, event_type, event_level, event_desc, event_time, handled, education_type) VALUES (?, ?, ?, ?, ?, 1, ?) ''', ('visitor', 'visitor_register', 'info',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not campus:
                        return {'success': False, 'error': '校园不存在'}
                    cursor.execute(''' INSERT INTO smart_energy ( energy_id, meter_type, meter_name, campus_id, location, last_reading, unit, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?, ?) ''', (energy_id, meter_type, meter_info.get('name'),
                          campus_id, kwargs.get('location'),
    ...
    def record_energy_reading(self, energy_id: str, reading: float) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT last_reading, unit, education_type FROM smart_energy WHERE energy_id = ?',
                    (energy_id,))
                    meter = cursor.fetchone()
                    if not meter:
                        return {'success': False, 'error': '仪表不存在'}
                    consumption = reading - meter[0] if meter[0] > 0 else 0
                    cursor.execute('UPDATE smart_energy SET last_reading = ?, updated_at = ? WHERE energy_id = ?',
                    (reading, now, energy_id))
                    cursor.execute(''' INSERT INTO energy_records (energy_id, reading, consumption, unit, record_date, education_type) VALUES (?, ?, ?, ?, ?, ?) ''', (energy_id, reading, consumption, meter[1], today, meter[2]))
                    conn.commit()
    ...
    def get_energy_statistics(self, campus_id: str, period: str = 'month') -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT er.unit, SUM(er.consumption) as total_consumption FROM energy_records er JOIN smart_energy se ON er.energy_id = se.energy_id WHERE se.campus_id = ? AND er.record_date >= ? GROUP BY er.unit ''', (campus_id, start_date))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not campus:
                        return {'success': False, 'error': '校园不存在'}
                    cursor.execute(''' INSERT INTO smart_library ( library_id, service_type, service_name, campus_id, description, status, config_json, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'active', ?, ?, ?, ?) ''', (library_id, service_type, service_info.get('name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    borrow_data = {'book_id': book_id, 'user_id': user_id, 'user_name': user_name, 'borrow_time': now,
                    'status': 'borrowed'}
                    cursor.execute(''' INSERT INTO library_records (library_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (library_id, 'borrow', json.dumps(borrow_data, ensure_ascii=False), now, library[0]))
                    conn.commit()
    ...
    def reserve_seat(self, library_id: str, seat_id: str, user_id: int, user_name: str, duration: int = 4) -> Dict[str,
    Any]:
        try:
            now = datetime.now().isoformat()
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not library:
                        return {'success': False, 'error': '图书馆服务不存在'}
                    cursor.execute(
                    'SELECT * FROM library_records WHERE library_id = ? AND record_type = ? AND data_json LIKE ? AND  record_time >= ?',
                                   (library_id, 'reservation', f'%{seat_id}%',
                                   (datetime.now() - timedelta(hours=duration)).isoformat()))
                    if cursor.fetchone():
                        return {'success': False, 'error': '座位已被预约'}
                    reserve_data = {'seat_id': seat_id, 'user_id': user_id, 'user_name': user_name, 'start_time': now,
                    'end_time': end_time, 'status': 'reserved'}
                    cursor.execute(''' INSERT INTO library_records (library_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (library_id, 'reservation', json.dumps(reserve_data, ensure_ascii=False), now, library[0]))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO library_records (library_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (library_id, 'search', json.dumps(search_data, ensure_ascii=False), now, library[0]))
                    conn.commit()
                    mock_results = [{'book_id': f"bk_{uuid.uuid4().hex[:8]}", 'title': f"《{keyword}相关书籍》",
                    'author': '佚名', 'location': 'A区1排'}]
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO smart_classroom ( classroom_id, classroom_name, campus_id, floor, capacity, tech_config, status, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'available', ?, ?, ?) ''', (classroom_id, classroom_name, campus_id,
    ...
    def reserve_classroom(self, classroom_id: str, user_id: int, user_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT education_type, status FROM smart_classroom WHERE classroom_id = ?',
                    (classroom_id,))
                    ...
                    cursor.execute('UPDATE smart_classroom SET status = ? WHERE classroom_id = ?', ('reserved',
                    classroom_id))
                    reserve_data = {'user_id': user_id, 'user_name': user_name, 'start_time': kwargs.get('start_time'),
                    'end_time': kwargs.get('end_time'), 'purpose': kwargs.get('purpose')}
                    cursor.execute(''' INSERT INTO classroom_records (classroom_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (classroom_id, 'reservation', json.dumps(reserve_data, ensure_ascii=False), now, classroom[0]))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO classroom_records (classroom_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (classroom_id, 'environment', json.dumps(env_data, ensure_ascii=False), now, classroom[0]))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not classroom:
                        return {'success': False, 'error': '教室不存在'}
                    recording_data = {'course_name': course_name, 'teacher_name': teacher_name, 'start_time': now,
                    'status': 'recording'}
                    cursor.execute(''' INSERT INTO classroom_records (classroom_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (classroom_id, 'recording', json.dumps(recording_data, ensure_ascii=False), now, classroom[0]))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not campus:
                        return {'success': False, 'error': '校园不存在'}
                    cursor.execute(''' INSERT INTO smart_logistics ( logistics_id, service_type, service_name, campus_id, description, status, config_json, education_type, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'active', ?, ?, ?, ?) ''', (logistics_id, service_type, service_info.get('name'),
                          campus_id, service_info.get('description'),
                          json.dumps(kwargs.get('config', {}), ensure_ascii=False),
                          campus[0], now, now))
                    conn.commit()
    ...
    def submit_maintenance_request(self, logistics_id: str, user_id: int, user_name: str, issue_desc: str,
    **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    request_data = {'request_id': request_id, 'user_id': user_id, 'user_name': user_name,
                    'issue_desc': issue_desc, 'location': kwargs.get('location'), 'priority': kwargs.get('priority',
                    'normal'), 'status': 'pending', 'created_at': now}
                    cursor.execute(''' INSERT INTO logistics_records (logistics_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (logistics_id, 'maintenance_request', json.dumps(request_data, ensure_ascii=False), now,
                    logistics[0]))
                    conn.commit()
    ...
    def order_canteen(self, logistics_id: str, user_id: int, user_name: str, items: List[Dict], **kwargs) -> Dict[str,
    Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    order_data = {'order_id': order_id, 'user_id': user_id, 'user_name': user_name, 'items': items,
                    'total_amount': total_amount, 'status': 'ordered', 'order_time': now}
                    cursor.execute(''' INSERT INTO logistics_records (logistics_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (logistics_id, 'canteen_order', json.dumps(order_data, ensure_ascii=False), now, logistics[0]))
                    conn.commit()
    ...
    def manage_dormitory(self, logistics_id: str, student_id: int, student_name: str, room_no: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not logistics:
                        return {'success': False, 'error': '后勤服务不存在'}
                    dorm_data = {'student_id': student_id, 'student_name': student_name, 'room_no': room_no,
                    'action': action, 'time': now}
                    cursor.execute(''' INSERT INTO logistics_records (logistics_id, record_type, data_json, record_time, education_type) VALUES (?, ?, ?, ?, ?) ''', (logistics_id, 'dormitory', json.dumps(dorm_data, ensure_ascii=False), now, logistics[0]))
                    conn.commit()
    ...
    def get_campus_overview(self, campus_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT SUM(consumption) as total FROM energy_records WHERE energy_id IN ( SELECT energy_id FROM smart_energy WHERE campus_id = ?) AND record_date >= ?', (campus_id,
