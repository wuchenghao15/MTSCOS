    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS monitoring_keywords ( keyword_id TEXT PRIMARY KEY, keyword TEXT NOT NULL, education_type TEXT NOT NULL, topic_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS monitoring_channels ( channel_id TEXT PRIMARY KEY, channel_code TEXT NOT NULL, channel_name TEXT NOT NULL, education_type TEXT NOT NULL, source_type TEXT, is_monitored INTEGER DEFAULT 1, last_sync_time TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS public_opinion ( opinion_id TEXT PRIMARY KEY, title TEXT NOT NULL, content TEXT, source_url TEXT, source_name TEXT, channel_code TEXT, education_type TEXT NOT NULL, topic_type TEXT, publish_time TEXT, sentiment_level TEXT DEFAULT 'neutral', sentiment_score REAL DEFAULT 0.5, heat_score INTEGER DEFAULT 0, share_count INTEGER DEFAULT 0, comment_count INTEGER DEFAULT 0, view_count INTEGER DEFAULT 0, is_crisis INTEGER DEFAULT 0, status TEXT DEFAULT 'pending', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS opinion_analysis ( analysis_id TEXT PRIMARY KEY, opinion_id TEXT NOT NULL, education_type TEXT NOT NULL, topic_type TEXT, key_points TEXT, influence_rating INTEGER DEFAULT 0, spread_path TEXT, related_topics TEXT, analysis_report TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS sentiment_analysis ( id INTEGER PRIMARY KEY AUTOINCREMENT, opinion_id TEXT NOT NULL, education_type TEXT NOT NULL, sentiment_level TEXT, sentiment_score REAL, positive_prob REAL, negative_prob REAL, neutral_prob REAL, keywords TEXT, analysis_time TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS crisis_events ( crisis_id TEXT PRIMARY KEY, crisis_name TEXT NOT NULL, education_type TEXT NOT NULL, crisis_level TEXT DEFAULT 'general', description TEXT, trigger_time TEXT, status TEXT DEFAULT 'active', impact_score INTEGER DEFAULT 0, risk_level TEXT DEFAULT 'medium', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS crisis_responses ( response_id TEXT PRIMARY KEY, crisis_id TEXT NOT NULL, education_type TEXT NOT NULL, response_type TEXT, response_content TEXT, response_time TEXT, responsible_person TEXT, status TEXT DEFAULT 'draft', effectiveness_rating INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS media_contacts ( media_id TEXT PRIMARY KEY, media_name TEXT NOT NULL, media_role TEXT, education_type TEXT NOT NULL, contact_name TEXT, contact_phone TEXT, contact_email TEXT, contact_wechat TEXT, coverage_scope TEXT, influence_level TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS media_interactions ( interaction_id TEXT PRIMARY KEY, media_id TEXT NOT NULL, education_type TEXT NOT NULL, interaction_type TEXT, content TEXT, interaction_time TEXT, outcome TEXT, notes TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS guidance_plans ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, education_type TEXT NOT NULL, strategy_type TEXT, target_topic TEXT, target_audience TEXT, content_plan TEXT, channel_plan TEXT, timeline TEXT, status TEXT DEFAULT 'draft', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS guidance_executions ( execution_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, education_type TEXT NOT NULL, execution_content TEXT, execution_channel TEXT, execution_time TEXT, reach_count INTEGER DEFAULT 0, engagement_count INTEGER DEFAULT 0, feedback TEXT, status TEXT DEFAULT 'pending', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS opinion_reports ( report_id TEXT PRIMARY KEY, report_name TEXT NOT NULL, education_type TEXT NOT NULL, report_type TEXT, report_period TEXT, content TEXT, summary TEXT, recommendations TEXT, generated_at TEXT, status TEXT DEFAULT 'generated', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS report_schedules ( schedule_id TEXT PRIMARY KEY, education_type TEXT NOT NULL, report_type TEXT, schedule_type TEXT, schedule_time TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS brand_reputation ( reputation_id TEXT PRIMARY KEY, brand_name TEXT NOT NULL, education_type TEXT NOT NULL, overall_score REAL DEFAULT 0, positive_rate REAL DEFAULT 0, neutral_rate REAL DEFAULT 0, negative_rate REAL DEFAULT 0, ranking INTEGER, assessment_date TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS reputation_trends ( id INTEGER PRIMARY KEY AUTOINCREMENT, reputation_id TEXT NOT NULL, education_type TEXT NOT NULL, date TEXT NOT NULL, score REAL, positive_count INTEGER DEFAULT 0, negative_count INTEGER DEFAULT 0, neutral_count INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS complaint_records ( complaint_id TEXT PRIMARY KEY, education_type TEXT NOT NULL, complaint_type TEXT, complaint_content TEXT, contact_info TEXT, complaint_time TEXT, source_channel TEXT, severity_level TEXT DEFAULT 'low', status TEXT DEFAULT 'pending', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS complaint_handlings ( handling_id TEXT PRIMARY KEY, complaint_id TEXT NOT NULL, education_type TEXT NOT NULL, handler_name TEXT, handling_content TEXT, handling_time TEXT, resolution_status TEXT DEFAULT 'processing', satisfaction_rating INTEGER, feedback TEXT, created_at TEXT, updated_at TEXT ) ''')
                conn.commit()
    ...
    def add_monitoring_keyword(self, keyword: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO monitoring_keywords ( keyword_id, keyword, education_type, topic_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, 1, ?, ?) ''', (keyword_id, keyword, education_type,
    ...
    def configure_monitoring_channel(self, channel_code: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO monitoring_channels ( channel_id, channel_code, channel_name, education_type, source_type, is_monitored, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 1, ?, ?) ''', (channel_id, channel_code, config.get('name', channel_code),
    ...
    def collect_public_opinion(self, title: str, content: str, source_url: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO public_opinion ( opinion_id, title, content, source_url, source_name, channel_code, education_type, topic_type, publish_time, sentiment_level, sentiment_score, heat_score, share_count, comment_count, view_count, is_crisis, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (opinion_id, title, content, source_url,
    ...
    def analyze_sentiment(self, opinion_id: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO sentiment_analysis ( opinion_id, education_type, sentiment_level, sentiment_score, positive_prob, negative_prob, neutral_prob, keywords, analysis_time ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (opinion_id, education_type, sentiment_level, sentiment_score,
    ...
    def identify_crisis(self, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO crisis_events ( crisis_id, crisis_name, education_type, crisis_level, description, trigger_time, status, impact_score, risk_level, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'active', ?, 'medium', ?, ?) ''', (crisis_id, kwargs.get('crisis_name', '未命名危机'),
        ...
        try:
            with self._get_connection() as conn:
                ...
                assessment = {
                    ...
                    'recommended_response_time': CRISIS_LEVELS.get(crisis['crisis_level'], {}).get('response_time',
                    '24小时')
                }
    ...
    def respond_to_crisis(self, crisis_id: str, education_type: str, response_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO crisis_responses ( response_id, crisis_id, education_type, response_type, response_content, response_time, responsible_person, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?) ''', (response_id, crisis_id, education_type, response_type,
    ...
    def add_media_contact(self, media_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO media_contacts ( media_id, media_name, media_role, education_type, contact_name, contact_phone, contact_email, contact_wechat, coverage_scope, influence_level, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (media_id, media_name, kwargs.get('media_role'), education_type,
    ...
    def record_media_interaction(self, media_id: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO media_interactions ( interaction_id, media_id, education_type, interaction_type, content, interaction_time, outcome, notes, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (interaction_id, media_id, education_type, interaction_type,
    ...
    def create_guidance_plan(self, plan_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO guidance_plans ( plan_id, plan_name, education_type, strategy_type, target_topic, target_audience, content_plan, channel_plan, timeline, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?) ''', (plan_id, plan_name, education_type, strategy_type,
    ...
    def execute_guidance(self, plan_id: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO guidance_executions ( execution_id, plan_id, education_type, execution_content, execution_channel, execution_time, reach_count, engagement_count, feedback, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (execution_id, plan_id, education_type, execution_content,
    ...
    def generate_report(self, report_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO opinion_reports ( report_id, report_name, education_type, report_type, report_period, content, summary, recommendations, generated_at, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'generated', ?) ''', (report_id, report_name, education_type, report_type,
    ...
    def schedule_report(self, education_type: str, report_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO report_schedules ( schedule_id, education_type, report_type, schedule_type, schedule_time, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 1, ?, ?) ''', (schedule_id, education_type, report_type, schedule_type,
    ...
    def monitor_brand(self, brand_name: str, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO brand_reputation ( reputation_id, brand_name, education_type, overall_score, positive_rate, neutral_rate, negative_rate, ranking, assessment_date, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (reputation_id, brand_name, education_type,
    ...
    def assess_reputation(self, reputation_id: str, education_type: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(
                'SELECT * FROM reputation_trends WHERE reputation_id = ? AND education_type = ? ORDER BY date DESC  LIMIT 30',
                             (reputation_id, education_type))
    ...
    def register_complaint(self, education_type: str, complaint_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO complaint_records ( complaint_id, education_type, complaint_type, complaint_content, contact_info, complaint_time, source_channel, severity_level, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'low', 'pending', ?) ''', (complaint_id, education_type, complaint_type,
    ...
    def assign_complaint(self, complaint_id: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO complaint_handlings ( handling_id, complaint_id, education_type, handler_name, handling_content, handling_time, resolution_status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'processing', ?, ?) ''', (handling_id, complaint_id, education_type, handler_name,
                          '', now, now, now))
                    cursor.execute('UPDATE complaint_records SET status = ? WHERE complaint_id = ?', ('processing',
                    complaint_id))
                    conn.commit()
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute('SELECT complaint_id FROM complaint_handlings WHERE handling_id = ?',
                        (handling_id,))
                        complaint_id = cursor.fetchone()[0]
                        cursor.execute('UPDATE complaint_records SET status = ? WHERE complaint_id = ?', ('resolved',
                        complaint_id))
                        conn.commit()
                        return {'success': True}
    ...
    def get_statistics(self, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT COUNT(*) FROM public_opinion WHERE education_type = ? AND sentiment_level = ?',
                (education_type, 'positive'))
                positive_count = cursor.fetchone()[0]
                cursor.execute('SELECT COUNT(*) FROM public_opinion WHERE education_type = ? AND sentiment_level = ?',
                (education_type, 'negative'))
                negative_count = cursor.fetchone()[0]
                cursor.execute('SELECT COUNT(*) FROM crisis_events WHERE education_type = ? AND status = ?',
                (education_type, 'active'))
                active_crises = cursor.fetchone()[0]
                cursor.execute('SELECT COUNT(*) FROM complaint_records WHERE education_type = ? AND status = ?',
                (education_type, 'pending'))
