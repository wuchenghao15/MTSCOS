    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_graph ( graph_id TEXT PRIMARY KEY, graph_name TEXT NOT NULL, description TEXT, education_type TEXT NOT NULL, subject TEXT, version TEXT DEFAULT '1.0', node_count INTEGER DEFAULT 0, relation_count INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS graph_config ( config_id TEXT PRIMARY KEY, graph_id TEXT NOT NULL, config_key TEXT NOT NULL, config_value TEXT, description TEXT, created_at TEXT, FOREIGN KEY (graph_id) REFERENCES knowledge_graph(graph_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_nodes ( node_id TEXT PRIMARY KEY, graph_id TEXT NOT NULL, node_type TEXT NOT NULL, knowledge_type TEXT, node_name TEXT NOT NULL, node_label TEXT, description TEXT, content TEXT, metadata TEXT, education_type TEXT NOT NULL, grade_level INTEGER, subject TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT, FOREIGN KEY (graph_id) REFERENCES knowledge_graph(graph_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS node_properties ( property_id INTEGER PRIMARY KEY AUTOINCREMENT, node_id TEXT NOT NULL, property_key TEXT NOT NULL, property_value TEXT, property_type TEXT DEFAULT 'string', created_at TEXT, FOREIGN KEY (node_id) REFERENCES knowledge_nodes(node_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_relations ( relation_id TEXT PRIMARY KEY, graph_id TEXT NOT NULL, source_node_id TEXT NOT NULL, target_node_id TEXT NOT NULL, relation_type TEXT NOT NULL, relation_name TEXT, weight REAL DEFAULT 1.0, description TEXT, education_type TEXT NOT NULL, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT, FOREIGN KEY (graph_id) REFERENCES knowledge_graph(graph_id), FOREIGN KEY (source_node_id) REFERENCES knowledge_nodes(node_id), FOREIGN KEY (target_node_id) REFERENCES knowledge_nodes(node_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS relation_properties ( property_id INTEGER PRIMARY KEY AUTOINCREMENT, relation_id TEXT NOT NULL, property_key TEXT NOT NULL, property_value TEXT, property_type TEXT DEFAULT 'string', created_at TEXT, FOREIGN KEY (relation_id) REFERENCES knowledge_relations(relation_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_query ( query_id TEXT PRIMARY KEY, query_name TEXT NOT NULL, query_type TEXT NOT NULL, query_template TEXT, description TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS query_records ( record_id INTEGER PRIMARY KEY AUTOINCREMENT, query_id TEXT, query_text TEXT NOT NULL, query_type TEXT, education_type TEXT, result_count INTEGER DEFAULT 0, execution_time REAL DEFAULT 0, status TEXT DEFAULT 'success', created_at TEXT, FOREIGN KEY (query_id) REFERENCES knowledge_query(query_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_reasoning ( reasoning_id TEXT PRIMARY KEY, reasoning_name TEXT NOT NULL, reasoning_type TEXT NOT NULL, reasoning_rules TEXT, description TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS reasoning_records ( record_id INTEGER PRIMARY KEY AUTOINCREMENT, reasoning_id TEXT, reasoning_type TEXT NOT NULL, input_data TEXT, output_data TEXT, confidence REAL DEFAULT 0, education_type TEXT, status TEXT DEFAULT 'success', created_at TEXT, FOREIGN KEY (reasoning_id) REFERENCES knowledge_reasoning(reasoning_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_recommendation ( rec_id TEXT PRIMARY KEY, rec_name TEXT NOT NULL, rec_type TEXT NOT NULL, rec_algorithm TEXT, description TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS recommendation_records ( record_id INTEGER PRIMARY KEY AUTOINCREMENT, rec_id TEXT, rec_type TEXT NOT NULL, user_id INTEGER, target_node_id TEXT, recommendations TEXT, education_type TEXT, status TEXT DEFAULT 'success', created_at TEXT, FOREIGN KEY (rec_id) REFERENCES knowledge_recommendation(rec_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_fusion ( fusion_id TEXT PRIMARY KEY, fusion_name TEXT NOT NULL, fusion_method TEXT NOT NULL, source_graph_id TEXT, target_graph_id TEXT, description TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS fusion_records ( record_id INTEGER PRIMARY KEY AUTOINCREMENT, fusion_id TEXT, fusion_method TEXT NOT NULL, source_data TEXT, target_data TEXT, fused_count INTEGER DEFAULT 0, conflict_count INTEGER DEFAULT 0, education_type TEXT, status TEXT DEFAULT 'success', created_at TEXT, FOREIGN KEY (fusion_id) REFERENCES knowledge_fusion(fusion_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_update ( update_id TEXT PRIMARY KEY, update_name TEXT NOT NULL, update_method TEXT NOT NULL, description TEXT, education_type TEXT, schedule TEXT, is_active INTEGER DEFAULT 1, last_run_at TEXT, next_run_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS update_records ( record_id INTEGER PRIMARY KEY AUTOINCREMENT, update_id TEXT, update_method TEXT NOT NULL, updated_count INTEGER DEFAULT 0, affected_nodes INTEGER DEFAULT 0, affected_relations INTEGER DEFAULT 0, education_type TEXT, status TEXT DEFAULT 'success', message TEXT, created_at TEXT, FOREIGN KEY (update_id) REFERENCES knowledge_update(update_id) ) ''')
                conn.commit()
    ...
    def create_graph(self, graph_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO knowledge_graph ( graph_id, graph_name, description, education_type, subject, version, node_count, relation_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, '1.0', 0, 0, 'active', ?, ?) ''', (graph_id, graph_name, kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    graph_id = f"kg_{uuid.uuid4().hex[:12]}"
                    cursor.execute(''' INSERT INTO knowledge_graph ( graph_id, graph_name, description, education_type, subject, version, node_count, relation_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (graph_id, graph_info.get('graph_name', 'Imported Graph'),
                    ...
                    for node in graph_data.get('nodes', []):
                        ...
                        cursor.execute(''' INSERT INTO knowledge_nodes ( node_id, graph_id, node_type, knowledge_type, node_name, node_label, description, content, metadata, education_type, grade_level, subject, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (new_node_id, graph_id, node.get('node_type'),
                    ...
                    for relation in graph_data.get('relations', []):
                        ...
                        cursor.execute(''' INSERT INTO knowledge_relations ( relation_id, graph_id, source_node_id, target_node_id, relation_type, relation_name, weight, description, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (new_relation_id, graph_id,
                    ...
                    cursor.execute(''' UPDATE knowledge_graph SET node_count = ?, relation_count = ?, updated_at = ? WHERE graph_id = ? ''', (node_count, relation_count, now, graph_id))
                    conn.commit()
                    logger.info(f'导入知识图谱: {graph_id}, nodes: {node_count}, relations: {relation_count}')
                    return {'success': True, 'graph_id': graph_id, 'node_count': node_count,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not graph:
                        return {'success': False, 'error': '知识图谱不存在'}
                    cursor.execute(''' INSERT INTO knowledge_nodes ( node_id, graph_id, node_type, knowledge_type, node_name, node_label, description, content, metadata, education_type, grade_level, subject, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (node_id, graph_id, node_type, kwargs.get('knowledge_type'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not result:
                        return {'success': False, 'error': '节点不存在'}
                    graph_id = result[0]
                    cursor.execute('UPDATE knowledge_nodes SET is_active = 0, updated_at = ? WHERE node_id = ?', (now,
                    node_id))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO knowledge_relations ( relation_id, graph_id, source_node_id, target_node_id, relation_type, relation_name, weight, description, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (relation_id, graph_id, source_node_id, target_node_id,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not result:
                        return {'success': False, 'error': '关系不存在'}
                    graph_id = result[0]
                    cursor.execute('UPDATE knowledge_relations SET is_active = 0, updated_at = ? WHERE relation_id = ?',
                    (now, relation_id))
                    cursor.execute('UPDATE knowledge_graph SET relation_count = relation_count - 1, updated_at = ? WHERE graph_id = ?', (now, graph_id))
                    conn.commit()
    ...
    def relation_query(self, graph_id: str, source_node_id: str = None,
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                relations = [dict(r) for r in cursor.fetchall()]
                cursor.execute('INSERT INTO query_records (query_text, query_type, education_type, result_count, created_at) VALUES (?, ?, ?, ?, ?)',
                             (f"relation_query_{source_node_id}_{target_node_id}", 'relation', education_type,
                             len(relations), now))
                conn.commit()
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute('INSERT INTO reasoning_records (reasoning_type, input_data, output_data, confidence, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                             ('deductive', premise, json.dumps([n['node_id'] for n in conclusions]), 0.85,
                             education_type, now))
                conn.commit()
    ...
    def inductive_reasoning(self, graph_id: str, examples: List[str],
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute('INSERT INTO reasoning_records (reasoning_type, input_data, output_data, confidence, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                             ('inductive', json.dumps(examples), json.dumps([p['node_id'] for p in patterns]), 0.75,
                             education_type, now))
                conn.commit()
    ...
    def analogical_reasoning(self, graph_id: str, source_node_id: str,
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute('INSERT INTO reasoning_records (reasoning_type, input_data, output_data, confidence, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                             ('analogical', source_node_id, json.dumps([n['node_id'] for n in similar_nodes]), 0.70,
                             education_type, now))
                conn.commit()
    ...
    def causal_reasoning(self, graph_id: str, event_node_id: str,
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                effects = []
                cursor.execute(
                'SELECT * FROM knowledge_relations WHERE graph_id = ? AND target_node_id = ? AND relation_type = ? AND  is_active = 1',
                ...
                cursor.execute(
                'SELECT * FROM knowledge_relations WHERE graph_id = ? AND source_node_id = ? AND relation_type = ? AND  is_active = 1',
                             (graph_id, event_node_id, 'causal'))
                for rel in cursor.fetchall():
                ...
                cursor.execute('INSERT INTO reasoning_records (reasoning_type, input_data, output_data, confidence, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                             ('causal', event_node_id, json.dumps({'causes': [c['node_id'] for c in causes],
                             'effects': [e['node_id'] for e in effects]}), 0.80, education_type, now))
                conn.commit()
    ...
    def recommend_related_knowledge(self, node_id: str,
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                related = []
                cursor.execute(
                'SELECT target_node_id FROM knowledge_relations WHERE graph_id = ? AND source_node_id = ? AND is_active  = 1',
                             (graph_id, node_id))
                for rel in cursor.fetchall():
                    cursor.execute('SELECT * FROM knowledge_nodes WHERE node_id = ? AND is_active = 1',
                    (rel['target_node_id'],))
                    node = cursor.fetchone()
                    if node:
                ...
                cursor.execute(
                'SELECT source_node_id FROM knowledge_relations WHERE graph_id = ? AND target_node_id = ? AND is_active  = 1',
                             (graph_id, node_id))
                for rel in cursor.fetchall():
                    cursor.execute('SELECT * FROM knowledge_nodes WHERE node_id = ? AND is_active = 1',
                    (rel['source_node_id'],))
                ...
                cursor.execute('INSERT INTO recommendation_records (rec_type, user_id, target_node_id, recommendations, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                             ('related_knowledge', None, node_id, json.dumps([r['node_id'] for r in related]),
                             education_type, now))
                conn.commit()
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                for _ in range(steps - 1):
                    cursor.execute(
                    'SELECT target_node_id FROM knowledge_relations WHERE graph_id = ? AND source_node_id = ? AND  relation_type = ? AND is_active = 1',
    ...
    def recommend_knowledge_points(self, subject: str,
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                points = points[:top_k]
                cursor.execute('INSERT INTO recommendation_records (rec_type, user_id, target_node_id, recommendations, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                             ('knowledge_point', None, subject, json.dumps([p['node_id'] for p in points]),
                             education_type, now))
                conn.commit()
    ...
    def recommend_resources(self, node_id: str,
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute('INSERT INTO recommendation_records (rec_type, user_id, target_node_id, recommendations, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                             ('resource', None, node_id, json.dumps([r['node_id'] for r in resources]), education_type,
                             now))
                conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    conn.row_factory = sqlite3.Row
                    cursor = conn.cursor()
                    cursor.execute('SELECT * FROM knowledge_nodes WHERE graph_id = ? AND is_active = 1',
                    (source_graph_id,))
                    ...
                    cursor.execute('INSERT INTO fusion_records (fusion_method, source_data, target_data, fused_count, conflict_count, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
                                 ('entity_alignment', source_graph_id, target_graph_id, len(aligned), conflicts,
                                 education_type, now))
                    conn.commit()
                    return {'success': True, 'aligned_count': len(aligned), 'conflicts': conflicts,
                    'alignments': aligned}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    conn.row_factory = sqlite3.Row
                    cursor = conn.cursor()
                    cursor.execute('SELECT * FROM knowledge_relations WHERE graph_id = ? AND is_active = 1',
                    (source_graph_id,))
                    ...
                    for source in source_rels:
                        ...
                        if not exists:
                            ...
                            conflicts += 1
                    cursor.execute('INSERT INTO fusion_records (fusion_method, source_data, target_data, fused_count, conflict_count, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
                                 ('relation_fusion', source_graph_id, target_graph_id, len(fused), conflicts,
                                 education_type, now))
                    conn.commit()
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    for conflict in conflict_data.get('conflicts', []):
                        ...
                        cursor.execute(
                        'UPDATE node_properties SET property_value = ? WHERE node_id = ? AND property_key = ?',
                                     (resolved_value, node_id, property_key))
                        if cursor.rowcount > 0:
                            resolved += 1
                    cursor.execute('INSERT INTO fusion_records (fusion_method, source_data, target_data, fused_count, conflict_count, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
                                 ('conflict_resolution', graph_id, json.dumps(conflict_data), resolved,
                                 len(conflict_data.get('conflicts', [])) - resolved, education_type, now))
                    conn.commit()
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT INTO update_records (update_method, updated_count, affected_nodes, affected_relations, education_type, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 ('incremental', updated_nodes + updated_relations, updated_nodes, updated_relations,
                                 education_type, now))
                    conn.commit()
    ...
    def full_update(self, graph_id: str, graph_data: Dict[str, Any],
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE knowledge_nodes SET is_active = 0, updated_at = ? WHERE graph_id = ?', (now,
                    graph_id))
                    cursor.execute('UPDATE knowledge_relations SET is_active = 0, updated_at = ? WHERE graph_id = ?',
                    (now, graph_id))
                    node_count = 0
                    relation_count = 0
                    for node in graph_data.get('nodes', []):
                        node_id = f"nd_{uuid.uuid4().hex[:12]}"
                        cursor.execute(''' INSERT INTO knowledge_nodes ( node_id, graph_id, node_type, knowledge_type, node_name, node_label, description, content, metadata, education_type, grade_level, subject, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (node_id, graph_id, node.get('node_type'),
                              node.get('knowledge_type'), node.get('node_name'),
                              node.get('node_label'), node.get('description'),
                              node.get('content'), node.get('metadata'),
                              education_type or node.get('education_type', 'adult'),
                    ...
                    for relation in graph_data.get('relations', []):
                        relation_id = f"rl_{uuid.uuid4().hex[:12]}"
                        cursor.execute(''' INSERT INTO knowledge_relations ( relation_id, graph_id, source_node_id, target_node_id, relation_type, relation_name, weight, description, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (relation_id, graph_id, relation.get('source_node_id'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if update_data.get('action') == 'add_node':
                        ...
                        cursor.execute('UPDATE knowledge_nodes SET is_active = 0, updated_at = ? WHERE node_id = ?',
                        (now, node_id))
                        cursor.execute('UPDATE knowledge_relations SET is_active = 0, updated_at = ? WHERE source_node_id = ? OR target_node_id = ?', (now, node_id, node_id))
                        cursor.execute('UPDATE knowledge_graph SET node_count = node_count - 1, updated_at = ? WHERE graph_id = ?', (now, graph_id))
                        updated_count = 1
    ...
    def periodic_update(self, graph_id: str, schedule_type: str = 'daily',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR REPLACE INTO knowledge_update ( update_id, update_name, update_method, description, education_type, schedule, is_active, last_run_at, next_run_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, ?) ''', (f"upd_{uuid.uuid4().hex[:12]}", f"{schedule_type}_update",
    ...
    def get_statistics(self, graph_id: str = None, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                stats = {}
                if graph_id:
                    cursor.execute('SELECT node_count, relation_count FROM knowledge_graph WHERE graph_id = ?',
                    (graph_id,))
                    graph = cursor.fetchone()
                    if graph:
