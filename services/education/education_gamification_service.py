    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS gamification_config ( config_id TEXT PRIMARY KEY, config_name TEXT NOT NULL, education_type TEXT NOT NULL, elements TEXT, point_rules TEXT, achievement_rules TEXT, task_rules TEXT, badge_rules TEXT, currency_rules TEXT, leaderboard_rules TEXT, social_game_rules TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS config_settings ( id INTEGER PRIMARY KEY AUTOINCREMENT, config_id TEXT NOT NULL, setting_key TEXT NOT NULL, setting_value TEXT, description TEXT, FOREIGN KEY (config_id) REFERENCES gamification_config(config_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS point_system ( point_id TEXT PRIMARY KEY, point_type TEXT NOT NULL, education_type TEXT NOT NULL, name TEXT, multiplier REAL DEFAULT 1.0, min_points INTEGER DEFAULT 0, max_points INTEGER DEFAULT 100000, description TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS point_records ( record_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, point_type TEXT NOT NULL, education_type TEXT, points INTEGER NOT NULL, reason TEXT, reference_id TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS achievement_system ( achievement_id TEXT PRIMARY KEY, achievement_type TEXT NOT NULL, education_type TEXT NOT NULL, name TEXT NOT NULL, icon TEXT, description TEXT, points_required INTEGER DEFAULT 0, tasks_required INTEGER DEFAULT 0, is_locked INTEGER DEFAULT 1, is_secret INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS achievement_records ( record_id TEXT PRIMARY KEY, achievement_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, unlocked_at TEXT, progress INTEGER DEFAULT 0, status TEXT DEFAULT 'in_progress', FOREIGN KEY (achievement_id) REFERENCES achievement_system(achievement_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS leaderboard ( board_id TEXT PRIMARY KEY, board_type TEXT NOT NULL, education_type TEXT NOT NULL, name TEXT NOT NULL, description TEXT, period TEXT DEFAULT 'monthly', is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS leaderboard_data ( data_id TEXT PRIMARY KEY, board_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, score INTEGER NOT NULL, rank INTEGER DEFAULT 0, period_start TEXT, period_end TEXT, created_at TEXT, FOREIGN KEY (board_id) REFERENCES leaderboard(board_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS task_system ( task_id TEXT PRIMARY KEY, task_type TEXT NOT NULL, education_type TEXT NOT NULL, name TEXT NOT NULL, description TEXT, points_reward INTEGER DEFAULT 0, currency_reward TEXT, badge_reward TEXT, achievement_reward TEXT, is_daily INTEGER DEFAULT 0, is_repeatable INTEGER DEFAULT 0, expires_at TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS task_records ( record_id TEXT PRIMARY KEY, task_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, status TEXT DEFAULT 'pending', progress INTEGER DEFAULT 0, completed_at TEXT, claimed_at TEXT, FOREIGN KEY (task_id) REFERENCES task_system(task_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS badge_system ( badge_id TEXT PRIMARY KEY, badge_type TEXT NOT NULL, education_type TEXT NOT NULL, name TEXT NOT NULL, color TEXT DEFAULT '#4CAF50', icon TEXT, description TEXT, rarity TEXT DEFAULT 'common', unlock_condition TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS badge_records ( record_id TEXT PRIMARY KEY, badge_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, unlocked_at TEXT, status TEXT DEFAULT 'locked', FOREIGN KEY (badge_id) REFERENCES badge_system(badge_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS virtual_currency ( currency_id TEXT PRIMARY KEY, currency_type TEXT NOT NULL, education_type TEXT NOT NULL, name TEXT NOT NULL, symbol TEXT, exchange_rate REAL DEFAULT 1.0, description TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS currency_transactions ( tx_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, currency_type TEXT NOT NULL, education_type TEXT, amount INTEGER NOT NULL, transaction_type TEXT, reason TEXT, reference_id TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS social_games ( game_id TEXT PRIMARY KEY, game_type TEXT NOT NULL, education_type TEXT NOT NULL, name TEXT NOT NULL, description TEXT, mode TEXT DEFAULT 'pvp', max_players INTEGER DEFAULT 2, min_players INTEGER DEFAULT 2, duration_minutes INTEGER DEFAULT 15, points_reward INTEGER DEFAULT 0, currency_reward TEXT, badge_reward TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS game_records ( record_id TEXT PRIMARY KEY, game_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, score INTEGER DEFAULT 0, result TEXT DEFAULT 'pending', participants TEXT, started_at TEXT, ended_at TEXT, FOREIGN KEY (game_id) REFERENCES social_games(game_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS gamification_alerts ( alert_id TEXT PRIMARY KEY, alert_type TEXT NOT NULL, education_type TEXT NOT NULL, name TEXT NOT NULL, description TEXT, threshold INTEGER DEFAULT 0, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS alert_history ( history_id TEXT PRIMARY KEY, alert_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, triggered_at TEXT, message TEXT, status TEXT DEFAULT 'triggered', FOREIGN KEY (alert_id) REFERENCES gamification_alerts(alert_id) ) ''')
                conn.commit()
    ...
    def create_gamification_config(self, config_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO gamification_config ( config_id, config_name, education_type, elements, point_rules, achievement_rules, task_rules, badge_rules, currency_rules, leaderboard_rules, social_game_rules, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (config_id, config_name, education_type, elements,
    ...
    def add_points(self, user_id: int, point_type: str, points: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO point_records ( record_id, user_id, user_name, point_type, education_type, points, reason, reference_id, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (record_id, user_id, kwargs.get('user_name'),
    ...
    def create_achievement(self, achievement_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO achievement_system ( achievement_id, achievement_type, education_type, name, icon, description, points_required, tasks_required, is_locked, is_secret, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?) ''', (achievement_id, achievement_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR REPLACE INTO achievement_records ( record_id, achievement_id, user_id, user_name, unlocked_at, progress, status ) VALUES (?, ?, ?, ?, ?, 100, 'unlocked') ''', (record_id, achievement_id, user_id, kwargs.get('user_name'), now))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR REPLACE INTO achievement_records ( record_id, achievement_id, user_id, user_name, unlocked_at, progress, status ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (record_id, achievement_id, user_id, kwargs.get('user_name'),
    ...
    def get_user_achievements(self, user_id: int, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT ar.*, asys.name, asys.icon, asys.description, asys.achievement_type FROM achievement_records ar JOIN achievement_system asys ON ar.achievement_id = asys.achievement_id WHERE ar.user_id = ? '''
                params = [user_id]
                if education_type:
    ...
    def create_leaderboard(self, board_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO leaderboard ( board_id, board_type, education_type, name, description, period, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (board_id, board_type, education_type, name,
                          kwargs.get('description', config.get('description')),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    period_start = datetime.now().replace(day=1, hour=0, minute=0,
                    second=0).isoformat()[:10] if board[1] == 'monthly' else \
                                   (datetime.now() - timedelta(days=7)).isoformat()[:10] if board[1] == 'weekly' else \
                                   datetime.now().replace(month=1, day=1, hour=0, minute=0,
                                   second=0).isoformat()[:10] if board[1] == 'yearly' else \
                                   None
                    cursor.execute(''' SELECT user_id, user_name, SUM(points) as total FROM point_records WHERE point_type = ? OR ? = 'total' GROUP BY user_id, user_name ORDER BY total DESC LIMIT 100 ''', (board[0], board[0]))
                    results = cursor.fetchall()
                    rank = 1
                    for row in results:
                        data_id = f"lbd_{uuid.uuid4().hex[:8]}"
                        cursor.execute(''' INSERT OR REPLACE INTO leaderboard_data ( data_id, board_id, user_id, user_name, score, rank, period_start, period_end, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (data_id, board_id, row[0], row[1], row[2], rank,
    ...
    def get_leaderboard(self, board_id: str, limit: int = 50) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not board:
                    return {'success': False, 'error': '排行榜不存在'}
                cursor.execute(''' SELECT * FROM leaderboard_data WHERE board_id = ? ORDER BY rank ASC LIMIT ? ''', (board_id, limit))
    ...
    def create_task(self, task_type: str, education_type: str, name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO task_system ( task_id, task_type, education_type, name, description, points_reward, currency_reward, badge_reward, achievement_reward, is_daily, is_repeatable, expires_at, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (task_id, task_type, education_type, name,
                          kwargs.get('description', config.get('description')),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO task_records ( record_id, task_id, user_id, user_name, status, progress, completed_at, claimed_at ) VALUES (?, ?, ?, ?, 'pending', 0, NULL, NULL) ''', (record_id, task_id, user_id, kwargs.get('user_name')))
                    conn.commit()
                    return {'success': True, 'record_id': record_id}
        except Exception as e:
    ...
    def claim_task_reward(self, record_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT tr.*, ts.points_reward, ts.currency_reward, ts.badge_reward, ts.achievement_reward FROM task_records tr JOIN task_system ts ON tr.task_id = ts.task_id WHERE tr.record_id = ? ''', (record_id,))
                    result = cursor.fetchone()
                    if not result:
                        return {'success': False, 'error': '任务记录不存在'}
    ...
    def create_badge(self, badge_type: str, education_type: str, name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO badge_system ( badge_id, badge_type, education_type, name, color, icon, description, rarity, unlock_condition, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (badge_id, badge_type, education_type, name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO badge_records ( record_id, badge_id, user_id, user_name, unlocked_at, status ) VALUES (?, ?, ?, ?, ?, 'unlocked') ''', (record_id, badge_id, user_id, kwargs.get('user_name'), now))
                    conn.commit()
                    return {'success': True, 'record_id': record_id}
        except Exception as e:
    ...
    def get_user_badges(self, user_id: int, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT br.*, bs.name, bs.color, bs.icon, bs.description, bs.badge_type, bs.rarity FROM badge_records br JOIN badge_system bs ON br.badge_id = bs.badge_id WHERE br.user_id = ? AND br.status = 'unlocked' '''
                params = [user_id]
                if education_type:
    ...
    def create_currency(self, currency_type: str, education_type: str, name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO virtual_currency ( currency_id, currency_type, education_type, name, symbol, exchange_rate, description, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (currency_id, currency_type, education_type, name,
    ...
    def add_currency(self, user_id: int, currency_type: str, amount: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO currency_transactions ( tx_id, user_id, user_name, currency_type, education_type, amount, transaction_type, reason, reference_id, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'credit', ?, ?, ?) ''', (tx_id, user_id, kwargs.get('user_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO currency_transactions ( tx_id, user_id, user_name, currency_type, education_type, amount, transaction_type, reason, reference_id, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'debit', ?, ?, ?) ''', (tx_id, user_id, kwargs.get('user_name'),
                          currency_type, education_type, -amount,
    ...
    def create_social_game(self, game_type: str, education_type: str, name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO social_games ( game_id, game_type, education_type, name, description, mode, max_players, min_players, duration_minutes, points_reward, currency_reward, badge_reward, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (game_id, game_type, education_type, name,
                          kwargs.get('description', config.get('description')),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO game_records ( record_id, game_id, user_id, user_name, score, result, participants, started_at, ended_at ) VALUES (?, ?, ?, ?, 0, 'pending', ?, ?, NULL) ''', (record_id, game_id, user_id, kwargs.get('user_name'),
    ...
    def end_game(self, record_id: str, result: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT gr.*, sg.points_reward, sg.currency_reward, sg.badge_reward FROM game_records gr JOIN social_games sg ON gr.game_id = sg.game_id WHERE gr.record_id = ? ''', (record_id,))
    ...
    def get_game_history(self, user_id: int, game_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT gr.*, sg.name, sg.mode FROM game_records gr JOIN social_games sg ON gr.game_id = sg.game_id WHERE gr.user_id = ? '''
                params = [user_id]
                if game_type:
    ...
    def create_alert(self, alert_type: str, education_type: str, name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO gamification_alerts ( alert_id, alert_type, education_type, name, description, threshold, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (alert_id, alert_type, education_type, name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO alert_history ( history_id, alert_id, user_id, user_name, triggered_at, message, status ) VALUES (?, ?, ?, ?, ?, ?, 'triggered') ''', (history_id, alert_id, user_id, kwargs.get('user_name'),
