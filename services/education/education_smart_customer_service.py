...
class EducationSmartCustomerService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_channels ( channel_id TEXT PRIMARY KEY, channel_name TEXT NOT NULL, icon TEXT, response_time TEXT, status TEXT DEFAULT 'active', education_type TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS chat_sessions ( session_id TEXT PRIMARY KEY, channel_id TEXT, customer_id INTEGER, customer_name TEXT, agent_id INTEGER, agent_name TEXT, dialog_type TEXT, education_type TEXT, start_time TEXT, end_time TEXT, status TEXT DEFAULT 'active', sentiment_score REAL DEFAULT 0, sentiment_level TEXT, satisfaction_score INTEGER, total_messages INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS chat_messages ( message_id TEXT PRIMARY KEY, session_id TEXT NOT NULL, sender_type TEXT NOT NULL, sender_id INTEGER, sender_name TEXT, content TEXT NOT NULL, message_type TEXT DEFAULT 'text', sentiment_score REAL, sentiment_level TEXT, timestamp TEXT, is_auto_reply INTEGER DEFAULT 0, knowledge_ref TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_base ( knowledge_id TEXT PRIMARY KEY, title TEXT NOT NULL, content TEXT NOT NULL, knowledge_type TEXT, category_id TEXT, tags TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, view_count INTEGER DEFAULT 0, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_categories ( category_id TEXT PRIMARY KEY, category_name TEXT NOT NULL, parent_id TEXT, education_type TEXT, sort_order INTEGER DEFAULT 0, is_active INTEGER DEFAULT 1, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS dialog_flows ( flow_id TEXT PRIMARY KEY, flow_name TEXT NOT NULL, dialog_type TEXT, education_type TEXT, start_node TEXT, nodes TEXT, edges TEXT, is_active INTEGER DEFAULT 1, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS sentiment_analysis ( analysis_id TEXT PRIMARY KEY, session_id TEXT, message_id TEXT, content TEXT, sentiment_score REAL, sentiment_level TEXT, confidence REAL, education_type TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS tickets ( ticket_id TEXT PRIMARY KEY, ticket_type TEXT NOT NULL, title TEXT NOT NULL, description TEXT, channel_id TEXT, customer_id INTEGER, customer_name TEXT, education_type TEXT, priority TEXT DEFAULT 'low', status TEXT DEFAULT 'open', sla_hours INTEGER DEFAULT 24, created_at TEXT, updated_at TEXT, resolved_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ticket_assignments ( id INTEGER PRIMARY KEY AUTOINCREMENT, ticket_id TEXT NOT NULL, agent_id INTEGER, agent_name TEXT, assign_time TEXT, status TEXT DEFAULT 'assigned', UNIQUE(ticket_id, agent_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS ticket_interactions ( interaction_id TEXT PRIMARY KEY, ticket_id TEXT NOT NULL, actor_id INTEGER, actor_name TEXT, actor_type TEXT, action TEXT NOT NULL, content TEXT, timestamp TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS routing_rules ( rule_id TEXT PRIMARY KEY, rule_name TEXT NOT NULL, rule_type TEXT, education_type TEXT, conditions TEXT, actions TEXT, priority INTEGER DEFAULT 100, is_active INTEGER DEFAULT 1, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS routing_logs ( log_id TEXT PRIMARY KEY, rule_id TEXT, ticket_id TEXT, session_id TEXT, source TEXT, target_agent_id INTEGER, target_agent_name TEXT, reason TEXT, timestamp TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS quality_inspections ( inspection_id TEXT PRIMARY KEY, session_id TEXT, ticket_id TEXT, agent_id INTEGER, agent_name TEXT, inspector_id INTEGER, inspector_name TEXT, education_type TEXT, status TEXT DEFAULT 'pending', created_at TEXT, completed_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS inspection_results ( result_id TEXT PRIMARY KEY, inspection_id TEXT NOT NULL, dimension TEXT NOT NULL, score REAL NOT NULL, comment TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS customer_feedback ( feedback_id TEXT PRIMARY KEY, session_id TEXT, ticket_id TEXT, customer_id INTEGER, customer_name TEXT, education_type TEXT, satisfaction_score INTEGER, rating INTEGER, comment TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_metrics ( metric_id TEXT PRIMARY KEY, date TEXT NOT NULL, education_type TEXT, channel_id TEXT, total_sessions INTEGER DEFAULT 0, resolved_sessions INTEGER DEFAULT 0, avg_response_time REAL DEFAULT 0, avg_handle_time REAL DEFAULT 0, satisfaction_rate REAL DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS agent_performance ( perf_id TEXT PRIMARY KEY, agent_id INTEGER NOT NULL, agent_name TEXT, education_type TEXT, date TEXT NOT NULL, total_tickets INTEGER DEFAULT 0, resolved_tickets INTEGER DEFAULT 0, avg_response_time REAL DEFAULT 0, avg_handle_time REAL DEFAULT 0, satisfaction_score REAL DEFAULT 0, quality_score REAL DEFAULT 0, created_at TEXT, UNIQUE(agent_id, date) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS auto_replies ( reply_id TEXT PRIMARY KEY, keyword TEXT NOT NULL, content TEXT NOT NULL, education_type TEXT, question_type TEXT, priority INTEGER DEFAULT 100, is_active INTEGER DEFAULT 1, usage_count INTEGER DEFAULT 0, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_schedules ( schedule_id TEXT PRIMARY KEY, agent_id INTEGER NOT NULL, agent_name TEXT, education_type TEXT, day_of_week TEXT NOT NULL, start_time TEXT, end_time TEXT, is_on_duty INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                conn.commit()
    ...
    def _find_auto_reply(self, question: str, education_type: str) -> Optional[str]:
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(''' SELECT content FROM auto_replies WHERE is_active = 1 AND education_type = ? AND keyword LIKE ? ORDER BY priority DESC LIMIT 1 ''', (education_type, f'%{question[:10]}%'))
    ...
    def _search_knowledge(self, query: str, education_type: str) -> Optional[Dict]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT * FROM knowledge_base WHERE is_active = 1 AND education_type = ? AND (title LIKE ? OR content LIKE ?) ORDER BY view_count DESC LIMIT 1 ''', (education_type, f'%{query}%', f'%{query}%'))
                result = cursor.fetchone()
                if result:
    ...
    def _create_session(self, customer_id: int, customer_name: str,
        ...
        with self._lock:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO chat_sessions ( session_id, channel_id, customer_id, customer_name, dialog_type, education_type, start_time, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?) ''', (session_id, channel_id, customer_id, customer_name,
    ...
    def add_knowledge(self, title: str, content: str, knowledge_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO knowledge_base ( knowledge_id, title, content, knowledge_type, category_id, tags, education_type, is_active, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?) ''', (knowledge_id, title, content, knowledge_type,
    ...
    def send_message(self, session_id: str, content: str, sender_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO chat_messages ( message_id, session_id, sender_type, sender_id, sender_name, content, sentiment_score, sentiment_level, timestamp, is_auto_reply, knowledge_ref, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (message_id, session_id, sender_type, kwargs.get('sender_id'),
    ...
    def close_session(self, session_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE chat_sessions SET status = 'closed', end_time = ?, sentiment_score = ?, sentiment_level = ?, satisfaction_score = ? WHERE session_id = ? AND status = 'active' ''', (now, kwargs.get('sentiment_score'), kwargs.get('sentiment_level'),
    ...
    def transfer_session(self, session_id: str, target_agent_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE chat_sessions SET agent_id = ?, agent_name = ? WHERE session_id = ? ''', (target_agent_id, target_agent_name, session_id))
                    cursor.execute(''' INSERT INTO routing_logs ( log_id, session_id, target_agent_id, target_agent_name, reason, timestamp, created_at ) VALUES (?, ?, ?, ?, '会话转接', ?, ?) ''', (f"log_{uuid.uuid4().hex[:12]}", session_id, target_agent_id,
    ...
    def analyze_sentiment(self, content: str) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' INSERT INTO sentiment_analysis ( analysis_id, content, sentiment_score, sentiment_level, confidence, created_at ) VALUES (?, ?, ?, ?, ?, ?) ''', (analysis_id, content[:500], result['score'], result['level'],
    ...
    def get_session_sentiment(self, session_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT AVG(sentiment_score) as avg_score, sentiment_level, COUNT(*) as count FROM chat_messages WHERE session_id = ? ''', (session_id,))
                result = cursor.fetchone()
                if result:
    ...
    def get_sentiment_trend(self, customer_id: int, days: int = 7) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT DATE(timestamp) as date, AVG(sentiment_score) as avg_score, COUNT(*) as count FROM chat_messages WHERE sender_id = ? AND DATE(timestamp) >= ? GROUP BY DATE(timestamp) ORDER BY date ''', (customer_id, start_date))
    ...
    def create_ticket(self, ticket_type: str, title: str, customer_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO tickets ( ticket_id, ticket_type, title, description, channel_id, customer_id, customer_name, education_type, priority, sla_hours, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (ticket_id, ticket_type, title, kwargs.get('description'),
                    ...
                    cursor.execute(''' INSERT INTO ticket_interactions ( interaction_id, ticket_id, actor_id, actor_name, actor_type, action, content, timestamp, created_at ) VALUES (?, ?, ?, ?, 'system', 'created', '工单已创建', ?, ?) ''', (f"int_{uuid.uuid4().hex[:12]}", ticket_id, customer_id,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO ticket_interactions ( interaction_id, ticket_id, actor_id, actor_name, actor_type, action, content, timestamp, created_at ) VALUES (?, ?, ?, ?, 'agent', 'assigned', ?, ?, ?) ''', (f"int_{uuid.uuid4().hex[:12]}", ticket_id, agent_id, agent_name,
    ...
    def add_ticket_interaction(self, ticket_id: str, actor_id: int, actor_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO ticket_interactions ( interaction_id, ticket_id, actor_id, actor_name, actor_type, action, content, timestamp, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (interaction_id, ticket_id, actor_id, actor_name,
    ...
    def add_routing_rule(self, rule_name: str, rule_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO routing_rules ( rule_id, rule_name, rule_type, education_type, conditions, actions, priority, is_active, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?) ''', (rule_id, rule_name, rule_type, education_type,
    ...
    def route_ticket(self, ticket_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT * FROM routing_rules WHERE is_active = 1 AND education_type = ? ORDER BY priority DESC LIMIT 1 ''', (ticket['education_type'],))
                rule = cursor.fetchone()
                
                cursor.execute(''' SELECT agent_id, agent_name FROM agent_performance WHERE date = ? AND education_type = ? ORDER BY resolved_tickets DESC LIMIT 1 ''', (now[:10], ticket['education_type']))
                agent = cursor.fetchone()
                
                if agent:
                    self.assign_ticket(ticket_id, agent['agent_id'], agent['agent_name'])
                    cursor.execute(''' INSERT INTO routing_logs ( log_id, rule_id, ticket_id, target_agent_id, target_agent_name, reason, timestamp, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (f"log_{uuid.uuid4().hex[:12]}", rule['rule_id'] if rule else None,
    ...
    def create_inspection(self, session_id: str = None, ticket_id: str = None,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO quality_inspections ( inspection_id, session_id, ticket_id, agent_id, agent_name, inspector_id, inspector_name, education_type, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?) ''', (inspection_id, session_id, ticket_id, agent_id,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    for result in results:
                        ...
                        total_weight += weight
                        cursor.execute(''' INSERT INTO inspection_results ( result_id, inspection_id, dimension, score, comment, created_at ) VALUES (?, ?, ?, ?, ?, ?) ''', (f"irs_{uuid.uuid4().hex[:12]}", inspection_id, dimension, score, comment, now))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if existing:
                        cursor.execute(''' UPDATE agent_performance SET total_tickets = total_tickets + ?, resolved_tickets = resolved_tickets + ?, avg_response_time = ?, avg_handle_time = ?, satisfaction_score = ?, quality_score = ? WHERE agent_id = ? AND date = ? ''', (kwargs.get('total_tickets', 0), kwargs.get('resolved_tickets', 0),
                              kwargs.get('avg_response_time', 0), kwargs.get('avg_handle_time', 0),
                              kwargs.get('satisfaction_score', 0), kwargs.get('quality_score', 0),
                              agent_id, date))
                    else:
                        cursor.execute(''' INSERT INTO agent_performance ( perf_id, agent_id, agent_name, education_type, date, total_tickets, resolved_tickets, avg_response_time, avg_handle_time, satisfaction_score, quality_score, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (perf_id, agent_id, agent_name, education_type, date,
    ...
    def get_agent_rankings(self, education_type: str = None, days: int = 7) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT agent_id, agent_name, education_type, SUM(total_tickets) as total_tickets, SUM(resolved_tickets) as resolved_tickets, AVG(avg_response_time) as avg_response_time, AVG(satisfaction_score) as satisfaction_score, AVG(quality_score) as quality_score FROM agent_performance WHERE date >= ? '''
                params = [start_date]
                if education_type:
                    query += ' AND education_type = ?'
                    params.append(education_type)
    ...
    def get_agent_workload(self, agent_id: int) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT (SELECT COUNT( *) FROM ticket_assignments WHERE agent_id = ? AND status = 'assigned') as assigned_tickets, (SELECT COUNT(*) FROM ticket_assignments ta JOIN tickets t ON ta.ticket_id = t.ticket_id WHERE ta.agent_id = ? AND t.status = 'open') as open_tickets, (SELECT COUNT( *) FROM chat_sessions WHERE agent_id = ? AND status = 'active') as active_sessions, (SELECT COUNT(*) FROM chat_sessions WHERE agent_id = ? AND DATE( start_time) = ?) as today_sessions ''', (agent_id, agent_id, agent_id, agent_id, today))
    ...
    def add_auto_reply(self, keyword: str, content: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO auto_replies ( reply_id, keyword, content, education_type, question_type, priority, is_active, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, ?) ''', (reply_id, keyword, content, education_type,
    ...
    def get_service_summary(self, date: str = None, education_type: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT (SELECT COUNT(*) FROM chat_sessions WHERE DATE(start_time) = ? {}) as total_sessions, (SELECT COUNT(*) FROM chat_sessions WHERE DATE( start_time) = ? {} AND status = 'closed') as closed_sessions, (SELECT COUNT(*) FROM tickets WHERE DATE(created_at) = ? {}) as total_tickets, (SELECT COUNT(*) FROM tickets WHERE DATE( created_at) = ? {} AND status = 'resolved') as resolved_tickets, (SELECT AVG(sentiment_score) FROM chat_sessions WHERE DATE(start_time) = ? {}) as avg_sentiment, (SELECT AVG(satisfaction_score) FROM customer_feedback WHERE DATE( created_at) = ? {}) as avg_satisfaction '''.format('AND education_type = ?' if education_type else '',) * 6,
                    query_params * 6)
                ...
                if summary:
                    return {
                        ...
                        'session_resolution_rate': round((summary['closed_sessions'] / summary['total_sessions']) * 100,
                        1) if summary['total_sessions'] else 0,
                        'ticket_resolution_rate': round((summary['resolved_tickets'] / summary['total_tickets']) * 100,
                        1) if summary['total_tickets'] else 0
                    }
