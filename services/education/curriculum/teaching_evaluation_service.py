...
class TeachingEvaluationService:
    ...
    def _update_dimension_stats(self, cursor, task_id: str, scores: dict):
        try:
            for dim_code, score in scores.items():
                cursor.execute('''
                    INSERT INTO evaluation_dimension_stats (task_id, dimension_code, dimension_name, average_score,
                    max_score, min_score, response_count)
                    VALUES (?, ?, ?, ?, ?, ?, 1)
                    ON CONFLICT(task_id, dimension_code) DO UPDATE SET
                        average_score = (average_score * response_count + excluded.average_score) / (
                        response_count + 1),
    ...
    def generate_evaluation_report(self, task_id: str, report_type: str = 'summary') -> Dict[str, Any]:
        try:
            ...
            content = json.dumps({                'overview': f'本次评价共收到{overall.get("total_responses", 0)}份有效评价，平均分为{avg_score}分，等级为{level.get("name",                "")}。',
                'strengths': strengths,
    ...
    def has_user_evaluated(self, task_id: str, user_id: int) -> bool:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('SELECT record_id FROM evaluation_records WHERE task_id = ? AND evaluator_id = ?',
                (task_id, user_id))
