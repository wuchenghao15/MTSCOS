...
class ExamManagementService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exams ( exam_id TEXT PRIMARY KEY, exam_name TEXT NOT NULL, education_type TEXT NOT NULL, exam_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, class_id TEXT, full_score REAL DEFAULT 100, pass_score REAL DEFAULT 60, exam_date TEXT, duration_minutes INTEGER DEFAULT 90, location TEXT, supervisor TEXT, status TEXT DEFAULT 'draft', description TEXT, created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exam_subjects ( id INTEGER PRIMARY KEY AUTOINCREMENT, exam_id TEXT NOT NULL, subject TEXT NOT NULL, full_score REAL DEFAULT 100, pass_score REAL DEFAULT 60, duration_minutes INTEGER DEFAULT 90, exam_date TEXT, UNIQUE(exam_id, subject) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exam_scores ( id INTEGER PRIMARY KEY AUTOINCREMENT, exam_id TEXT NOT NULL, student_id INTEGER NOT NULL, subject TEXT NOT NULL, score REAL, grade TEXT, gpa REAL, rank INTEGER, class_rank INTEGER, is_absent INTEGER DEFAULT 0, is_exempt INTEGER DEFAULT 0, remark TEXT, graded_by INTEGER, graded_at TEXT, created_at TEXT, UNIQUE(exam_id, student_id, subject) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exam_statistics ( id INTEGER PRIMARY KEY AUTOINCREMENT, exam_id TEXT NOT NULL, subject TEXT, total_students INTEGER DEFAULT 0, actual_count INTEGER DEFAULT 0, average_score REAL DEFAULT 0, max_score REAL DEFAULT 0, min_score REAL DEFAULT 0, pass_count INTEGER DEFAULT 0, pass_rate REAL DEFAULT 0, excellent_count INTEGER DEFAULT 0, excellent_rate REAL DEFAULT 0, std_deviation REAL DEFAULT 0, median_score REAL DEFAULT 0, updated_at TEXT, UNIQUE(exam_id, subject) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS score_warnings ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, exam_id TEXT NOT NULL, subject TEXT, warning_type TEXT NOT NULL, warning_value REAL, threshold REAL, description TEXT, is_resolved INTEGER DEFAULT 0, resolved_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS report_cards ( report_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, semester TEXT NOT NULL, education_type TEXT, grade_level INTEGER, class_id TEXT, subjects_data TEXT, total_score REAL, average_score REAL, total_gpa REAL, class_rank INTEGER, grade_rank INTEGER, teacher_comment TEXT, head_teacher_comment TEXT, generated_at TEXT, is_published INTEGER DEFAULT 0 ) ''')
                conn.commit()
    ...
    def create_exam(self, exam_name: str, education_type: str, exam_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO exams ( exam_id, exam_name, education_type, exam_type, subject, grade_level, class_id, full_score, pass_score, exam_date, duration_minutes, location, supervisor, status, description, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
                    ...
                    if kwargs.get('subjects'):
                        for subj in kwargs['subjects']:
                            cursor.execute(''' INSERT OR IGNORE INTO exam_subjects ( exam_id, subject, full_score, pass_score, duration_minutes, exam_date ) VALUES (?, ?, ?, ?, ?, ?) ''', (exam_id, subj.get('name'),
    ...
    def update_exam_status(self, exam_id: str, status: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE exams SET status = ?, updated_at = ? WHERE exam_id = ?', (status, now,
                    exam_id))
                    conn.commit()
    ...
    def input_score(self, exam_id: str, student_id: int, subject: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO exam_scores ( exam_id, student_id, subject, score, grade, gpa, is_absent, is_exempt, remark, graded_by, graded_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(exam_id, student_id, subject) DO UPDATE SET score = excluded.score, grade = excluded.grade, gpa = excluded.gpa, is_absent = excluded.is_absent, is_exempt = excluded.is_exempt, remark = excluded.remark, graded_by = excluded.graded_by, graded_at = excluded.graded_at ''', (exam_id, student_id, subject, score, grade, gpa,
    ...
    def get_student_scores(self, student_id: int, exam_id: str = None,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT es.*, e.exam_name, e.exam_type, e.education_type, e.exam_date FROM exam_scores es JOIN exams e ON es.exam_id = e.exam_id WHERE es.student_id = ? '''
                params = [student_id]
                if exam_id:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    for subj in subjects_to_calc:
                        cursor.execute(''' SELECT score FROM exam_scores WHERE exam_id = ? AND subject = ? AND is_absent = 0 AND is_exempt = 0 ''', (exam_id, subj))
                        ...
                        median = scores_sorted[total // 2] if total % 2 == 1 else (
                        scores_sorted[total // 2 - 1] + scores_sorted[total // 2]) / 2
                        variance = sum((s - avg) ** 2 for s in scores) / total
                        std_dev = variance ** 0.5
                        now = datetime.now().isoformat()
                        cursor.execute(''' INSERT INTO exam_statistics ( exam_id, subject, total_students, actual_count, average_score, max_score, min_score, pass_count, pass_rate, excellent_count, excellent_rate, std_deviation, median_score, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(exam_id, subject) DO UPDATE SET total_students = excluded.total_students, actual_count = excluded.actual_count, average_score = excluded.average_score, max_score = excluded.max_score, min_score = excluded.min_score, pass_count = excluded.pass_count, pass_rate = excluded.pass_rate, excellent_count = excluded.excellent_count, excellent_rate = excluded.excellent_rate, std_deviation = excluded.std_deviation, median_score = excluded.median_score, updated_at = excluded.updated_at ''', (exam_id, subj, total, total, round(avg, 2), max_score, min_score,
    ...
    def _calculate_ranks(self, cursor, exam_id: str, subjects: list):
        for subj in subjects:
            cursor.execute(''' SELECT id, student_id, score FROM exam_scores WHERE exam_id = ? AND subject = ? AND is_absent = 0 AND is_exempt = 0 ORDER BY score DESC ''', (exam_id, subj))
                ...
                with self._get_connection() as conn:
                    ...
                    for student_id, subject, score in scores:
                        if score is not None and score < 60:
                            cursor.execute(''' INSERT INTO score_warnings ( student_id, exam_id, subject, warning_type, warning_value, threshold, description, created_at ) VALUES (?, ?, ?, 'fail_risk', ?, 60, ?, ?) ''', (student_id, exam_id, subject, score,
                                  f'{subject}成绩{score}分，低于及格线60分', now))
                            warnings_created += 1
                    cursor.execute(''' SELECT es.student_id, es.subject, es.score FROM exam_scores es JOIN exams e ON es.exam_id = e.exam_id WHERE es.exam_id = ? AND es.is_absent = 0 ''', (exam_id,))
                    conn.commit()
    ...
    def get_student_warnings(self, student_id: int, resolved: int = 0) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT sw.*, e.exam_name FROM score_warnings sw LEFT JOIN exams e ON sw.exam_id = e.exam_id WHERE sw.student_id = ? AND sw.is_resolved = ? ORDER BY sw.created_at DESC ''', (student_id, resolved))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    conn.row_factory = sqlite3.Row
                    cursor = conn.cursor()
                    query = ''' SELECT es.subject, es.score, es.grade, es.gpa, es.rank, e.exam_name, e.exam_type FROM exam_scores es JOIN exams e ON es.exam_id = e.exam_id WHERE es.student_id = ? AND e.exam_date LIKE ? '''
                    ...
                    cursor.execute(''' INSERT INTO report_cards ( report_id, student_id, semester, education_type, grade_level, class_id, subjects_data, total_score, average_score, total_gpa, teacher_comment, head_teacher_comment, generated_at, is_published ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0) ''', (report_id, student_id, semester, education_type,
    ...
    def get_class_ranking(self, exam_id: str, subject: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT es.student_id, es.subject, es.score, es.grade, es.rank FROM exam_scores es WHERE es.exam_id = ? AND es.is_absent = 0 AND es.is_exempt = 0 '''
                params = [exam_id]
                if subject:
