    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS classes ( class_id TEXT PRIMARY KEY, class_name TEXT NOT NULL, education_type TEXT NOT NULL, grade_level INTEGER, class_type TEXT, subject TEXT, head_teacher_id INTEGER, max_students INTEGER DEFAULT 30, current_students INTEGER DEFAULT 0, status TEXT DEFAULT 'planning', start_date TEXT, end_date TEXT, description TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS class_teachers ( id INTEGER PRIMARY KEY AUTOINCREMENT, class_id TEXT NOT NULL, teacher_id INTEGER NOT NULL, subject TEXT, role TEXT DEFAULT 'subject_teacher', assigned_at TEXT, UNIQUE(class_id, teacher_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS class_students ( id INTEGER PRIMARY KEY AUTOINCREMENT, class_id TEXT NOT NULL, student_id INTEGER NOT NULL, enrollment_status TEXT DEFAULT 'enrolled', enrolled_at TEXT, left_at TEXT, leave_reason TEXT, UNIQUE(class_id, student_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS student_profiles ( user_id INTEGER PRIMARY KEY, student_no TEXT UNIQUE, education_type TEXT NOT NULL, grade_level INTEGER, current_class_id TEXT, enrollment_date TEXT, graduation_date TEXT, status TEXT DEFAULT 'enrolled', guardian_name TEXT, guardian_phone TEXT, address TEXT, remark TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS transfer_records ( transfer_id TEXT PRIMARY KEY, student_id INTEGER NOT NULL, from_class_id TEXT, to_class_id TEXT, from_grade INTEGER, to_grade INTEGER, transfer_type TEXT, reason TEXT, approved_by INTEGER, status TEXT DEFAULT 'pending', created_at TEXT, approved_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS grade_levels ( id INTEGER PRIMARY KEY AUTOINCREMENT, education_type TEXT NOT NULL, grade_level INTEGER NOT NULL, grade_name TEXT, stage TEXT, UNIQUE(education_type, grade_level) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS attendance_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, class_id TEXT NOT NULL, student_id INTEGER NOT NULL, record_date TEXT NOT NULL, status TEXT NOT NULL, remark TEXT, recorded_by INTEGER, created_at TEXT, UNIQUE(class_id, student_id, record_date) ) ''')
                conn.commit()
    ...
    def create_class(self, class_name: str, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO classes ( class_id, class_name, education_type, grade_level, class_type, subject, head_teacher_id, max_students, status, start_date, end_date, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
                    ...
                    if kwargs.get('head_teacher_id'):
                        cursor.execute(''' INSERT INTO class_teachers (class_id, teacher_id, subject, role, assigned_at) VALUES (?, ?, ?, ?, ?) ''', (class_id, kwargs['head_teacher_id'], kwargs.get('subject'), 'head_teacher', now))
                    conn.commit()
    ...
    def add_student_to_class(self, class_id: str, student_id: int) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT current_students, max_students, status FROM classes WHERE class_id = ?',
                    (class_id,))
                    cls = cursor.fetchone()
                    if not cls:
                    ...
                    cursor.execute('SELECT id FROM class_students WHERE class_id = ? AND student_id = ?', (class_id,
                    student_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '学生已在班级中'}
                    cursor.execute(''' INSERT INTO class_students (class_id, student_id, enrollment_status, enrolled_at) VALUES (?, ?, 'enrolled', ?) ''', (class_id, student_id, now))
                    cursor.execute('UPDATE classes SET current_students = current_students + 1, updated_at = ? WHERE class_id = ?', (now, class_id))
                    cursor.execute(''' INSERT OR IGNORE INTO student_profiles (user_id, education_type, status, created_at, updated_at) SELECT ?, (SELECT education_type FROM classes WHERE class_id = ?), 'enrolled', ?, ? WHERE NOT EXISTS (SELECT 1 FROM student_profiles WHERE user_id = ?) ''', (student_id, class_id, now, now, student_id))
                    cursor.execute(''' UPDATE student_profiles SET current_class_id = ?, updated_at = ? WHERE user_id = ? ''', (class_id, now, student_id))
                    conn.commit()
    ...
    def remove_student_from_class(self, class_id: str, student_id: int, reason: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE class_students SET enrollment_status = 'dropped', left_at = ?, leave_reason = ? WHERE class_id = ? AND student_id = ? ''', (now, reason, class_id, student_id))
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not from_cls or not to_cls:
                        return {'success': False, 'error': '班级不存在'}
                    cursor.execute(''' INSERT INTO transfer_records ( transfer_id, student_id, from_class_id, to_class_id, from_grade, to_grade, transfer_type, reason, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'approved', ?) ''', (transfer_id, student_id, from_class_id, to_class_id,
                          from_cls[0], to_cls[0], transfer_type, reason, now))
                    cursor.execute(''' UPDATE class_students SET enrollment_status = 'transferred', left_at = ? WHERE class_id = ? AND student_id = ? ''', (now, from_class_id, student_id))
                    cursor.execute('UPDATE classes SET current_students = current_students - 1, updated_at = ? WHERE class_id = ?', (now, from_class_id))
                    cursor.execute(''' INSERT INTO class_students (class_id, student_id, enrollment_status, enrolled_at) VALUES (?, ?, 'enrolled', ?) ''', (to_class_id, student_id, now))
                    cursor.execute('UPDATE classes SET current_students = current_students + 1, updated_at = ? WHERE class_id = ?', (now, to_class_id))
                    cursor.execute(''' UPDATE student_profiles SET current_class_id = ?, grade_level = ?, updated_at = ? WHERE user_id = ? ''', (to_class_id, to_cls[0], now, student_id))
                    conn.commit()
    ...
    def get_student_profile(self, user_id: int) -> Optional[Dict[str, Any]]:
        try:
            with self._get_connection() as conn:
                ...
                if row:
                    profile = dict(row)
                    cursor.execute(''' SELECT c.* FROM classes c JOIN class_students cs ON c.class_id = cs.class_id WHERE cs.student_id = ? AND cs.enrollment_status = 'enrolled' ''', (user_id,))
    ...
    def update_student_profile(self, user_id: int, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(f''' UPDATE student_profiles SET {", ".join(fields)} WHERE user_id = ? ''', params)
                    if cursor.rowcount == 0:
                        cursor.execute(''' INSERT INTO student_profiles (user_id, created_at, updated_at) VALUES (?, ?, ?) ''', (user_id, now, now))
                        cursor.execute(f''' UPDATE student_profiles SET {", ".join(fields)} WHERE user_id = ? ''', params)
                    conn.commit()
    ...
    def record_attendance(self, class_id: str, student_id: int, record_date: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR REPLACE INTO attendance_records (class_id, student_id, record_date, status, remark, recorded_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (class_id, student_id, record_date, status, remark, recorded_by, now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_class_statistics(self, class_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not cls:
                    return {'success': False, 'error': '班级不存在'}
                cursor.execute(''' SELECT enrollment_status, COUNT(*) as cnt FROM class_students WHERE class_id = ? GROUP BY enrollment_status ''', (class_id,))
                ...
                return {
                    ...
                    'occupancy_rate': round(cls['current_students'] / cls['max_students'] * 100,
                    2) if cls['max_students'] > 0 else 0
                }
        except Exception as e:
    ...
    def add_teacher_to_class(self, class_id: str, teacher_id: int, subject: str = None,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR IGNORE INTO class_teachers (class_id, teacher_id, subject, role, assigned_at) VALUES (?, ?, ?, ?, ?) ''', (class_id, teacher_id, subject, role, now))
                    conn.commit()
    ...
    def promote_grade(self, education_type: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT class_id, grade_level FROM classes WHERE education_type = ? AND status = 'active' ''', (education_type,))
                    classes = cursor.fetchall()
                    for cls_id, grade in classes:
                        if grade and grade < 12:
                            cursor.execute(''' UPDATE classes SET grade_level = grade_level + 1, updated_at = ? WHERE class_id = ? ''', (now, cls_id))
                            promoted += 1
                    cursor.execute(''' UPDATE student_profiles SET grade_level = grade_level + 1, updated_at = ? WHERE education_type = ? AND status = 'enrolled' AND grade_level < 12 ''', (now, education_type))
                    conn.commit()
    ...
    def graduate_class(self, class_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE class_students SET enrollment_status = 'graduated', left_at = ? WHERE class_id = ? AND enrollment_status = 'enrolled' ''', (now, class_id))
                    cursor.execute(''' UPDATE classes SET status = 'completed', current_students = 0, updated_at = ? WHERE class_id = ? ''', (now, class_id))
                    cursor.execute(''' UPDATE student_profiles SET status = 'graduated', graduation_date = ?, updated_at = ? WHERE current_class_id = ? ''', (now, now, class_id))
                    conn.commit()
    ...
    def get_transfer_history(self, student_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT * FROM transfer_records WHERE student_id = ? ORDER BY created_at DESC ''', (student_id,))
    ...
    def list_student_classes(self, student_id: int, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT c.*, cs.enrollment_status, cs.enrolled_at FROM classes c JOIN class_students cs ON c.class_id = cs.class_id WHERE cs.student_id = ? '''
                params = [student_id]
                if education_type:
