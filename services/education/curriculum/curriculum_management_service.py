...
class CurriculumManagementService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS courses ( course_id TEXT PRIMARY KEY, course_name TEXT NOT NULL, course_code TEXT UNIQUE, education_type TEXT NOT NULL, subject TEXT, category TEXT, course_type TEXT DEFAULT 'compulsory', grade_level INTEGER, description TEXT, objectives TEXT, credit REAL DEFAULT 0, total_hours INTEGER DEFAULT 36, theory_hours INTEGER DEFAULT 30, practice_hours INTEGER DEFAULT 6, semester TEXT, prerequisite TEXT, status TEXT DEFAULT 'draft', created_by INTEGER, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_syllabus ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, chapter TEXT NOT NULL, chapter_title TEXT NOT NULL, section TEXT, section_title TEXT, knowledge_points TEXT, teaching_objectives TEXT, teaching_methods TEXT, estimated_hours INTEGER DEFAULT 2, difficulty INTEGER DEFAULT 3, sort_order INTEGER DEFAULT 0, UNIQUE(course_id, chapter, section) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS teaching_plans ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, plan_type TEXT DEFAULT 'semester', course_id TEXT NOT NULL, teacher_id INTEGER, class_id TEXT, education_type TEXT, semester TEXT, start_date TEXT, end_date TEXT, total_weeks INTEGER DEFAULT 18, weekly_hours INTEGER DEFAULT 2, objectives TEXT, content TEXT, assessment_method TEXT, status TEXT DEFAULT 'draft', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS plan_items ( id INTEGER PRIMARY KEY AUTOINCREMENT, plan_id TEXT NOT NULL, week_number INTEGER, session_number INTEGER, date TEXT, topic TEXT NOT NULL, content TEXT, knowledge_points TEXT, teaching_methods TEXT, materials TEXT, homework TEXT, estimated_hours INTEGER DEFAULT 2, status TEXT DEFAULT 'not_started', completed_at TEXT, remark TEXT, UNIQUE(plan_id, week_number, session_number) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS schedules ( schedule_id TEXT PRIMARY KEY, class_id TEXT NOT NULL, course_id TEXT, teacher_id INTEGER, weekday INTEGER NOT NULL, time_slot INTEGER NOT NULL, subject TEXT, classroom TEXT, semester TEXT, is_active INTEGER DEFAULT 1, note TEXT, created_at TEXT, updated_at TEXT, UNIQUE(class_id, weekday, time_slot, semester) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS teaching_logs ( id INTEGER PRIMARY KEY AUTOINCREMENT, plan_id TEXT, plan_item_id TEXT, course_id TEXT, teacher_id INTEGER, class_id TEXT, date TEXT NOT NULL, topic TEXT, content TEXT, actual_hours REAL, attendance_count INTEGER, total_students INTEGER, method_used TEXT, effectiveness TEXT, issues TEXT, improvements TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_teachers ( id INTEGER PRIMARY KEY AUTOINCREMENT, course_id TEXT NOT NULL, teacher_id INTEGER NOT NULL, role TEXT DEFAULT 'primary', assigned_at TEXT, UNIQUE(course_id, teacher_id) ) ''')
                conn.commit()
    ...
    def create_course(self, course_name: str, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO courses ( course_id, course_name, course_code, education_type, subject, category, course_type, grade_level, description, objectives, credit, total_hours, theory_hours, practice_hours, semester, prerequisite, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def add_syllabus_item(self, course_id: str, chapter: str, chapter_title: str,
        ...
        try:
            ...
            objectives = json.dumps(kwargs.get('teaching_objectives'),
            ensure_ascii=False) if kwargs.get('teaching_objectives') else None
            methods = json.dumps(kwargs.get('teaching_methods'),
            ensure_ascii=False) if kwargs.get('teaching_methods') else None
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR REPLACE INTO course_syllabus ( course_id, chapter, chapter_title, section, section_title, knowledge_points, teaching_objectives, teaching_methods, estimated_hours, difficulty, sort_order ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (course_id, chapter, chapter_title,
    ...
    def create_teaching_plan(self, plan_name: str, course_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO teaching_plans ( plan_id, plan_name, plan_type, course_id, teacher_id, class_id, education_type, semester, start_date, end_date, total_weeks, weekly_hours, objectives, content, assessment_method, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (plan_id, plan_name, plan_type, course_id,
    ...
    def add_plan_item(self, plan_id: str, week_number: int, topic: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR REPLACE INTO plan_items ( plan_id, week_number, session_number, date, topic, content, knowledge_points, teaching_methods, materials, homework, estimated_hours, status ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (plan_id, week_number, kwargs.get('session_number', 1),
    ...
    def update_plan_item_status(self, plan_id: str, week_number: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE plan_items SET status = ?, completed_at = ?, remark = ? WHERE plan_id = ? AND week_number = ? AND session_number = ? ''', (status, now if status == 'completed' else None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                if row:
                    ...
                    cursor.execute('SELECT * FROM plan_items WHERE plan_id = ? ORDER BY week_number, session_number',
                    (plan_id,))
    ...
    def add_schedule(self, class_id: str, weekday: int, time_slot: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT schedule_id FROM schedules WHERE class_id = ? AND weekday = ? AND time_slot = ? AND semester = ? AND is_active = 1 ''', (class_id, weekday, time_slot, kwargs.get('semester')))
                    if cursor.fetchone():
                        return {'success': False, 'error': '该时段已有课程安排'}
                    if kwargs.get('teacher_id'):
                        cursor.execute(''' SELECT schedule_id FROM schedules WHERE teacher_id = ? AND weekday = ? AND time_slot = ? AND semester = ? AND is_active = 1 ''', (kwargs['teacher_id'], weekday, time_slot, kwargs.get('semester')))
                        if cursor.fetchone():
                            return {'success': False, 'error': '教师该时段已有课程'}
                    cursor.execute(''' INSERT INTO schedules ( schedule_id, class_id, course_id, teacher_id, weekday, time_slot, subject, classroom, semester, is_active, note, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?) ''', (schedule_id, class_id, kwargs.get('course_id'),
    ...
    def log_teaching(self, teacher_id: int, date: str, topic: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO teaching_logs ( plan_id, plan_item_id, course_id, teacher_id, class_id, date, topic, content, actual_hours, attendance_count, total_students, method_used, effectiveness, issues, improvements, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (kwargs.get('plan_id'), kwargs.get('plan_item_id'),
    ...
    def assign_teacher(self, course_id: str, teacher_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR IGNORE INTO course_teachers (course_id, teacher_id, role, assigned_at) VALUES (?, ?, ?, ?) ''', (course_id, teacher_id, role, now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
