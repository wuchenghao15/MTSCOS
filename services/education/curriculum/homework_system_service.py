...
class HomeworkSystemService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS homeworks ( homework_id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT, education_type TEXT NOT NULL, subject TEXT, class_id TEXT, teacher_id INTEGER, homework_type TEXT DEFAULT 'daily', difficulty INTEGER DEFAULT 3, total_score REAL DEFAULT 100, grading_method TEXT DEFAULT 'manual', status TEXT DEFAULT 'draft', publish_time TEXT, deadline TEXT, late_allowed INTEGER DEFAULT 1, late_penalty REAL DEFAULT 0.1, resubmit_allowed INTEGER DEFAULT 0, max_resubmit INTEGER DEFAULT 1, question_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS homework_questions ( id INTEGER PRIMARY KEY AUTOINCREMENT, homework_id TEXT NOT NULL, question_id TEXT, question_type TEXT NOT NULL, question_text TEXT NOT NULL, options TEXT, correct_answer TEXT, score REAL DEFAULT 10, difficulty INTEGER DEFAULT 3, sort_order INTEGER DEFAULT 0, knowledge_points TEXT, explanation TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS homework_submissions ( submission_id TEXT PRIMARY KEY, homework_id TEXT NOT NULL, student_id INTEGER NOT NULL, answers TEXT, score REAL, grade_status TEXT DEFAULT 'not_submitted', submitted_at TEXT, first_submitted_at TEXT, graded_at TEXT, graded_by INTEGER, feedback TEXT, comment TEXT, attempt_count INTEGER DEFAULT 0, is_late INTEGER DEFAULT 0, auto_score REAL, manual_score REAL ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS practice_questions ( question_id TEXT PRIMARY KEY, education_type TEXT NOT NULL, subject TEXT NOT NULL, question_type TEXT NOT NULL, question_text TEXT NOT NULL, options TEXT, correct_answer TEXT, difficulty INTEGER DEFAULT 3, chapter TEXT, knowledge_points TEXT, explanation TEXT, tags TEXT, source TEXT, used_count INTEGER DEFAULT 0, correct_count INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS practice_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, question_id TEXT NOT NULL, user_answer TEXT, is_correct INTEGER, score REAL, time_spent_seconds INTEGER, practice_mode TEXT, subject TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS wrong_questions ( id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER NOT NULL, question_id TEXT NOT NULL, subject TEXT, question_type TEXT, wrong_count INTEGER DEFAULT 1, last_wrong_at TEXT, mastered INTEGER DEFAULT 0, notes TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS homework_stats ( id INTEGER PRIMARY KEY AUTOINCREMENT, homework_id TEXT NOT NULL UNIQUE, total_students INTEGER DEFAULT 0, submitted_count INTEGER DEFAULT 0, graded_count INTEGER DEFAULT 0, average_score REAL DEFAULT 0, highest_score REAL DEFAULT 0, lowest_score REAL DEFAULT 0, pass_rate REAL DEFAULT 0, completion_rate REAL DEFAULT 0, updated_at TEXT ) ''')
                conn.commit()
    ...
    def create_homework(self, title: str, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO homeworks ( homework_id, title, description, education_type, subject, class_id, teacher_id, homework_type, difficulty, total_score, grading_method, status, publish_time, deadline, late_allowed, late_penalty, resubmit_allowed, max_resubmit, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO homework_questions ( homework_id, question_id, question_type, question_text, options, correct_answer, score, difficulty, sort_order, knowledge_points, explanation ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
                    ...
                    cursor.execute(''' UPDATE homeworks SET question_count = question_count + 1, updated_at = ? WHERE homework_id = ? ''', (datetime.now().isoformat(), homework_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def publish_homework(self, homework_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE homeworks SET status = 'published', publish_time = ?, updated_at = ? WHERE homework_id = ? AND status = 'draft' ''', (now, now, homework_id))
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if deadline and now > deadline:
                        ...
                        is_late = 1
                    cursor.execute('SELECT submission_id, attempt_count FROM homework_submissions WHERE homework_id = ? AND student_id = ?', (homework_id,
                    student_id))
                    existing = cursor.fetchone()
                    if existing:
                        if not resubmit_allowed or existing[1] >= max_resubmit:
                            return {'success': False, 'error': '不允许再次提交'}
                        cursor.execute(''' UPDATE homework_submissions SET answers = ?, submitted_at = ?, attempt_count = attempt_count + 1, grade_status = 'submitted', is_late = ? WHERE submission_id = ? ''', (json.dumps(answers, ensure_ascii=False), now, is_late, existing[0]))
                        submission_id = existing[0]
                    else:
                        cursor.execute(''' INSERT INTO homework_submissions ( submission_id, homework_id, student_id, answers, grade_status, submitted_at, first_submitted_at, attempt_count, is_late ) VALUES (?, ?, ?, ?, 'submitted', ?, ?, 1, ?) ''', (submission_id, homework_id, student_id, json.dumps(answers, ensure_ascii=False),
                              now, now, is_late))
                    auto_score = None
                    if grading_method in ('auto', 'semi_auto'):
                        auto_score = self._auto_grade(homework_id, answers)
                        cursor.execute('UPDATE homework_submissions SET auto_score = ? WHERE submission_id = ?',
                        (auto_score, submission_id))
                        if grading_method == 'auto':
                            cursor.execute(''' UPDATE homework_submissions SET grade_status = 'graded', score = auto_score, graded_at = ? WHERE submission_id = ? ''', (now, submission_id))
                    conn.commit()
    ...
    def _auto_grade(self, homework_id: str, answers: dict) -> float:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' SELECT id, question_type, correct_answer, score FROM homework_questions WHERE homework_id = ? ORDER BY sort_order, id ''', (homework_id,))
    ...
    def grade_homework(self, submission_id: str, score: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE homework_submissions SET grade_status = 'graded', score = ?, manual_score = ?, graded_at = ?, graded_by = ?, feedback = ?, comment = ? WHERE submission_id = ? ''', (score, score, now, graded_by, feedback, comment, submission_id))
                    conn.commit()
    ...
    def get_homework(self, homework_id: str) -> Optional[Dict[str, Any]]:
        try:
            with self._get_connection() as conn:
                ...
                if row:
                    hw = dict(row)
                    cursor.execute('SELECT * FROM homework_questions WHERE homework_id = ? ORDER BY sort_order, id',
                    (homework_id,))
    ...
    def get_homework_submissions(self, homework_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT hs.* FROM homework_submissions hs WHERE hs.homework_id = ? ORDER BY hs.submitted_at DESC ''', (homework_id,))
    ...
    def get_student_homework_status(self, homework_id: str, student_id: int) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT * FROM homework_submissions WHERE homework_id = ? AND student_id = ? ''', (homework_id, student_id))
                row = cursor.fetchone()
                if row:
    ...
    def add_practice_question(self, education_type: str, subject: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO practice_questions ( question_id, education_type, subject, question_type, question_text, options, correct_answer, difficulty, chapter, knowledge_points, explanation, tags, source, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (
    ...
    def record_practice(self, student_id: int, question_id: str, user_answer: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO practice_records ( student_id, question_id, user_answer, is_correct, score, time_spent_seconds, practice_mode, subject, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (student_id, question_id, user_answer, 1 if is_correct else 0,
                          score, time_spent, practice_mode, subject, now))
                    if is_correct:
                        cursor.execute(''' UPDATE practice_questions SET used_count = used_count + 1, correct_count = correct_count + 1 WHERE question_id = ? ''', (question_id,))
                    else:
                        cursor.execute(''' UPDATE practice_questions SET used_count = used_count + 1 WHERE question_id = ? ''', (question_id,))
                        cursor.execute(''' INSERT INTO wrong_questions (student_id, question_id, subject, question_type, wrong_count, last_wrong_at) VALUES (?, ?, ?, (SELECT question_type FROM practice_questions WHERE question_id = ?), 1, ?) ON CONFLICT(student_id, question_id) DO UPDATE SET wrong_count = wrong_count + 1, last_wrong_at = ?, mastered = 0 ''', (student_id, question_id, subject, question_id, now, now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_wrong_questions(self, student_id: int, subject: str = None,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT wq.*, pq.question_text, pq.question_type, pq.difficulty, pq.chapter FROM wrong_questions wq LEFT JOIN practice_questions pq ON wq.question_id = pq.question_id WHERE wq.student_id = ? AND wq.mastered = ? '''
    ...
    def get_homework_stats(self, homework_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT COUNT(*) as total, SUM(CASE WHEN grade_status IN ('submitted', 'graded') THEN 1 ELSE 0 END) as submitted, SUM(CASE WHEN grade_status = 'graded' THEN 1 ELSE 0 END) as graded, AVG(CASE WHEN grade_status = 'graded' THEN score END) as avg_score, MAX(CASE WHEN grade_status = 'graded' THEN score END) as max_score, MIN(CASE WHEN grade_status = 'graded' THEN score END) as min_score FROM homework_submissions WHERE homework_id = ? ''', (homework_id,))
                s = cursor.fetchone()
                return {
                    'success': True,
                    'stats': {
    ...
    def get_student_practice_stats(self, student_id: int, days: int = 30) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT COUNT(*) as total_questions, SUM(CASE WHEN is_correct = 1 THEN 1 ELSE 0 END) as correct_count, AVG(score) as avg_score, SUM(time_spent_seconds) as total_time, COUNT(DISTINCT DATE(created_at)) as active_days FROM practice_records WHERE student_id = ? AND created_at >= ? ''', (student_id, start_date))
                s = cursor.fetchone()
                total = s[0] or 0
                correct = s[1] or 0
                cursor.execute(''' SELECT subject, COUNT(*), SUM(is_correct) FROM practice_records WHERE student_id = ? AND created_at >= ? GROUP BY subject ''', (student_id, start_date))
                by_subject = {}
    ...
    def mark_wrong_mastered(self, student_id: int, question_id: str) -> Dict[str, Any]:
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE wrong_questions SET mastered = 1 WHERE student_id = ? AND question_id = ? ''', (student_id, question_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_student_homework_list(self, student_id: int, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT h.*, hs.grade_status, hs.score, hs.submitted_at, hs.submission_id FROM homeworks h LEFT JOIN homework_submissions hs ON h.homework_id = hs.homework_id AND hs.student_id = ? WHERE 1=1 '''
                params = [student_id]
                if education_type:
