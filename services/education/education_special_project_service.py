...
class EducationSpecialProjectService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS special_projects ( project_id TEXT PRIMARY KEY, project_name TEXT NOT NULL, project_type TEXT, education_type TEXT, specialization_level TEXT, status TEXT DEFAULT 'planning', start_date TEXT, end_date TEXT, budget REAL DEFAULT 0, funding_source TEXT, leader_id INTEGER, leader_name TEXT, description TEXT, objectives TEXT, target_participants INTEGER DEFAULT 0, actual_participants INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_details ( detail_id INTEGER PRIMARY KEY AUTOINCREMENT, project_id TEXT NOT NULL, stage TEXT, stage_start_date TEXT, stage_end_date TEXT, responsible_person TEXT, status TEXT DEFAULT 'pending', progress REAL DEFAULT 0, notes TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS curriculum_design ( curriculum_id TEXT PRIMARY KEY, curriculum_name TEXT NOT NULL, curriculum_type TEXT, education_type TEXT, project_id TEXT, subject TEXT, grade_level TEXT, credits INTEGER DEFAULT 2, duration TEXT, objectives TEXT, content TEXT, teaching_methods TEXT, assessment_methods TEXT, materials TEXT, teacher_id INTEGER, teacher_name TEXT, status TEXT DEFAULT 'draft', created_at TEXT, updated_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_learning ( learning_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, curriculum_id TEXT, education_type TEXT, start_date TEXT, end_date TEXT, description TEXT, status TEXT DEFAULT 'in_progress', created_at TEXT, updated_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id), FOREIGN KEY (curriculum_id) REFERENCES curriculum_design(curriculum_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_groups ( group_id TEXT PRIMARY KEY, learning_id TEXT NOT NULL, group_name TEXT, member_count INTEGER DEFAULT 0, leader_id INTEGER, leader_name TEXT, status TEXT DEFAULT 'active', created_at TEXT, FOREIGN KEY (learning_id) REFERENCES project_learning(learning_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_deliverables ( deliverable_id TEXT PRIMARY KEY, learning_id TEXT NOT NULL, group_id TEXT, title TEXT, description TEXT, file_url TEXT, status TEXT DEFAULT 'submitted', score REAL, feedback TEXT, created_at TEXT, updated_at TEXT, FOREIGN KEY (learning_id) REFERENCES project_learning(learning_id), FOREIGN KEY (group_id) REFERENCES learning_groups(group_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS special_activities ( activity_id TEXT PRIMARY KEY, activity_name TEXT NOT NULL, category TEXT, education_type TEXT, project_id TEXT, location TEXT, start_date TEXT, end_date TEXT, max_participants INTEGER DEFAULT 100, registered_count INTEGER DEFAULT 0, organizer TEXT, description TEXT, budget REAL DEFAULT 0, status TEXT DEFAULT 'planned', created_at TEXT, updated_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS activity_participants ( id INTEGER PRIMARY KEY AUTOINCREMENT, activity_id TEXT NOT NULL, participant_id INTEGER, participant_name TEXT, participant_type TEXT DEFAULT 'student', register_time TEXT, attended INTEGER DEFAULT 0, feedback TEXT, FOREIGN KEY (activity_id) REFERENCES special_activities(activity_id), UNIQUE(activity_id, participant_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS special_bases ( base_id TEXT PRIMARY KEY, base_name TEXT NOT NULL, base_type TEXT, education_type TEXT, location TEXT, capacity INTEGER DEFAULT 100, description TEXT, facilities TEXT, contact_person TEXT, contact_phone TEXT, cooperation_status TEXT DEFAULT 'pending', established_date TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS base_cooperations ( cooperation_id TEXT PRIMARY KEY, base_id TEXT NOT NULL, project_id TEXT, cooperation_type TEXT, start_date TEXT, end_date TEXT, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, FOREIGN KEY (base_id) REFERENCES special_bases(base_id), FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS special_clubs ( club_id TEXT PRIMARY KEY, club_name TEXT NOT NULL, club_type TEXT, education_type TEXT, project_id TEXT, description TEXT, leader_id INTEGER, leader_name TEXT, advisor_id INTEGER, advisor_name TEXT, member_count INTEGER DEFAULT 0, meeting_frequency TEXT DEFAULT 'weekly', status TEXT DEFAULT 'active', founded_date TEXT, created_at TEXT, updated_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS club_achievements ( achievement_id TEXT PRIMARY KEY, club_id TEXT NOT NULL, title TEXT, description TEXT, date TEXT, level TEXT, evidence_url TEXT, created_at TEXT, FOREIGN KEY (club_id) REFERENCES special_clubs(club_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS special_competitions ( competition_id TEXT PRIMARY KEY, competition_name TEXT NOT NULL, competition_type TEXT, education_type TEXT, project_id TEXT, organizer TEXT, location TEXT, start_date TEXT, end_date TEXT, registration_deadline TEXT, max_participants INTEGER DEFAULT 200, registered_count INTEGER DEFAULT 0, has_ranking INTEGER DEFAULT 1, description TEXT, status TEXT DEFAULT 'open', created_at TEXT, updated_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS competition_results ( result_id TEXT PRIMARY KEY, competition_id TEXT NOT NULL, participant_id INTEGER, participant_name TEXT, team_name TEXT, category TEXT, rank INTEGER, score REAL, award TEXT, certificate_no TEXT, status TEXT DEFAULT 'pending', created_at TEXT, FOREIGN KEY (competition_id) REFERENCES special_competitions(competition_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_assessments ( assessment_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, assessment_type TEXT, education_type TEXT, assessor_id INTEGER, assessor_name TEXT, assessment_date TEXT, overall_score REAL, specialization_level TEXT, status TEXT DEFAULT 'in_progress', created_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS assessment_records ( record_id INTEGER PRIMARY KEY AUTOINCREMENT, assessment_id TEXT NOT NULL, dimension TEXT, score REAL, weight REAL, comment TEXT, FOREIGN KEY (assessment_id) REFERENCES project_assessments(assessment_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_showcases ( showcase_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, showcase_name TEXT, education_type TEXT, location TEXT, start_date TEXT, end_date TEXT, description TEXT, visitor_count INTEGER DEFAULT 0, status TEXT DEFAULT 'planned', created_at TEXT, updated_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS showcase_items ( item_id TEXT PRIMARY KEY, showcase_id TEXT NOT NULL, item_type TEXT, title TEXT, description TEXT, media_url TEXT, order_num INTEGER DEFAULT 0, FOREIGN KEY (showcase_id) REFERENCES project_showcases(showcase_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS promotion_records ( promotion_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, channel TEXT, content TEXT, target_audience TEXT, start_date TEXT, end_date TEXT, reach_count INTEGER DEFAULT 0, engagement_count INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS project_archives ( archive_id TEXT PRIMARY KEY, project_id TEXT NOT NULL, archive_name TEXT, archive_type TEXT, file_url TEXT, description TEXT, archived_at TEXT, FOREIGN KEY (project_id) REFERENCES special_projects(project_id) ) ''')
                conn.commit()
    ...
    def create_special_project(self, project_name: str, project_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO special_projects ( project_id, project_name, project_type, education_type, specialization_level, status, start_date, end_date, budget, funding_source, leader_id, leader_name, description, objectives, target_participants, actual_participants, created_at, updated_at ) VALUES (?, ?, ?, ?, NULL, 'planning', ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?) ''', (project_id, project_name, project_type, education_type,
                    ...
                    for stage_key, stage_config in PROJECT_STAGES.items():
                        cursor.execute(''' INSERT INTO project_details (project_id, stage, stage_start_date, stage_end_date, status) VALUES (?, ?, NULL, NULL, 'pending') ''', (project_id, stage_key))
                    conn.commit()
    ...
    def update_project_stage(self, project_id: str, stage: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE project_details SET stage_start_date = ?, stage_end_date = ?, responsible_person = ?, status = ?, progress = ?, notes = ? WHERE project_id = ? AND stage = ? ''', (kwargs.get('stage_start_date'), kwargs.get('stage_end_date'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE special_projects SET status = ? WHERE project_id = ?',
                                 ('archived', project_id))
                    cursor.execute(''' INSERT INTO project_archives (archive_id, project_id, archive_name, archive_type, description, archived_at) VALUES (?, ?, ?, 'project', ?, ?) ''', (archive_id, project_id, project[0], kwargs.get('description', '项目归档'), now))
                    conn.commit()
    ...
    def create_curriculum(self, curriculum_name: str, curriculum_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO curriculum_design ( curriculum_id, curriculum_name, curriculum_type, education_type, project_id, subject, grade_level, credits, duration, objectives, content, teaching_methods, assessment_methods, materials, teacher_id, teacher_name, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?) ''', (curriculum_id, curriculum_name, curriculum_type, education_type,
    ...
    def create_project_learning(self, project_id: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO project_learning ( learning_id, project_id, curriculum_id, education_type, start_date, end_date, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'in_progress', ?, ?) ''', (learning_id, project_id, kwargs.get('curriculum_id'), education_type,
                          kwargs.get('start_date', now[:10]), kwargs.get('end_date'),
    ...
    def create_learning_group(self, learning_id: str, group_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_groups ( group_id, learning_id, group_name, member_count, leader_id, leader_name, status, created_at ) VALUES (?, ?, ?, 0, ?, ?, 'active', ?) ''', (group_id, learning_id, group_name, kwargs.get('leader_id'),
    ...
    def submit_deliverable(self, learning_id: str, group_id: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO project_deliverables ( deliverable_id, learning_id, group_id, title, description, file_url, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'submitted', ?, ?) ''', (deliverable_id, learning_id, group_id, title,
    ...
    def create_special_activity(self, activity_name: str, category: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO special_activities ( activity_id, activity_name, category, education_type, project_id, location, start_date, end_date, max_participants, registered_count, organizer, description, budget, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, 'planned', ?, ?) ''', (activity_id, activity_name, category, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR IGNORE INTO activity_participants (activity_id, participant_id, participant_name, participant_type, register_time) VALUES (?, ?, ?, ?, ?) ''', (activity_id, participant_id, participant_name,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'UPDATE activity_participants SET attended = ? WHERE activity_id = ? AND participant_id = ?',
    ...
    def create_special_base(self, base_name: str, base_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO special_bases ( base_id, base_name, base_type, education_type, location, capacity, description, facilities, contact_person, contact_phone, cooperation_status, established_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, 'active', ?, ?) ''', (base_id, base_name, base_type, education_type,
    ...
    def establish_cooperation(self, base_id: str, project_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO base_cooperations ( cooperation_id, base_id, project_id, cooperation_type, start_date, end_date, description, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?) ''', (cooperation_id, base_id, project_id, cooperation_type,
                          kwargs.get('start_date', now[:10]), kwargs.get('end_date'),
    ...
    def create_special_club(self, club_name: str, club_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO special_clubs ( club_id, club_name, club_type, education_type, project_id, description, leader_id, leader_name, advisor_id, advisor_name, member_count, meeting_frequency, status, founded_date, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'active', ?, ?, ?) ''', (club_id, club_name, club_type, education_type,
    ...
    def record_club_achievement(self, club_id: str, title: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO club_achievements ( achievement_id, club_id, title, description, date, level, evidence_url, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (achievement_id, club_id, title, kwargs.get('description'),
    ...
    def create_competition(self, competition_name: str, competition_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO special_competitions ( competition_id, competition_name, competition_type, education_type, project_id, organizer, location, start_date, end_date, registration_deadline, max_participants, registered_count, has_ranking, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'open', ?, ?) ''', (competition_id, competition_name, competition_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(
                    'SELECT result_id FROM competition_results WHERE competition_id = ? AND participant_id = ?',
                    (competition_id, participant_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已报名该竞赛'}
                    result_id = f"crs_{uuid.uuid4().hex[:12]}"
                    cursor.execute(''' INSERT INTO competition_results (result_id, competition_id, participant_id, participant_name, team_name, category, status) VALUES (?, ?, ?, ?, ?, ?, 'pending') ''', (result_id, competition_id, participant_id, participant_name,
    ...
    def record_competition_result(self, result_id: str, score: float, **kwargs) -> Dict[str, Any]:
        try:
            now = datetime.now().isoformat()
            certificate_no = f"CC{datetime.now().strftime('%Y%m%d')}{uuid.uuid4().hex[:6].upper()}" if kwargs.get(
            'award') else None
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE competition_results SET score = ?, rank = ?, award = ?, certificate_no = ?, status = ? WHERE result_id = ? ''', (score, kwargs.get('rank'), kwargs.get('award'), certificate_no, 'completed', result_id))
                    if cursor.rowcount > 0:
                        conn.commit()
    ...
    def get_competition_ranking(self, competition_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT participant_name, team_name, score, rank, award FROM competition_results WHERE competition_id = ? AND status = 'completed' ORDER BY score DESC ''', (competition_id,))
    ...
    def create_assessment(self, project_id: str, assessment_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO project_assessments ( assessment_id, project_id, assessment_type, education_type, assessor_id, assessor_name, assessment_date, overall_score, specialization_level, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, NULL, NULL, 'in_progress', ?) ''', (assessment_id, project_id, assessment_type, education_type,
                    ...
                    for dim_key, dim_config in ASSESSMENT_DIMENSIONS.items():
                        cursor.execute(''' INSERT INTO assessment_records (assessment_id, dimension, score, weight) VALUES (?, ?, NULL, ?) ''', (assessment_id, dim_key, dim_config.get('weight', 0)))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute('SELECT SUM( score * weight) as total FROM assessment_records WHERE assessment_id = ? AND score IS NOT NULL',
                        (assessment_id,))
                        total = cursor.fetchone()[0]
                        if total:
                            overall_score = round(total / sum(d.get('weight',
                            0) for d in ASSESSMENT_DIMENSIONS.values()), 2)
                            level = 'school'
    ...
    def complete_assessment(self, assessment_id: str) -> Dict[str, Any]:
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT overall_score, project_id FROM project_assessments WHERE assessment_id = ?',
                    (assessment_id,))
    ...
    def get_assessment_report(self, project_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT a.*, ar.dimension, ar.score, ar.weight, ar.comment FROM project_assessments a LEFT JOIN assessment_records ar ON a.assessment_id = ar.assessment_id WHERE a.project_id = ? ORDER BY a.created_at DESC, ar.dimension ''', (project_id,))
                ...
                for row in rows:
                    if current_assessment != row['assessment_id']:
                        ...
                        assessment_data = {k: row[k] for k in ['assessment_id', 'project_id', 'assessment_type',
                        'assessor_name', 'assessment_date', 'overall_score', 'specialization_level', 'status']}
                        dimensions = []
                    dimensions.append({'dimension': row['dimension'], 'score': row['score'], 'weight': row['weight'],
                    'comment': row['comment']})
                reports.append({'assessment': dict(assessment_data), 'dimensions': dimensions})
    ...
    def create_showcase(self, project_id: str, showcase_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO project_showcases ( showcase_id, project_id, showcase_name, education_type, location, start_date, end_date, description, visitor_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 'planned', ?, ?) ''', (showcase_id, project_id, showcase_name, education_type,
    ...
    def add_showcase_item(self, showcase_id: str, item_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO showcase_items ( item_id, showcase_id, item_type, title, description, media_url, order_num ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (item_id, showcase_id, item_type, title,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'UPDATE project_showcases SET visitor_count = visitor_count + ? WHERE showcase_id = ?',
                                 (count, showcase_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
    ...
    def create_promotion(self, project_id: str, channel: str, content: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO promotion_records ( promotion_id, project_id, channel, content, target_audience, start_date, end_date, reach_count, engagement_count, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, 'active', ?) ''', (promotion_id, project_id, channel, content,
    ...
    def get_promotion_effect(self, project_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT channel, SUM(reach_count) as total_reach, SUM(engagement_count) as total_engagement FROM promotion_records WHERE project_id = ? GROUP BY channel ''', (project_id,))
