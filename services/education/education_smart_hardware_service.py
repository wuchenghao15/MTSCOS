    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_devices ( device_id TEXT PRIMARY KEY, device_name TEXT NOT NULL, device_type TEXT NOT NULL, category TEXT, education_type TEXT, manufacturer TEXT, model TEXT, serial_number TEXT UNIQUE, firmware_version TEXT, ip_address TEXT, mac_address TEXT UNIQUE, location TEXT, status TEXT DEFAULT 'offline', last_online TEXT, metadata TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_registry ( id INTEGER PRIMARY KEY AUTOINCREMENT, device_id TEXT NOT NULL, registered_by INTEGER, registered_name TEXT, registration_date TEXT, classroom_id TEXT, assigned_user_id INTEGER, assigned_user_name TEXT, status TEXT DEFAULT 'registered', FOREIGN KEY(device_id) REFERENCES smart_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_status ( id INTEGER PRIMARY KEY AUTOINCREMENT, device_id TEXT NOT NULL, status TEXT NOT NULL, battery_level INTEGER, signal_strength INTEGER, temperature REAL, cpu_usage REAL, memory_usage REAL, disk_usage REAL, uptime INTEGER, timestamp TEXT, FOREIGN KEY(device_id) REFERENCES smart_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS iot_connections ( connection_id TEXT PRIMARY KEY, device_id TEXT NOT NULL, protocol TEXT NOT NULL, gateway_id TEXT, connection_status TEXT DEFAULT 'disconnected', connected_at TEXT, disconnected_at TEXT, signal_strength INTEGER, latency INTEGER, data_rate INTEGER, FOREIGN KEY(device_id) REFERENCES smart_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS connection_logs ( id INTEGER PRIMARY KEY AUTOINCREMENT, connection_id TEXT NOT NULL, device_id TEXT NOT NULL, event_type TEXT NOT NULL, event_description TEXT, timestamp TEXT, FOREIGN KEY(device_id) REFERENCES smart_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS sensor_network ( network_id TEXT PRIMARY KEY, network_name TEXT NOT NULL, topology TEXT DEFAULT 'star', education_type TEXT, gateway_id TEXT, node_count INTEGER DEFAULT 0, coverage_area TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS sensor_data ( id INTEGER PRIMARY KEY AUTOINCREMENT, network_id TEXT NOT NULL, sensor_id TEXT NOT NULL, sensor_type TEXT NOT NULL, value REAL NOT NULL, unit TEXT, timestamp TEXT, location TEXT, FOREIGN KEY(network_id) REFERENCES sensor_network(network_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS smart_classrooms ( classroom_id TEXT PRIMARY KEY, classroom_name TEXT NOT NULL, education_type TEXT, school_id INTEGER, school_name TEXT, grade_level TEXT, capacity INTEGER DEFAULT 50, floor TEXT, building TEXT, has_ac INTEGER DEFAULT 0, has_air_purifier INTEGER DEFAULT 0, has_smart_board INTEGER DEFAULT 0, has_sensor_network INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS classroom_devices ( id INTEGER PRIMARY KEY AUTOINCREMENT, classroom_id TEXT NOT NULL, device_id TEXT NOT NULL, device_role TEXT, installed_at TEXT, FOREIGN KEY(classroom_id) REFERENCES smart_classrooms(classroom_id), FOREIGN KEY(device_id) REFERENCES smart_devices(device_id), UNIQUE(classroom_id, device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_maintenance ( maintenance_id TEXT PRIMARY KEY, device_id TEXT NOT NULL, maintenance_type TEXT NOT NULL, scheduled_date TEXT, priority TEXT DEFAULT 'medium', assignee_id INTEGER, assignee_name TEXT, status TEXT DEFAULT 'pending', description TEXT, FOREIGN KEY(device_id) REFERENCES smart_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS maintenance_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, maintenance_id TEXT NOT NULL, device_id TEXT NOT NULL, action TEXT NOT NULL, result TEXT, technician_id INTEGER, technician_name TEXT, start_time TEXT, end_time TEXT, notes TEXT, FOREIGN KEY(device_id) REFERENCES smart_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_updates ( update_id TEXT PRIMARY KEY, device_type TEXT NOT NULL, firmware_version TEXT NOT NULL, previous_version TEXT, release_date TEXT, update_size INTEGER, changelog TEXT, is_mandatory INTEGER DEFAULT 0, status TEXT DEFAULT 'available', education_type TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS update_history ( id INTEGER PRIMARY KEY AUTOINCREMENT, update_id TEXT NOT NULL, device_id TEXT NOT NULL, start_time TEXT, end_time TEXT, result TEXT, error_message TEXT, FOREIGN KEY(device_id) REFERENCES smart_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS data_collectors ( collector_id TEXT PRIMARY KEY, collector_name TEXT NOT NULL, data_source TEXT NOT NULL, collection_type TEXT DEFAULT 'periodic', interval INTEGER DEFAULT 60, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS collection_rules ( id INTEGER PRIMARY KEY AUTOINCREMENT, collector_id TEXT NOT NULL, rule_name TEXT NOT NULL, filter_condition TEXT, aggregation_type TEXT, retention_days INTEGER DEFAULT 30, enabled INTEGER DEFAULT 1, FOREIGN KEY(collector_id) REFERENCES data_collectors(collector_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS hardware_integrations ( integration_id TEXT PRIMARY KEY, integration_name TEXT NOT NULL, integration_level TEXT DEFAULT 'basic', provider TEXT, api_endpoint TEXT, education_type TEXT, status TEXT DEFAULT 'configured', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS integration_config ( id INTEGER PRIMARY KEY AUTOINCREMENT, integration_id TEXT NOT NULL, config_key TEXT NOT NULL, config_value TEXT, FOREIGN KEY(integration_id) REFERENCES hardware_integrations(integration_id), UNIQUE(integration_id, config_key) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_alerts ( alert_id TEXT PRIMARY KEY, device_id TEXT NOT NULL, alert_type TEXT NOT NULL, severity TEXT DEFAULT 'warning', message TEXT, status TEXT DEFAULT 'active', triggered_at TEXT, resolved_at TEXT, FOREIGN KEY(device_id) REFERENCES smart_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS alert_rules ( id INTEGER PRIMARY KEY AUTOINCREMENT, rule_name TEXT NOT NULL, device_type TEXT, alert_type TEXT NOT NULL, condition TEXT NOT NULL, severity TEXT DEFAULT 'warning', notification_enabled INTEGER DEFAULT 1, auto_resolve INTEGER DEFAULT 0, education_type TEXT, enabled INTEGER DEFAULT 1, created_at TEXT ) ''')
                conn.commit()
    ...
    def register_device(self, device_name: str, device_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO smart_devices ( device_id, device_name, device_type, category, education_type, manufacturer, model, serial_number, firmware_version, ip_address, mac_address, location, status, last_online, metadata, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'offline', NULL, ?, ?, ?) ''', (device_id, device_name, device_type,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute(''' INSERT INTO device_status (device_id, status, battery_level, signal_strength, temperature, cpu_usage, memory_usage, disk_usage, uptime, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (device_id, status, kwargs.get('battery_level'),
    ...
    def establish_connection(self, device_id: str, protocol: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO iot_connections ( connection_id, device_id, protocol, gateway_id, connection_status, connected_at, signal_strength, latency, data_rate ) VALUES (?, ?, ?, ?, 'connected', ?, ?, ?, ?) ''', (connection_id, device_id, protocol,
    ...
    def get_connection_status(self, device_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM iot_connections WHERE device_id = ? ORDER BY connected_at DESC LIMIT 1',
                (device_id,))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'SELECT connection_id FROM iot_connections WHERE device_id = ? AND connection_status = ?',
                    (device_id, 'connected'))
                    row = cursor.fetchone()
                    if row:
                        connection_id = row[0]
    ...
    def get_connection_logs(self, device_id: str, limit: int = 50) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM connection_logs WHERE device_id = ? ORDER BY timestamp DESC LIMIT ?',
                (device_id, limit))
    ...
    def create_sensor_network(self, network_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO sensor_network ( network_id, network_name, topology, education_type, gateway_id, node_count, coverage_area, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 0, ?, 'active', ?, ?) ''', (network_id, network_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO sensor_data (network_id, sensor_id, sensor_type, value, unit, timestamp, location) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (network_id, sensor_id, sensor_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if sensor_id:
                        cursor.execute(
                        'SELECT sensor_type FROM sensor_data WHERE network_id = ? AND sensor_id = ? ORDER BY timestamp  DESC LIMIT 1', (network_id, sensor_id))
                        row = cursor.fetchone()
                        if not row:
                        ...
                        cursor.execute('SELECT DISTINCT sensor_id, sensor_type FROM sensor_data WHERE network_id = ?',
                        (network_id,))
                        sensors = cursor.fetchall()
                        for sensor in sensors:
                            sid, stype = sensor
                            config = SENSOR_TYPES.get(stype, {})
                            cursor.execute(''' INSERT INTO sensor_data (network_id, sensor_id, sensor_type, value, unit, timestamp, location) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (network_id, sid, stype, kwargs.get('value', 0),
                                  config.get('unit', ''), now, kwargs.get('location')))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def create_smart_classroom(self, classroom_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO smart_classrooms ( classroom_id, classroom_name, education_type, school_id, school_name, grade_level, capacity, floor, building, has_ac, has_air_purifier, has_smart_board, has_sensor_network, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (classroom_id, classroom_name,
                          kwargs.get('education_type'),
    ...
    def get_classroom_devices(self, classroom_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT sd.*, cd.device_role, cd.installed_at FROM smart_devices sd JOIN classroom_devices cd ON sd.device_id = cd.device_id WHERE cd.classroom_id = ? ''', (classroom_id,))
    ...
    def control_classroom_device(self, classroom_id: str, device_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT * FROM classroom_devices WHERE classroom_id = ? AND device_id = ?',
                    (classroom_id, device_id))
                    ...
                    if not device:
                        return {'success': False, 'error': '设备不存在'}
                    new_status = 'online' if action in ['turn_on', 'activate'] else ('offline' if action in ['turn_off',
                    'deactivate'] else device[0])
                    cursor.execute('UPDATE smart_devices SET status = ?, updated_at = ? WHERE device_id = ?',
                    (new_status, now, device_id))
                    cursor.execute('INSERT INTO device_status (device_id, status, timestamp) VALUES (?, ?, ?)',
                    (device_id, new_status, now))
                    conn.commit()
    ...
    def create_maintenance_task(self, device_id: str, maintenance_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO device_maintenance ( maintenance_id, device_id, maintenance_type, scheduled_date, priority, assignee_id, assignee_name, status, description ) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?) ''', (maintenance_id, device_id, maintenance_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE device_maintenance SET status = ? WHERE maintenance_id = ?', ('in_progress',
                    maintenance_id))
                    cursor.execute(''' INSERT INTO maintenance_records ( maintenance_id, device_id, action, result, technician_id, technician_name, start_time, end_time, notes ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (maintenance_id, device_id, action,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    for device in devices:
                        device_id = device[0]
                        maintenance_id = f"mnt_{uuid.uuid4().hex[:12]}"
                        cursor.execute(''' INSERT INTO device_maintenance ( maintenance_id, device_id, maintenance_type, scheduled_date, priority, status, description ) VALUES (?, ?, 'preventive', ?, 'low', 'pending', '定期预防性维护') ''', (maintenance_id, device_id, scheduled_date))
                        count += 1
                    conn.commit()
    ...
    def create_data_collector(self, collector_name: str, data_source: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO data_collectors ( collector_id, collector_name, data_source, collection_type, interval, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (collector_id, collector_name, data_source,
    ...
    def add_collection_rule(self, collector_id: str, rule_name: str,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO collection_rules ( collector_id, rule_name, filter_condition, aggregation_type, retention_days, enabled ) VALUES (?, ?, ?, ?, ?, 1) ''', (collector_id, rule_name, condition,
    ...
    def create_integration(self, integration_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO hardware_integrations ( integration_id, integration_name, integration_level, provider, api_endpoint, education_type, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'configured', ?, ?) ''', (integration_id, integration_name,
                ...
                with self._get_connection() as conn:
                    ...
                    for key, value in config.items():
                        cursor.execute('INSERT OR REPLACE INTO integration_config (integration_id, config_key, config_value) VALUES (?, ?, ?)',
                                     (integration_id, key, str(value)))
                    cursor.execute('UPDATE hardware_integrations SET updated_at = ? WHERE integration_id = ?', (now,
                    integration_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_integration_status(self, integration_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT config_key, config_value FROM integration_config WHERE integration_id = ?',
                (integration_id,))
    ...
    def create_alert_rule(self, rule_name: str, alert_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO alert_rules ( rule_name, device_type, alert_type, condition, severity, notification_enabled, auto_resolve, education_type, enabled, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?) ''', (rule_name, kwargs.get('device_type'), alert_type, condition,
    ...
    def trigger_alert(self, device_id: str, alert_type: str, message: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO device_alerts ( alert_id, device_id, alert_type, severity, message, status, triggered_at ) VALUES (?, ?, ?, ?, ?, 'active', ?) ''', (alert_id, device_id, alert_type,
    ...
    def create_device_update(self, device_type: str, firmware_version: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO device_updates ( update_id, device_type, firmware_version, previous_version, release_date, update_size, changelog, is_mandatory, status, education_type ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'available', ?) ''', (update_id, device_type, firmware_version,
