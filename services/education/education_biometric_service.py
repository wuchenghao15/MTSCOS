...
BIOMETRIC_TYPES = {
    ...
    'fingerprint': {'name': '指纹识别', 'description': '基于指纹特征的身份识别', 'data_format': 'image',
    'required_resolution': '500dpi'},
    'iris': {'name': '虹膜识别', 'description': '基于虹膜特征的身份识别', 'data_format': 'image', 'required_resolution': '2048x1536'},
    'voiceprint': {'name': '声纹识别', 'description': '基于声纹特征的身份识别', 'data_format': 'audio',
    'required_sample_rate': '16kHz'},
    'palmprint': {'name': '掌纹识别', 'description': '基于掌纹特征的身份识别', 'data_format': 'image',
    'required_resolution': '500dpi'},
    ...
    'multimodal': {'name': '多模态识别', 'description': '融合多种生物特征的身份识别', 'data_format': 'mixed',
...
RECOGNITION_ACCURACY = {
    ...
    'offline': {'name': '离线识别', 'threshold': 0.75, 'sensitivity': '中', 'processing_time': '中',
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS biometric_templates ( template_id TEXT PRIMARY KEY, person_id INTEGER NOT NULL, person_name TEXT, biometric_type TEXT NOT NULL, education_type TEXT, template_hash TEXT UNIQUE, accuracy_level TEXT DEFAULT 'standard', enrollment_date TEXT, last_update TEXT, status TEXT DEFAULT 'active', expires_at TEXT, version INTEGER DEFAULT 1 ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS template_data ( id INTEGER PRIMARY KEY AUTOINCREMENT, template_id TEXT NOT NULL, data_blob BLOB, data_format TEXT, metadata TEXT, created_at TEXT, FOREIGN KEY (template_id) REFERENCES biometric_templates(template_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS recognition_records ( record_id TEXT PRIMARY KEY, template_id TEXT, person_id INTEGER, biometric_type TEXT, education_type TEXT, input_data_hash TEXT, recognition_time TEXT, accuracy_level TEXT, confidence REAL, result TEXT, matched_template_id TEXT, matched_person_id INTEGER, matched_person_name TEXT, location TEXT, device_id TEXT, scenario TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS recognition_results ( result_id TEXT PRIMARY KEY, record_id TEXT NOT NULL, person_id INTEGER, person_name TEXT, biometric_type TEXT, match_score REAL, threshold REAL, is_match INTEGER, verification_time TEXT, status TEXT, FOREIGN KEY (record_id) REFERENCES recognition_records(record_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS authentication_logs ( log_id TEXT PRIMARY KEY, person_id INTEGER, person_name TEXT, auth_method TEXT, auth_factors TEXT, education_type TEXT, auth_time TEXT, location TEXT, device_id TEXT, ip_address TEXT, success INTEGER DEFAULT 0, failure_reason TEXT, session_id TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS auth_results ( result_id TEXT PRIMARY KEY, log_id TEXT NOT NULL, person_id INTEGER, auth_method TEXT, step TEXT, step_result TEXT, overall_result TEXT, auth_time TEXT, FOREIGN KEY (log_id) REFERENCES authentication_logs(log_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS attendance_records ( record_id TEXT PRIMARY KEY, person_id INTEGER NOT NULL, person_name TEXT, education_type TEXT, attendance_type TEXT, check_in_time TEXT, check_out_time TEXT, location TEXT, device_id TEXT, biometric_type TEXT, confidence REAL, status TEXT DEFAULT 'normal', remarks TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS attendance_stats ( stat_id TEXT PRIMARY KEY, person_id INTEGER, education_type TEXT, year INTEGER, month INTEGER, total_days INTEGER DEFAULT 0, present_days INTEGER DEFAULT 0, absent_days INTEGER DEFAULT 0, late_days INTEGER DEFAULT 0, early_leave_days INTEGER DEFAULT 0, overtime_hours REAL DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS access_control ( access_id TEXT PRIMARY KEY, person_id INTEGER NOT NULL, person_name TEXT, education_type TEXT, door_id TEXT, door_name TEXT, access_level TEXT, valid_from TEXT, valid_to TEXT, allowed_days TEXT, allowed_hours TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS access_logs ( log_id TEXT PRIMARY KEY, access_id TEXT, person_id INTEGER, person_name TEXT, education_type TEXT, door_id TEXT, door_name TEXT, access_time TEXT, access_result TEXT, biometric_type TEXT, confidence REAL, reason TEXT, device_id TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS exam_proctoring ( proctor_id TEXT PRIMARY KEY, exam_id TEXT NOT NULL, exam_name TEXT, person_id INTEGER NOT NULL, person_name TEXT, education_type TEXT, start_time TEXT, end_time TEXT, seat_number TEXT, camera_feed TEXT, status TEXT DEFAULT 'in_progress', violations TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS proctoring_data ( data_id TEXT PRIMARY KEY, proctor_id TEXT NOT NULL, timestamp TEXT, data_type TEXT, data_content TEXT, is_suspicious INTEGER DEFAULT 0, FOREIGN KEY (proctor_id) REFERENCES exam_proctoring(proctor_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS biometric_devices ( device_id TEXT PRIMARY KEY, device_name TEXT, device_type TEXT, manufacturer TEXT, model TEXT, ip_address TEXT, location TEXT, education_type TEXT, status TEXT DEFAULT 'online', last_sync TEXT, installed_at TEXT, calibrated_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_registry ( id INTEGER PRIMARY KEY AUTOINCREMENT, device_id TEXT NOT NULL, person_id INTEGER, person_name TEXT, registered_at TEXT, expires_at TEXT, status TEXT DEFAULT 'active', FOREIGN KEY (device_id) REFERENCES biometric_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_policies ( policy_id TEXT PRIMARY KEY, policy_name TEXT NOT NULL, security_level TEXT, education_type TEXT, description TEXT, applies_to TEXT, enforcement_level TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS policy_config ( config_id INTEGER PRIMARY KEY AUTOINCREMENT, policy_id TEXT NOT NULL, config_key TEXT, config_value TEXT, created_at TEXT, FOREIGN KEY (policy_id) REFERENCES security_policies(policy_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS data_protection ( protection_id TEXT PRIMARY KEY, policy_id TEXT, data_type TEXT, protection_method TEXT, encryption_key_id TEXT, retention_days INTEGER, backup_frequency TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS protection_logs ( log_id TEXT PRIMARY KEY, protection_id TEXT, operation TEXT, data_id TEXT, operator TEXT, operation_time TEXT, result TEXT, details TEXT, FOREIGN KEY (protection_id) REFERENCES data_protection(protection_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS biometric_alerts ( alert_id TEXT PRIMARY KEY, alert_type TEXT, severity TEXT, education_type TEXT, description TEXT, source TEXT, location TEXT, device_id TEXT, person_id INTEGER, person_name TEXT, timestamp TEXT, status TEXT DEFAULT 'pending', acknowledged_by TEXT, acknowledged_at TEXT, resolved_at TEXT, resolution TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS alert_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, alert_id TEXT NOT NULL, action TEXT, operator TEXT, action_time TEXT, notes TEXT, FOREIGN KEY (alert_id) REFERENCES biometric_alerts(alert_id) ) ''')
                conn.commit()
    ...
    def enroll_biometric(self, person_id: int, biometric_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO biometric_templates ( template_id, person_id, person_name, biometric_type, education_type, template_hash, accuracy_level, enrollment_date, last_update, status, expires_at, version ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, 1) ''', (template_id, person_id, kwargs.get('person_name'),
                          biometric_type, education_type, template_hash,
                          accuracy_level, now, now,
                          (datetime.now() + timedelta(days=365)).isoformat()))
                    cursor.execute(''' INSERT INTO template_data (template_id, data_blob, data_format, metadata, created_at) VALUES (?, ?, ?, ?, ?) ''', (template_id, kwargs.get('data_blob'),
    ...
    def recognize(self, input_data_hash: str, biometric_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO recognition_records ( record_id, person_id, biometric_type, education_type, input_data_hash, recognition_time, accuracy_level, confidence, result, location, device_id, scenario ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (record_id, kwargs.get('person_id'), biometric_type,
                    ...
                    cursor.execute(''' INSERT INTO recognition_results ( result_id, record_id, person_id, person_name, biometric_type, match_score, threshold, is_match, verification_time, status ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (result_id, record_id, kwargs.get('person_id'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO recognition_records ( record_id, template_id, person_id, person_name, biometric_type, education_type, input_data_hash, recognition_time, accuracy_level, confidence, result, matched_template_id, matched_person_id, matched_person_name, location, device_id, scenario ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (record_id, template[0], person_id, template[1],
                    ...
                    cursor.execute(''' INSERT INTO recognition_results ( result_id, record_id, person_id, person_name, biometric_type, match_score, threshold, is_match, verification_time, status ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (result_id, record_id, person_id, template[1],
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO recognition_records ( record_id, person_id, biometric_type, education_type, input_data_hash, recognition_time, accuracy_level, confidence, result, matched_template_id, matched_person_id, matched_person_name, location, device_id, scenario ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (record_id, kwargs.get('person_id'), biometric_type,
                          education_type, input_data_hash, now,
                          kwargs.get('accuracy_level', 'standard'), confidence,
                          'match' if is_match else 'no_match',
    ...
    def authenticate(self, person_id: int, auth_method: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO authentication_logs ( log_id, person_id, person_name, auth_method, auth_factors, education_type, auth_time, location, device_id, ip_address, success, failure_reason, session_id ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, person_id, kwargs.get('person_name'),
                    ...
                    cursor.execute(''' INSERT INTO auth_results ( result_id, log_id, person_id, auth_method, step, step_result, overall_result, auth_time ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (result_id, log_id, person_id, auth_method,
                          'primary', 'success',
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    for i, result in enumerate(results):
                        ...
                        cursor.execute(''' INSERT INTO auth_results ( result_id, log_id, person_id, auth_method, step, step_result, overall_result, auth_time ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (ar_id, log_id, person_id, 'multi_factor',
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO authentication_logs ( log_id, person_id, person_name, auth_method, auth_factors, education_type, auth_time, location, device_id, ip_address, success, failure_reason, session_id ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, person_id, kwargs.get('person_name'),
                          ...
                          kwargs.get('failure_reason'), session_id))
                    result_id = f"ars_{uuid.uuid4().hex[:12]}"
                    cursor.execute(''' INSERT INTO auth_results ( result_id, log_id, person_id, auth_method, step, step_result, overall_result, auth_time ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (result_id, log_id, person_id, 'continuous',
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO authentication_logs ( log_id, person_id, person_name, auth_method, auth_factors, education_type, auth_time, location, device_id, ip_address, success, failure_reason, session_id ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, person_id, kwargs.get('person_name'),
                          ...
                          session_id))
                    result_id = f"ars_{uuid.uuid4().hex[:12]}"
                    cursor.execute(''' INSERT INTO auth_results ( result_id, log_id, person_id, auth_method, step, step_result, overall_result, auth_time ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (result_id, log_id, person_id, 'adaptive',
    ...
    def record_attendance(self, person_id: int, attendance_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO attendance_records ( record_id, person_id, person_name, education_type, attendance_type, check_in_time, check_out_time, location, device_id, biometric_type, confidence, status, remarks ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (record_id, person_id, kwargs.get('person_name'),
    ...
    def get_attendance_stats(self, person_id: int, year: int, month: int,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor = conn.cursor()
                cursor.execute(''' SELECT * FROM attendance_stats WHERE person_id = ? AND year = ? AND month = ? AND education_type = ? ''', (person_id, year, month, education_type))
    ...
    def calculate_attendance(self, person_id: int, year: int, month: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT COUNT(*) as total, SUM(CASE WHEN status = 'normal' THEN 1 ELSE 0 END) as present, SUM(CASE WHEN status = 'absent' THEN 1 ELSE 0 END) as absent, SUM(CASE WHEN status = 'late' THEN 1 ELSE 0 END) as late, SUM(CASE WHEN status = 'early_leave' THEN 1 ELSE 0 END) as early_leave FROM attendance_records WHERE person_id = ? AND education_type = ? AND strftime('%Y', check_in_time) = ? AND strftime('%m', check_in_time) = ? ''', (person_id, education_type, str(year), f'{month:02d}'))
                    result = cursor.fetchone()
                    stat_id = f"ats_{uuid.uuid4().hex[:12]}"
                    cursor.execute(''' INSERT OR REPLACE INTO attendance_stats ( stat_id, person_id, education_type, year, month, total_days, present_days, absent_days, late_days, early_leave_days, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (stat_id, person_id, education_type, year, month,
    ...
    def grant_access(self, person_id: int, door_id: str, access_level: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO access_control ( access_id, person_id, person_name, education_type, door_id, door_name, access_level, valid_from, valid_to, allowed_days, allowed_hours, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (access_id, person_id, kwargs.get('person_name'),
    ...
    def check_access(self, person_id: int, door_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' SELECT access_id, door_name, access_level, valid_from, valid_to, allowed_days, allowed_hours, status FROM access_control WHERE person_id = ? AND door_id = ? AND education_type = ? AND status = ? ''', (person_id, door_id, education_type, 'active'))
                    access = cursor.fetchone()
                    has_access = bool(access)
                    cursor.execute(''' INSERT INTO access_logs ( log_id, access_id, person_id, person_name, education_type, door_id, door_name, access_time, access_result, biometric_type, confidence, reason, device_id ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, access[0] if access else None, person_id,
    ...
    def open_door(self, door_id: str, person_id: int = None,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO access_logs ( log_id, person_id, person_name, education_type, door_id, door_name, access_time, access_result, biometric_type, confidence, reason, device_id ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, person_id, kwargs.get('person_name'),
                          education_type, door_id, kwargs.get('door_name'),
    ...
    def start_proctoring(self, exam_id: str, person_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO exam_proctoring ( proctor_id, exam_id, exam_name, person_id, person_name, education_type, start_time, end_time, seat_number, camera_feed, status, violations, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'in_progress', ?, ?, ?) ''', (proctor_id, exam_id, kwargs.get('exam_name'),
    ...
    def record_proctoring_data(self, proctor_id: str, data_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO proctoring_data ( data_id, proctor_id, timestamp, data_type, data_content, is_suspicious ) VALUES (?, ?, ?, ?, ?, ?) ''', (data_id, proctor_id, now, data_type, data_content, is_suspicious))
                    conn.commit()
    ...
    def register_device(self, device_type: str, ip_address: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO biometric_devices ( device_id, device_name, device_type, manufacturer, model, ip_address, location, education_type, status, last_sync, installed_at, calibrated_at, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'online', ?, ?, ?, ?, ?) ''', (device_id, kwargs.get('device_name'), device_type,
    ...
    def create_protection_policy(self, policy_id: str, data_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO data_protection ( protection_id, policy_id, data_type, protection_method, encryption_key_id, retention_days, backup_frequency, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (f"prt_{uuid.uuid4().hex[:12]}", policy_id, data_type,
    ...
    def encrypt_data(self, data_id: str, protection_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO protection_logs ( log_id, protection_id, operation, data_id, operator, operation_time, result, details ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, protection_id, 'encrypt', data_id,
    ...
    def audit_data_access(self, data_id: str, protection_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO protection_logs ( log_id, protection_id, operation, data_id, operator, operation_time, result, details ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, protection_id, 'audit', data_id,
                          kwargs.get('operator'), now, 'success',
                          json.dumps(kwargs.get('details', {}))))
                    conn.commit()
                    return {'success': True, 'log_id': log_id}
    ...
    def backup_data(self, protection_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO protection_logs ( log_id, protection_id, operation, data_id, operator, operation_time, result, details ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, protection_id, 'backup', kwargs.get('data_id'),
                          kwargs.get('operator'), now, 'success',
                          json.dumps(kwargs.get('details', {}))))
                    conn.commit()
                    return {'success': True, 'log_id': log_id}
    ...
    def create_security_policy(self, policy_name: str, security_level: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO security_policies ( policy_id, policy_name, security_level, education_type, description, applies_to, enforcement_level, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (policy_id, policy_name, security_level, education_type,
    ...
    def create_alert(self, alert_type: str, severity: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO biometric_alerts ( alert_id, alert_type, severity, education_type, description, source, location, device_id, person_id, person_name, timestamp, status, acknowledged_by, acknowledged_at, resolved_at, resolution ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?) ''', (alert_id, alert_type, severity, education_type,
    ...
    def get_biometric_stats(self, education_type: str = None, start_time: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f'SELECT COUNT(*) as success FROM recognition_records {query_tmpl} AND result = ?',
                [*params, 'match'])
                ...
                cursor.execute(f'SELECT COUNT(*) as success FROM authentication_logs {query_tmpl} AND success = ?',
                [*params, 1])
                ...
                cursor.execute(f'SELECT COUNT(*) as success FROM access_logs {query_tmpl} AND access_result = ?',
                [*params, 'granted'])
                ...
                cursor.execute(f'SELECT COUNT(*) as active FROM biometric_alerts {query_tmpl} AND status = ?', [*params,
                'pending'])
