...
QKD_PROTOCOLS = {
    ...
    'QSDC': {'name': '量子安全直接通信', 'type': 'direct_communication', 'security': 'theoretical',
    'bit_rate': '100bps-10Mbps'},
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS quantum_channels ( channel_id TEXT PRIMARY KEY, channel_name TEXT NOT NULL, channel_type TEXT NOT NULL, education_type TEXT, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS channel_config ( id INTEGER PRIMARY KEY AUTOINCREMENT, channel_id TEXT NOT NULL, bandwidth REAL, distance REAL, loss_rate REAL, noise_level REAL, encryption_type TEXT, error_correction TEXT, FOREIGN KEY (channel_id) REFERENCES quantum_channels(channel_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS qkd_protocols ( protocol_id TEXT PRIMARY KEY, protocol_name TEXT NOT NULL, protocol_type TEXT, education_type TEXT, security_level TEXT, bit_rate TEXT, description TEXT, is_enabled INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS protocol_params ( id INTEGER PRIMARY KEY AUTOINCREMENT, protocol_id TEXT NOT NULL, param_name TEXT, param_value TEXT, FOREIGN KEY (protocol_id) REFERENCES qkd_protocols(protocol_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS quantum_keys ( key_id TEXT PRIMARY KEY, key_value TEXT NOT NULL, key_length INTEGER DEFAULT 256, protocol_id TEXT, channel_id TEXT, education_type TEXT, status TEXT DEFAULT 'generated', generated_at TEXT, used_at TEXT, expires_at TEXT, FOREIGN KEY (protocol_id) REFERENCES qkd_protocols(protocol_id), FOREIGN KEY (channel_id) REFERENCES quantum_channels(channel_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS key_management ( id INTEGER PRIMARY KEY AUTOINCREMENT, key_id TEXT NOT NULL, action TEXT NOT NULL, operator TEXT, timestamp TEXT, details TEXT, FOREIGN KEY (key_id) REFERENCES quantum_keys(key_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS quantum_security ( security_id TEXT PRIMARY KEY, security_level TEXT NOT NULL, education_type TEXT, description TEXT, policy_name TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_policies ( id INTEGER PRIMARY KEY AUTOINCREMENT, security_id TEXT NOT NULL, policy_type TEXT, policy_content TEXT, priority INTEGER DEFAULT 1, FOREIGN KEY (security_id) REFERENCES quantum_security(security_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS quantum_authentication ( auth_id TEXT PRIMARY KEY, auth_method TEXT NOT NULL, education_type TEXT, description TEXT, security_level TEXT, is_enabled INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS auth_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, auth_id TEXT NOT NULL, user_id INTEGER, user_name TEXT, auth_time TEXT, auth_result TEXT, details TEXT, FOREIGN KEY (auth_id) REFERENCES quantum_authentication(auth_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS quantum_network ( network_id TEXT PRIMARY KEY, network_name TEXT NOT NULL, topology_type TEXT, education_type TEXT, description TEXT, node_count INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS network_topology ( id INTEGER PRIMARY KEY AUTOINCREMENT, network_id TEXT NOT NULL, node_name TEXT, node_type TEXT, position_x REAL, position_y REAL, FOREIGN KEY (network_id) REFERENCES quantum_network(network_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS qc_applications ( app_id TEXT PRIMARY KEY, app_name TEXT NOT NULL, app_scenario TEXT, education_type TEXT, description TEXT, security_level TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS application_data ( id INTEGER PRIMARY KEY AUTOINCREMENT, app_id TEXT NOT NULL, data_type TEXT, data_size INTEGER, encryption_status TEXT, transmitted_at TEXT, FOREIGN KEY (app_id) REFERENCES qc_applications(app_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS quantum_devices ( device_id TEXT PRIMARY KEY, device_name TEXT NOT NULL, device_type TEXT, education_type TEXT, manufacturer TEXT, model TEXT, status TEXT DEFAULT 'active', location TEXT, installed_at TEXT, last_maintenance TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_usage ( id INTEGER PRIMARY KEY AUTOINCREMENT, device_id TEXT NOT NULL, usage_type TEXT, start_time TEXT, end_time TEXT, usage_duration REAL, data_processed INTEGER, FOREIGN KEY (device_id) REFERENCES quantum_devices(device_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS qc_monitoring ( monitor_id TEXT PRIMARY KEY, monitor_name TEXT NOT NULL, education_type TEXT, monitored_object TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS monitoring_data ( id INTEGER PRIMARY KEY AUTOINCREMENT, monitor_id TEXT NOT NULL, metric_type TEXT, metric_value REAL, timestamp TEXT, FOREIGN KEY (monitor_id) REFERENCES qc_monitoring(monitor_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS qc_alerts ( alert_id TEXT PRIMARY KEY, alert_type TEXT NOT NULL, education_type TEXT, severity TEXT DEFAULT 'medium', status TEXT DEFAULT 'pending', description TEXT, created_at TEXT, resolved_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS alert_history ( id INTEGER PRIMARY KEY AUTOINCREMENT, alert_id TEXT NOT NULL, action TEXT, operator TEXT, timestamp TEXT, FOREIGN KEY (alert_id) REFERENCES qc_alerts(alert_id) ) ''')
                conn.commit()
    ...
    def generate_quantum_key(self, protocol_type: str, channel_id: str = None,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO quantum_keys ( key_id, key_value, key_length, protocol_id, channel_id, education_type, status, generated_at, expires_at ) VALUES (?, ?, ?, ?, ?, ?, 'generated', ?, ?) ''', (key_id, key_value[:key_length//4], key_length,
                          protocol_type, channel_id, education_type,
                          now, expires_at))
                    cursor.execute(''' INSERT INTO key_management (key_id, action, operator, timestamp, details) VALUES (?, 'generate', ?, ?, ?) ''', (key_id, kwargs.get('operator', 'system'), now,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO key_management (key_id, action, operator, timestamp, details) VALUES (?, 'distribute', ?, ?, ?) ''', (key_id, kwargs.get('operator', 'system'), now,
    ...
    def create_channel(self, channel_name: str, channel_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO quantum_channels ( channel_id, channel_name, channel_type, education_type, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'active', ?, ?) ''', (channel_id, channel_name, channel_type, education_type,
                          kwargs.get('description', channel_info.get('name', '')),
                          now, now))
                    cursor.execute(''' INSERT INTO channel_config (channel_id, bandwidth, distance, loss_rate, noise_level, encryption_type, error_correction) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (channel_id, kwargs.get('bandwidth'), kwargs.get('distance'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if updates:
                        ...
                        cursor.execute('UPDATE quantum_channels SET updated_at = ? WHERE channel_id = ?', (now,
                        channel_id))
                        conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not key:
                        return {'success': False, 'error': '密钥不存在'}
                    cursor.execute(''' INSERT INTO key_management (key_id, action, operator, timestamp, details) VALUES (?, 'store', ?, ?, ?) ''', (key_id, kwargs.get('operator', 'system'), now,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute(''' INSERT INTO key_management (key_id, action, operator, timestamp, details) VALUES (?, 'update', ?, ?, ?) ''', (key_id, kwargs.get('operator', 'system'), now,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE quantum_keys SET key_value = ?, status = ? WHERE key_id = ?',
                                  ('', 'destroyed', key_id))
                    if cursor.rowcount > 0:
                        cursor.execute(''' INSERT INTO key_management (key_id, action, operator, timestamp, details) VALUES (?, 'destroy', ?, ?, ?) ''', (key_id, kwargs.get('operator', 'system'), now,
    ...
    def create_security_policy(self, security_level: str, policy_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO quantum_security ( security_id, security_level, education_type, description, policy_name, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 1, ?, ?) ''', (security_id, security_level, education_type,
                          kwargs.get('description', level_info.get('name', '')),
                          policy_name, now, now))
                    if 'policies' in kwargs:
                        for i, policy in enumerate(kwargs['policies']):
                            cursor.execute(''' INSERT INTO security_policies (security_id, policy_type, policy_content, priority) VALUES (?, ?, ?, ?) ''', (security_id, policy.get('type'), json.dumps(policy.get('content', {})), i + 1))
                    conn.commit()
    ...
    def create_authentication(self, auth_method: str, education_type: str = 'k12',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO quantum_authentication ( auth_id, auth_method, education_type, description, security_level, is_enabled, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 1, ?, ?) ''', (auth_id, auth_method, education_type,
    ...
    def perform_authentication(self, auth_id: str, user_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT is_enabled, security_level FROM quantum_authentication WHERE auth_id = ?',
                    (auth_id,))
                    ...
                    cursor.execute(''' INSERT INTO auth_records (auth_id, user_id, user_name, auth_time, auth_result, details) VALUES (?, ?, ?, ?, ?, ?) ''', (auth_id, user_id, user_name, now, auth_result,
    ...
    def create_network(self, network_name: str, topology_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO quantum_network ( network_id, network_name, topology_type, education_type, description, node_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 0, 'active', ?, ?) ''', (network_id, network_name, topology_type, education_type,
                          kwargs.get('description', topology_info.get('name', '')),
                          now, now))
                    if 'nodes' in kwargs:
                        for node in kwargs['nodes']:
                            cursor.execute(''' INSERT INTO network_topology (network_id, node_name, node_type, position_x, position_y) VALUES (?, ?, ?, ?, ?) ''', (network_id, node.get('name'), node.get('type', 'node'),
                                  node.get('x', 0), node.get('y', 0)))
                            cursor.execute(
                            'UPDATE quantum_network SET node_count = node_count + 1 WHERE network_id = ?', (network_id,
                            ))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO network_topology (network_id, node_name, node_type, position_x, position_y) VALUES (?, ?, ?, ?, ?) ''', (network_id, node_name, kwargs.get('node_type', 'node'),
    ...
    def select_protocol(self, protocol_type: str, education_type: str = 'k12',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO qkd_protocols ( protocol_id, protocol_name, protocol_type, education_type, security_level, bit_rate, description, is_enabled, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (protocol_id, protocol_info['name'], protocol_type,
        ...
        try:
            with self._get_connection() as conn:
                ...
                test_result = {
                    ...
                    'status': 'success',
                    'latency': 0.001,
                    'throughput': float(protocol['bit_rate'].split('-')[1].replace('bps', '').replace('G',
                    '000000000').replace('M', '000000')),
                    'error_rate': 0.0001
                }
    ...
    def create_application(self, app_name: str, app_scenario: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO qc_applications ( app_id, app_name, app_scenario, education_type, description, security_level, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (app_id, app_name, app_scenario, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO application_data (app_id, data_type, data_size, encryption_status, transmitted_at) VALUES (?, ?, ?, ?, ?) ''', (app_id, 'encrypted', len(data), 'encrypted', now))
                    conn.commit()
    ...
    def transmit_data(self, app_id: str, data: str, destination: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO application_data (app_id, data_type, data_size, encryption_status, transmitted_at) VALUES (?, ?, ?, ?, ?) ''', (app_id, 'transmitted', len(data), 'transmitted', now))
                    conn.commit()
    ...
    def register_device(self, device_name: str, device_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO quantum_devices ( device_id, device_name, device_type, education_type, manufacturer, model, status, location, installed_at, last_maintenance, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?) ''', (device_id, device_name, device_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO device_usage (device_id, usage_type, start_time, end_time, usage_duration, data_processed) VALUES (?, ?, ?, ?, ?, ?) ''', (device_id, usage_type, start_time, end_time, usage_duration, data_processed))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_device_status(self, device_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not device:
                    return {'success': False, 'error': '设备不存在'}
                cursor.execute('SELECT * FROM device_usage WHERE device_id = ? ORDER BY start_time DESC LIMIT 10',
                (device_id,))
