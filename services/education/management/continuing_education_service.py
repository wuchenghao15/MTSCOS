...
class ContinuingEducationService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                # 继续教育项目
                cursor.execute('''CREATE TABLE IF NOT EXISTS ce_programs ( program_id TEXT PRIMARY KEY, program_name TEXT NOT NULL, program_type TEXT, degree_level TEXT, description TEXT, objectives TEXT, target_audience TEXT, education_type TEXT DEFAULT 'adult', duration_months INTEGER, total_credits REAL, total_hours INTEGER, tuition_fee REAL, learning_mode TEXT, enrollment_quota INTEGER, enrolled_count INTEGER DEFAULT 0, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'recruiting', partner_institution TEXT, created_at TEXT, updated_at TEXT)''')
                # 继续教育课程
                cursor.execute('''CREATE TABLE IF NOT EXISTS ce_courses ( course_id TEXT PRIMARY KEY, program_id TEXT, course_name TEXT NOT NULL, course_code TEXT, credit REAL, hours INTEGER, learning_mode TEXT, instructor_id TEXT, instructor_name TEXT, description TEXT, prerequisites TEXT, assessment_method TEXT, is_required INTEGER DEFAULT 1, max_students INTEGER DEFAULT 50, enrolled_count INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT)''')
                # 继续教育学员
                cursor.execute('''CREATE TABLE IF NOT EXISTS ce_learners ( learner_id TEXT PRIMARY KEY, user_id TEXT, name TEXT NOT NULL, gender TEXT, birth_date TEXT, id_number TEXT, phone TEXT, email TEXT, current_education_level TEXT, occupation TEXT, work_unit TEXT, program_id TEXT, enroll_date TEXT, expected_complete_date TEXT, status TEXT DEFAULT 'active', total_credits REAL DEFAULT 0, completed_credits REAL DEFAULT 0, gpa REAL DEFAULT 0, advisor_id TEXT, created_at TEXT, updated_at TEXT)''')
                # 课程选课
                cursor.execute('''CREATE TABLE IF NOT EXISTS ce_enrollments ( enrollment_id TEXT PRIMARY KEY, learner_id TEXT NOT NULL, course_id TEXT NOT NULL, program_id TEXT, enroll_date TEXT, complete_date TEXT, attendance_rate REAL, assignment_score REAL, exam_score REAL, final_score REAL, credit_earned REAL DEFAULT 0, status TEXT DEFAULT 'enrolled', created_at TEXT, updated_at TEXT)''')
                # 证书
                cursor.execute('''CREATE TABLE IF NOT EXISTS ce_certificates ( certificate_id TEXT PRIMARY KEY, learner_id TEXT NOT NULL, learner_name TEXT, program_id TEXT, program_name TEXT, certificate_type TEXT, certificate_number TEXT UNIQUE, issue_date TEXT, expiry_date TEXT, credits REAL, gpa REAL, degree_level TEXT, issued_by TEXT, verification_code TEXT, status TEXT DEFAULT 'issued', created_at TEXT, updated_at TEXT)''')
                # 合作机构
                cursor.execute('''CREATE TABLE IF NOT EXISTS ce_institutions ( institution_id TEXT PRIMARY KEY, institution_name TEXT NOT NULL, institution_type TEXT, accreditation_number TEXT, legal_person TEXT, address TEXT, contact_phone TEXT, contact_email TEXT, cooperation_type TEXT, provided_programs TEXT, rating REAL DEFAULT 0, is_approved INTEGER DEFAULT 0, cooperation_start TEXT, cooperation_end TEXT, created_at TEXT, updated_at TEXT)''')
                # 先前学习认证申请
                cursor.execute('''CREATE TABLE IF NOT EXISTS rpl_applications ( rpl_id TEXT PRIMARY KEY, learner_id TEXT NOT NULL, learner_name TEXT, rpl_type TEXT, learning_description TEXT, evidence TEXT, requested_credits REAL, assessed_credits REAL, assessor_id TEXT, assessor_name TEXT, assessment_date TEXT, assessment_result TEXT, status TEXT DEFAULT 'pending', created_at TEXT, updated_at TEXT)''')
                # 终身学习档案
                cursor.execute('''CREATE TABLE IF NOT EXISTS lifelong_portfolios ( portfolio_id TEXT PRIMARY KEY, learner_id TEXT NOT NULL, learner_name TEXT, total_learning_hours REAL DEFAULT 0, total_courses INTEGER DEFAULT 0, total_credits REAL DEFAULT 0, total_certificates INTEGER DEFAULT 0, skill_tags TEXT, learning_history TEXT, level TEXT DEFAULT 'bronze', badges TEXT, last_activity TEXT, created_at TEXT, updated_at TEXT)''')
                # 学习计划
                cursor.execute('''CREATE TABLE IF NOT EXISTS learning_plans ( plan_id TEXT PRIMARY KEY, learner_id TEXT NOT NULL, learner_name TEXT, program_id TEXT, plan_name TEXT, target_credits REAL, target_complete_date TEXT, planned_courses TEXT, completed_courses TEXT, progress_percent REAL DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT)''')
                # 考核记录
                cursor.execute('''CREATE TABLE IF NOT EXISTS ce_assessments ( assessment_id TEXT PRIMARY KEY, enrollment_id TEXT NOT NULL, learner_id TEXT, course_id TEXT, assessment_type TEXT, assessment_name TEXT, score REAL, max_score REAL DEFAULT 100, weight REAL DEFAULT 0, weighted_score REAL DEFAULT 0, assessed_date TEXT, assessed_by TEXT, created_at TEXT)''')
                # 学员进度
                cursor.execute('''CREATE TABLE IF NOT EXISTS learner_progress ( progress_id TEXT PRIMARY KEY, learner_id TEXT NOT NULL, program_id TEXT, total_courses INTEGER DEFAULT 0, completed_courses INTEGER DEFAULT 0, in_progress_courses INTEGER DEFAULT 0, total_credits REAL DEFAULT 0, earned_credits REAL DEFAULT 0, gpa REAL DEFAULT 0, completion_rate REAL DEFAULT 0, last_activity TEXT, status TEXT DEFAULT 'active', updated_at TEXT, created_at TEXT)''')
                conn.commit()
    ...
    def create_program(self, program_name: str, program_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            objectives = json.dumps(objectives, ensure_ascii=False) if isinstance(objectives, (list,
            dict)) else objectives
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('''INSERT INTO ce_programs ( program_id, program_name, program_type, degree_level, description, objectives, target_audience, education_type, duration_months, total_credits, total_hours, tuition_fee, learning_mode, enrollment_quota, enrolled_count, start_date, end_date, status, partner_institution, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?)''',
    ...
    def add_course(self, program_id: str, course_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('''INSERT INTO ce_courses ( course_id, program_id, course_name, course_code, credit, hours, learning_mode, instructor_id, instructor_name, description, prerequisites, assessment_method, is_required, max_students, enrolled_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'active', ?, ?)''',
    ...
    def register_learner(self, name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('''INSERT INTO ce_learners ( learner_id, user_id, name, gender, birth_date, id_number, phone, email, current_education_level, occupation, work_unit, program_id, enroll_date, expected_complete_date, status, total_credits, completed_credits, gpa, advisor_id, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, ?, ?, ?)''',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT 1 FROM ce_enrollments WHERE learner_id = ? AND course_id = ?', (learner_id,
                    course_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已选该课程'}
                    cursor.execute('''INSERT INTO ce_enrollments ( enrollment_id, learner_id, course_id, program_id, enroll_date, complete_date, attendance_rate, assignment_score, exam_score, final_score, credit_earned, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, NULL, 0, 0, 0, 0, 0, 'enrolled', ?, ?)''',
    ...
    def record_assessment(self, enrollment_id: str, assessment_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT learner_id, course_id FROM ce_enrollments WHERE enrollment_id = ?',
                    (enrollment_id,))
                    row = cursor.fetchone()
                    if not row:
                        return {'success': False, 'error': '选课记录不存在'}
                    learner_id, course_id = row
                    cursor.execute('''INSERT INTO ce_assessments ( assessment_id, enrollment_id, learner_id, course_id, assessment_type, assessment_name, score, max_score, weight, weighted_score, assessed_date, assessed_by, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    enr = dict(enr)
                    # 最终成绩: 出勤20% + 作业30% + 考试50%
                    final_score = (enr.get('attendance_rate') or 0) * 0.2 + (enr.get('assignment_score') or 0) * 0.3 + (
                    enr.get('exam_score') or 0) * 0.5
                    ...
                    cursor.execute('''UPDATE ce_enrollments SET final_score = ?, credit_earned = ?, complete_date = ?, status = 'completed', updated_at = ? WHERE enrollment_id = ?''',
    ...
    def get_learner_progress(self, learner_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('''SELECT COUNT(*) as total, SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) as completed, SUM(CASE WHEN status != 'completed' THEN 1 ELSE 0 END) as in_progress, SUM(credit_earned) as earned, AVG(CASE WHEN status = 'completed' THEN final_score END) as avg_score FROM ce_enrollments WHERE learner_id = ?''', (learner_id,))
                ...
                cursor.execute('''SELECT COALESCE(SUM(c.credit), 0) as total_credits FROM ce_enrollments e JOIN ce_courses c ON e.course_id = c.course_id WHERE e.learner_id = ?''', (learner_id,))
                ...
                if existing:
                    cursor.execute('''UPDATE learner_progress SET total_courses = ?, completed_courses = ?, in_progress_courses = ?, total_credits = ?, earned_credits = ?, gpa = ?, completion_rate = ?, last_activity = ?, updated_at = ? WHERE learner_id = ?''',
                    (total, completed, in_progress, total_credits, earned,
                          gpa, completion_rate, now, now, learner_id))
                else:
                    cursor.execute('''INSERT INTO learner_progress ( progress_id, learner_id, program_id, total_courses, completed_courses, in_progress_courses, total_credits, earned_credits, gpa, completion_rate, last_activity, status, updated_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)''',
    ...
    def list_enrollments(self, learner_id: str = None, page: int = 1, page_size: int = 20) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'enrollments': enrollments, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if program_id:
                        cursor.execute('SELECT program_name, degree_level FROM ce_programs WHERE program_id = ?',
                        (program_id,))
                        prog = cursor.fetchone()
                        if prog:
                            program_name = program_name or dict(prog)['program_name']
                            degree_level = degree_level or dict(prog)['degree_level']
                    cursor.execute('''INSERT INTO ce_certificates ( certificate_id, learner_id, learner_name, program_id, program_name, certificate_type, certificate_number, issue_date, expiry_date, credits, gpa, degree_level, issued_by, verification_code, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'issued', ?, ?)''',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('''INSERT INTO rpl_applications ( rpl_id, learner_id, learner_name, rpl_type, learning_description, evidence, requested_credits, assessed_credits, assessor_id, assessor_name, assessment_date, assessment_result, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, NULL, NULL, NULL, NULL, 'pending', ?, ?)''',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('''UPDATE rpl_applications SET assessed_credits = ?, assessor_id = ?, assessor_name = ?, assessment_date = ?, assessment_result = ?, status = ?, updated_at = ? WHERE rpl_id = ?''',
                    ...
                    if status == 'approved' and assessed_credits > 0:
                        ...
                        cursor.execute('SELECT portfolio_id FROM lifelong_portfolios WHERE learner_id = ?',
                        (rpl['learner_id'],))
                        pf = cursor.fetchone()
                        if pf:
    ...
    def list_rpl_applications(self, learner_id: str = None, page: int = 1, page_size: int = 20) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'applications': applications, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def create_lifelong_portfolio(self, learner_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            skill_tags = json.dumps(skill_tags, ensure_ascii=False) if isinstance(skill_tags, (list,
            dict)) else skill_tags
            learning_history = kwargs.get('learning_history', [])
            learning_history = json.dumps(learning_history, ensure_ascii=False) if isinstance(learning_history, (list,
            dict)) else learning_history
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('''INSERT INTO lifelong_portfolios ( portfolio_id, learner_id, learner_name, total_learning_hours, total_courses, total_credits, total_certificates, skill_tags, learning_history, level, badges, last_activity, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 0, 0, ?, ?, 'bronze', ?, ?, ?, ?)''',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    dict)) else skill_tags
                    learning_history = kwargs.get('learning_history')
                    learning_history = json.dumps(learning_history, ensure_ascii=False) if isinstance(learning_history,
                    (list, dict)) else learning_history
                    badges = kwargs.get('badges')
                    badges = json.dumps(badges, ensure_ascii=False) if isinstance(badges, (list, dict)) else badges
                    cursor.execute('''UPDATE lifelong_portfolios SET total_learning_hours = ?, total_courses = ?, total_credits = ?, total_certificates = ?, skill_tags = COALESCE(?, skill_tags), learning_history = COALESCE(?, learning_history), badges = COALESCE(?, badges), last_activity = ?, updated_at = ? WHERE portfolio_id = ?''', (total_hours, total_courses, total_credits, total_certificates,
    ...
    def create_learning_plan(self, learner_id: str, program_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            planned_courses = json.dumps(planned_courses, ensure_ascii=False) if isinstance(planned_courses, (list,
            dict)) else planned_courses
            completed_courses = kwargs.get('completed_courses', [])
            completed_courses = json.dumps(completed_courses, ensure_ascii=False) if isinstance(completed_courses,
            (list, dict)) else completed_courses
            with self._lock:
                with self._get_connection() as conn:
                    conn.row_factory = sqlite3.Row
                    cursor = conn.cursor()
                    ...
                    row = cursor.fetchone()
                    learner_name = dict(row)['name'] if row else kwargs.get('learner_name')
                    cursor.execute('''INSERT INTO learning_plans ( plan_id, learner_id, learner_name, program_id, plan_name, target_credits, target_complete_date, planned_courses, completed_courses, progress_percent, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'active', ?, ?)''',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    completed = json.dumps(completed, ensure_ascii=False) if isinstance(completed, (list,
                    dict)) else completed
                    progress = kwargs.get('progress_percent', plan['progress_percent'] or 0)
                    status = kwargs.get('status', plan['status'])
                    cursor.execute('''UPDATE learning_plans SET plan_name = COALESCE(?, plan_name), target_credits = COALESCE(?, target_credits), target_complete_date = COALESCE(?, target_complete_date), planned_courses = COALESCE(?, planned_courses), completed_courses = COALESCE(?, completed_courses), progress_percent = ?, status = ?, updated_at = ? WHERE plan_id = ?''',
    ...
    def register_institution(self, institution_name: str, institution_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            provided_programs = json.dumps(provided_programs, ensure_ascii=False) if isinstance(provided_programs,
            (list, dict)) else provided_programs
            config = INSTITUTION_TYPES.get(institution_type, {})
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('''INSERT INTO ce_institutions ( institution_id, institution_name, institution_type, accreditation_number, legal_person, address, contact_phone, contact_email, cooperation_type, provided_programs, rating, is_approved, cooperation_start, cooperation_end, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?)''',
    ...
    def list_institutions(self, page: int = 1, page_size: int = 20, **filters) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'institutions': institutions, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def get_statistics(self, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                # 项目数与类型分布
                stats['total_programs'] = cursor.execute(f'SELECT COUNT(*) FROM ce_programs WHERE 1=1{edu_cond}',
                params).fetchone()[0]
                ...
                stats['course_completion_rate'] = round(completed_enrollments / total_enrollments,
                4) if total_enrollments else 0
