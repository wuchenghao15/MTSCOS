    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                # 教师档案
                cursor.execute('''CREATE TABLE IF NOT EXISTS teacher_profiles ( teacher_id TEXT PRIMARY KEY, user_id TEXT, name TEXT NOT NULL, gender TEXT, birth_date TEXT, education_background TEXT, graduation_school TEXT, major TEXT, title TEXT, role TEXT, hire_date TEXT, education_type TEXT, subjects TEXT, certifications TEXT, teaching_years INTEGER DEFAULT 0, total_training_hours REAL DEFAULT 0, total_research_count INTEGER DEFAULT 0, total_awards INTEGER DEFAULT 0, ethics_score REAL DEFAULT 100, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT)''')
                # 培训计划
                cursor.execute('''CREATE TABLE IF NOT EXISTS training_programs ( program_id TEXT PRIMARY KEY, program_name TEXT NOT NULL, training_type TEXT, description TEXT, target_audience TEXT, instructor TEXT, location TEXT, start_date TEXT, end_date TEXT, total_hours REAL DEFAULT 0, credit_points REAL DEFAULT 0, max_participants INTEGER DEFAULT 50, registered_count INTEGER DEFAULT 0, status TEXT DEFAULT 'planned', created_at TEXT, updated_at TEXT)''')
                # 培训报名
                cursor.execute('''CREATE TABLE IF NOT EXISTS training_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, program_id TEXT NOT NULL, teacher_id TEXT NOT NULL, teacher_name TEXT, register_date TEXT, attendance_hours REAL DEFAULT 0, completion_status TEXT DEFAULT 'registered', score REAL, credit_earned REAL DEFAULT 0, certificate_issued INTEGER DEFAULT 0, created_at TEXT)''')
                # 培训课时
                cursor.execute('''CREATE TABLE IF NOT EXISTS training_sessions ( session_id TEXT PRIMARY KEY, program_id TEXT NOT NULL, session_name TEXT NOT NULL, session_date TEXT, start_time TEXT, end_time TEXT, duration_hours REAL DEFAULT 0, instructor TEXT, location TEXT, content TEXT, created_at TEXT)''')
                # 教研活动
                cursor.execute('''CREATE TABLE IF NOT EXISTS research_activities ( activity_id TEXT PRIMARY KEY, activity_name TEXT NOT NULL, research_type TEXT, organizer TEXT, description TEXT, location TEXT, start_date TEXT, end_date TEXT, participants_count INTEGER DEFAULT 0, outcome TEXT, created_at TEXT, updated_at TEXT)''')
                # 课题研究
                cursor.execute('''CREATE TABLE IF NOT EXISTS research_topics ( topic_id TEXT PRIMARY KEY, topic_name TEXT NOT NULL, research_type TEXT, level TEXT, leader_id TEXT, leader_name TEXT, members TEXT, start_date TEXT, end_date TEXT, budget REAL DEFAULT 0, status TEXT DEFAULT 'ongoing', findings TEXT, created_at TEXT, updated_at TEXT)''')
                # 教研成果
                cursor.execute('''CREATE TABLE IF NOT EXISTS research_achievements ( achievement_id TEXT PRIMARY KEY, topic_id TEXT, teacher_id TEXT NOT NULL, achievement_type TEXT, title TEXT NOT NULL, publication TEXT, publish_date TEXT, level TEXT, description TEXT, created_at TEXT)''')
                # 职称评定
                cursor.execute('''CREATE TABLE IF NOT EXISTS title_assessments ( assessment_id TEXT PRIMARY KEY, teacher_id TEXT NOT NULL, teacher_name TEXT, current_title TEXT, target_title TEXT, application_date TEXT, qualifications TEXT, review_status TEXT DEFAULT 'pending', review_score REAL, review_comment TEXT, reviewed_by TEXT, reviewed_at TEXT, created_at TEXT, updated_at TEXT)''')
                # 师德考核
                cursor.execute('''CREATE TABLE IF NOT EXISTS ethics_evaluations ( eval_id TEXT PRIMARY KEY, teacher_id TEXT NOT NULL, teacher_name TEXT, eval_period TEXT, eval_year INTEGER, self_score REAL DEFAULT 0, peer_score REAL DEFAULT 0, student_score REAL DEFAULT 0, supervisor_score REAL DEFAULT 0, total_score REAL DEFAULT 0, grade TEXT, evaluator TEXT, violations TEXT, created_at TEXT)''')
                # 师德违规记录
                cursor.execute('''CREATE TABLE IF NOT EXISTS ethics_violations ( violation_id TEXT PRIMARY KEY, teacher_id TEXT NOT NULL, teacher_name TEXT, violation_type TEXT, violation_date TEXT, description TEXT, severity TEXT, penalty TEXT, reporter TEXT, handled_by TEXT, status TEXT DEFAULT 'reported', created_at TEXT)''')
                # 教学竞赛
                cursor.execute('''CREATE TABLE IF NOT EXISTS teaching_competitions ( competition_id TEXT PRIMARY KEY, competition_name TEXT NOT NULL, competition_type TEXT, organizer TEXT, level TEXT, description TEXT, start_date TEXT, end_date TEXT, registration_deadline TEXT, max_participants INTEGER DEFAULT 50, registered_count INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', created_at TEXT, updated_at TEXT)''')
                # 竞赛报名
                cursor.execute('''CREATE TABLE IF NOT EXISTS competition_registrations ( id INTEGER PRIMARY KEY AUTOINCREMENT, competition_id TEXT NOT NULL, teacher_id TEXT NOT NULL, teacher_name TEXT, category TEXT, work_title TEXT, work_desc TEXT, submit_date TEXT, score REAL, award_level TEXT, award_certificate TEXT, status TEXT DEFAULT 'registered', created_at TEXT)''')
                # 导师制结对
                cursor.execute('''CREATE TABLE IF NOT EXISTS mentorships ( mentorship_id TEXT PRIMARY KEY, mentor_id TEXT NOT NULL, mentor_name TEXT, mentee_id TEXT NOT NULL, mentee_name TEXT, start_date TEXT, end_date TEXT, plan_desc TEXT, goals TEXT, progress TEXT, mentor_eval REAL, mentee_eval REAL, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT)''')
                # 培养记录
                cursor.execute('''CREATE TABLE IF NOT EXISTS mentorship_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, mentorship_id TEXT NOT NULL, record_date TEXT, activity_type TEXT, content TEXT, outcome TEXT, mentor_feedback TEXT, mentee_reflection TEXT, created_at TEXT)''')
                conn.commit()
    ...
    def register_teacher(self, name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO teacher_profiles ( teacher_id, user_id, name, gender, birth_date, education_background, graduation_school, major, title, role, hire_date, education_type, subjects, certifications, teaching_years, total_training_hours, total_research_count, total_awards, ethics_score, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, 0, 100, 'active', ?, ?) ''', (teacher_id, kwargs.get('user_id'), name,
    ...
    def create_training_program(self, program_name: str, training_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO training_programs ( program_id, program_name, training_type, description, target_audience, instructor, location, start_date, end_date, total_hours, credit_points, max_participants, registered_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?) ''', (program_id, program_name, training_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('SELECT id FROM training_registrations WHERE program_id = ? AND teacher_id = ?',
                    ...
                    cursor.execute(''' INSERT INTO training_registrations (program_id, teacher_id, teacher_name, register_date, created_at) VALUES (?, ?, ?, ?, ?) ''', (program_id, teacher_id, teacher_name, now[:10], now))
    ...
    def record_training_session(self, program_id: str, session_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO training_sessions ( session_id, program_id, session_name, session_date, start_time, end_time, duration_hours, instructor, location, content, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (session_id, program_id, session_name,
                    ...
                    if kwargs.get('duration_hours'):
                        cursor.execute('SELECT id, teacher_id FROM training_registrations WHERE program_id = ?',
                        (program_id,))
                        registrations = cursor.fetchall()
                        for reg in registrations:
                            cursor.execute(
                            'UPDATE training_registrations SET attendance_hours = attendance_hours + ? WHERE id = ?',
                                         (kwargs['duration_hours'], reg[0]))
                            cursor.execute(
                            'UPDATE teacher_profiles SET total_training_hours = total_training_hours + ? WHERE  teacher_id = ?',
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE training_registrations SET completion_status = 'completed', score = ?, credit_earned = ?, certificate_issued = ? WHERE id = ? ''', (score, credit_earned, certificate_issued, reg_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def organize_research_activity(self, activity_name: str, research_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO research_activities ( activity_id, activity_name, research_type, organizer, description, location, start_date, end_date, participants_count, outcome, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (activity_id, activity_name, research_type,
    ...
    def create_research_topic(self, topic_name: str, research_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO research_topics ( topic_id, topic_name, research_type, level, leader_id, leader_name, members, start_date, end_date, budget, status, findings, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (topic_id, topic_name, research_type, level,
                    ...
                    if kwargs.get('leader_id'):
                        cursor.execute(
                        'UPDATE teacher_profiles SET total_research_count = total_research_count + 1 WHERE teacher_id =  ?',
                                     (kwargs['leader_id'],))
                    conn.commit()
    ...
    def record_research_achievement(self, topic_id: str, teacher_id: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO research_achievements ( achievement_id, topic_id, teacher_id, achievement_type, title, publication, publish_date, level, description, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (achievement_id, topic_id, teacher_id, achievement_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO title_assessments ( assessment_id, teacher_id, teacher_name, current_title, target_title, application_date, qualifications, review_status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (assessment_id, teacher_id, teacher_name, current_title,
    ...
    def review_title_assessment(self, assessment_id: str, approved: bool,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT teacher_id, target_title FROM title_assessments WHERE assessment_id = ? AND review_status = ?', (assessment_id,
                    'pending'))
                    assessment = cursor.fetchone()
                    if not assessment:
                        return {'success': False, 'error': '评定申请不存在或已审核'}
                    cursor.execute(''' UPDATE title_assessments SET review_status = ?, review_score = ?, review_comment = ?, reviewed_by = ?, reviewed_at = ?, updated_at = ? WHERE assessment_id = ? ''', (review_status, kwargs.get('review_score'),
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not teacher:
                        return {'success': False, 'error': '教师不存在'}
                    teacher_name = teacher[0]
                    cursor.execute(''' INSERT INTO ethics_evaluations ( eval_id, teacher_id, teacher_name, eval_period, eval_year, self_score, peer_score, student_score, supervisor_score, total_score, grade, evaluator, violations, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (eval_id, teacher_id, teacher_name, eval_period, eval_year,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO ethics_violations ( violation_id, teacher_id, teacher_name, violation_type, violation_date, description, severity, penalty, reporter, handled_by, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (violation_id, teacher_id, teacher_name, violation_type,
    ...
    def organize_competition(self, competition_name: str, competition_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO teaching_competitions ( competition_id, competition_name, competition_type, organizer, level, description, start_date, end_date, registration_deadline, max_participants, registered_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?) ''', (competition_id, competition_name, competition_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(
                    'SELECT id FROM competition_registrations WHERE competition_id = ? AND teacher_id = ?',
                    (competition_id, teacher_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已报名该竞赛'}
                    cursor.execute('SELECT name FROM teacher_profiles WHERE teacher_id = ?', (teacher_id,))
                    teacher = cursor.fetchone()
                    teacher_name = teacher[0] if teacher else None
                    cursor.execute(''' INSERT INTO competition_registrations ( competition_id, teacher_id, teacher_name, category, work_title, work_desc, submit_date, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'registered', ?) ''', (competition_id, teacher_id, teacher_name,
    ...
    def score_competition(self, reg_id: int, score: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT competition_id, teacher_id FROM competition_registrations WHERE id = ?',
                    (reg_id,))
                    reg = cursor.fetchone()
                    if not reg:
                        return {'success': False, 'error': '竞赛报名记录不存在'}
                    cursor.execute(''' UPDATE competition_registrations SET score = ?, award_level = ?, award_certificate = ?, status = 'scored' WHERE id = ? ''', (score, award_level, certificate, reg_id))
                    if award_level:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO mentorships ( mentorship_id, mentor_id, mentor_name, mentee_id, mentee_name, start_date, end_date, plan_desc, goals, progress, mentor_eval, mentee_eval, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (mentorship_id, mentor_id, mentor[0], mentee_id, mentee[0],
                          kwargs.get('start_date', now[:10]),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO mentorship_records ( mentorship_id, record_date, activity_type, content, outcome, mentor_feedback, mentee_reflection, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (mentorship_id, kwargs.get('record_date', now[:10]),
    ...
    def complete_mentorship(self, mentorship_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT mentorship_id FROM mentorships WHERE mentorship_id = ? AND status = ?',
                    (mentorship_id, 'active'))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '导师制结对不存在或已结束'}
                    cursor.execute(''' UPDATE mentorships SET status = 'completed', end_date = ?, mentor_eval = ?, mentee_eval = ?, progress = ?, updated_at = ? WHERE mentorship_id = ? ''', (kwargs.get('end_date', now[:10]),
