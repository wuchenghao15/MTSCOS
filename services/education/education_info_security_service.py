    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_policies ( policy_id TEXT PRIMARY KEY, policy_name TEXT NOT NULL, security_level TEXT, education_type TEXT, description TEXT, scope TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS policy_rules ( rule_id TEXT PRIMARY KEY, policy_id TEXT NOT NULL, rule_name TEXT, rule_type TEXT, rule_content TEXT, priority INTEGER DEFAULT 1, status TEXT DEFAULT 'enabled', FOREIGN KEY (policy_id) REFERENCES security_policies(policy_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS data_classification ( classification_id TEXT PRIMARY KEY, data_name TEXT NOT NULL, data_category TEXT, sensitivity_level TEXT, owner TEXT, education_type TEXT, description TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS data_protection ( protection_id TEXT PRIMARY KEY, classification_id TEXT, protection_method TEXT, encryption_algorithm TEXT, anonymization_level TEXT, backup_frequency TEXT, retention_period INTEGER, status TEXT DEFAULT 'active', FOREIGN KEY (classification_id) REFERENCES data_classification(classification_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS network_security ( security_id TEXT PRIMARY KEY, security_type TEXT NOT NULL, device_name TEXT, device_type TEXT, ip_address TEXT, location TEXT, education_type TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_devices ( device_id TEXT PRIMARY KEY, device_name TEXT NOT NULL, device_type TEXT, vendor TEXT, model TEXT, firmware_version TEXT, ip_address TEXT, status TEXT DEFAULT 'online', last_check TEXT, FOREIGN KEY (device_id) REFERENCES network_security(security_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS access_control ( control_id TEXT PRIMARY KEY, control_type TEXT NOT NULL, authentication_method TEXT, permission_model TEXT, mfa_enabled INTEGER DEFAULT 0, sso_enabled INTEGER DEFAULT 0, education_type TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS user_permissions ( permission_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, role TEXT, permissions TEXT, education_type TEXT, granted_at TEXT, expires_at TEXT, status TEXT DEFAULT 'active' ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_monitoring ( monitoring_id TEXT PRIMARY KEY, monitoring_type TEXT NOT NULL, log_source TEXT, alert_level TEXT, threshold TEXT, education_type TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS monitoring_alerts ( alert_id TEXT PRIMARY KEY, monitoring_id TEXT, alert_type TEXT, alert_level TEXT, alert_message TEXT, source_ip TEXT, target_ip TEXT, occurred_at TEXT, status TEXT DEFAULT 'pending', acknowledged_by TEXT, acknowledged_at TEXT, FOREIGN KEY (monitoring_id) REFERENCES security_monitoring(monitoring_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS emergency_plans ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, plan_type TEXT, education_type TEXT, scope TEXT, description TEXT, procedures TEXT, responsible_person TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS incident_response ( incident_id TEXT PRIMARY KEY, plan_id TEXT, incident_type TEXT, severity TEXT, description TEXT, detected_at TEXT, response_started_at TEXT, response_completed_at TEXT, status TEXT DEFAULT 'detected', handled_by TEXT, root_cause TEXT, remediation TEXT, FOREIGN KEY (plan_id) REFERENCES emergency_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_training ( training_id TEXT PRIMARY KEY, training_name TEXT NOT NULL, training_type TEXT, education_type TEXT, target_audience TEXT, duration INTEGER, content TEXT, status TEXT DEFAULT 'planned', scheduled_date TEXT, completed_count INTEGER DEFAULT 0, total_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS training_participants ( participant_id TEXT PRIMARY KEY, training_id TEXT, user_id INTEGER, user_name TEXT, education_type TEXT, status TEXT DEFAULT 'registered', completion_status TEXT DEFAULT 'not_started', score INTEGER, completed_at TEXT, FOREIGN KEY (training_id) REFERENCES security_training(training_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_audit ( audit_id TEXT PRIMARY KEY, audit_type TEXT NOT NULL, audit_scope TEXT, education_type TEXT, status TEXT DEFAULT 'planned', started_at TEXT, completed_at TEXT, findings_count INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS audit_records ( record_id TEXT PRIMARY KEY, audit_id TEXT, finding_type TEXT, severity TEXT, description TEXT, recommendation TEXT, status TEXT DEFAULT 'open', FOREIGN KEY (audit_id) REFERENCES security_audit(audit_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS vulnerability_scan ( scan_id TEXT PRIMARY KEY, scan_name TEXT NOT NULL, scan_type TEXT, target TEXT, education_type TEXT, status TEXT DEFAULT 'scheduled', started_at TEXT, completed_at TEXT, total_vulnerabilities INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS scan_results ( result_id TEXT PRIMARY KEY, scan_id TEXT, vulnerability_id TEXT, severity TEXT, title TEXT, description TEXT, cve_id TEXT, remediation TEXT, status TEXT DEFAULT 'open', FOREIGN KEY (scan_id) REFERENCES vulnerability_scan(scan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_events ( event_id TEXT PRIMARY KEY, event_type TEXT NOT NULL, event_category TEXT, severity TEXT, description TEXT, source TEXT, target TEXT, occurred_at TEXT, status TEXT DEFAULT 'active', education_type TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS event_logs ( log_id TEXT PRIMARY KEY, event_id TEXT, log_type TEXT, log_message TEXT, timestamp TEXT, FOREIGN KEY (event_id) REFERENCES security_events(event_id) ) ''')
                conn.commit()
    ...
    def create_security_policy(self, policy_name: str, security_level: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO security_policies ( policy_id, policy_name, security_level, education_type, description, scope, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (policy_id, policy_name, security_level,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO policy_rules (rule_id, policy_id, rule_name, rule_type, rule_content, priority, status) VALUES (?, ?, ?, ?, ?, ?, 'enabled') ''', (rule_id, policy_id, rule_name, rule_type,
    ...
    def classify_data(self, data_name: str, data_category: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO data_classification ( classification_id, data_name, data_category, sensitivity_level, owner, education_type, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (classification_id, data_name, data_category,
    ...
    def configure_protection(self, classification_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT classification_id FROM data_classification WHERE classification_id = ?',
                    (classification_id,))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '数据分类不存在'}
                    cursor.execute(''' INSERT INTO data_protection ( protection_id, classification_id, protection_method, encryption_algorithm, anonymization_level, backup_frequency, retention_period, status ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active') ''', (protection_id, classification_id,
    ...
    def anonymize_data(self, classification_id: str, anonymization_level: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT sensitivity_level FROM data_classification WHERE classification_id = ?',
                    (classification_id,))
                    ...
                    cursor.execute(''' UPDATE data_protection SET anonymization_level = ?, updated_at = ? WHERE classification_id = ? ''', (anonymization_level, now, classification_id))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not data:
                        return {'success': False, 'error': '数据分类不存在'}
                    cursor.execute(''' UPDATE data_protection SET backup_frequency = ?, updated_at = ? WHERE classification_id = ? ''', (backup_type, now, classification_id))
                    conn.commit()
    ...
    def configure_network_security(self, security_type: str, device_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO network_security ( security_id, security_type, device_name, device_type, ip_address, location, education_type, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (security_id, security_type, device_name,
    ...
    def register_security_device(self, device_name: str, device_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO security_devices ( device_id, device_name, device_type, vendor, model, firmware_version, ip_address, status, last_check ) VALUES (?, ?, ?, ?, ?, ?, ?, 'online', ?) ''', (device_id, device_name, device_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO policy_rules (rule_id, policy_id, rule_name, rule_type, rule_content, priority, status) VALUES (?, ?, ?, 'firewall', ?, ?, 'enabled') ''', (rule_id, f"pol_{device_id[:8]}", rule_name,
    ...
    def configure_access_control(self, control_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO access_control ( control_id, control_type, authentication_method, permission_model, mfa_enabled, sso_enabled, education_type, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (control_id, control_type, kwargs.get('authentication_method'),
    ...
    def grant_permission(self, user_id: int, role: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO user_permissions ( permission_id, user_id, user_name, role, permissions, education_type, granted_at, expires_at, status ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active') ''', (permission_id, user_id, kwargs.get('user_name'),
    ...
    def revoke_permission(self, permission_id: str, reason: str = None) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE user_permissions SET status = ?, updated_at = ? WHERE permission_id = ?',
                    ('revoked', now, permission_id))
    ...
    def configure_monitoring(self, monitoring_type: str, log_source: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO security_monitoring ( monitoring_id, monitoring_type, log_source, alert_level, threshold, education_type, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (monitoring_id, monitoring_type, log_source,
    ...
    def trigger_alert(self, monitoring_id: str, alert_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT monitoring_id FROM security_monitoring WHERE monitoring_id = ?',
                    (monitoring_id,))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '监控配置不存在'}
                    cursor.execute(''' INSERT INTO monitoring_alerts ( alert_id, monitoring_id, alert_type, alert_level, alert_message, source_ip, target_ip, occurred_at, status ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending') ''', (alert_id, monitoring_id, alert_type, alert_level,
    ...
    def get_security_situation(self, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                threat_level = 'critical' if risk_index >= 80 else ('high' if risk_index >= 50 else (
                'medium' if risk_index >= 20 else 'low'))
                return {
                    'success': True,
    ...
    def create_emergency_plan(self, plan_name: str, plan_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO emergency_plans ( plan_id, plan_name, plan_type, education_type, scope, description, procedures, responsible_person, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (plan_id, plan_name, plan_type, kwargs.get('education_type'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO incident_response ( incident_id, plan_id, incident_type, severity, description, detected_at, response_started_at, status, handled_by ) VALUES (?, ?, ?, ?, ?, ?, ?, 'detected', ?) ''', (incident_id, plan_id, incident_type, severity,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(f'UPDATE incident_response SET {", ".join(update_fields)} WHERE incident_id = ?',
                    update_values)
                    if cursor.rowcount > 0:
                        conn.commit()
    ...
    def create_training(self, training_name: str, training_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO security_training ( training_id, training_name, training_type, education_type, target_audience, duration, content, status, scheduled_date, completed_count, total_count, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'planned', ?, 0, ?, ?, ?) ''', (training_id, training_name, training_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute(
                        'UPDATE security_training SET total_count = total_count + 1 WHERE training_id = ?',
                        (training_id,))
                        conn.commit()
    ...
    def record_training_completion(self, participant_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT training_id FROM training_participants WHERE participant_id = ?',
                    (participant_id,))
                    result = cursor.fetchone()
                    if not result:
                        return {'success': False, 'error': '参与者不存在'}
                    training_id = result[0]
                    cursor.execute(''' UPDATE training_participants SET completion_status = ?, score = ?, completed_at = ? WHERE participant_id = ? ''', ('completed', kwargs.get('score'), now, participant_id))
                    cursor.execute(
                    'UPDATE security_training SET completed_count = completed_count + 1 WHERE training_id = ?',
                    (training_id,))
                    conn.commit()
    ...
    def create_audit(self, audit_type: str, audit_scope: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO security_audit ( audit_id, audit_type, audit_scope, education_type, status, started_at, completed_at, findings_count, created_at, updated_at ) VALUES (?, ?, ?, ?, 'planned', NULL, NULL, 0, ?, ?) ''', (audit_id, audit_type, audit_scope, kwargs.get('education_type'),
                          now, now))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO audit_records ( record_id, audit_id, finding_type, severity, description, recommendation, status ) VALUES (?, ?, ?, ?, ?, ?, 'open') ''', (record_id, audit_id, finding_type, severity,
                          description, kwargs.get('recommendation')))
                    cursor.execute('UPDATE security_audit SET findings_count = findings_count + 1 WHERE audit_id = ?',
                    (audit_id,))
                    conn.commit()
    ...
    def create_vulnerability_scan(self, scan_name: str, scan_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO vulnerability_scan ( scan_id, scan_name, scan_type, target, education_type, status, started_at, completed_at, total_vulnerabilities, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'scheduled', NULL, NULL, 0, ?, ?) ''', (scan_id, scan_name, scan_type, target,
                          kwargs.get('education_type'), now, now))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO scan_results ( result_id, scan_id, vulnerability_id, severity, title, description, cve_id, remediation, status ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open') ''', (result_id, scan_id, kwargs.get('vulnerability_id', f"vul_{uuid.uuid4().hex[:8]}"),
                          severity, title, description, kwargs.get('cve_id'),
                          kwargs.get('remediation')))
                    cursor.execute(
                    'UPDATE vulnerability_scan SET total_vulnerabilities = total_vulnerabilities + 1 WHERE scan_id = ?',
                    (scan_id,))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        conn.commit()
                        cursor.execute('SELECT total_vulnerabilities FROM vulnerability_scan WHERE scan_id = ?',
                        (scan_id,))
    ...
    def get_security_statistics(self, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f'SELECT COUNT(*) FROM vulnerability_scan WHERE status = ? {query_base}',
                ['completed'] + params)
                ...
                cursor.execute(f'SELECT COUNT(*) FROM security_audit WHERE status = ? {query_base}',
                ['completed'] + params)
                ...
                cursor.execute(f'SELECT SUM(completed_count), SUM(total_count) FROM security_training {query_base}',
                params)
