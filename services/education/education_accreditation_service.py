    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS accreditation_programs ( program_id TEXT PRIMARY KEY, program_name TEXT NOT NULL, accreditation_type TEXT NOT NULL, education_type TEXT NOT NULL, description TEXT, standards_level TEXT, review_cycle TEXT DEFAULT '5y', start_date TEXT, end_date TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS accreditation_standards ( standard_id TEXT PRIMARY KEY, standard_name TEXT NOT NULL, standards_level TEXT, education_type TEXT, description TEXT, dimensions TEXT, version TEXT DEFAULT '1.0', effective_date TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS assessment_criteria ( criteria_id TEXT PRIMARY KEY, standard_id TEXT NOT NULL, dimension TEXT NOT NULL, criteria_name TEXT NOT NULL, weight REAL DEFAULT 0.1, passing_score REAL DEFAULT 60, description TEXT, education_type TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS school_accreditations ( accreditation_id TEXT PRIMARY KEY, school_id INTEGER NOT NULL, school_name TEXT, program_id TEXT NOT NULL, education_type TEXT NOT NULL, application_date TEXT, review_date TEXT, decision_date TEXT, status TEXT DEFAULT 'applying', overall_score REAL, decision TEXT, certificate_no TEXT, certificate_url TEXT, valid_from TEXT, valid_until TEXT, next_review_date TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS program_accreditations ( accreditation_id TEXT PRIMARY KEY, program_id TEXT NOT NULL, program_name TEXT, school_id INTEGER NOT NULL, education_type TEXT NOT NULL, application_date TEXT, review_date TEXT, decision_date TEXT, status TEXT DEFAULT 'applying', overall_score REAL, decision TEXT, certificate_no TEXT, valid_from TEXT, valid_until TEXT, next_review_date TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS course_accreditations ( accreditation_id TEXT PRIMARY KEY, course_id INTEGER NOT NULL, course_name TEXT, school_id INTEGER NOT NULL, education_type TEXT NOT NULL, application_date TEXT, review_date TEXT, decision_date TEXT, status TEXT DEFAULT 'applying', rating TEXT, decision TEXT, certificate_no TEXT, valid_from TEXT, valid_until TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS teacher_certifications ( certification_id TEXT PRIMARY KEY, teacher_id INTEGER NOT NULL, teacher_name TEXT, school_id INTEGER NOT NULL, education_type TEXT NOT NULL, certification_type TEXT, issue_date TEXT, valid_until TEXT, status TEXT DEFAULT 'active', certificate_no TEXT, certificate_url TEXT, training_hours INTEGER DEFAULT 0, last_renewal_date TEXT, next_renewal_date TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS assessment_reports ( report_id TEXT PRIMARY KEY, accreditation_id TEXT NOT NULL, report_type TEXT, assessor_id INTEGER, assessor_name TEXT, assessment_date TEXT, overall_score REAL, detailed_scores TEXT, findings TEXT, recommendations TEXT, status TEXT DEFAULT 'draft', approved INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS accreditation_teams ( team_id TEXT PRIMARY KEY, team_name TEXT NOT NULL, accreditation_type TEXT, education_type TEXT, leader_id INTEGER, leader_name TEXT, members TEXT, expertise TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS site_visits ( visit_id TEXT PRIMARY KEY, accreditation_id TEXT NOT NULL, school_id INTEGER NOT NULL, team_id TEXT, visit_date TEXT, duration INTEGER DEFAULT 1, activities TEXT, observations TEXT, photos TEXT, report_url TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS evidence_documents ( document_id TEXT PRIMARY KEY, accreditation_id TEXT NOT NULL, document_name TEXT NOT NULL, document_type TEXT, file_url TEXT, uploaded_by INTEGER, upload_date TEXT, verified INTEGER DEFAULT 0, verified_by INTEGER, verified_date TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS accreditation_decisions ( decision_id TEXT PRIMARY KEY, accreditation_id TEXT NOT NULL, decision_type TEXT, decision TEXT, decision_date TEXT, committee_name TEXT, members TEXT, remarks TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS certification_management ( cert_id TEXT PRIMARY KEY, certificate_no TEXT UNIQUE NOT NULL, certification_type TEXT, holder_id INTEGER NOT NULL, holder_name TEXT, education_type TEXT, issue_date TEXT, valid_from TEXT, valid_until TEXT, status TEXT DEFAULT 'active', revoked INTEGER DEFAULT 0, revoked_reason TEXT, reissued_from TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS international_accreditations ( acc_id TEXT PRIMARY KEY, school_id INTEGER NOT NULL, school_name TEXT, education_type TEXT NOT NULL, international_body TEXT, accreditation_name TEXT, application_date TEXT, approval_date TEXT, valid_until TEXT, status TEXT DEFAULT 'applying', recognition_level TEXT, equivalence TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS quality_assurance_records ( record_id TEXT PRIMARY KEY, school_id INTEGER NOT NULL, education_type TEXT NOT NULL, qa_type TEXT, audit_date TEXT, auditor_id INTEGER, auditor_name TEXT, findings TEXT, corrective_actions TEXT, status TEXT DEFAULT 'open', completed_date TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_accreditation_program(self, program_name: str, accreditation_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO accreditation_programs ( program_id, program_name, accreditation_type, education_type, description, standards_level, review_cycle, start_date, end_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (program_id, program_name, accreditation_type, education_type,
    ...
    def create_standard(self, standard_name: str, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO accreditation_standards ( standard_id, standard_name, standards_level, education_type, description, dimensions, version, effective_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (standard_id, standard_name, kwargs.get('standards_level'),
    ...
    def add_criteria(self, standard_id: str, dimension: str, criteria_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT education_type FROM accreditation_standards WHERE standard_id = ?',
                    (standard_id,))
                    std = cursor.fetchone()
                    if not std:
                    ...
                    cursor.execute(''' INSERT INTO assessment_criteria ( criteria_id, standard_id, dimension, criteria_name, weight, passing_score, description, education_type, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (criteria_id, standard_id, dimension, criteria_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                    'SELECT review_cycle FROM accreditation_programs WHERE program_id = ? AND education_type = ?',
                    ...
                    cursor.execute(''' INSERT INTO school_accreditations ( accreditation_id, school_id, school_name, program_id, education_type, application_date, status, next_review_date, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'applying', ?, ?, ?) ''', (accreditation_id, school_id, school_name, program_id,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE school_accreditations SET review_date = ?, decision_date = ?, status = ?, overall_score = ?, decision = ?, certificate_no = ?, valid_from = ?, valid_until = ?, next_review_date = ?, updated_at = ? WHERE accreditation_id = ? ''', (now[:10], now[:10], status, overall_score, decision,
    ...
    def schedule_site_visit(self, accreditation_id: str, school_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO site_visits ( visit_id, accreditation_id, school_id, team_id, visit_date, duration, activities, observations, photos, report_url, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (visit_id, accreditation_id, school_id, kwargs.get('team_id'),
    ...
    def upload_evidence(self, accreditation_id: str, document_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO evidence_documents ( document_id, accreditation_id, document_name, document_type, file_url, uploaded_by, upload_date, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (document_id, accreditation_id, document_name,
    ...
    def apply_program_accreditation(self, program_id: str, program_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO program_accreditations ( accreditation_id, program_id, program_name, school_id, education_type, application_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'applying', ?, ?) ''', (accreditation_id, program_id, program_name, school_id,
    ...
    def assess_program(self, accreditation_id: str, overall_score: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE program_accreditations SET review_date = ?, decision_date = ?, status = ?, overall_score = ?, decision = ?, certificate_no = ?, valid_from = ?, valid_until = ?, next_review_date = ?, updated_at = ? WHERE accreditation_id = ? ''', (now[:10], now[:10], status, overall_score, decision,
    ...
    def renew_program_accreditation(self, accreditation_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT valid_until FROM program_accreditations WHERE accreditation_id = ?',
                    (accreditation_id,))
                    record = cursor.fetchone()
                    if not record:
                        return {'success': False, 'error': '专业认证记录不存在'}
                    new_valid_until = (datetime.now() + timedelta(days=5 * 365)).strftime('%Y-%m-%d')
                    cursor.execute(''' UPDATE program_accreditations SET status = 'reviewing', valid_until = ?, next_review_date = ?, updated_at = ? WHERE accreditation_id = ? ''', (new_valid_until, new_valid_until, now, accreditation_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def list_program_accreditations(self, school_id: int = None, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'accreditations': accreditations, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def apply_course_accreditation(self, course_id: int, course_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO course_accreditations ( accreditation_id, course_id, course_name, school_id, education_type, application_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'applying', ?, ?) ''', (accreditation_id, course_id, course_name, school_id,
                          education_type, now[:10], now, now))
                    conn.commit()
    ...
    def evaluate_course(self, accreditation_id: str, rating: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE course_accreditations SET review_date = ?, decision_date = ?, status = ?, rating = ?, decision = ?, certificate_no = ?, valid_from = ?, valid_until = ?, updated_at = ? WHERE accreditation_id = ? ''', (now[:10], now[:10], status, rating, decision,
    ...
    def issue_teacher_certification(self, teacher_id: int, teacher_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO teacher_certifications ( certification_id, teacher_id, teacher_name, school_id, education_type, certification_type, issue_date, valid_until, status, certificate_no, next_renewal_date, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?) ''', (certification_id, teacher_id, teacher_name, school_id,
    ...
    def renew_teacher_certification(self, certification_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE teacher_certifications SET valid_until = ?, last_renewal_date = ?, next_renewal_date = ?, updated_at = ? WHERE certification_id = ? ''', (valid_until, now[:10], valid_until, now, certification_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
                    return {'success': False, 'error': '教师证书不存在'}
    ...
    def create_assessment_report(self, accreditation_id: str, report_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO assessment_reports ( report_id, accreditation_id, report_type, assessor_id, assessor_name, assessment_date, overall_score, detailed_scores, findings, recommendations, status, approved, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', 0, ?, ?) ''', (report_id, accreditation_id, report_type, assessor_id,
    ...
    def create_accreditation_team(self, team_name: str, accreditation_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO accreditation_teams ( team_id, team_name, accreditation_type, education_type, leader_id, leader_name, members, expertise, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (team_id, team_name, accreditation_type, education_type,
    ...
    def record_decision(self, accreditation_id: str, decision_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO accreditation_decisions ( decision_id, accreditation_id, decision_type, decision, decision_date, committee_name, members, remarks, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (decision_id, accreditation_id, decision_type, decision,
    ...
    def issue_certificate(self, certification_type: str, holder_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO certification_management ( cert_id, certificate_no, certification_type, holder_id, holder_name, education_type, issue_date, valid_from, valid_until, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (cert_id, certificate_no, certification_type, holder_id,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO certification_management ( cert_id, certificate_no, certification_type, holder_id, holder_name, education_type, issue_date, valid_from, valid_until, status, reissued_from, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?) ''', (new_cert_id, new_certificate_no, cert[1], cert[2],
    ...
    def apply_international_accreditation(self, school_id: int, school_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO international_accreditations ( acc_id, school_id, school_name, education_type, international_body, accreditation_name, application_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'applying', ?, ?) ''', (acc_id, school_id, school_name, education_type,
    ...
    def approve_international_accreditation(self, acc_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE international_accreditations SET approval_date = ?, valid_until = ?, status = ?, recognition_level = ?, equivalence = ?, updated_at = ? WHERE acc_id = ? ''', (now[:10], kwargs.get('valid_until'), 'accredited',
    ...
    def create_qa_audit(self, school_id: int, education_type: str, qa_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO quality_assurance_records ( record_id, school_id, education_type, qa_type, audit_date, auditor_id, auditor_name, findings, corrective_actions, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?) ''', (record_id, school_id, education_type, qa_type,
    ...
    def update_qa_status(self, record_id: str, status: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE quality_assurance_records SET status = ?, completed_date = ?, updated_at = ? WHERE record_id = ? ''', (status, kwargs.get('completed_date', now[:10]), now, record_id))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
