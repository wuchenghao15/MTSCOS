...
class EducationInfrastructureService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS infrastructure_planning ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, education_type TEXT NOT NULL, school_name TEXT, plan_type TEXT, description TEXT, budget REAL DEFAULT 0, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'draft', created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS planning_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, plan_id TEXT NOT NULL, phase TEXT, activity TEXT, responsible TEXT, deadline TEXT, status TEXT DEFAULT 'pending', progress INTEGER DEFAULT 0, remarks TEXT, created_at TEXT, FOREIGN KEY(plan_id) REFERENCES infrastructure_planning(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS infrastructure_construction ( construction_id TEXT PRIMARY KEY, construction_name TEXT NOT NULL, education_type TEXT NOT NULL, infrastructure_type TEXT, school_name TEXT, location TEXT, budget REAL DEFAULT 0, actual_cost REAL DEFAULT 0, start_date TEXT, end_date TEXT, phase TEXT DEFAULT 'planning', status TEXT DEFAULT 'active', created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS construction_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, construction_id TEXT NOT NULL, activity TEXT, equipment_name TEXT, quantity INTEGER DEFAULT 1, cost REAL DEFAULT 0, completed_at TEXT, remarks TEXT, created_at TEXT, FOREIGN KEY(construction_id) REFERENCES infrastructure_construction(construction_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS network_construction ( network_id TEXT PRIMARY KEY, network_name TEXT NOT NULL, education_type TEXT NOT NULL, network_type TEXT, school_name TEXT, coverage_area TEXT, bandwidth TEXT, device_count INTEGER DEFAULT 0, ip_range TEXT, vlan_config TEXT, status TEXT DEFAULT 'planning', created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS network_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, network_id TEXT NOT NULL, device_type TEXT, device_name TEXT, ip_address TEXT, location TEXT, installed_at TEXT, remarks TEXT, FOREIGN KEY(network_id) REFERENCES network_construction(network_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS data_center ( center_id TEXT PRIMARY KEY, center_name TEXT NOT NULL, education_type TEXT NOT NULL, center_type TEXT, school_name TEXT, location TEXT, rack_count INTEGER DEFAULT 0, server_count INTEGER DEFAULT 0, storage_capacity TEXT, power_capacity TEXT, cooling_system TEXT, status TEXT DEFAULT 'planning', created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS center_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, center_id TEXT NOT NULL, asset_type TEXT, asset_name TEXT, serial_number TEXT, purchase_date TEXT, warranty_expire TEXT, status TEXT DEFAULT 'active', remarks TEXT, FOREIGN KEY(center_id) REFERENCES data_center(center_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS system_integration ( integration_id TEXT PRIMARY KEY, integration_name TEXT NOT NULL, education_type TEXT NOT NULL, integration_type TEXT, school_name TEXT, source_system TEXT, target_system TEXT, description TEXT, status TEXT DEFAULT 'planning', created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS integration_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, integration_id TEXT NOT NULL, step TEXT, description TEXT, status TEXT DEFAULT 'pending', completed_at TEXT, FOREIGN KEY(integration_id) REFERENCES system_integration(integration_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS cloud_services ( service_id TEXT PRIMARY KEY, service_name TEXT NOT NULL, education_type TEXT NOT NULL, service_type TEXT, school_name TEXT, provider TEXT, subscription_type TEXT, resource_spec TEXT, monthly_cost REAL DEFAULT 0, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'active', created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, service_id TEXT NOT NULL, record_type TEXT, usage_data TEXT, cost REAL DEFAULT 0, recorded_at TEXT, FOREIGN KEY(service_id) REFERENCES cloud_services(service_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_construction ( security_id TEXT PRIMARY KEY, security_name TEXT NOT NULL, education_type TEXT NOT NULL, security_level TEXT, school_name TEXT, description TEXT, compliance_requirement TEXT, budget REAL DEFAULT 0, status TEXT DEFAULT 'planning', created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS security_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, security_id TEXT NOT NULL, record_type TEXT, event_type TEXT, severity TEXT, description TEXT, resolved INTEGER DEFAULT 0, resolved_at TEXT, FOREIGN KEY(security_id) REFERENCES security_construction(security_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS operation_services ( operation_id TEXT PRIMARY KEY, operation_name TEXT NOT NULL, education_type TEXT NOT NULL, operation_type TEXT, school_name TEXT, description TEXT, service_level TEXT, status TEXT DEFAULT 'active', created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS operation_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, operation_id TEXT NOT NULL, incident_type TEXT, title TEXT, description TEXT, priority TEXT DEFAULT 'medium', status TEXT DEFAULT 'open', assigned_to TEXT, resolved_at TEXT, FOREIGN KEY(operation_id) REFERENCES operation_services(operation_id) ) ''')
                conn.commit()
    ...
    def create_planning(self, plan_name: str, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO infrastructure_planning ( plan_id, plan_name, education_type, school_name, plan_type, description, budget, start_date, end_date, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?) ''', (plan_id, plan_name, education_type,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if updates:
                        ...
                        cursor.execute(f'UPDATE infrastructure_planning SET {", ".join(updates)} WHERE plan_id = ?',
                        params)
                        conn.commit()
                        return {'success': True}
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO planning_records (plan_id, phase, activity, responsible, deadline, status, progress, remarks, created_at) VALUES (?, ?, ?, ?, ?, 'pending', 0, ?, ?) ''', (plan_id, phase, activity, kwargs.get('responsible'),
    ...
    def get_planning_progress(self, plan_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT COUNT(*) FROM planning_records WHERE plan_id = ? AND status = ?', (plan_id,
                'completed'))
                completed = cursor.fetchone()[0] or 0
                progress = round((completed / total) * 100) if total > 0 else 0
                return {'success': True, 'plan_name': plan[0], 'status': plan[1], 'total_records': total,
                'completed_records': completed, 'progress': progress}
        except Exception as e:
    ...
    def create_construction(self, construction_name: str, education_type: str, infrastructure_type: str,
    **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO infrastructure_construction ( construction_id, construction_name, education_type, infrastructure_type, school_name, location, budget, actual_cost, start_date, end_date, phase, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'planning', 'active', ?, ?, ?) ''', (construction_id, construction_name, education_type,
    ...
    def add_construction_record(self, construction_id: str, activity: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT construction_id FROM infrastructure_construction WHERE construction_id = ?',
                    (construction_id,))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '建设项目不存在'}
                    cursor.execute(''' INSERT INTO construction_records (construction_id, activity, equipment_name, quantity, cost, completed_at, remarks, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (construction_id, activity, kwargs.get('equipment_name'),
                          kwargs.get('quantity', 1), kwargs.get('cost', 0),
                          kwargs.get('completed_at'), kwargs.get('remarks'), now))
                    if kwargs.get('cost', 0) > 0:
                        cursor.execute(
                        'UPDATE infrastructure_construction SET actual_cost = actual_cost + ? WHERE construction_id =  ?',
    ...
    def get_construction_cost(self, construction_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not construction:
                    return {'success': False, 'error': '建设项目不存在'}
                return {'success': True, 'construction_name': construction[0], 'budget': construction[1],
                'actual_cost': construction[2], 'remaining': max(0, construction[1] - construction[2])}
        except Exception as e:
    ...
    def create_network(self, network_name: str, education_type: str, network_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO network_construction ( network_id, network_name, education_type, network_type, school_name, coverage_area, bandwidth, device_count, ip_range, vlan_config, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'planning', ?, ?, ?) ''', (network_id, network_name, education_type, network_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO network_records (network_id, device_type, device_name, ip_address, location, installed_at, remarks) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (network_id, device_type, device_name, kwargs.get('ip_address'),
    ...
    def get_network_devices(self, network_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute('SELECT network_name, device_count FROM network_construction WHERE network_id = ?',
                (network_id,))
    ...
    def create_data_center(self, center_name: str, education_type: str, center_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO data_center ( center_id, center_name, education_type, center_type, school_name, location, rack_count, server_count, storage_capacity, power_capacity, cooling_system, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, 'planning', ?, ?, ?) ''', (center_id, center_name, education_type, center_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO center_records (center_id, asset_type, asset_name, serial_number, purchase_date, warranty_expire, status, remarks) VALUES (?, ?, ?, ?, ?, ?, 'active', ?) ''', (center_id, asset_type, asset_name, kwargs.get('serial_number'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not cursor.fetchone():
                        return {'success': False, 'error': '数据中心不存在'}
                    cursor.execute(''' INSERT INTO center_records (center_id, asset_type, asset_name, serial_number, purchase_date, warranty_expire, status, remarks) VALUES (?, 'disaster_recovery', ?, ?, ?, ?, 'active', ?) ''', (center_id, strategy, kwargs.get('serial_number'), now[:10],
    ...
    def create_integration(self, integration_name: str, education_type: str, integration_type: str,
    **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO system_integration ( integration_id, integration_name, education_type, integration_type, school_name, source_system, target_system, description, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'planning', ?, ?, ?) ''', (integration_id, integration_name, education_type, integration_type,
    ...
    def add_integration_step(self, integration_id: str, step: str, description: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT integration_id FROM system_integration WHERE integration_id = ?',
                    (integration_id,))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '集成项目不存在'}
                    cursor.execute(''' INSERT INTO integration_records (integration_id, step, description, status, completed_at) VALUES (?, ?, ?, 'pending', NULL) ''', (integration_id, step, description))
                    conn.commit()
                    return {'success': True, 'record_id': cursor.lastrowid}
        except Exception as e:
    ...
    def get_integration_progress(self, integration_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('SELECT integration_name, status FROM system_integration WHERE integration_id = ?',
                (integration_id,))
                ...
                total = cursor.fetchone()[0] or 0
                cursor.execute('SELECT COUNT(*) FROM integration_records WHERE integration_id = ? AND status = ?',
                (integration_id, 'completed'))
                completed = cursor.fetchone()[0] or 0
                progress = round((completed / total) * 100) if total > 0 else 0
                return {'success': True, 'integration_name': integration[0], 'status': integration[1],
                'total_steps': total, 'completed_steps': completed, 'progress': progress}
        except Exception as e:
    ...
    def create_cloud_service(self, service_name: str, education_type: str, service_type: str, **kwargs) -> Dict[str,
    Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO cloud_services ( service_id, service_name, education_type, service_type, school_name, provider, subscription_type, resource_spec, monthly_cost, start_date, end_date, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?) ''', (service_id, service_name, education_type, service_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO service_records (service_id, record_type, usage_data, cost, recorded_at) VALUES (?, ?, ?, ?, ?) ''', (service_id, record_type, json.dumps(kwargs.get('usage_data', {})),
    ...
    def create_security_project(self, security_name: str, education_type: str, security_level: str,
    **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO security_construction ( security_id, security_name, education_type, security_level, school_name, description, compliance_requirement, budget, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'planning', ?, ?, ?) ''', (security_id, security_name, education_type, security_level,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO security_records (security_id, record_type, event_type, severity, description, resolved) VALUES (?, 'event', ?, ?, ?, 0) ''', (security_id, event_type, severity, description))
                    conn.commit()
                    return {'success': True, 'record_id': cursor.lastrowid}
        except Exception as e:
    ...
    def get_security_status(self, security_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not security:
                    return {'success': False, 'error': '安全项目不存在'}
                cursor.execute('SELECT COUNT(*) FROM security_records WHERE security_id = ? AND resolved = 0',
                (security_id,))
                unresolved = cursor.fetchone()[0] or 0
                cursor.execute('SELECT COUNT(*) FROM security_records WHERE security_id = ?', (security_id,))
                total = cursor.fetchone()[0] or 0
                return {'success': True, 'security_name': security[0], 'security_level': security[1],
                'status': security[2], 'total_events': total, 'unresolved_events': unresolved}
        except Exception as e:
    ...
    def create_operation_service(self, operation_name: str, education_type: str, operation_type: str,
    **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO operation_services ( operation_id, operation_name, education_type, operation_type, school_name, description, service_level, status, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?) ''', (operation_id, operation_name, education_type, operation_type,
                          kwargs.get('school_name'), kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO operation_records (operation_id, incident_type, title, description, priority, status, assigned_to) VALUES (?, ?, ?, ?, ?, 'open', ?) ''', (operation_id, incident_type, title, kwargs.get('description'),
    ...
    def get_operation_stats(self, operation_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                total = cursor.fetchone()[0] or 0
                cursor.execute('SELECT COUNT(*) FROM operation_records WHERE operation_id = ? AND status = ?',
                (operation_id, 'resolved'))
                resolved = cursor.fetchone()[0] or 0
                cursor.execute('SELECT COUNT(*) FROM operation_records WHERE operation_id = ? AND status = ?',
                (operation_id, 'open'))
                open_count = cursor.fetchone()[0] or 0
                return {'success': True, 'operation_name': operation[0], 'operation_type': operation[1],
                'service_level': operation[2], 'status': operation[3], 'total_incidents': total,
                'resolved_incidents': resolved, 'open_incidents': open_count}
        except Exception as e:
    ...
    def get_overall_statistics(self, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f'SELECT COALESCE(SUM(budget), 0) FROM infrastructure_construction {where_clause}',
                params)
