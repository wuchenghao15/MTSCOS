    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_levels ( service_id TEXT PRIMARY KEY, service_name TEXT NOT NULL, service_category TEXT, service_level TEXT, education_type TEXT, availability_target REAL, response_time TEXT, priority INTEGER, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS sla_agreements ( sla_id TEXT PRIMARY KEY, service_id TEXT NOT NULL, education_type TEXT, availability_guarantee REAL, response_time_guarantee TEXT, resolution_time_guarantee TEXT, penalties TEXT, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, FOREIGN KEY(service_id) REFERENCES service_levels(service_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS incident_management ( incident_id TEXT PRIMARY KEY, title TEXT NOT NULL, incident_type TEXT, severity TEXT, education_type TEXT, service_id TEXT, description TEXT, reporter_id INTEGER, reporter_name TEXT, assignee_id INTEGER, assignee_name TEXT, status TEXT DEFAULT 'open', impact TEXT, urgency TEXT, created_at TEXT, updated_at TEXT, resolved_at TEXT, FOREIGN KEY(service_id) REFERENCES service_levels(service_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS incident_history ( id INTEGER PRIMARY KEY AUTOINCREMENT, incident_id TEXT NOT NULL, action TEXT, actor_id INTEGER, actor_name TEXT, status_before TEXT, status_after TEXT, comment TEXT, created_at TEXT, FOREIGN KEY(incident_id) REFERENCES incident_management(incident_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS performance_monitoring ( monitor_id TEXT PRIMARY KEY, service_id TEXT, education_type TEXT, metric_type TEXT, threshold REAL, alert_enabled INTEGER DEFAULT 1, alert_level TEXT, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, FOREIGN KEY(service_id) REFERENCES service_levels(service_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS monitoring_data ( id INTEGER PRIMARY KEY AUTOINCREMENT, monitor_id TEXT NOT NULL, timestamp TEXT, value REAL, unit TEXT, is_alert INTEGER DEFAULT 0, FOREIGN KEY(monitor_id) REFERENCES performance_monitoring(monitor_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS capacity_management ( capacity_id TEXT PRIMARY KEY, resource_type TEXT, education_type TEXT, current_capacity REAL, used_capacity REAL, peak_capacity REAL, threshold REAL, forecast_method TEXT, status TEXT DEFAULT 'normal', description TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS capacity_plans ( plan_id TEXT PRIMARY KEY, capacity_id TEXT, plan_name TEXT, education_type TEXT, scaling_strategy TEXT, target_capacity REAL, implementation_date TEXT, estimated_cost REAL, status TEXT DEFAULT 'planned', created_at TEXT, updated_at TEXT, FOREIGN KEY(capacity_id) REFERENCES capacity_management(capacity_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS change_management ( change_id TEXT PRIMARY KEY, change_type TEXT, education_type TEXT, service_id TEXT, title TEXT NOT NULL, description TEXT, risk_level TEXT, priority TEXT, requester_id INTEGER, requester_name TEXT, status TEXT DEFAULT 'pending', approval_status TEXT DEFAULT 'pending', planned_start TEXT, planned_end TEXT, actual_start TEXT, actual_end TEXT, created_at TEXT, updated_at TEXT, FOREIGN KEY(service_id) REFERENCES service_levels(service_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS change_requests ( id INTEGER PRIMARY KEY AUTOINCREMENT, change_id TEXT NOT NULL, approver_id INTEGER, approver_name TEXT, approval_action TEXT, approval_comment TEXT, approved_at TEXT, FOREIGN KEY(change_id) REFERENCES change_management(change_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS configuration_items ( config_id TEXT PRIMARY KEY, item_name TEXT NOT NULL, item_type TEXT, education_type TEXT, service_id TEXT, current_value TEXT, previous_value TEXT, owner_id INTEGER, owner_name TEXT, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, FOREIGN KEY(service_id) REFERENCES service_levels(service_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS config_history ( id INTEGER PRIMARY KEY AUTOINCREMENT, config_id TEXT NOT NULL, change_type TEXT, old_value TEXT, new_value TEXT, changed_by_id INTEGER, changed_by_name TEXT, change_reason TEXT, created_at TEXT, FOREIGN KEY(config_id) REFERENCES configuration_items(config_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS release_management ( release_id TEXT PRIMARY KEY, release_name TEXT NOT NULL, education_type TEXT, service_id TEXT, version TEXT, description TEXT, status TEXT DEFAULT 'planned', release_date TEXT, deployed_at TEXT, rollback_plan TEXT, created_at TEXT, updated_at TEXT, FOREIGN KEY(service_id) REFERENCES service_levels(service_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS release_records ( id INTEGER PRIMARY KEY AUTOINCREMENT, release_id TEXT NOT NULL, environment TEXT, status TEXT, started_at TEXT, completed_at TEXT, result TEXT, rollback_executed INTEGER DEFAULT 0, FOREIGN KEY(release_id) REFERENCES release_management(release_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_continuity ( continuity_id TEXT PRIMARY KEY, service_id TEXT, education_type TEXT, business_criticality TEXT, impact_level TEXT, recovery_time_objective TEXT, recovery_point_objective TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, FOREIGN KEY(service_id) REFERENCES service_levels(service_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS continuity_plans ( plan_id TEXT PRIMARY KEY, continuity_id TEXT, plan_name TEXT, education_type TEXT, recovery_strategy TEXT, redundancy_level TEXT, failover_type TEXT, backup_method TEXT, drill_frequency TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, FOREIGN KEY(continuity_id) REFERENCES service_continuity(continuity_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS disaster_recovery ( dr_id TEXT PRIMARY KEY, continuity_id TEXT, education_type TEXT, recovery_site TEXT, recovery_type TEXT, rto_target TEXT, rpo_target TEXT, mttr_target TEXT, last_tested TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, FOREIGN KEY(continuity_id) REFERENCES service_continuity(continuity_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS recovery_tests ( test_id TEXT PRIMARY KEY, dr_id TEXT, education_type TEXT, test_type TEXT, test_date TEXT, test_results TEXT, issues_found TEXT, status TEXT DEFAULT 'completed', created_at TEXT, FOREIGN KEY(dr_id) REFERENCES disaster_recovery(dr_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS service_catalog ( catalog_id TEXT PRIMARY KEY, catalog_name TEXT NOT NULL, education_type TEXT, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS catalog_items ( item_id TEXT PRIMARY KEY, catalog_id TEXT NOT NULL, service_id TEXT, education_type TEXT, item_name TEXT NOT NULL, item_description TEXT, service_level TEXT, price REAL DEFAULT 0, status TEXT DEFAULT 'available', created_at TEXT, updated_at TEXT, FOREIGN KEY(catalog_id) REFERENCES service_catalog(catalog_id), FOREIGN KEY(service_id) REFERENCES service_levels(service_id) ) ''')
                conn.commit()
    ...
    def create_service_level(self, service_name: str, service_category: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO service_levels ( service_id, service_name, service_category, service_level, education_type, availability_target, response_time, priority, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (service_id, service_name, service_category,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO sla_agreements ( sla_id, service_id, education_type, availability_guarantee, response_time_guarantee, resolution_time_guarantee, penalties, start_date, end_date, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (sla_id, service_id, education_type,
    ...
    def create_incident(self, title: str, incident_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO incident_management ( incident_id, title, incident_type, severity, education_type, service_id, description, reporter_id, reporter_name, assignee_id, assignee_name, status, impact, urgency, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?, ?, ?) ''', (incident_id, title, incident_type, severity,
                    ...
                    cursor.execute(''' INSERT INTO incident_history (incident_id, action, actor_id, actor_name, status_before, status_after, comment, created_at) VALUES (?, 'created', ?, ?, NULL, 'open', ?, ?) ''', (incident_id, reporter_id, kwargs.get('reporter_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE incident_management SET assignee_id = ?, assignee_name = ?, status = 'assigned', updated_at = ? WHERE incident_id = ? ''', (assignee_id, assignee_name, now, incident_id))
                    cursor.execute(''' INSERT INTO incident_history (incident_id, action, actor_id, actor_name, status_before, status_after, comment, created_at) VALUES (?, 'assigned', ?, ?, ?, 'assigned', ?, ?) ''', (incident_id, assignee_id, assignee_name, incident[0], now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO incident_history (incident_id, action, actor_id, actor_name, status_before, status_after, comment, created_at) VALUES (?, 'status_update', ?, ?, ?, ?, ?, ?) ''', (incident_id, kwargs.get('actor_id'), kwargs.get('actor_name'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not incident:
                        return {'success': False, 'error': '故障事件不存在'}
                    cursor.execute(''' INSERT INTO incident_history (incident_id, action, actor_id, actor_name, status_before, status_after, comment, created_at) VALUES (?, 'comment', ?, ?, ?, ?, ?, ?) ''', (incident_id, actor_id, actor_name, incident[0], incident[0], comment, now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def create_monitor(self, service_id: str, metric_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO performance_monitoring ( monitor_id, service_id, education_type, metric_type, threshold, alert_enabled, alert_level, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (monitor_id, service_id, education_type, metric_type,
    ...
    def record_metric_data(self, monitor_id: str, value: float, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT threshold, alert_enabled FROM performance_monitoring WHERE monitor_id = ?',
                    (monitor_id,))
                    ...
                    if monitor[1] and monitor[0] is not None:
                        ...
                        if metric_type in ['availability', 'data_integrity', 'user_satisfaction']:
                            ...
                            is_alert = 1 if value > monitor[0] else 0
                    cursor.execute(''' INSERT INTO monitoring_data (monitor_id, timestamp, value, unit, is_alert) VALUES (?, ?, ?, ?, ?) ''', (monitor_id, now, value, kwargs.get('unit', ''), is_alert))
                    conn.commit()
    ...
    def get_monitor_alerts(self, education_type: str = None, limit: int = 50) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                query = ''' SELECT md.*, pm.metric_type, pm.service_id FROM monitoring_data md JOIN performance_monitoring pm ON md.monitor_id = pm.monitor_id WHERE md.is_alert = 1 '''
                params = []
                if education_type:
    ...
    def create_capacity_plan(self, resource_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO capacity_management ( capacity_id, resource_type, education_type, current_capacity, used_capacity, peak_capacity, threshold, forecast_method, status, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'normal', ?, ?, ?) ''', (capacity_id, resource_type, education_type,
    ...
    def update_capacity_usage(self, capacity_id: str, used_capacity: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT current_capacity, threshold FROM capacity_management WHERE capacity_id = ?',
                    (capacity_id,))
                    capacity = cursor.fetchone()
                    if not capacity:
                        return {'success': False, 'error': '容量管理不存在'}
                    usage_percent = (used_capacity / capacity[0]) * 100 if capacity[0] > 0 else 0
                    status = 'normal' if usage_percent < capacity[1] else (
                    'warning' if usage_percent < 90 else 'critical')
                    cursor.execute(''' UPDATE capacity_management SET used_capacity = ?, peak_capacity = ?, status = ?, updated_at = ? WHERE capacity_id = ? ''', (used_capacity, max(capacity[1], used_capacity) if kwargs.get('is_peak') else capacity[1],
    ...
    def create_scaling_plan(self, capacity_id: str, plan_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO capacity_plans ( plan_id, capacity_id, plan_name, education_type, scaling_strategy, target_capacity, implementation_date, estimated_cost, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'planned', ?, ?) ''', (plan_id, capacity_id, plan_name, education_type,
    ...
    def create_change_request(self, change_type: str, title: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO change_management ( change_id, change_type, education_type, service_id, title, description, risk_level, priority, requester_id, requester_name, status, approval_status, planned_start, planned_end, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (change_id, change_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE change_management SET approval_status = ?, status = ?, updated_at = ? WHERE change_id = ? ''', (approval_action, 'approved' if approved else 'rejected', now, change_id))
                    cursor.execute(''' INSERT INTO change_requests (change_id, approver_id, approver_name, approval_action, approval_comment, approved_at) VALUES (?, ?, ?, ?, ?, ?) ''', (change_id, approver_id, approver_name, approval_action,
    ...
    def implement_change(self, change_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT approval_status, status FROM change_management WHERE change_id = ?',
                    (change_id,))
                    change = cursor.fetchone()
                    if not change:
                        return {'success': False, 'error': '变更请求不存在'}
                    ...
                    cursor.execute(''' UPDATE change_management SET status = 'in_progress', actual_start = ?, updated_at = ? WHERE change_id = ? ''', (now, now, change_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE change_management SET status = 'completed', actual_end = ?, updated_at = ? WHERE change_id = ? ''', (now, now, change_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def create_config_item(self, item_name: str, item_type: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO configuration_items ( config_id, item_name, item_type, education_type, service_id, current_value, previous_value, owner_id, owner_name, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, 'active', ?, ?) ''', (config_id, item_name, item_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    old_value = config[0]
                    cursor.execute(''' UPDATE configuration_items SET previous_value = ?, current_value = ?, updated_at = ? WHERE config_id = ? ''', (old_value, current_value, now, config_id))
                    cursor.execute(''' INSERT INTO config_history (config_id, change_type, old_value, new_value, changed_by_id, changed_by_name, change_reason, created_at) VALUES (?, 'update', ?, ?, ?, ?, ?, ?) ''', (config_id, old_value, current_value, changed_by_id,
    ...
    def get_config_history(self, config_id: str, limit: int = 50) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT * FROM config_history WHERE config_id = ? ORDER BY created_at DESC LIMIT ? ''', (config_id, limit))
    ...
    def create_release(self, release_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO release_management ( release_id, release_name, education_type, service_id, version, description, status, release_date, rollback_plan, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'planned', ?, ?, ?, ?) ''', (release_id, release_name, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO release_records (release_id, environment, status, started_at) VALUES (?, ?, 'deploying', ?) ''', (release_id, environment, now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def complete_deployment(self, release_id: str, environment: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE release_records SET status = ?, completed_at = ?, result = ? WHERE release_id = ? AND environment = ? AND status = 'deploying' ''', (result, now, result, release_id, environment))
                    if cursor.rowcount == 0:
                        return {'success': False, 'error': '未找到部署记录'}
                    if result == 'success':
                        cursor.execute(''' UPDATE release_management SET status = 'deployed', deployed_at = ?, updated_at = ? WHERE release_id = ? ''', (now, now, release_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not release:
                        return {'success': False, 'error': '发布版本不存在'}
                    cursor.execute(''' UPDATE release_records SET status = 'rolled_back', completed_at = ?, result = 'rollback', rollback_executed = 1 WHERE release_id = ? AND environment = ? ''', (now, release_id, environment))
                    cursor.execute(''' UPDATE release_management SET status = 'rolled_back', updated_at = ? WHERE release_id = ? ''', (now, release_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def create_continuity_plan(self, service_id: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO service_continuity ( continuity_id, service_id, education_type, business_criticality, impact_level, recovery_time_objective, recovery_point_objective, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (continuity_id, service_id, education_type,
    ...
    def create_disaster_recovery(self, continuity_id: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO disaster_recovery ( dr_id, continuity_id, education_type, recovery_site, recovery_type, rto_target, rpo_target, mttr_target, last_tested, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, 'active', ?, ?) ''', (dr_id, continuity_id, education_type,
    ...
    def run_dr_test(self, dr_id: str, education_type: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO recovery_tests ( test_id, dr_id, education_type, test_type, test_date, test_results, issues_found, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'completed', ?) ''', (test_id, dr_id, education_type,
    ...
    def create_service_catalog(self, catalog_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO service_catalog ( catalog_id, catalog_name, education_type, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, 'active', ?, ?) ''', (catalog_id, catalog_name, education_type,
                          kwargs.get('description'), now, now))
                    conn.commit()
    ...
    def add_catalog_item(self, catalog_id: str, item_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO catalog_items ( item_id, catalog_id, service_id, education_type, item_name, item_description, service_level, price, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'available', ?, ?) ''', (item_id, catalog_id, kwargs.get('service_id'), education_type,
        ...
        try:
            with self._get_connection() as conn:
                ...
                if catalog_id and catalogs:
                    cursor.execute('SELECT * FROM catalog_items WHERE catalog_id = ? AND status = ?', (catalog_id,
                    'available'))
                    items = [dict(i) for i in cursor.fetchall()]
    ...
    def get_operations_stats(self, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                avg_resolution = 0
                if resolved_count > 0:
                    cursor.execute(f''' SELECT AVG(julianday(resolved_at) - julianday(created_at)) * 24 FROM incident_management WHERE status = "resolved" {filters} ''', params)
