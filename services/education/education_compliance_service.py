    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS compliance_policies ( policy_id TEXT PRIMARY KEY, policy_name TEXT NOT NULL, compliance_area TEXT NOT NULL, policy_code TEXT UNIQUE, education_type TEXT, description TEXT, scope TEXT, status TEXT DEFAULT 'active', effective_date TEXT, expiry_date TEXT, created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS policy_versions ( version_id TEXT PRIMARY KEY, policy_id TEXT NOT NULL, version_number TEXT NOT NULL, change_description TEXT, status TEXT DEFAULT 'draft', effective_date TEXT, created_by TEXT, created_at TEXT, FOREIGN KEY (policy_id) REFERENCES compliance_policies(policy_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS compliance_checks ( check_id TEXT PRIMARY KEY, check_name TEXT NOT NULL, compliance_area TEXT, education_type TEXT, check_scope TEXT, frequency TEXT DEFAULT 'quarterly', description TEXT, status TEXT DEFAULT 'planned', scheduled_date TEXT, created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS check_results ( result_id TEXT PRIMARY KEY, check_id TEXT NOT NULL, check_date TEXT, findings TEXT, issues_found INTEGER DEFAULT 0, severity TEXT, status TEXT DEFAULT 'open', assignee TEXT, due_date TEXT, completed_date TEXT, created_at TEXT, FOREIGN KEY (check_id) REFERENCES compliance_checks(check_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS risk_assessments ( assessment_id TEXT PRIMARY KEY, assessment_name TEXT NOT NULL, education_type TEXT, risk_type TEXT, description TEXT, risk_level TEXT DEFAULT 'medium', risk_score INTEGER DEFAULT 50, probability INTEGER DEFAULT 50, impact INTEGER DEFAULT 50, status TEXT DEFAULT 'pending', assessed_by TEXT, assessed_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS risk_registers ( register_id TEXT PRIMARY KEY, assessment_id TEXT NOT NULL, risk_category TEXT, risk_description TEXT, risk_level TEXT, risk_score INTEGER, mitigation_status TEXT DEFAULT 'pending', owner TEXT, created_at TEXT, FOREIGN KEY (assessment_id) REFERENCES risk_assessments(assessment_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS risk_mitigation ( mitigation_id TEXT PRIMARY KEY, register_id TEXT NOT NULL, strategy TEXT, target_risk_level TEXT, resources_required TEXT, timeline TEXT, status TEXT DEFAULT 'planned', created_by TEXT, created_at TEXT, FOREIGN KEY (register_id) REFERENCES risk_registers(register_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS mitigation_actions ( action_id TEXT PRIMARY KEY, mitigation_id TEXT NOT NULL, action_description TEXT NOT NULL, responsible_party TEXT, due_date TEXT, status TEXT DEFAULT 'pending', completed_date TEXT, created_at TEXT, FOREIGN KEY (mitigation_id) REFERENCES risk_mitigation(mitigation_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS internal_controls ( control_id TEXT PRIMARY KEY, control_name TEXT NOT NULL, control_type TEXT, compliance_area TEXT, education_type TEXT, description TEXT, effectiveness TEXT DEFAULT 'unknown', status TEXT DEFAULT 'active', owner TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS control_testing ( test_id TEXT PRIMARY KEY, control_id TEXT NOT NULL, test_type TEXT, test_date TEXT, test_results TEXT, pass_rate REAL, status TEXT DEFAULT 'pending', tester TEXT, created_at TEXT, FOREIGN KEY (control_id) REFERENCES internal_controls(control_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS audit_plans ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, audit_type TEXT, education_type TEXT, audit_scope TEXT, start_date TEXT, end_date TEXT, status TEXT DEFAULT 'planned', auditor TEXT, budget TEXT, created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS audit_findings ( finding_id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, finding_description TEXT NOT NULL, severity TEXT DEFAULT 'medium', recommendation TEXT, status TEXT DEFAULT 'open', assignee TEXT, due_date TEXT, completed_date TEXT, created_at TEXT, FOREIGN KEY (plan_id) REFERENCES audit_plans(plan_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS compliance_training ( training_id TEXT PRIMARY KEY, training_name TEXT NOT NULL, training_topic TEXT, education_type TEXT, target_audience TEXT, duration TEXT, format TEXT DEFAULT 'online', description TEXT, status TEXT DEFAULT 'planned', scheduled_date TEXT, created_by TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS training_records ( record_id TEXT PRIMARY KEY, training_id TEXT NOT NULL, participant_id INTEGER NOT NULL, participant_name TEXT, completion_status TEXT DEFAULT 'incomplete', score INTEGER, completed_date TEXT, created_at TEXT, FOREIGN KEY (training_id) REFERENCES compliance_training(training_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS risk_alerts ( alert_id TEXT PRIMARY KEY, alert_name TEXT NOT NULL, indicator_type TEXT, education_type TEXT, threshold_value REAL, current_value REAL, alert_level TEXT DEFAULT 'warning', status TEXT DEFAULT 'active', trigger_time TEXT, acknowledged_by TEXT, acknowledged_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS alert_history ( history_id TEXT PRIMARY KEY, alert_id TEXT NOT NULL, status TEXT, action_taken TEXT, handled_by TEXT, handled_at TEXT, created_at TEXT, FOREIGN KEY (alert_id) REFERENCES risk_alerts(alert_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS compliance_reports ( report_id TEXT PRIMARY KEY, report_name TEXT NOT NULL, report_type TEXT, education_type TEXT, period_start TEXT, period_end TEXT, status TEXT DEFAULT 'draft', generated_by TEXT, file_path TEXT, generated_at TEXT, approved_at TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS report_schedules ( schedule_id TEXT PRIMARY KEY, report_type TEXT NOT NULL, education_type TEXT, frequency TEXT DEFAULT 'monthly', next_run_date TEXT, status TEXT DEFAULT 'active', created_by TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS whistleblower_reports ( report_id TEXT PRIMARY KEY, report_type TEXT, education_type TEXT, description TEXT NOT NULL, evidence TEXT, reporter_identity TEXT DEFAULT 'anonymous', status TEXT DEFAULT 'pending', priority TEXT DEFAULT 'medium', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS report_handlings ( handling_id TEXT PRIMARY KEY, report_id TEXT NOT NULL, action_description TEXT, investigator TEXT, investigation_status TEXT, findings TEXT, resolution TEXT, handled_at TEXT, created_at TEXT, FOREIGN KEY (report_id) REFERENCES whistleblower_reports(report_id) ) ''')
                conn.commit()
    ...
    def create_policy(self, policy_name: str, compliance_area: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO compliance_policies ( policy_id, policy_name, compliance_area, policy_code, education_type, description, scope, status, effective_date, expiry_date, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?) ''', (policy_id, policy_name, compliance_area, policy_code,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO policy_versions ( version_id, policy_id, version_number, change_description, status, effective_date, created_by, created_at ) VALUES (?, ?, ?, ?, 'draft', ?, ?, ?) ''', (version_id, policy_id, version_number, change_description,
    ...
    def create_risk_assessment(self, assessment_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO risk_assessments ( assessment_id, assessment_name, education_type, risk_type, description, risk_level, risk_score, probability, impact, status, assessed_by, assessed_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?) ''', (assessment_id, assessment_name, kwargs.get('education_type'),
                    ...
                    return {'success': True, 'assessment_id': assessment_id, 'risk_level': risk_level,
    ...
    def add_risk_register(self, assessment_id: str, risk_description: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO risk_registers ( register_id, assessment_id, risk_category, risk_description, risk_level, risk_score, mitigation_status, owner, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (register_id, assessment_id, kwargs.get('risk_category'),
    ...
    def create_mitigation(self, register_id: str, strategy: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO risk_mitigation ( mitigation_id, register_id, strategy, target_risk_level, resources_required, timeline, status, created_by, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'planned', ?, ?) ''', (mitigation_id, register_id, strategy,
    ...
    def add_mitigation_action(self, mitigation_id: str, action_description: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO mitigation_actions ( action_id, mitigation_id, action_description, responsible_party, due_date, status, created_at ) VALUES (?, ?, ?, ?, ?, 'pending', ?) ''', (action_id, mitigation_id, action_description,
    ...
    def create_control(self, control_name: str, control_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO internal_controls ( control_id, control_name, control_type, compliance_area, education_type, description, effectiveness, status, owner, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'unknown', 'active', ?, ?, ?) ''', (control_id, control_name, control_type,
    ...
    def perform_control_test(self, control_id: str, test_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO control_testing ( test_id, control_id, test_type, test_date, test_results, pass_rate, status, tester, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (test_id, control_id, test_type, now[:10],
    ...
    def create_audit_plan(self, plan_name: str, audit_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO audit_plans ( plan_id, plan_name, audit_type, education_type, audit_scope, start_date, end_date, status, auditor, budget, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'planned', ?, ?, ?, ?, ?) ''', (plan_id, plan_name, audit_type, kwargs.get('education_type'),
    ...
    def add_audit_finding(self, plan_id: str, finding_description: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO audit_findings ( finding_id, plan_id, finding_description, severity, recommendation, status, assignee, due_date, created_at ) VALUES (?, ?, ?, ?, ?, 'open', ?, ?, ?) ''', (finding_id, plan_id, finding_description,
    ...
    def create_training(self, training_name: str, training_topic: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO compliance_training ( training_id, training_name, training_topic, education_type, target_audience, duration, format, description, status, scheduled_date, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'planned', ?, ?, ?, ?) ''', (training_id, training_name, training_topic,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(
                    'SELECT record_id FROM training_records WHERE training_id = ? AND participant_id = ?',
                                 (training_id, participant_id))
                    if cursor.fetchone():
                        return {'success': False, 'error': '已报名该培训'}
                    cursor.execute(''' INSERT INTO training_records ( record_id, training_id, participant_id, participant_name, completion_status, created_at ) VALUES (?, ?, ?, ?, 'incomplete', ?) ''', (record_id, training_id, participant_id, participant_name, now))
                    conn.commit()
    ...
    def create_risk_alert(self, alert_name: str, indicator_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO risk_alerts ( alert_id, alert_name, indicator_type, education_type, threshold_value, current_value, alert_level, status, trigger_time, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (alert_id, alert_name, indicator_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute(''' INSERT INTO alert_history (history_id, alert_id, status, action_taken, handled_by, handled_at, created_at) VALUES (?, ?, ?, 'Acknowledged', ?, ?, ?) ''', (f"ahl_{uuid.uuid4().hex[:12]}", alert_id, 'acknowledged', acknowledged_by, now[:10], now))
                        conn.commit()
                        return {'success': True}
    ...
    def generate_report(self, report_name: str, report_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO compliance_reports ( report_id, report_name, report_type, education_type, period_start, period_end, status, generated_by, file_path, generated_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?) ''', (report_id, report_name, report_type,
    ...
    def create_check(self, check_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO compliance_checks ( check_id, check_name, compliance_area, education_type, check_scope, frequency, description, status, scheduled_date, created_by, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'planned', ?, ?, ?, ?) ''', (check_id, check_name, kwargs.get('compliance_area'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO check_results ( result_id, check_id, check_date, findings, issues_found, severity, status, assignee, due_date, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'open', ?, ?, ?) ''', (result_id, check_id, now[:10], kwargs.get('findings'),
    ...
    def submit_whistleblower_report(self, description: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO whistleblower_reports ( report_id, report_type, education_type, description, evidence, reporter_identity, status, priority, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (report_id, kwargs.get('report_type'), kwargs.get('education_type'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO report_handlings ( handling_id, report_id, action_description, investigator, investigation_status, created_at ) VALUES (?, ?, ?, ?, 'in_progress', ?) ''', (handling_id, report_id, kwargs.get('action_description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE whistleblower_reports SET status = ? WHERE report_id = ?',
                                 ('resolved', report_id))
                    cursor.execute(''' INSERT INTO report_handlings ( handling_id, report_id, action_description, investigator, investigation_status, findings, resolution, handled_at, created_at ) VALUES (?, ?, ?, ?, 'completed', ?, ?, ?, ?) ''', (f"whh_{uuid.uuid4().hex[:12]}", report_id, kwargs.get('action_description'),
