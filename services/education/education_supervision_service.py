...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
""" MTSCOS 教育督导管理服务 (v15.12.0) ==================================== 提供教育督导、教育监察、办学行为规范、督导评估、整改追踪等综合管理服务。  核心能力： 1. 督导计划 - 计划制定、审核、发布、查询 2. 督导任务 - 任务分配、执行、反馈、归档 3. 督导队伍 - 成员管理、资质审核、培训记录 4. 督导执行 - 现场检查、记录录入、评分评估、问题反馈 5. 教育监察 - 监察立项、调查取证、结果认定、通报处理 6. 违规处理 - 违规登记、等级评定、处罚决定、申诉处理 7. 整改追踪 - 整改任务、进度跟踪、复核验收、销号管理 8. 师德师风 - 考核评价、问题查处、培训提升、档案管理 9. 合规检查 - 办学资质、课程设置、财务规范、安全制度 10. 投诉处理 - 投诉受理、调查核实、处理回复、满意度评价 11. 报告管理 - 报告生成、审核发布、归档保存、统计分析 12. 统计分析 - 数据汇总、趋势分析、报表生成  差异化支持：成人教育 / K12教育 """
import os
import json
import uuid
import sqlite3
...
class EducationSupervisionService:
    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_plans ( plan_id TEXT PRIMARY KEY, plan_name TEXT NOT NULL, supervision_type TEXT, education_type TEXT, target_school TEXT, target_level TEXT, start_date TEXT, end_date TEXT, scope TEXT, objectives TEXT, budget REAL DEFAULT 0, status TEXT DEFAULT 'draft', approved_by INTEGER, approved_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_tasks ( task_id TEXT PRIMARY KEY, plan_id TEXT, task_name TEXT NOT NULL, task_type TEXT, assignee_id INTEGER, assignee_name TEXT, target_school TEXT, scheduled_date TEXT, status TEXT DEFAULT 'pending', priority TEXT DEFAULT 'normal', description TEXT, completed_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_teams ( team_id TEXT PRIMARY KEY, team_name TEXT NOT NULL, leader_id INTEGER, leader_name TEXT, education_type TEXT, specialty TEXT, description TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_members ( id INTEGER PRIMARY KEY AUTOINCREMENT, team_id TEXT NOT NULL, member_id INTEGER NOT NULL, member_name TEXT, role TEXT DEFAULT 'member', qualification TEXT, joined_at TEXT, UNIQUE(team_id, member_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_records ( record_id TEXT PRIMARY KEY, task_id TEXT, plan_id TEXT, school_id INTEGER, school_name TEXT, education_type TEXT, supervisor_id INTEGER, supervisor_name TEXT, record_date TEXT, findings TEXT, score REAL, rating TEXT, issues_found INTEGER DEFAULT 0, photos TEXT, status TEXT DEFAULT 'draft', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS inspection_items ( item_id TEXT PRIMARY KEY, inspection_category TEXT, item_name TEXT NOT NULL, education_type TEXT, standard TEXT, weight REAL DEFAULT 1, pass_score REAL DEFAULT 60, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS violation_records ( violation_id TEXT PRIMARY KEY, record_id TEXT, school_id INTEGER, school_name TEXT, education_type TEXT, violation_type TEXT, violation_level TEXT, description TEXT, evidence TEXT, responsible_person TEXT, status TEXT DEFAULT 'pending', penalty_decision TEXT, penalty_date TEXT, appealed INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS rectification_tasks ( rect_id TEXT PRIMARY KEY, violation_id TEXT, record_id TEXT, school_id INTEGER, school_name TEXT, education_type TEXT, problem_description TEXT, rectification_measures TEXT, deadline TEXT, responsible_person TEXT, status TEXT DEFAULT 'pending', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS rectification_progress ( id INTEGER PRIMARY KEY AUTOINCREMENT, rect_id TEXT NOT NULL, progress REAL DEFAULT 0, status TEXT DEFAULT 'in_progress', update_date TEXT, description TEXT, evidence TEXT, updated_by TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_reports ( report_id TEXT PRIMARY KEY, report_type TEXT, plan_id TEXT, record_id TEXT, school_id INTEGER, school_name TEXT, education_type TEXT, title TEXT NOT NULL, content TEXT, summary TEXT, recommendations TEXT, author_id INTEGER, author_name TEXT, status TEXT DEFAULT 'draft', published_at TEXT, archived INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS dedication_evaluations ( eval_id TEXT PRIMARY KEY, teacher_id INTEGER NOT NULL, teacher_name TEXT, education_type TEXT, evaluation_period TEXT, political_score REAL DEFAULT 0, professional_score REAL DEFAULT 0, role_model_score REAL DEFAULT 0, teaching_attitude_score REAL DEFAULT 0, academic_integrity_score REAL DEFAULT 0, care_students_score REAL DEFAULT 0, overall_score REAL DEFAULT 0, overall_rating TEXT, evaluator_id INTEGER, evaluator_name TEXT, comments TEXT, status TEXT DEFAULT 'pending', created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS compliance_checks ( check_id TEXT PRIMARY KEY, school_id INTEGER, school_name TEXT, education_type TEXT, check_date TEXT, check_items TEXT, status TEXT DEFAULT 'in_progress', completed_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS compliance_results ( id INTEGER PRIMARY KEY AUTOINCREMENT, check_id TEXT NOT NULL, item_code TEXT, item_name TEXT, result TEXT DEFAULT 'pending', score REAL, evidence TEXT, comments TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_comments ( id INTEGER PRIMARY KEY AUTOINCREMENT, record_id TEXT NOT NULL, commenter_id INTEGER, commenter_name TEXT, comment TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS complaint_handling ( complaint_id TEXT PRIMARY KEY, complaint_type TEXT, school_id INTEGER, school_name TEXT, education_type TEXT, complainant_name TEXT, contact_info TEXT, content TEXT, evidence TEXT, status TEXT DEFAULT 'pending', handler_id INTEGER, handler_name TEXT, investigation_result TEXT, handling_measures TEXT, reply_content TEXT, satisfaction_rating INTEGER, closed_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS supervision_archive ( archive_id TEXT PRIMARY KEY, related_type TEXT, related_id TEXT, title TEXT, education_type TEXT, archive_date TEXT, archived_by INTEGER, file_path TEXT, metadata TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_supervision_plan(self, plan_name: str, supervision_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO supervision_plans ( plan_id, plan_name, supervision_type, education_type, target_school, target_level, start_date, end_date, scope, objectives, budget, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?) ''', (plan_id, plan_name, supervision_type, education_type,
    ...
    def approve_supervision_plan(self, plan_id: str, approved_by: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE supervision_plans SET status = ?, approved_by = ?, approved_at = ?, updated_at = ? WHERE plan_id = ? AND status = 'draft' ''', (status, approved_by, now, now, plan_id))
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO supervision_tasks ( task_id, plan_id, task_name, task_type, assignee_id, assignee_name, target_school, scheduled_date, status, priority, description, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?) ''', (task_id, plan_id, task_name, kwargs.get('task_type'),
    ...
    def create_supervision_team(self, team_name: str, leader_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO supervision_teams ( team_id, team_name, leader_id, leader_name, education_type, specialty, description, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (team_id, team_name, leader_id, leader_name,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT OR IGNORE INTO supervision_members (team_id, member_id, member_name, role, qualification, joined_at) VALUES (?, ?, ?, ?, ?, ?)',
                                 (team_id, member_id, member_name, kwargs.get('role', 'member'),
                                 kwargs.get('qualification'), now))
                    if cursor.rowcount > 0:
                        conn.commit()
                        return {'success': True}
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO supervision_records ( record_id, task_id, plan_id, school_id, school_name, education_type, supervisor_id, supervisor_name, record_date, findings, score, rating, issues_found, photos, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 'draft', ?, ?) ''', (record_id, task_id, plan_id, school_id, school_name,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if 'score' in kwargs:
                        ...
                        score = kwargs['score']
                        rating = 'excellent' if score >= 90 else ('good' if score >= 80 else (
    ...
    def create_inspection_item(self, inspection_category: str, item_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO inspection_items ( item_id, inspection_category, item_name, education_type, standard, weight, pass_score, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (item_id, inspection_category, item_name,
    ...
    def conduct_inspection(self, school_id: int, school_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO compliance_checks ( check_id, school_id, school_name, education_type, check_date, check_items, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'in_progress', ?, ?) ''', (check_id, school_id, school_name, education_type,
    ...
    def record_compliance_result(self, check_id: str, item_code: str,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR REPLACE INTO compliance_results (check_id, item_code, item_name, result, score, evidence, comments) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (check_id, item_code, item_name,
    ...
    def create_violation_record(self, record_id: str, school_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO violation_records ( violation_id, record_id, school_id, school_name, education_type, violation_type, violation_level, description, evidence, responsible_person, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (violation_id, record_id, school_id, school_name,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO rectification_tasks ( rect_id, violation_id, record_id, school_id, school_name, education_type, problem_description, rectification_measures, deadline, responsible_person, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (rect_id, violation_id, record_id, school_id, school_name,
    ...
    def update_rectification_progress(self, rect_id: str, progress: float,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO rectification_progress (rect_id, progress, status, update_date, description, evidence, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (rect_id, progress, status, now[:10], kwargs.get('description'), kwargs.get('evidence'),
    ...
    def create_dedication_evaluation(self, teacher_id: int, teacher_name: str,
        ...
        try:
            ...
            now = datetime.now().isoformat()
            scores = {k: kwargs.get(k, 0) for k in ['political_score', 'professional_score', 'role_model_score',
            'teaching_attitude_score', 'academic_integrity_score', 'care_students_score']}
            weights = {'political_score': 0.2, 'professional_score': 0.25, 'role_model_score': 0.2,
            'teaching_attitude_score': 0.15, 'academic_integrity_score': 0.1, 'care_students_score': 0.1}
            overall_score = sum(scores[k] * weights[k] for k in scores)
            overall_rating = 'excellent' if overall_score >= 90 else ('good' if overall_score >= 80 else (
            'pass' if overall_score >= 60 else 'fail'))
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO dedication_evaluations ( eval_id, teacher_id, teacher_name, education_type, evaluation_period, political_score, professional_score, role_model_score, teaching_attitude_score, academic_integrity_score, care_students_score, overall_score, overall_rating, evaluator_id, evaluator_name, comments, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (eval_id, teacher_id, teacher_name, education_type,
                    ...
                    return {'success': True, 'eval_id': eval_id, 'overall_score': round(overall_score, 1),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    scores = dict(zip(['political_score', 'professional_score', 'role_model_score',
                    'teaching_attitude_score', 'academic_integrity_score', 'care_students_score'], current))
                    ...
                    overall_rating = 'excellent' if overall_score >= 90 else ('good' if overall_score >= 80 else (
                    'pass' if overall_score >= 60 else 'fail'))
                    cursor.execute(''' UPDATE dedication_evaluations SET political_score = ?, professional_score = ?, role_model_score = ?, teaching_attitude_score = ?, academic_integrity_score = ?, care_students_score = ?, overall_score = ?, overall_rating = ?, updated_at = ? WHERE eval_id = ? ''', (scores['political_score'], scores['professional_score'],
                          scores['role_model_score'], scores['teaching_attitude_score'],
                          scores['academic_integrity_score'], scores['care_students_score'],
    ...
    def add_compliance_result(self, check_id: str, item_code: str,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO compliance_results (check_id, item_code, item_name, result, score, evidence, comments) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (check_id, item_code, item_name, result,
    ...
    def create_complaint(self, complaint_type: str, school_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO complaint_handling ( complaint_id, complaint_type, school_id, school_name, education_type, complainant_name, contact_info, content, evidence, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (complaint_id, complaint_type, school_id, school_name,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE complaint_handling SET investigation_result = ?, handling_measures = ?, reply_content = ?, status = ?, updated_at = ? WHERE complaint_id = ? AND status = ?',
                                 (investigation_result, handling_measures, kwargs.get('reply_content'), 'resolved', now,
                                 complaint_id, 'processing'))
                    if cursor.rowcount > 0:
                        conn.commit()
    ...
    def create_supervision_report(self, report_type: str, title: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO supervision_reports ( report_id, report_type, plan_id, record_id, school_id, school_name, education_type, title, content, summary, recommendations, author_id, author_name, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?) ''', (report_id, report_type, kwargs.get('plan_id'), kwargs.get('record_id'),
                ...
                with self._get_connection() as conn:
                    ...
                    if cursor.rowcount > 0:
                        cursor.execute('INSERT INTO supervision_archive (archive_id, related_type, related_id, title, education_type, archive_date, archived_by, metadata, created_at) SELECT ?, ?, ?, title, education_type, ?, ?, ?, ? FROM supervision_reports WHERE report_id = ?',
                                     (f"arc_{uuid.uuid4().hex[:12]}", 'report', report_id, now[:10], None, '{}', now,
                                     report_id))
                        conn.commit()
                        return {'success': True}
    ...
    def get_supervision_statistics(self, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f'SELECT AVG(score) FROM supervision_records{where_clause} AND score IS NOT NULL',
                params)
                ...
                cursor.execute(f'SELECT status, COUNT(*) FROM supervision_records{where_clause} GROUP BY status',
                params)
                record_status_distribution = {r[0]: r[1] for r in cursor.fetchall()}
                cursor.execute(f'SELECT status, COUNT(*) FROM rectification_tasks{where_clause} GROUP BY status',
                params)
