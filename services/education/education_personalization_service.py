    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' CREATE TABLE IF NOT EXISTS user_profiles ( profile_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, grade_level INTEGER, age INTEGER, gender TEXT, learning_interests TEXT, learning_ability TEXT, learning_style TEXT, learning_goals TEXT, learning_preferences TEXT, learning_history TEXT, learning_behavior TEXT, learning_motivation TEXT, profile_score REAL DEFAULT 0, last_updated TEXT, created_at TEXT, UNIQUE(user_id, education_type) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS profile_data ( id INTEGER PRIMARY KEY AUTOINCREMENT, profile_id TEXT NOT NULL, dimension TEXT NOT NULL, data_key TEXT NOT NULL, data_value TEXT, confidence REAL DEFAULT 1.0, source TEXT, updated_at TEXT, FOREIGN KEY (profile_id) REFERENCES user_profiles(profile_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS personalization_rules ( rule_id TEXT PRIMARY KEY, rule_name TEXT NOT NULL, rule_type TEXT, education_type TEXT, priority INTEGER DEFAULT 1, condition TEXT, action TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS rule_config ( id INTEGER PRIMARY KEY AUTOINCREMENT, rule_id TEXT NOT NULL, config_key TEXT NOT NULL, config_value TEXT, FOREIGN KEY (rule_id) REFERENCES personalization_rules(rule_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS adaptive_learning ( adaptive_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, education_type TEXT, course_id TEXT, strategy TEXT, current_difficulty REAL DEFAULT 1.0, progress REAL DEFAULT 0, performance REAL DEFAULT 0, adaptation_count INTEGER DEFAULT 0, last_adapted TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_state ( id INTEGER PRIMARY KEY AUTOINCREMENT, adaptive_id TEXT NOT NULL, state_key TEXT NOT NULL, state_value TEXT, recorded_at TEXT, FOREIGN KEY (adaptive_id) REFERENCES adaptive_learning(adaptive_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS personalized_content ( content_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, education_type TEXT, content_type TEXT, source_content_id TEXT, customization_level TEXT DEFAULT 'low', delivery_status TEXT DEFAULT 'pending', delivered_at TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS content_preferences ( id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, education_type TEXT, content_type TEXT NOT NULL, preference_score REAL DEFAULT 0.5, last_interaction TEXT, interaction_count INTEGER DEFAULT 0, UNIQUE(user_id, education_type, content_type) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS personalized_assessment ( assessment_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, education_type TEXT, assessment_type TEXT, target_dimension TEXT, difficulty REAL DEFAULT 1.0, question_count INTEGER DEFAULT 10, duration_minutes INTEGER DEFAULT 30, status TEXT DEFAULT 'created', scheduled_at TEXT, completed_at TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS assessment_results ( result_id TEXT PRIMARY KEY, assessment_id TEXT NOT NULL, user_id INTEGER NOT NULL, score REAL DEFAULT 0, max_score REAL DEFAULT 100, correct_count INTEGER DEFAULT 0, total_count INTEGER DEFAULT 0, performance_level TEXT, feedback TEXT, analyzed_at TEXT, FOREIGN KEY (assessment_id) REFERENCES personalized_assessment(assessment_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS personalized_feedback ( feedback_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, education_type TEXT, feedback_type TEXT, source_type TEXT, source_id TEXT, content TEXT, tone TEXT DEFAULT 'neutral', depth TEXT DEFAULT 'shallow', action_required INTEGER DEFAULT 0, read_status INTEGER DEFAULT 0, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS feedback_history ( id INTEGER PRIMARY KEY AUTOINCREMENT, feedback_id TEXT NOT NULL, user_id INTEGER NOT NULL, action TEXT, action_time TEXT, FOREIGN KEY (feedback_id) REFERENCES personalized_feedback(feedback_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS learning_paths ( path_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, education_type TEXT, path_name TEXT NOT NULL, strategy TEXT, start_date TEXT, end_date TEXT, progress REAL DEFAULT 0, status TEXT DEFAULT 'active', total_nodes INTEGER DEFAULT 0, completed_nodes INTEGER DEFAULT 0, created_at TEXT, updated_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS path_config ( id INTEGER PRIMARY KEY AUTOINCREMENT, path_id TEXT NOT NULL, node_order INTEGER NOT NULL, node_type TEXT, node_content TEXT, prerequisites TEXT, estimated_duration INTEGER, FOREIGN KEY (path_id) REFERENCES learning_paths(path_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS personalized_experience ( experience_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, education_type TEXT, interface_style TEXT, interaction_mode TEXT, content_presentation TEXT, learning_rhythm TEXT, incentive_mechanism TEXT, social_interaction TEXT, personalized_messages TEXT, exclusive_service TEXT, last_updated TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS experience_settings ( id INTEGER PRIMARY KEY AUTOINCREMENT, experience_id TEXT NOT NULL, setting_key TEXT NOT NULL, setting_value TEXT, FOREIGN KEY (experience_id) REFERENCES personalized_experience(experience_id) ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS personalization_alerts ( alert_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, education_type TEXT, alert_type TEXT, severity TEXT DEFAULT 'medium', title TEXT, description TEXT, status TEXT DEFAULT 'active', threshold_value TEXT, current_value TEXT, created_at TEXT ) ''')

                cursor.execute(''' CREATE TABLE IF NOT EXISTS alert_history ( id INTEGER PRIMARY KEY AUTOINCREMENT, alert_id TEXT NOT NULL, user_id INTEGER NOT NULL, action TEXT, action_result TEXT, action_time TEXT, FOREIGN KEY (alert_id) REFERENCES personalization_alerts(alert_id) ) ''')
    ...
    def create_user_profile(self, user_id: int, user_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR REPLACE INTO user_profiles ( profile_id, user_id, user_name, education_type, grade_level, age, gender, learning_interests, learning_ability, learning_style, learning_goals, learning_preferences, learning_history, learning_behavior, learning_motivation, profile_score, last_updated, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?) ''', (profile_id, user_id, user_name, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT OR REPLACE INTO profile_data ( profile_id, dimension, data_key, data_value, confidence, source, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (profile_id, dimension, data_key, json.dumps(data_value),
    ...
    def export_profile(self, profile_id: str, format_type: str = 'json') -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if format_type == 'json':
                    return {'success': True, 'data': export_data, 'content': json.dumps(export_data, ensure_ascii=False,
                    indent=2)}
    ...
    def initialize_adaptive_learning(self, user_id: int, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO adaptive_learning ( adaptive_id, user_id, education_type, course_id, strategy, current_difficulty, progress, performance, adaptation_count, last_adapted, created_at ) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0, ?, ?) ''', (adaptive_id, user_id, education_type, course_id,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' UPDATE adaptive_learning SET current_difficulty = ?, performance = ?, adaptation_count = ?, last_adapted = ? WHERE adaptive_id = ? ''', (new_diff, performance, adaptation_count + 1, now, adaptive_id))
                    cursor.execute(''' INSERT INTO learning_state (adaptive_id, state_key, state_value, recorded_at) VALUES (?, 'difficulty_change', ?, ?) ''', (adaptive_id, json.dumps({'old': current_diff, 'new': new_diff, 'reason': 'performance'}),
                    now))
                    conn.commit()
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not cursor.fetchone():
                        return {'success': False, 'error': '自适应学习不存在'}
                    cursor.execute('UPDATE adaptive_learning SET progress = ?, updated_at = ? WHERE adaptive_id = ?',
                    (progress, now, adaptive_id))
                    cursor.execute(''' INSERT INTO learning_state (adaptive_id, state_key, state_value, recorded_at) VALUES (?, 'progress_update', ?, ?) ''', (adaptive_id, json.dumps({'progress': progress, 'details': kwargs.get('details', {})}), now))
                    conn.commit()
    ...
    def get_adaptive_learning_state(self, adaptive_id: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                if not adaptive:
                    return {'success': False, 'error': '自适应学习不存在'}
                cursor.execute('SELECT * FROM learning_state WHERE adaptive_id = ? ORDER BY recorded_at DESC',
                (adaptive_id,))
    ...
    def create_personalized_content(self, user_id: int, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO personalized_content ( content_id, user_id, education_type, content_type, source_content_id, customization_level, delivery_status, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'pending', ?) ''', (content_id, user_id, education_type, content_type,
    ...
    def update_content_preference(self, user_id: int, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT OR REPLACE INTO content_preferences ( user_id, education_type, content_type, preference_score, last_interaction, interaction_count ) VALUES (?, ?, ?, ?, ?, COALESCE(( SELECT interaction_count FROM content_preferences WHERE user_id = ? AND education_type = ? AND content_type = ?), 0) + 1) ''', (user_id, education_type, content_type, preference_score, now,
    ...
    def get_content_preferences(self, user_id: int, education_type: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM content_preferences WHERE user_id = ? AND education_type = ?', (user_id,
                education_type))
    ...
    def create_personalized_assessment(self, user_id: int, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO personalized_assessment ( assessment_id, user_id, education_type, assessment_type, target_dimension, difficulty, question_count, duration_minutes, status, scheduled_at, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'created', ?, ?) ''', (assessment_id, user_id, education_type, assessment_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE personalized_assessment SET status = ? WHERE assessment_id = ?',
                    ('in_progress', assessment_id))
                    result_id = f"ars_{uuid.uuid4().hex[:12]}"
                    score = kwargs.get('score', 0)
                    correct_count = kwargs.get('correct_count', 0)
                    total_count = assessment[7]
                    performance_level = 'excellent' if score >= 90 else ('good' if score >= 80 else (
                    'pass' if score >= 60 else 'fail'))
                    cursor.execute(''' INSERT INTO assessment_results ( result_id, assessment_id, user_id, score, max_score, correct_count, total_count, performance_level, feedback, analyzed_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (result_id, assessment_id, assessment[1], score, 100,
                    ...
                    conn.commit()
                    return {'success': True, 'result_id': result_id, 'score': score,
    ...
    def get_assessment_history(self, user_id: int, education_type: str,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT pa.*, ar.score, ar.performance_level FROM personalized_assessment pa LEFT JOIN assessment_results ar ON pa.assessment_id = ar.assessment_id WHERE pa.user_id = ? AND pa.education_type = ? ORDER BY pa.created_at DESC LIMIT ? OFFSET ? ''', (user_id, education_type, page_size, (page - 1) * page_size))
                assessments = cursor.fetchall()
                cursor.execute('SELECT COUNT( *) as cnt FROM personalized_assessment WHERE user_id = ? AND education_type = ?', (user_id,
                education_type))
                total = cursor.fetchone()['cnt']
                return {'success': True, 'assessments': [dict(a) for a in assessments], 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def create_personalized_feedback(self, user_id: int, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO personalized_feedback ( feedback_id, user_id, education_type, feedback_type, source_type, source_id, content, tone, depth, action_required, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (feedback_id, user_id, education_type, feedback_type,
    ...
    def deliver_feedback(self, feedback_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE personalized_feedback SET read_status = 1 WHERE feedback_id = ?',
                    (feedback_id,))
                    if cursor.rowcount > 0:
                        cursor.execute(''' INSERT INTO feedback_history (feedback_id, user_id, action, action_time) SELECT feedback_id, user_id, 'delivered', ? FROM personalized_feedback WHERE feedback_id = ? ''', (now, feedback_id))
                        conn.commit()
                        return {'success': True}
    ...
    def get_feedback_history(self, user_id: int, education_type: str,
        ...
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute(''' SELECT pf.*, fh.action, fh.action_time FROM personalized_feedback pf LEFT JOIN feedback_history fh ON pf.feedback_id = fh.feedback_id WHERE pf.user_id = ? AND pf.education_type = ? ORDER BY pf.created_at DESC LIMIT ? OFFSET ? ''', (user_id, education_type, page_size, (page - 1) * page_size))
                feedbacks = cursor.fetchall()
                cursor.execute('SELECT COUNT( *) as cnt FROM personalized_feedback WHERE user_id = ? AND education_type = ?', (user_id,
                education_type))
                total = cursor.fetchone()['cnt']
                return {'success': True, 'feedbacks': [dict(f) for f in feedbacks], 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not user:
                        return {'success': False, 'error': '反馈不存在'}
                    cursor.execute(''' INSERT INTO feedback_history (feedback_id, user_id, action, action_result, action_time) VALUES (?, ?, ?, ?, ?) ''', (feedback_id, user[0], action, kwargs.get('action_result', ''), now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def create_learning_path(self, user_id: int, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO learning_paths ( path_id, user_id, education_type, path_name, strategy, start_date, end_date, progress, status, total_nodes, completed_nodes, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 'active', ?, 0, ?, ?) ''', (path_id, user_id, education_type, path_name, strategy,
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO path_config (path_id, node_order, node_type, node_content, prerequisites, estimated_duration) VALUES (?, ?, ?, ?, ?, ?) ''', (path_id, node_order, node_type, node_content,
    ...
    def update_path_progress(self, path_id: str, completed_node_order: int) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT total_nodes, completed_nodes FROM learning_paths WHERE path_id = ?',
                    (path_id,))
                    path = cursor.fetchone()
                    if not path:
                        return {'success': False, 'error': '学习路径不存在'}
                    ...
                    cursor.execute(''' UPDATE learning_paths SET completed_nodes = ?, progress = ?, status = ?, updated_at = ? WHERE path_id = ? ''', (completed, progress, status, now, path_id))
                    conn.commit()
                    return {'success': True, 'progress': progress, 'status': status}
        except Exception as e:
    ...
    def create_personalized_experience(self, user_id: int, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO personalized_experience ( experience_id, user_id, education_type, interface_style, interaction_mode, content_presentation, learning_rhythm, incentive_mechanism, social_interaction, personalized_messages, exclusive_service, last_updated, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (experience_id, user_id, education_type,
    ...
    def update_experience_setting(self, experience_id: str, setting_key: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT experience_id FROM personalized_experience WHERE experience_id = ?',
                    (experience_id,))
                    if not cursor.fetchone():
                    ...
                    cursor.execute('UPDATE personalized_experience SET last_updated = ? WHERE experience_id = ?', (now,
                    experience_id))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_personalized_experience(self, user_id: int, education_type: str) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM personalized_experience WHERE user_id = ? AND education_type = ?',
                (user_id, education_type))
                experience = cursor.fetchone()
                if not experience:
                    return {'success': False, 'error': '个性化体验不存在'}
                cursor.execute('SELECT * FROM experience_settings WHERE experience_id = ?',
    ...
    def reset_experience_to_default(self, experience_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' UPDATE personalized_experience SET interface_style = ?, interaction_mode = ?, content_presentation = ?, learning_rhythm = ?, incentive_mechanism = ?, social_interaction = ?, personalized_messages = ?, exclusive_service = ?, last_updated = ? WHERE experience_id = ? ''', (defaults['interface_style'], defaults['interaction_mode'],
    ...
    def create_alert(self, user_id: int, education_type: str, alert_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO personalization_alerts ( alert_id, user_id, education_type, alert_type, severity, title, description, status, threshold_value, current_value, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?) ''', (alert_id, user_id, education_type, alert_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('UPDATE personalization_alerts SET status = ? WHERE alert_id = ?', ('resolved',
                    alert_id))
                    cursor.execute(''' INSERT INTO alert_history (alert_id, user_id, action, action_result, action_time) VALUES (?, ?, ?, ?, ?) ''', (alert_id, user[0], action, kwargs.get('action_result', ''), now))
                    conn.commit()
                    return {'success': True}
        except Exception as e:
    ...
    def get_alert_history(self, user_id: int, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(query, params)
                alerts = cursor.fetchall()
                return {'success': True, 'alerts': [dict(a) for a in alerts], 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def get_personalization_stats(self, user_id: int = None, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                if user_id:
                    cursor.execute('SELECT COUNT(*) as cnt FROM user_profiles WHERE user_id = ?' + (
                    ' AND education_type = ?' if education_type else ''),
                                  [user_id] + ([education_type] if education_type else []))
                    stats['profile_count'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT COUNT(*) as cnt FROM adaptive_learning WHERE user_id = ?' + (
                    ' AND education_type = ?' if education_type else ''),
                                  [user_id] + ([education_type] if education_type else []))
                    stats['adaptive_learning_count'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT COUNT(*) as cnt FROM personalized_assessment WHERE user_id = ?' + (
                    ' AND education_type = ?' if education_type else ''),
                                  [user_id] + ([education_type] if education_type else []))
                    stats['assessment_count'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT COUNT(*) as cnt FROM personalized_feedback WHERE user_id = ?' + (
                    ' AND education_type = ?' if education_type else ''),
                                  [user_id] + ([education_type] if education_type else []))
                    stats['feedback_count'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT COUNT(*) as cnt FROM learning_paths WHERE user_id = ?' + (
                    ' AND education_type = ?' if education_type else ''),
                                  [user_id] + ([education_type] if education_type else []))
                    stats['path_count'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT COUNT(*) as cnt FROM personalization_alerts WHERE user_id = ?' + (
                    ' AND education_type = ?' if education_type else ''),
                                  [user_id] + ([education_type] if education_type else []))
                    stats['alert_count'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT AVG(progress) as avg FROM learning_paths WHERE user_id = ?' + (
                    ' AND education_type = ?' if education_type else ''),
                                  [user_id] + ([education_type] if education_type else []))
                    ...
                    ' WHERE education_type = ?' if education_type else ''),
                                  [education_type] if education_type else [])
                    stats['total_profiles'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT COUNT(*) as cnt FROM adaptive_learning' + (
                    ' WHERE education_type = ?' if education_type else ''),
                                  [education_type] if education_type else [])
                    stats['total_adaptive_learning'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT COUNT(*) as cnt FROM personalized_assessment' + (
                    ' WHERE education_type = ?' if education_type else ''),
                                  [education_type] if education_type else [])
                    stats['total_assessments'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT COUNT(*) as cnt FROM personalized_feedback' + (
                    ' WHERE education_type = ?' if education_type else ''),
                                  [education_type] if education_type else [])
                    ...
                    cursor.execute('SELECT COUNT(*) as cnt FROM personalization_alerts' + (
                    ' WHERE education_type = ?' if education_type else ''),
                                  [education_type] if education_type else [])
                    stats['total_alerts'] = cursor.fetchone()['cnt']

                    cursor.execute('SELECT AVG(progress) as avg FROM learning_paths' + (
                    ' WHERE education_type = ?' if education_type else ''),
                                  [education_type] if education_type else [])
                    avg_progress = cursor.fetchone()['avg']
