    ...
    def _init_db(self):
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(''' CREATE TABLE IF NOT EXISTS virtual_spaces ( space_id TEXT PRIMARY KEY, space_name TEXT NOT NULL, space_type TEXT NOT NULL, education_type TEXT NOT NULL, capacity INTEGER DEFAULT 100, current_users INTEGER DEFAULT 0, status TEXT DEFAULT 'available', description TEXT, scene_url TEXT, thumbnail_url TEXT, creator_id INTEGER, creator_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS space_configurations ( config_id TEXT PRIMARY KEY, space_id TEXT NOT NULL, config_key TEXT NOT NULL, config_value TEXT, created_at TEXT, UNIQUE(space_id, config_key) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS immersive_content ( content_id TEXT PRIMARY KEY, content_name TEXT NOT NULL, content_type TEXT NOT NULL, education_type TEXT NOT NULL, immersive_type TEXT, duration INTEGER DEFAULT 0, description TEXT, scene_url TEXT, thumbnail_url TEXT, file_size INTEGER DEFAULT 0, status TEXT DEFAULT 'draft', creator_id INTEGER, creator_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS content_assets ( asset_id TEXT PRIMARY KEY, content_id TEXT NOT NULL, asset_name TEXT, asset_type TEXT, file_url TEXT, file_size INTEGER DEFAULT 0, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS virtual_experiments ( experiment_id TEXT PRIMARY KEY, experiment_name TEXT NOT NULL, education_type TEXT NOT NULL, subject TEXT, grade_level INTEGER, description TEXT, duration INTEGER DEFAULT 60, equipment_requirements TEXT, safety_notes TEXT, status TEXT DEFAULT 'draft', creator_id INTEGER, creator_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS experiment_sessions ( session_id TEXT PRIMARY KEY, experiment_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, start_time TEXT, end_time TEXT, duration INTEGER DEFAULT 0, data_collected TEXT, results TEXT, status TEXT DEFAULT 'running', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS vr_ar_devices ( device_id TEXT PRIMARY KEY, device_name TEXT NOT NULL, device_type TEXT NOT NULL, education_type TEXT, serial_number TEXT, status TEXT DEFAULT 'available', location TEXT, last_maintenance TEXT, next_maintenance TEXT, assigned_user_id INTEGER, assigned_user_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS device_usage ( usage_id TEXT PRIMARY KEY, device_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, start_time TEXT, end_time TEXT, duration INTEGER DEFAULT 0, usage_type TEXT, content_id TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS virtual_social ( social_id TEXT PRIMARY KEY, social_name TEXT NOT NULL, social_type TEXT NOT NULL, education_type TEXT NOT NULL, max_users INTEGER DEFAULT 100, current_users INTEGER DEFAULT 0, description TEXT, scene_url TEXT, status TEXT DEFAULT 'available', creator_id INTEGER, creator_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS social_sessions ( session_id TEXT PRIMARY KEY, social_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, avatar_id TEXT, join_time TEXT, leave_time TEXT, duration INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS virtual_events ( event_id TEXT PRIMARY KEY, event_name TEXT NOT NULL, event_type TEXT NOT NULL, education_type TEXT NOT NULL, description TEXT, location TEXT, start_date TEXT, end_date TEXT, start_time TEXT, end_time TEXT, max_participants INTEGER DEFAULT 100, registered_count INTEGER DEFAULT 0, status TEXT DEFAULT 'scheduled', cover_image TEXT, organizer_id INTEGER, organizer_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS event_participants ( participant_id TEXT PRIMARY KEY, event_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, avatar_id TEXT, register_time TEXT, status TEXT DEFAULT 'registered', attended INTEGER DEFAULT 0, created_at TEXT, UNIQUE(event_id, user_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS digital_collectibles ( collectible_id TEXT PRIMARY KEY, collectible_name TEXT NOT NULL, collectible_type TEXT NOT NULL, education_type TEXT, rarity TEXT DEFAULT 'common', description TEXT, image_url TEXT, metadata TEXT, supply INTEGER DEFAULT 1, issued INTEGER DEFAULT 0, status TEXT DEFAULT 'active', issuer_id INTEGER, issuer_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS collectible_ownership ( ownership_id TEXT PRIMARY KEY, collectible_id TEXT NOT NULL, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, acquire_time TEXT, acquire_method TEXT, status TEXT DEFAULT 'owned', created_at TEXT, UNIQUE(collectible_id, user_id) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS digital_twin ( twin_id TEXT PRIMARY KEY, twin_name TEXT NOT NULL, education_type TEXT, entity_type TEXT, description TEXT, model_url TEXT, status TEXT DEFAULT 'syncing', sync_interval INTEGER DEFAULT 60, creator_id INTEGER, creator_name TEXT, created_at TEXT, updated_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS twin_data ( data_id TEXT PRIMARY KEY, twin_id TEXT NOT NULL, data_type TEXT, data_value TEXT, timestamp TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS user_avatars ( avatar_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_name TEXT, education_type TEXT, avatar_name TEXT NOT NULL, avatar_type TEXT DEFAULT 'human', appearance TEXT, accessories TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT, UNIQUE(user_id, avatar_name) ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS avatar_customization ( customization_id TEXT PRIMARY KEY, avatar_id TEXT NOT NULL, customization_type TEXT, item_name TEXT, item_value TEXT, created_at TEXT ) ''')
                cursor.execute(''' CREATE TABLE IF NOT EXISTS metaverse_logs ( log_id TEXT PRIMARY KEY, log_type TEXT, user_id INTEGER, user_name TEXT, education_type TEXT, action TEXT, target_id TEXT, target_type TEXT, details TEXT, timestamp TEXT, created_at TEXT ) ''')
                conn.commit()
    ...
    def create_virtual_space(self, space_name: str, space_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO virtual_spaces ( space_id, space_name, space_type, education_type, capacity, current_users, status, description, scene_url, thumbnail_url, creator_id, creator_name, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 0, 'available', ?, ?, ?, ?, ?, ?, ?) ''', (space_id, space_name, space_type, education_type,
    ...
    def create_immersive_content(self, content_name: str, content_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO immersive_content ( content_id, content_name, content_type, education_type, immersive_type, duration, description, scene_url, thumbnail_url, file_size, status, creator_id, creator_name, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?) ''', (content_id, content_name, content_type, education_type,
    ...
    def add_content_asset(self, content_id: str, asset_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO content_assets ( asset_id, content_id, asset_name, asset_type, file_url, file_size, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (asset_id, content_id, asset_name, asset_type,
    ...
    def create_virtual_experiment(self, experiment_name: str, education_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO virtual_experiments ( experiment_id, experiment_name, education_type, subject, grade_level, description, duration, equipment_requirements, safety_notes, status, creator_id, creator_name, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?) ''', (experiment_id, experiment_name, education_type,
    ...
    def start_experiment_session(self, experiment_id: str, user_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT status, education_type FROM virtual_experiments WHERE experiment_id = ?',
                    (experiment_id,))
                    ...
                    cursor.execute(''' INSERT INTO experiment_sessions ( session_id, experiment_id, user_id, user_name, education_type, start_time, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'running', ?) ''', (session_id, experiment_id, user_id, user_name,
    ...
    def end_experiment_session(self, session_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT start_time FROM experiment_sessions WHERE session_id = ? AND status = ?',
                    (session_id, 'running'))
                    ...
                    cursor.execute(''' UPDATE experiment_sessions SET end_time = ?, duration = ?, data_collected = ?, results = ?, status = 'completed' WHERE session_id = ? ''', (now, duration, kwargs.get('data_collected'),
    ...
    def register_device(self, device_name: str, device_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO vr_ar_devices ( device_id, device_name, device_type, education_type, serial_number, status, location, last_maintenance, next_maintenance, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 'available', ?, NULL, ?, ?, ?) ''', (device_id, device_name, device_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not device:
                        return {'success': False, 'error': '设备不存在'}
                    cursor.execute(''' INSERT INTO device_usage ( usage_id, device_id, user_id, user_name, start_time, usage_type, content_id, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (usage_id, device_id, user_id, user_name, now,
    ...
    def create_social_space(self, social_name: str, social_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO virtual_social ( social_id, social_name, social_type, education_type, max_users, current_users, description, scene_url, status, creator_id, creator_name, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, 0, ?, ?, 'available', ?, ?, ?, ?) ''', (social_id, social_name, social_type, education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO social_sessions ( session_id, social_id, user_id, user_name, education_type, avatar_id, join_time, status, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?) ''', (session_id, social_id, user_id, user_name,
    ...
    def create_virtual_event(self, event_name: str, event_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO virtual_events ( event_id, event_name, event_type, education_type, description, location, start_date, end_date, start_time, end_time, max_participants, registered_count, status, cover_image, organizer_id, organizer_name, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'scheduled', ?, ?, ?, ?, ?) ''', (event_id, event_name, event_type, education_type,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT OR IGNORE INTO event_participants (participant_id, event_id, user_id, user_name, education_type, avatar_id, register_time, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, \'registered\', ?)',
                                 (participant_id, event_id, user_id, user_name, event[3], kwargs.get('avatar_id'), now,
                                 now))
                    if cursor.rowcount > 0:
    ...
    def cancel_event_registration(self, participant_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT event_id FROM event_participants WHERE participant_id = ? AND status = ?',
                    (participant_id, 'registered'))
                    participant = cursor.fetchone()
                    if not participant:
                        return {'success': False, 'error': '报名记录不存在或已签到'}
                    cursor.execute('UPDATE event_participants SET status = ? WHERE participant_id = ?', ('cancelled',
                    participant_id))
    ...
    def create_digital_collectible(self, collectible_name: str, collectible_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO digital_collectibles ( collectible_id, collectible_name, collectible_type, education_type, rarity, description, image_url, metadata, supply, issued, status, issuer_id, issuer_name, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'active', ?, ?, ?, ?) ''', (collectible_id, collectible_name, collectible_type,
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute('INSERT OR IGNORE INTO collectible_ownership (ownership_id, collectible_id, user_id, user_name, education_type, acquire_time, acquire_method, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, \'owned\', ?)',
                                 (ownership_id, collectible_id, user_id, user_name, collectible[3], now,
                                 kwargs.get('acquire_method', 'issued'), now))
                    if cursor.rowcount > 0:
    ...
    def get_user_collectibles(self, user_id: int, education_type: str = None,
        ...
        try:
            with self._get_connection() as conn:
                ...
                return {'success': True, 'collectibles': ownerships, 'total': total, 'page': page,
                'page_size': page_size}
        except Exception as e:
    ...
    def create_digital_twin(self, twin_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO digital_twin ( twin_id, twin_name, education_type, entity_type, description, model_url, status, sync_interval, creator_id, creator_name, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 'syncing', ?, ?, ?, ?, ?) ''', (twin_id, twin_name, kwargs.get('education_type'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO twin_data ( data_id, twin_id, data_type, data_value, timestamp, created_at ) VALUES (?, ?, ?, ?, ?, ?) ''', (data_id, twin_id, data_type, data_value, now, now))
                    conn.commit()
    ...
    def create_user_avatar(self, user_id: int, avatar_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO user_avatars ( avatar_id, user_id, user_name, education_type, avatar_name, avatar_type, appearance, accessories, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (avatar_id, user_id, kwargs.get('user_name'), education_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO avatar_customization ( customization_id, avatar_id, customization_type, item_name, item_value, created_at ) VALUES (?, ?, ?, ?, ?, ?) ''', (customization_id, avatar_id, customization_type,
    ...
    def get_metaverse_statistics(self, education_type: str = None) -> Dict[str, Any]:
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(f'SELECT COUNT(*) FROM immersive_content WHERE status = ?{base_query}',
                ['published'] + base_params)
