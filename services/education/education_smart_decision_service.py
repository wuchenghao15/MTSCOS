...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
""" MTSCOS 教育智慧决策服务 (v15.15.0) ==================================== 提供教育大数据决策、智能分析、预测预警、决策支持、知识图谱、可视化仪表盘、智能推荐、绩效评估等综合服务。  核心能力： 1. 决策模型 - 模型管理、规则配置、策略优化、方案生成 2. 数据分析 - 数据查询、统计分析、趋势分析、对比分析 3. 知识图谱 - 图谱构建、实体管理、关系挖掘、路径分析 4. 可视化 - 仪表盘、报表、图表、数据大屏 5. 智能推荐 - 课程推荐、学习路径、教师推荐、资源推荐 6. 预测预警 - 成绩预警、辍学预警、资源预警、财务预警、安全预警 7. 绩效评估 - 指标管理、数据采集、评估计算、报告生成 8. 决策日志 - 日志记录、查询分析、审计追踪 9. 反馈管理 - 反馈收集、分析处理、改进建议 10. 统计分析 - 综合统计、趋势预测 """
import os
import json
import uuid
import sqlite3
        ...
        try:
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS decision_models ( model_id TEXT PRIMARY KEY, model_name TEXT NOT NULL, model_type TEXT, decision_type TEXT, analysis_model TEXT, education_type TEXT, description TEXT, config_json TEXT, accuracy REAL DEFAULT 0, status TEXT DEFAULT 'active', version TEXT DEFAULT '1.0', created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS decision_rules ( rule_id TEXT PRIMARY KEY, model_id TEXT NOT NULL, rule_name TEXT NOT NULL, rule_expression TEXT, priority INTEGER DEFAULT 1, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT, FOREIGN KEY (model_id) REFERENCES decision_models(model_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS analysis_tasks ( task_id TEXT PRIMARY KEY, task_name TEXT NOT NULL, data_domain TEXT, analysis_model TEXT, education_type TEXT, parameters_json TEXT, status TEXT DEFAULT 'pending', progress INTEGER DEFAULT 0, created_at TEXT, started_at TEXT, completed_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS analysis_results ( result_id TEXT PRIMARY KEY, task_id TEXT NOT NULL, data_domain TEXT, education_type TEXT, result_json TEXT, summary TEXT, recommendations TEXT, confidence REAL DEFAULT 0, created_at TEXT, FOREIGN KEY (task_id) REFERENCES analysis_tasks(task_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS data_sources ( source_id TEXT PRIMARY KEY, source_name TEXT NOT NULL, data_domain TEXT, source_type TEXT, connection_string TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, last_sync_at TEXT, created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS data_pipelines ( pipeline_id TEXT PRIMARY KEY, pipeline_name TEXT NOT NULL, source_id TEXT, data_domain TEXT, education_type TEXT, transform_rules TEXT, schedule_type TEXT DEFAULT 'daily', status TEXT DEFAULT 'active', last_run_at TEXT, created_at TEXT, updated_at TEXT, FOREIGN KEY (source_id) REFERENCES data_sources(source_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS knowledge_graph ( graph_id TEXT PRIMARY KEY, graph_name TEXT NOT NULL, description TEXT, education_type TEXT, node_count INTEGER DEFAULT 0, edge_count INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS graph_nodes ( node_id TEXT PRIMARY KEY, graph_id TEXT NOT NULL, node_name TEXT NOT NULL, node_type TEXT, properties_json TEXT, education_type TEXT, created_at TEXT, FOREIGN KEY (graph_id) REFERENCES knowledge_graph(graph_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS graph_edges ( edge_id TEXT PRIMARY KEY, graph_id TEXT NOT NULL, source_node_id TEXT NOT NULL, target_node_id TEXT NOT NULL, edge_type TEXT, properties_json TEXT, education_type TEXT, created_at TEXT, FOREIGN KEY (graph_id) REFERENCES knowledge_graph(graph_id), FOREIGN KEY (source_node_id) REFERENCES graph_nodes(node_id), FOREIGN KEY (target_node_id) REFERENCES graph_nodes(node_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS dashboards ( dashboard_id TEXT PRIMARY KEY, dashboard_name TEXT NOT NULL, decision_level TEXT, education_type TEXT, layout_json TEXT, refresh_interval INTEGER DEFAULT 300, is_public INTEGER DEFAULT 0, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS dashboard_widgets ( widget_id TEXT PRIMARY KEY, dashboard_id TEXT NOT NULL, widget_name TEXT NOT NULL, visualization_type TEXT, data_domain TEXT, config_json TEXT, position_json TEXT, education_type TEXT, is_active INTEGER DEFAULT 1, created_at TEXT, FOREIGN KEY (dashboard_id) REFERENCES dashboards(dashboard_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS recommendations ( rec_id TEXT PRIMARY KEY, user_id INTEGER NOT NULL, user_type TEXT, rec_type TEXT, education_type TEXT, items_json TEXT, reason TEXT, confidence REAL DEFAULT 0, is_viewed INTEGER DEFAULT 0, created_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS recommendation_rules ( rule_id TEXT PRIMARY KEY, rec_type TEXT NOT NULL, education_type TEXT, rule_name TEXT NOT NULL, rule_logic TEXT, weight REAL DEFAULT 1.0, is_active INTEGER DEFAULT 1, created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS performance_metrics ( metric_id TEXT PRIMARY KEY, metric_name TEXT NOT NULL, metric_type TEXT, data_domain TEXT, education_type TEXT, calculation_formula TEXT, target_value REAL, unit TEXT, description TEXT, status TEXT DEFAULT 'active', created_at TEXT, updated_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS metric_data ( data_id TEXT PRIMARY KEY, metric_id TEXT NOT NULL, education_type TEXT, period TEXT, actual_value REAL, target_value REAL, trend TEXT, score REAL DEFAULT 0, created_at TEXT, FOREIGN KEY (metric_id) REFERENCES performance_metrics(metric_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS prediction_results ( pred_id TEXT PRIMARY KEY, model_id TEXT NOT NULL, data_domain TEXT, education_type TEXT, prediction_json TEXT, confidence REAL DEFAULT 0, forecast_period TEXT, created_at TEXT, FOREIGN KEY (model_id) REFERENCES decision_models(model_id) ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS alert_records ( alert_id TEXT PRIMARY KEY, alert_type TEXT NOT NULL, education_type TEXT, severity TEXT DEFAULT 'medium', title TEXT NOT NULL, description TEXT, affected_objects TEXT, status TEXT DEFAULT 'pending', trigger_time TEXT, acknowledged_at TEXT, resolved_at TEXT, created_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS decision_logs ( log_id TEXT PRIMARY KEY, decision_type TEXT, education_type TEXT, decision_level TEXT, user_id INTEGER, user_name TEXT, action TEXT, details_json TEXT, result TEXT, created_at TEXT ) ''')
                    cursor.execute(''' CREATE TABLE IF NOT EXISTS decision_feedback ( feedback_id TEXT PRIMARY KEY, decision_id TEXT, education_type TEXT, user_id INTEGER, user_name TEXT, rating INTEGER, comment TEXT, improvement_suggestions TEXT, created_at TEXT ) ''')
                    conn.commit()
    ...
    def create_decision_model(self, model_name: str, model_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO decision_models ( model_id, model_name, model_type, decision_type, analysis_model, education_type, description, config_json, accuracy, status, version, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', '1.0', ?, ?) ''', (model_id, model_name, model_type, decision_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO decision_rules ( rule_id, model_id, rule_name, rule_expression, priority, education_type, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (rule_id, model_id, rule_name, rule_expression,
    ...
    def create_analysis_task(self, task_name: str, data_domain: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO analysis_tasks ( task_id, task_name, data_domain, analysis_model, education_type, parameters_json, status, progress, created_at ) VALUES (?, ?, ?, ?, ?, ?, 'pending', 0, ?) ''', (task_id, task_name, data_domain, analysis_model,
    ...
    def execute_analysis(self, task_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO analysis_results ( result_id, task_id, data_domain, education_type, result_json, summary, recommendations, confidence, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (result_id, task_id, task_info[0], task_info[2],
    ...
    def create_knowledge_graph(self, graph_name: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO knowledge_graph ( graph_id, graph_name, description, education_type, node_count, edge_count, status, created_at, updated_at ) VALUES (?, ?, ?, ?, 0, 0, 'active', ?, ?) ''', (graph_id, graph_name, kwargs.get('description'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO graph_nodes ( node_id, graph_id, node_name, node_type, properties_json, education_type, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?) ''', (node_id, graph_id, node_name, node_type,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    if not cursor.fetchone():
                        return {'success': False, 'error': '知识图谱不存在'}
                    cursor.execute('SELECT node_id FROM graph_nodes WHERE node_id = ? AND graph_id = ?',
                    (source_node_id, graph_id))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '源节点不存在'}
                    cursor.execute('SELECT node_id FROM graph_nodes WHERE node_id = ? AND graph_id = ?',
                    (target_node_id, graph_id))
                    if not cursor.fetchone():
                        return {'success': False, 'error': '目标节点不存在'}
                    cursor.execute(''' INSERT INTO graph_edges ( edge_id, graph_id, source_node_id, target_node_id, edge_type, properties_json, education_type, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (edge_id, graph_id, source_node_id, target_node_id,
    ...
    def analyze_graph_path(self, graph_id: str, start_node_id: str,
        ...
        try:
            with self._get_connection() as conn:
                ...
                cursor.execute(''' SELECT e.edge_id, e.edge_type, e.source_node_id, e.target_node_id, sn.node_name as source_name, tn.node_name as target_name FROM graph_edges e JOIN graph_nodes sn ON e.source_node_id = sn.node_id JOIN graph_nodes tn ON e.target_node_id = tn.node_id WHERE e.graph_id = ? AND (e.source_node_id = ? OR e.target_node_id = ?) ''', (graph_id, start_node_id, start_node_id))
    ...
    def create_dashboard(self, dashboard_name: str, decision_level: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO dashboards ( dashboard_id, dashboard_name, decision_level, education_type, layout_json, refresh_interval, is_public, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (dashboard_id, dashboard_name, decision_level,
                          kwargs.get('education_type'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO dashboard_widgets ( widget_id, dashboard_id, widget_name, visualization_type, data_domain, config_json, position_json, education_type, is_active, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?) ''', (widget_id, dashboard_id, widget_name, visualization_type,
    ...
    def generate_report(self, dashboard_id: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._get_connection() as conn:
                ...
                if not dashboard:
                    return {'success': False, 'error': '仪表盘不存在'}
                cursor.execute('SELECT * FROM dashboard_widgets WHERE dashboard_id = ? AND is_active = 1',
                (dashboard_id,))
    ...
    def generate_recommendation(self, user_id: int, user_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('SELECT rule_name, rule_logic, weight FROM recommendation_rules WHERE rec_type = ? AND education_type = ? AND is_active = 1',
                    (rec_type, education_type))
                    rules = cursor.fetchall()
                    ...
                    cursor.execute(''' INSERT INTO recommendations ( rec_id, user_id, user_type, rec_type, education_type, items_json, reason, confidence, is_viewed, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?) ''', (rec_id, user_id, user_type, rec_type, education_type,
    ...
    def add_recommendation_rule(self, rec_type: str, rule_name: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO recommendation_rules ( rule_id, rec_type, education_type, rule_name, rule_logic, weight, is_active, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?) ''', (rule_id, rec_type, kwargs.get('education_type'),
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    cursor.execute(''' INSERT INTO prediction_results ( pred_id, model_id, data_domain, education_type, prediction_json, confidence, forecast_period, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ''', (pred_id, model_id, data_domain, kwargs.get('education_type'),
    ...
    def trigger_alert(self, alert_type: str, title: str, **kwargs) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO alert_records ( alert_id, alert_type, education_type, severity, title, description, affected_objects, status, trigger_time, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?) ''', (alert_id, alert_type, kwargs.get('education_type'),
    ...
    def resolve_alert(self, alert_id: str) -> Dict[str, Any]:
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE alert_records SET status = ?, resolved_at = ? WHERE alert_id = ?',
            ...
            with self._get_connection() as conn:
                ...
                for row in rows:
                    ...
                    if alert_type not in summary['by_type']:
                        summary['by_type'][alert_type] = {'name': ALERT_TYPES.get(alert_type, {}).get('name', ''),
                        'count': 0, 'status': {}}
    ...
    def create_performance_metric(self, metric_name: str, metric_type: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO performance_metrics ( metric_id, metric_name, metric_type, data_domain, education_type, calculation_formula, target_value, unit, description, status, created_at, updated_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?) ''', (metric_id, metric_name, metric_type, data_domain,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    ...
                    trend = 'up' if actual_value > target_value * 0.9 else (
                    'down' if actual_value < target_value * 0.7 else 'stable')
                    cursor.execute(''' INSERT INTO metric_data ( data_id, metric_id, education_type, period, actual_value, target_value, trend, score, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (data_id, metric_id, kwargs.get('education_type'), period,
    ...
    def calculate_performance(self, metric_ids: List[str], period: str,
        ...
        try:
            with self._get_connection() as conn:
                ...
                query = f''' SELECT m.metric_id, m.metric_name, m.metric_type, m.data_domain, md.actual_value, md.target_value, md.score, md.trend FROM performance_metrics m LEFT JOIN metric_data md ON m.metric_id = md.metric_id AND md.period = ? WHERE m.metric_id IN ({placeholders}) '''
                ...
                result = {
                    ...
                    'overall_level': 'excellent' if avg_score >= 90 else ('good' if avg_score >= 75 else (
                    'medium' if avg_score >= 60 else 'poor')),
                    'details': rows
                }
    ...
    def record_decision_log(self, decision_type: str, action: str,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO decision_logs ( log_id, decision_type, education_type, decision_level, user_id, user_name, action, details_json, result, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (log_id, decision_type, kwargs.get('education_type'),
    ...
    def submit_decision_feedback(self, decision_id: str, user_id: int,
        ...
        try:
            ...
            with self._lock:
                with self._get_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute(''' INSERT INTO decision_feedback ( feedback_id, decision_id, education_type, user_id, user_name, rating, comment, improvement_suggestions, created_at ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', (feedback_id, decision_id, kwargs.get('education_type'),
