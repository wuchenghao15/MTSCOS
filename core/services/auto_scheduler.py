...
class AutoScheduler:
    ...
    def task_git_sync_check(self):
        """Git同步 - 基于核心文件变更检测触发同步"""
        task = 'git_sync_check'
        # 缩短检测间隔到60秒，但只有检测到变更才会真正同步
        if not self._should_run(task, 'MAINT_GIT_SYNC_INTERVAL', 60):
            return

        logger.info("[任务] Git同步变更检测...")
        try:
            from app.git_auto_sync import GitAutoSync
            sync = GitAutoSync({
                'sync_remote': 'mtscos_origin',
                'sync_branch': 'main',
                'auto_commit': True,
                'auto_push': True,
                'retry_count': 2,
            ...
            result = sync.sync_if_changed()

            if result['synced']:
                logger.info(f"  ✓ Git同步完成: {result['reason']}")
                self._log_maintenance('sync', 'git', 'success',
                                      f'变更触发同步: {result["reason"]}')
            else:
                logger.debug("  - 无核心文件变更，跳过同步")

            self._update_stats(task, True)
        except Exception as e:
            logger.warning(f"  ⚠ Git同步检测失败: {e}")
            self._update_stats(task, False)

        self.last_run[task] = datetime.now()

    def task_ai_security_defend(self):
        """AI安全自动防御 - 分析攻击模式并自动调整安全参数"""
        task = 'ai_security_defend'
        if not self._should_run(task, 'AI_SECURITY_DEFEND_INTERVAL', 300):
            return

        if not self._get_rule_bool('AI_SECURITY_DEFEND_ENABLED', True):
            return

        logger.info("[任务] AI安全自动防御...")
        try:
            from app.middlewares.security_middleware import SecurityMiddlewareClass
            actions = SecurityMiddlewareClass.auto_defend()

            if actions:
                logger.info(f"  ✓ AI防御执行: {len(actions)}个动作")
                self._log_maintenance('security', 'ai_defend', 'success',
                                      f'执行{len(actions)}个防御动作: {"; ".join(actions)}')
            else:
                logger.debug("  - 无需防御动作")

            # 同时获取安全统计供AI学习
            stats = SecurityMiddlewareClass.get_security_stats()
            if stats['locked_users'] > 0 or stats['blacklisted_ips'] > 0:
                logger.info(f"  ⚠ 安全状态: 锁定用户{stats['locked_users']}个, "
                           f"黑名单IP{stats['blacklisted_ips']}个")

            self._update_stats(task, True)
        except Exception as e:
            logger.warning(f"  ⚠ AI安全防御失败: {e}")
            self._update_stats(task, False)
    ...
    def task_document_cleanup(self):
        ...
        try:
            ...
            with self._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                "UPDATE document_rules SET status = 'archived' WHERE expiry_date < ? AND status = 'published'", (cutoff,
                ))
    ...
    def task_self_learning(self):
        """AI自我学习"""
        task = 'self_learning'
        if not self._should_run(task, 'SELF_LEARNING_INTERVAL', 3600):
            return

        if not self._get_rule_bool('SELF_LEARNING_ENABLED', True):
            return

        logger.info("[任务] AI自我学习...")
        try:
    ...
    def get_all_tasks(self):
        ...
        return [
            ...
            ('ai_security_defend', self.task_ai_security_defend),
            ...
            ('self_learning', self.task_self_learning),
        ]
