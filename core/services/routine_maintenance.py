...
def create_system_version_history_table(conn):
    ...
    conn.commit()
    logger.info("✓ system_version_history表创建成功")
...
def insert_version_history(conn, version_data, upgrade_time=None):
    ...
    if cursor.fetchone()[0] == 0:
        ...
        conn.commit()
        logger.info(f"✓ 版本 {version_data['version']} 历史记录已插入")
    else:
        logger.info(f"✓ 版本 {version_data['version']} 历史记录已存在，跳过")
...
def update_system_version(conn, version):
    ...
    conn.commit()
    logger.info(f"✓ 系统版本号已更新为 {version}")
...
def run_maintenance_tasks(conn):
    ...
    try:
        ...
        logger.info(f"   ✓ 清理了 {deleted} 条过期会话")
    except Exception as e:
        logger.info(f"   ✗ 清理会话失败: {e}")
    ...
    try:
        ...
        logger.info(f"   ✓ 更新了 {updated} 条规则状态")
    except Exception as e:
        logger.info(f"   ✗ 更新规则状态失败: {e}")
    ...
    try:
        ...
        logger.info(f"   ✓ 规则总数: {total}, 启用规则: {active}")
    except Exception as e:
        logger.info(f"   ✗ 统计规则失败: {e}")
    ...
    try:
        ...
        if result == 'ok':
            ...
            logger.info(f"   ✗ 数据库完整性检查失败: {result}")
    except Exception as e:
        logger.info(f"   ✗ 数据库完整性检查失败: {e}")
...
def log_maintenance(conn, action, details):
    ...
    try:
        ...
        logger.info(f"记录维护日志失败: {e}")

def main():
    logger.info("=" * 60)
    logger.info("     MTSCOS AI 例行维护系统")
    logger.info("=" * 60)
    logger.info(f"维护时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info()
